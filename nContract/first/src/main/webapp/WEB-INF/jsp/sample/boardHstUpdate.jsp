    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
			...
			$('#EXPIRE_CONDITION').change(function(e){ 
				...
				cnt++; //최초0에서 카운트를 시작 함, 처음 불러왔을 때는 값을 초기화하지 않으며 이후 변경점이 있을 경우에는 값을 초기화 
			});
			...
			switch(tempId)
			{
				case "EXPIRE_CONDITION" : 
					if (tempValue == "D0001") {
						...
						if(cnt > 0){//최초 불러온 데이터는 지우지 않는다, 그 이후 변경하게 되면 값을 초기화 한다
							$("#VALID_DT").val("");
							$("#EXPIRE_DT").val("");
						}
					}
        ...
        function fn_openBoardList(){
            ...
            comSubmit.addParam("CONTRACT_NO", "${map.CONTRACT_NO}");
            ...
            var valid = fn_validCheck();
   			...
   			if (valid == true){
				...
				$("#update").attr("disabled",true);
   	            ...
   	            comSubmit.setUrl("<c:url value='/sample/updateBoarHst.do' />");
   	            comSubmit.addParam("userid", "<sec:authentication property='principal.username'/>");
   	            comSubmit.addParam("HST_SEQ", "${map.HST_SEQ}");
        ...
        function fn_validCheck(){
			...
			if ($("#BU_CODE").val().length < 1) {
				//alert("사업부를 선택 해주세요.");
				Lobibox.alert("error", //AVAILABLE TYPES: "error", "info", "success", "error"
		   	    		 {
		   	    		     msg: "사업부를 선택 바랍니다."
		   	    		 });
