...
//                 fn_openBoardSearch();
        ...
        function fn_openBoardSearch(pageNo){
        	...
        	if($('#selctBucode option:selected').val() == undefined) 
        		...
        		busor_Code = $('#selctBucode option:selected').val();
        	
         	if(pageNo == 1) 
         		comAjax.setCallback("fn_selectBoardListFirstCallback");
         	else 
        ...
        function fn_selectBoardListFirstCallback(data){
        	var total = data.TOTAL;
        	
            if(total == '0'){
            	var str = "<tr><td colspan='9'>조회된 결과가 없습니다.</td></tr>";
                fn_SetNocountGridDate(str);
            }
            else{
                var params = {
                    divId : "PAGE_NAVI",
        ...
        function fn_SetGridDate(gridDate){
            ...
            $.each(gridDate.list, function(key, value){
            	...
            	if(value.EXPIRE_CONDITION == 'D0001') str2 = "<td>" + value.VALID_DT + "~" + value.EXPIRE_DT + "</td>"
				else str2 = "<td>" + value.EXPIRE_CONDITIONNM + "</td>"
        ...
        function fn_SetNocountGridDate(str){
        	var body = $("table>tbody");
