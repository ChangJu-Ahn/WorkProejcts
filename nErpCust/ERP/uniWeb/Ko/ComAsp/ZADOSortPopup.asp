...
Const C_SEQ_NO     = 6
Const C_Update     = 7     '--- 2003-01-04 김승진 추가 spreadUpdae 
Const C_LockFlg    = 8     '--- 2003-01-04 김승진 추가 spreadLockFlag 
Const C_NEXT_SEQ   = 9
...
Dim iMessage 	             '--- 2003-01-09 김승진 추가  
...
Sub InitSpreadSheet()
	...
	With vspdData
		...
		'.MaxCols = C_SEQ_NO
		'.Col = .MaxCols
		.MaxCols = C_PairField    ' 2003-01-04 김승진 수정 
		.Col = C_SEQ_NO        ' 2003-01-04 김승진 수정 
		.ColHidden = True
        ...
        vspdData.Col = C_LockFlg   ' 2003-01-08 김승진 추가 
        vspdData.Row = 0
        .Colhidden = True
...
'========================== 2003-01-09 김승진 추가 : DefaultT값이 모두 0인지 체크=============
...
Function InitDefaultT(ByVal obj)
   ...
   For maxNum=1 to obj.MaxRows   '2003-01-08 김승진 추가 
     obj.Row = maxNum
     obj.Col = C_LockFlg
...
'========================== 2003-01-08 김승진 추가 : 해당 값보다 큰 수에 +1 =============
...
Function ZADOSort(ByVal obj ,ByVal Row ,ByVal ChangeValue)
				...
				For maxNum=1 to obj.MaxRows
				  ...
				  if (maxNum <> Row) AND (CLng(obj.value) <> 0) then
                    'msgbox changeValue & " | " & arrRowValue(Row-1)
						...
						if CLng(ChangeValue) > CLng(arrRowValue(Row-1)) then  '작은값에서 큰값으로 변환 
							
							if CLng(arrRowValue(Row-1)) = 0 then        ' 원래의 값이 0일때 변환값보다 큰것만 +1
   ...
   Call InitRowValue(obj)  '2002-12-16 김승진 추가 
   
End Function
'=======================================================================
'========================== 2003-01-08 김승진 추가 : SpreadLock Convert=
'=======================================================================
...
End Function
'=======================================================================
'========================== 2003-01-08 김승진 추가 :  RowLockComboList =
'=======================================================================
...
'        if objDOMNodeList.item(ii-1).attributes.getNamedItem("FIELD_TYPE").nodeValue <> "HH" And objDOMNodeList.item(ii-1).attributes.getNamedItem("DEFAULT_T").nodeValue <> "HH" then
		 ...
		 If iStrTemp = "L" OR iStrTemp = "HH" Then
            ...
            Else 
                 ' 2003-01-11 김승진 추가 : 화면에 보여지지 않는 열을 선택 후 자동으로 선택되어지는 열이 L일 경우를 위한 추가 
			...
			ggoSpread.SpreadLock C_PairField, iColNum1 , C_PairField , iColNum1  '2003-01-20 김승진 추가 
...
Sub vspdData_Change(ByVal Col ,ByVal Row)
    ...
    vspdData.ReDraw = false    
    if vspdData.Col = C_DefaultT then  '2002-12-16 추가 
      Call ZADOSort(ggoSpread.Source,Row,vspdData.Value)
    end if
    
    'If vspdData.Col = C_HIDESHOW then  '2003-01-08 추가 
