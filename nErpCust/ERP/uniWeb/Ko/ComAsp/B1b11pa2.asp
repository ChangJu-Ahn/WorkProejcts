...
'*  2. Function Name        : Popup Item By Plant(with Phantom)											*	
'*  3. Program ID           : b1b11pa2.asp																*
'*  4. Program Name         :																			*
'*  5. Program Desc         : Item by Plant Popup(with Phantom)											*
'*  7. Modified date(First) : 2002/04/09																*
'*  8. Modified date(Last)  : 2002/11/29																*
...
Const BIZ_PGM_ID = "b1b11pb2.asp"							'☆: Asp name of Biz logic
...
'========================================================================================================
Sub InitComboBox()
	Dim i, iItemAcctArr, iItemAcctNmArr, iProcurTypeArr, iProcurTypeNmArr
	...
	For i = 0 to UBound(iItemAcctArr) - 1
		If UCase(lgItemAcct(0)) = "Y" Then
			If (UCase(iItemAcctArr(i)) >= UCase(lgItemAcct(1))) And (UCase(iItemAcctArr(i)) <= UCase(lgItemAcct(2))) Then 
				Call SetCombo(cboItemAccount, UCase(iItemAcctArr(i)), iItemAcctNmArr(i))
			End If
		Else
			Call SetCombo(cboItemAccount, UCase(iItemAcctArr(i)), iItemAcctNmArr(i))
		End If
...
Sub SetDefaultVal()
	...
	Else
		...
		ReDim lgItemAcct(Fix(ILen/2)) 
		lgItemAcct(0) = "Y"

        For i = 1 To ILen Step 2
		    j=j+1
		    lgItemAcct(j) = UCase(Mid(IStr1, i, 2))
        Next    
    End If
...
'    vspdData.OperationMode = 3
	...
	ggoSpread.SSSetEdit C_BaseItemCd,		"기준품목",		10										
	...
	Call ggoSpread.SSSetColHidden(C_FormalNm, C_AbcFlg, True)
...
<TABLE CELLSPACING=0 CLASS="basicTB">
	<TR><TD HEIGHT=40>
		<FIELDSET CLASS="CLSFLD"><TABLE WIDTH=100% CELLSPACING=0>
			...
			<TR>
				...
				<TD CLASS=TD6 NOWRAP><SELECT NAME="cboItemClass" ALT="품목그룹" STYLE="Width: 160px;" tag="11XXXU"><OPTION VALUE = ""></OPTION></SELECT></TD>
			</TR>
			<TR>
			...
			<TR>
				...
				<TD CLASS=TD6 NOWRAP>
					<script language =javascript src='./js/b1b11pa2_I187401121_txtBaseDt.js'></script>
				</TD>
			</TR>
			<TR>
	...
	<TR><TD HEIGHT=100%>
			<script language =javascript src='./js/b1b11pa2_I944487392_vspdData.js'></script>
	</TD></TR>
	<TR><TD HEIGHT=30>
