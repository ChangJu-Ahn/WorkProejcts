...
<%													'☜ : 여기서 부터 개발자 비지니스 로직을 처리하는 내용이 시작된다 
    ...
    Const C_SHEETMAXROWS = 1000									'한화면에 보일수 있는 최대 Row 수 
...
'hanc	if Request.QueryString("isFlag") = 1 Then isWhere = 0 End If
...
'Call ServerMesgBox("qqqq", vbInformation, I_MKSCRIPT)
...
'hanc				  IF isWhere = 1 THEN 'LIKE 검색인 경우만 적용됨 
'hanc					if txtNextCode <> "" or txtNextName <> "" then
'hanc						IF iLoop = C_SHEETMAXROWS - 1 THEN
'hanc						isCondition = isCondition & "'" & CSTR(tmp2By2Array(0,iLoop)) & "'"
'hanc						ELSE
'hanc						isCondition = isCondition & "'" & tmp2By2Array(0,iLoop) & "',"
'hanc						END IF
'hanc					end if
'hanc                  END IF
