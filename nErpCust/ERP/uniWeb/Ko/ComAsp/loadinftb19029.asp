...
Sub LoadInfTB19029(ByVal pCurrency, ByVal pFormType, ByVal pModuleCd)
    ...
    On Error Resume Next												            '¢Ð: Server Side Process
...
'    If lgQtyIntegeral > 12 Then
'       lgQtyIntegeral =  12
'    End If

'    If lgAmtOfMoneyIntegeral > 16 Then
'       lgAmtOfMoneyIntegeral =  16
'    End If

'    If lgUnitCostIntegeral > 12 Then
'       lgUnitCostIntegeral =  12
'    End If
...
Sub SetDefaultDec(Byval pDataType,Byval pDecimals)
    ...
    If iMaxdecimal(2) = -1 Or iMaxdecimal(2) > 2 Then
        iMaxdecimal(2) = 2
    End If    
    If iMaxdecimal(3) = -1 Or iMaxdecimal(3) > 6 Then
        iMaxdecimal(3) = 6
    End If    
    If iMaxdecimal(4) = -1 Or iMaxdecimal(4) > 6 Then
        iMaxdecimal(4) = 6
    End If    
...
Function GetGlobalData(pData)   '2003-08-07 leejinsoo
	...
	xmlDoc.async = False 
	xmlDoc.LoadXML(xSessionDll.DMakeDic(Request.Cookies("unierp")("SessionKey")))	
