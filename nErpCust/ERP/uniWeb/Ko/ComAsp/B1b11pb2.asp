...
If Request("cboItemAccount") <> "" Then		'품목계정 
	...
	strToItemAcct = FilterVar(Trim(Request("cboItemAccount"))	,"''", "S")
Else
	If Request("ToItemAcct") <> "" Then
		strFromItemAcct = FilterVar(Trim(Request("FromItemAcct"))	,"''", "S")
		strToItemAcct = FilterVar(Trim(Request("ToItemAcct"))	,"''", "S")
	End If
End If
...
Sub SubBizQueryMulti(pType)
    ...
    '---------- Developer Coding part (Start) ---------------------------------------------------------------
		Call SubMakeSQLStatements("CD",strPlantCd,strItemCd,strItemNm,strFromItemClass,strToItemClass,strFromItemAcct,strToItemAcct,strFromProcType,strToProcType,strFromProdEnv,strToProdEnv,strBaseDt,strSpec,strTrackingFlg,strInspClass,strNextKey)           '☜ : Make sql statements
	Else	
		Call SubMakeSQLStatements("NM",strPlantCd,strItemCd,strItemNm,strFromItemClass,strToItemClass,strFromItemAcct,strToItemAcct,strFromProcType,strToProcType,strFromProdEnv,strToProdEnv,strBaseDt,strSpec,strTrackingFlg,strInspClass,strNextKey)           '☜ : Make sql statements
	End If
    ...
    Do While Not lgObjRs.EOF
	    ...
	    lgstrData = lgstrData & Chr(11)	& Trim(lgObjRs("HS_CD"))		'HS코드						'HS코드	
...
'============================================================================================================
Sub SubMakeSQLStatements(pDataType,pCode,pCode1,pCode2,pCode3,pCode4,pCode5,pCode6,pCode7,pCode8,pCode9,pCode10,pCode11,pCode12,pCode13,pCode14,pCode15)
    Dim iSelCount
	'------ Developer Coding part (Start ) ------------------------------------------------------------------
    Select Case pDataType

		Case "CD"
			lgStrSQL = "SELECT TOP 51 A.*, B.*, dbo.ufn_GetCodeName('P1001', A.ITEM_ACCT) ITEM_ACCT_NM, "
			...
			lgStrSQL = lgStrSQL & " AND B.ITEM_NM LIKE " & pCode2	'2003-08-29
			...
			lgStrSQL = lgStrSQL & " ) AND A.ITEM_ACCT >= " & pCode5
			lgStrSQL = lgStrSQL & " AND A.ITEM_ACCT <= " & pCode6
