...
*  2. Function Name        : 근속수당기준등록 
*  3. Program ID           : H1007ma1
*  4. Program Name         : H1007ma1
*  5. Program Desc         : 기준정보관리/근속수당기준등록 
*  6. Comproxy List        :
...
'========================================================================================================
Const BIZ_PGM_ID = "H1007mb1.asp"                                      'Biz Logic ASP 
...
Sub InitComboBox()
    ...
    Call  CommonQueryRs(" MINOR_CD,MINOR_NM "," B_MINOR "," MAJOR_CD = " & FilterVar("H0091", "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    iCodeArr = lgF0
    iNameArr = lgF1
     ggoSpread.SetCombo Replace(iCodeArr,Chr(11),vbTab), C_APPLY_TYPE
     ggoSpread.SetCombo Replace(iNameArr,Chr(11),vbTab), C_APPLY_TYPE_NM
End Sub
...
Sub InitSpreadSheet()
    ...
    With frm1.vspdData
	    ...
	    .ReDraw = false
        .MaxCols = C_APPLY_TYPE_NM + 1												<%'☜: 최대 Columns의 항상 1개 증가시킴 %>
       ...
       Call  GetSpreadColumnPos("A")
		 ...
		 ggoSpread.SSSetEdit     C_DEPT_CD_NM,   "부서",     28,,,20,2
		 ggoSpread.SSSetFloat    C_DUTY_STRT,    "근속From", 15,"6", ggStrIntegeralPart,  ggStrDeciPointPart, parent.gComNum1000, parent.gComNumDec,,,"Z"
		 ggoSpread.SSSetFloat    C_DUTY_END,     "근속To",   15,"6", ggStrIntegeralPart,  ggStrDeciPointPart, parent.gComNum1000, parent.gComNumDec,,,"Z"
		 ggoSpread.SSSetFloat    C_ALLOW,        "수당",     20,  parent.ggAmtOfMoneyNo, ggStrIntegeralPart,  ggStrDeciPointPart, parent.gComNum1000, parent.gComNumDec,,,"Z"
	     ggoSpread.SSSetCombo    C_APPLY_TYPE,   "적용구분", 20
	     ggoSpread.SSSetCombo    C_APPLY_TYPE_NM,"적용구분", 24
	     
	     Call ggoSpread.MakePairsColumn(C_DEPT_CD,    C_DEPT_CD_POP)
         Call ggoSpread.SSSetColHidden(C_APPLY_TYPE,  C_APPLY_TYPE, True)
...
Sub SetSpreadLock()
    ...
    .vspdData.ReDraw = False
     ...
     ggoSpread.SpreadLock    C_DUTY_STRT     , -1, C_DUTY_STRT
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			C_DUTY_STRT			= iCurColumnPos(4)
			C_DUTY_END			= iCurColumnPos(5)
			C_ALLOW				= iCurColumnPos(6)
			C_APPLY_TYPE		= iCurColumnPos(7)
			C_APPLY_TYPE_NM		= iCurColumnPos(8)           
...
Sub Form_Load()
    ...
    ' 수당코드에 값을 
    Call  CommonQueryRs(" MAX(allow_cd) "," hda080t ","",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    frm1.txtAllow_cd.value = Trim(Replace(lgF0,Chr(11),""))

    Call  CommonQueryRs(" allow_nm "," HDA010T "," code_type = " & FilterVar("1", "''", "S") & "  and allow_cd =  " & FilterVar(frm1.txtAllow_cd.value , "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Function FncSave()
    ...
    IntRetCd =  CommonQueryRs(" distinct ALLOW_CD "," HDA080T "," ALLOW_CD <>  " & FilterVar(frm1.txtAllow_cd.value, "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    strAllow_cd = Trim(Replace(lgF0,Chr(11),""))

    If IsNull(strAllow_cd) OR strAllow_cd = "" then
    Else
        Call  DisplayMsgBox("800492","x",UCase(strAllow_cd),"근속수당")                        
        Exit Function          
    End if 
       ...
       For lRow = 1 To .vspdData.MaxRows
           ...
           if   .vspdData.Text =  ggoSpread.InsertFlag OR .vspdData.Text =  ggoSpread.UpdateFlag then
                ...
                if Trim(.vspdData.text) = "" then
                    Call  DisplayMsgBox("800062","X","X","X")	'부서정보에 등록되어 있지 않은 코드입니다.
  					.vspdData.focus
                    exit function
                end if
                ...
                if  .vspdData.Text <> "" then
                    if   UNICDbl(.vspdData.Text) <= 0 then
                        call  DisplayMsgBox("970027", "x","근속From","x")
                ...
                if  .vspdData.Text <> "" then
                    if   UNICDbl(.vspdData.Text) <= 0 then
                        call  DisplayMsgBox("970021", "x","근속To","x")
                        .vspdData.Action = 0 ' go to 
                        exit function
                    end if
                else
                ...
                .vspdData.Col = C_DUTY_STRT
                intStrt =  UNICDbl(.vspdData.Text)
                
                .vspdData.Col = C_DUTY_END
                intEnd =  UNICDbl(.vspdData.Text)
                if  intStrt >= intEnd then                
                    call  DisplayMsgBox("800097", "x","x","x")
                    .vspdData.Action = 0 ' go to
                    exit function
                end if
       ...
       For lRow = 1 To .vspdData.MaxRows
           ...
           Select Case .vspdData.Text
               ...
               Case  ggoSpread.InsertFlag                                      '☜: Update
                   ...
                   .vspdData.Col = C_APPLY_TYPE	: strVal = strVal & Trim(.vspdData.Text) & parent.gColSep
...
Sub vspdData_Change(ByVal Col , ByVal Row )
    ...
    Select Case Col
         Case  C_APPLY_TYPE_NM
                iDx = Frm1.vspdData.value
   	            Frm1.vspdData.Col = C_APPLY_TYPE
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>근속수당기준등록</font></td>
			...
			<TABLE <%=LR_SPACE_TYPE_20%>>
							...
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
                                    ...
                                    <TD CLASS=TD6 NOWRAP>
										...
										<INPUT TYPE=TEXT ID=txtAllow_cd NAME="txtAllow_cd" MAXLENGTH=3 SIZE=10 MAXLENGTH=8 tag=12XXXU ALT="수당코드"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnWarrentNo" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenAllowCd()">
										<INPUT TYPE=TEXT ID=txtAllow_nm NAME="txtAllow_nm" tag="14X"></TD>
				...
				<TR>
					<TD WIDTH=100% HEIGHT=100% valign=top>
							...
							<TR>
								<TD HEIGHT="100%">
									<script language =javascript src='./js/h1007ma1_vspdData_vspdData.js'></script>
								</TD>
							</TR>
						</TABLE>
					</TD>
