...
Const BIZ_PGM_ID = "ksm111qb1.asp"
Const BIZ_PGM_ID2 = "ksm111qb01.asp"
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetEdit 	C_CFM_FLAG		,"수주확정여부"		,10		'수주확정여부 
...
Sub SetSpreadColor(ByVal pvStartRow, ByVal pvEndRow)
	With frm1.vspdData
		...
		ggoSpread.SSSetProtected 	C_SOLD_TO_PARTY		, pvStartRow, pvEndRow		'발주법인 
		...
		ggoSpread.SSSetProtected 	C_CFM_FLAG		, pvStartRow, pvEndRow		'수주확정여부 
...
Sub InitSpreadPosVariables()
	C_SOLD_TO_PARTY		= 1		'발주법인 
	C_BP_FULL_NM		= 2		'발주법인명 
	C_CUST_PO_NO		= 3		'고객발주번호 
	C_SO_NO			= 4		'수주번호 
	C_EXPORT_FLAG		= 5		'내외자구분 
	C_CFM_FLAG		= 6		'수주확정여부 
	C_SO_DT			= 7		'수주일 
	C_SALES_GRP		= 8		'영업그룹 
	C_SALES_GRP_FULL_NM	= 9		'영업그룹명 
	C_CUR			= 10		'화페 
	C_NET_AMT		= 11		'수주금액 
	C_VAT_AMT		= 12		'부가세금액 
	C_NET_VAT_TOTAMT	= 13		'수주총금액 
	C_VAT_TYPE		= 14		'부가세유형 
	C_VAT_TYPE_NM		= 15		'부가세유형명 
	C_VAT_RATE		= 16		'부가세율 
	C_PAY_METH		= 17		'결제방법 
	C_PAY_METH_NM		= 18		'결제방법명 
	C_INCOTERMS		= 19		'가격조건 
	C_INCOTERMS_NM		= 20		'가격조건명 
End Sub
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
	...
	Select Case UCase(pvSpdNo)
		Case "A"
			...
			C_SOLD_TO_PARTY		= iCurColumnPos(1)	'발주법인 
			C_BP_FULL_NM		= iCurColumnPos(2)	'발주법인명 
			C_CUST_PO_NO		= iCurColumnPos(3)	'고객발주번호 
			C_SO_NO			= iCurColumnPos(4)	'수주번호 
			C_EXPORT_FLAG		= iCurColumnPos(5)	'내외자구분 
			C_CFM_FLAG		= iCurColumnPos(6)	'수주확정여부 
			C_SO_DT			= iCurColumnPos(7)	'수주일 
			C_SALES_GRP		= iCurColumnPos(8)	'영업그룹 
			C_SALES_GRP_FULL_NM	= iCurColumnPos(9)	'영업그룹명 
			C_CUR			= iCurColumnPos(10)	'화페 
			C_NET_AMT		= iCurColumnPos(11)	'수주금액 
			C_VAT_AMT		= iCurColumnPos(12)	'부가세금액 
			C_NET_VAT_TOTAMT	= iCurColumnPos(13)	'수주총금액 
			C_VAT_TYPE		= iCurColumnPos(14)	'부가세유형 
			C_VAT_TYPE_NM		= iCurColumnPos(15)	'부가세유형명 
			C_VAT_RATE		= iCurColumnPos(16)	'부가세율 
			C_PAY_METH		= iCurColumnPos(17)	'결제방법 
			C_PAY_METH_NM		= iCurColumnPos(18)	'결제방법명 
			C_INCOTERMS		= iCurColumnPos(19)	'가격조건 
			C_INCOTERMS_NM		= iCurColumnPos(20)	'가격조건명 
...
Function CookiePage(Byval Kubun)
	...
	If Kubun = 0 Then
		...
		If ReadCookie("txtPO_NO") <> "" Then
			frm1.txtPO_NO.value = ReadCookie("txtPO_NO")
		End If
	...
	With frm1
		If lgIntFlgMode = parent.OPMD_UMODE Then
			...
			if .rdoCfmFlag(0).checked = true Then					'수주확정여부 
				strVal = strVal & "&rdoCfmFlag=" & ""	'전체 
			elseif .rdoCfmFlag(1).checked = true then
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>멀티컴퍼니 수주조회</font></td>
						...
						<TABLE <%=LR_SPACE_TYPE_40%>>
								...
								<TD CLASS="TD6" NOWRAP>
									<table cellspacing=0 cellpadding=0>
										<tr>
											<td>
												<script language =javascript src='./js/ksm111qa1_fpDateTime2_txtFrDt.js'></script>
											</td>
											<td>~</td>
											<td>
												<script language =javascript src='./js/ksm111qa1_fpDateTime2_txtToDt.js'></script>
											</td>
										<tr>
									</table>
								</TD>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP>
									<table cellspacing=0 cellpadding=0>
										<tr>
											<td>
												<script language =javascript src='./js/ksm111qa1_fpDateTime2_txtSo_Frdt.js'></script>
											</td>
											<td>~</td>
											<td>
												<script language =javascript src='./js/ksm111qa1_fpDateTime2_txtSo_Todt.js'></script>
											</td>
										<tr>
									</table>
								</TD>
								...
								<TD CLASS="TD6" NOWRAP>
									<input type=radio CLASS="RADIO" name="rdoCfmFlag" id="rdoCfmFlagY" value="N" tag = "11" checked>
										<label for="rdoCfmFlag">전체</label>&nbsp;&nbsp;&nbsp;&nbsp;
									<input type=radio CLASS = "RADIO" name="rdoCfmFlag" id="rdoCfmFlagN" value="Y" tag = "11" >
				  ...
				  <TABLE <%=LR_SPACE_TYPE_60%>>
					<TR HEIGHT=60%>
						<TD WIDTH=100% COLSPAN=4>
							<TABLE <%=LR_SPACE_TYPE_20%>>
								<TR>
									<TD HEIGHT="100%">
										<script language =javascript src='./js/ksm111qa1_A_vspdData.js'></script>
									</TD>
								</TR>
							</TABLE>
						</TD>
					...
					<TR HEIGHT= 40%>
						<TD HEIGHT=100% WIDTH=100% COLSPAN=4>
						     <script language =javascript src='./js/ksm111qa1_B_vspdData2.js'></script>
						</TD>
					</TR>
				  </TABLE>
				 </TD>
