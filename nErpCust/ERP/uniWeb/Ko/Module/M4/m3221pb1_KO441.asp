...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "M3221QA001"  										' main query(spread sheet에 뿌려지는 query statement)
	...
	IF Len(Trim(Request("txtFrDt"))) THEN 
		strVal = strVal & " AND AMEND_REQ_DT >= " & FilterVar(UniconvDate(Trim(Request("txtFrDt"))), "''", "S") & " "		
	END IF
		
	IF Len(Trim(Request("txtToDt"))) THEN 
		strVal = strVal & " AND AMEND_REQ_DT <= " & FilterVar(UniconvDate(Trim(Request("txtToDt"))), "''", "S") & " "		
	END IF

	IF Len(Trim(Request("gBizArea"))) THEN 
		strVal = strVal & " AND BIZ_AREA = " & FilterVar(UniconvDate(Trim(Request("gBizArea"))), "''", "S") & " "		
	END IF

	IF Len(Trim(Request("gPurGrp"))) THEN 
		strVal = strVal & " AND PUR_GRP = " & FilterVar(UniconvDate(Trim(Request("gPurGrp"))), "''", "S") & " "		
	END IF

	IF Len(Trim(Request("gPurOrg"))) THEN 
		strVal = strVal & " AND PUR_ORG = " & FilterVar(UniconvDate(Trim(Request("gPurOrg"))), "''", "S") & " "		
	END IF
