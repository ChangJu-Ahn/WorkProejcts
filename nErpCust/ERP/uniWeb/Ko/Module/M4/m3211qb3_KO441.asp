...
'----------------------------------------------------------------------------------------------------------
 Sub FixUNISQLData()
    ...
    Redim UNIValue(6,14)                                                  '⊙: DB-Agent로 전송될 parameter를 위한 변수 
     ...
     UNISqlId(5) = "M3211QA302"								              '근거서류유형명     	
																		  'Reusage is Recommende
     ...
     UNIValue(0,11) = UCase(Trim(strOLcKindFrom))       '---근거서류유형 
     UNIValue(0,12) = UCase(Trim(strOLcKind))
     ...
     UNIValue(5,0)  = UCase(Trim(strOLcKind))
...
'----------------------------------------------------------------------------------------------------------
 Sub QueryData()
    ...
    If  rs5.EOF And rs5.BOF Then
        rs5.Close
        Set rs5 = Nothing
        If Len(Request("txtOLcKind")) And FalsechkFlg = False Then
		   Call DisplayMsgBox("970000", vbInformation, "근거서류유형", "", I_MKSCRIPT)	'⊙: you must release this line if you change msg into code
	       FalsechkFlg = True	
		End If
    Else    
		arrRsVal(8) = rs5(0)
...
'----------------------------------------------------------------------------------------------------------
 Sub TrimData()
    ...
    '---가격조건 
    If Len(Trim(Request("txtOLcKind"))) Then
    	strOLcKind	= " " & FilterVar(Trim(UCase(Request("txtOLcKind"))), " " , "S") & " "
    	strOLcKindFrom = strOLcKind
    Else
    	strOLcKind	= "" & FilterVar("zzzzzzzzz", "''", "S") & ""
    	strOLcKindFrom = "''"
    End If
...
<Script Language=vbscript>
    With Parent
		...
		Call .ReFormatSpreadCellByCellByCurrency2(.frm1.vspdData,"<%=iFrPoint+1%>",.frm1.vspddata.maxrows, Parent.Parent.gCurrency,.GetKeyPos("A",12),"D","Q","X","X")
        ...
        .frm1.hdnOLcKind.value			= "<%=ConvSPChars(Request("txtOLcKind"))%>"
		...
		.frm1.txtPlantNm.value			=  "<%=ConvSPChars(arrRsVal(1))%>" 	
  		...
  		.frm1.txtOLcKindNm.value		=  "<%=ConvSPChars(arrRsVal(9))%>"
