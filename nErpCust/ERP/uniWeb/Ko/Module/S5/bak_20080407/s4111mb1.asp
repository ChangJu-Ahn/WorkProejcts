...
Sub SubBizQuery()
	...
	'--출고예정일--
	Response.Write ".txtPlanned_gi_dt.Text		= """ & UNIDateClientFormat(iArrSDnHdr(S5G211_RS_PROMISE_DT))	& """" & vbcr
	'--실제출고일--
	Response.Write ".txtActGi_dt.value			= """ & UNIDateClientFormat(iArrSDnHdr(S5G211_RS_ACTUAL_GI_DT))	& """" & vbcr
	'--납품장소--
	Response.Write ".txtShip_to_place.value		= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_SHIP_TO_PLCE))			& """" & vbcr
	'--운송방법--
	...
	'--납기일--
	Response.Write ".txtDlvy_dt.value			= """ & UNIDateClientFormat(iArrSDnHdr(S5G211_RS_DLVY_DT))		& """" & vbcr
	'--영업그룹--
	...
	'--출하타입--
	Response.Write ".txtMovType.value			= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_MOV_TYPE))				& """" & vbcr
	'--출하타입명--
	Response.Write ".txtMovTypeNm.value			= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_MOV_TYPE_NM))			& """" & vbcr
	'--출고번호--
	Response.Write ".txtGoods_mv_no.value		= """ & Trim(ConvSPChars(iArrSDnHdr(S5G211_RS_GOODS_MV_NO)))	& """" & vbcr
	'--수주번호--
	Response.Write ".txtSo_no.value				= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_SO_NO))				& """" & vbcr
	'--수주번호지정--
	If iArrSDnHdr(S5G211_RS_SO_NO) <> "" Then
		Response.Write ".chkSoNo.checked = True" & vbcr
	End If
	'--수주타입--
	Response.Write ".txtSo_Type.value			= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_SO_TYPE))				& """" & vbcr
	'--수주타입명--
	Response.Write ".txtSo_TypeNm.value			= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_SO_TYPE_NM))			& """" & vbcr
	'--납품처--
	...
	'--납품처상세정보번호--
	Response.Write ".txtSTP_Inf_No.value	= """ & ConvSPChars(iArrSDnHdr(S5G211_RS_STP_INFO_NO))		& """" & vbcr
	'--운송정보번호--
...
Sub SubBizSave()
    ...
    Const S5G211_DnHdr_EXT1_QTY = 11       '(O)여유필드(수량)
    Const S5G211_DnHdr_EXT2_QTY = 12       '(O)여유필드(수량)
    Const S5G211_DnHdr_EXT3_QTY = 13       '(O)여유필드(수량)
    Const S5G211_DnHdr_EXT1_AMT = 14       '(O)여유필드(금액)
    Const S5G211_DnHdr_EXT2_AMT = 15       '(O)여유필드(금액)
    Const S5G211_DnHdr_EXT3_AMT = 16       '(O)여유필드(금액)
    Const S5G211_DnHdr_EXT1_CD = 17        '(O)여유필드(Text)
    Const S5G211_DnHdr_EXT2_CD = 18        '(O)여유필드(Text)
    Const S5G211_DnHdr_EXT3_CD = 19        '(O)여유필드(Text)
    Const S5G211_DnHdr_TEMP_SO_NO = 20     '(O)S/O 번호 
	...
	Redim iArrDnHdr(S5G211_DnHdr_INV_MGR)
    ...
    '-----------------------
	iArrDnHdr(S5G211_DnHdr_DN_NO) = UCase(Trim(Request("txtDnNo")))						'(O)출하번호 
	iArrDnHdr(S5G211_DnHdr_EXCEPT_DN_FLAG) = "N"										'(M)예외출고여부 
	iArrDnHdr(S5G211_DnHdr_SHIP_TO_PARTY) = UCase(Trim(Request("txtShip_to_party")))	'(M)납품처 
	iArrDnHdr(S5G211_DnHdr_SALES_GRP) = UCase(Trim(Request("txtSales_Grp")))			'(M)영업그룹 
	iArrDnHdr(S5G211_DnHdr_MOV_TYPE) = UCase(Trim(Request("txtMovType")))				'(M)출하형태 
	iArrDnHdr(S5G211_DnHdr_DLVY_DT) = UNIConvDate(Request("txtDlvy_dt"))				'(M)납기일 
	iArrDnHdr(S5G211_DnHdr_PROMISE_DT) = UNIConvDate(Request("txtPlanned_gi_dt"))		'(M)출고예정일 
	iArrDnHdr(S5G211_DnHdr_TRANS_METH) = UCase(Trim(Request("txtTrans_meth")))			'(O)운송방법 
	iArrDnHdr(S5G211_DnHdr_SO_TYPE) = UCase(Trim(Request("txtSo_Type")))					'(M)수주형태 
	iArrDnHdr(S5G211_DnHdr_SO_NO) = UCase(Trim(Request("txtSo_no")))					'(O)S/O번호 
	iArrDnHdr(S5G211_DnHdr_SHIP_TO_PLCE) = Trim(Request("txtShip_to_place"))			'(O)납품장소 
	iArrDnHdr(S5G211_DnHdr_EXT1_QTY) = 0												'(O)여유필드(수량)
	iArrDnHdr(S5G211_DnHdr_EXT2_QTY) = 0												'(O)여유필드(수량)
	iArrDnHdr(S5G211_DnHdr_EXT3_QTY) = 0       											'(O)여유필드(수량)
	iArrDnHdr(S5G211_DnHdr_EXT1_AMT) = 0       											'(O)여유필드(금액)
	iArrDnHdr(S5G211_DnHdr_EXT2_AMT) = 0       											'(O)여유필드(금액)
	iArrDnHdr(S5G211_DnHdr_EXT3_AMT) = 0       											'(O)여유필드(금액)
	iArrDnHdr(S5G211_DnHdr_EXT1_CD) = ""												'(O)여유필드(Text)
	iArrDnHdr(S5G211_DnHdr_EXT2_CD) = ""		    									'(O)여유필드(Text)
	iArrDnHdr(S5G211_DnHdr_EXT3_CD) = ""	        									'(O)여유필드(Text)
	iArrDnHdr(S5G211_DnHdr_TEMP_SO_NO) = UCase(Trim(Request("txtTempSoNo")))   			'(O)S/O 번호 
	'iArrDnHdr(S5G211_DnHdr_CUR) = ""            										'(O)화폐단위 
	'iArrDnHdr(S5G211_DnHdr_XCHG_RATE) = ""      										'(0)환율 
	'iArrDnHdr(S5G211_DnHdr_XCHG_RATE_OP) = ""   										'(O)환율연산자 
	iArrDnHdr(S5G211_DnHdr_REMARK) = UCase(Trim(Request("txtRemark")))					'(O)비고 
	iArrDnHdr(S5G211_DnHdr_ARRIVAL_DT) = UNIConvDate(Request("txtArriv_dt"))			'(O)실제납품일 
	iArrDnHdr(S5G211_DnHdr_ARRIVAL_TIME) = Trim(Request("txtArriv_Tm"))					'(O)납품시간 
	iArrDnHdr(S5G211_DnHdr_SO_AUTO_FLAG) = "N"											'(O)납품시간 
	iArrDnHdr(S5G211_DnHdr_PLANT_CD) = Trim(Request("txtPlantCd"))						'(M)공장 
	iArrDnHdr(S5G211_DnHdr_INV_MGR) = Trim(Request("txtInvMgr"))						'(O)재고담당 
    ...
    Call iObjS5G211.Maintain(pvCB, gStrGlobalCollection, iStrCUDFlag, iArrDnHdr, , "Y",_
...
Sub SubBizDelete()
    ...
    Call iObjS5G211.Maintain(pvCB, gStrGlobalCollection, iStrCUDFlag, iArrDnHdr)
