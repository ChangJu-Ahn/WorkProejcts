...
Const BIZ_PGM_ID = "s4511mb1.asp"												'☆: Head Query 비지니스 로직 ASP명 
...
Function OpenSORef()
	...
	iCalledAspName = AskPRAspName("S4511AA1")
		
	if Trim(iCalledAspName) = "" then
		IntRetCD = DisplayMsgBox("900040",parent.VB_INFORMATION, "S4511AA1", "x")
		IsOpenPop = False
		exit Function
	end if
...
Function SetSORef(ByRef prArrRet) 
	...
	With frm1
		...
		.txtDlvy_dt.value = prArrRet(2)					'납기일	
		.txtMovType.value = prArrRet(3)					'출하형태 
		.txtMovTypeNm.value = prArrRet(4)				'출하형태명 
		.txtSo_no.value = prArrRet(5)					'수주번호 
		.txtTempSoNo.value = prArrRet(5)					'수주번호 
		.chkSoNo.checked  = True
'		.chkSoNo.disabled = True
		.txtSales_Grp.value = prArrRet(6)				'영업그룹 
		.txtSales_GrpNm.value = prArrRet(7)				'영업그룹명	
		.txtSo_Type.value = prArrRet(8)					'수주형태	
		.txtSo_TypeNm.value = prArrRet(9)				'수주형태명 
		.txtTrans_meth.value = prArrRet(10)				'운송방법 
		.txtTrans_meth_nm.value = prArrRet(11)			'운송방법명 
		.txtPlantCd.value = prArrRet(12)				'공장 
		.txtPlantNm.value = prArrRet(13)				'공장명 
