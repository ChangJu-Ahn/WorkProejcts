...
Sub FixUNISQLData()
	...
	If Request("lgStrAllocInvFlag") = "N" Then
		UNISqlId(0) = "S3111AA101"
	Else
		UNISqlId(0) = "S3111AA102"
	End If
	...
	' 출고예정일 
	iStrVal = iStrVal & " AND SC.PROMISE_DT >=  " & FilterVar(UNIConvDate(Request("txtConFrPromiseDt")), "''", "S") & ""			
	iStrVal = iStrVal & " AND SC.PROMISE_DT <=  " & FilterVar(UNIConvDate(Request("txtConToPromiseDt")), "''", "S") & ""		
	...
	' 납기일 
	If Len(Request("txtConFrDlvyDt")) Then
		iStrVal = iStrVal & " AND SC.DLVY_DT >=  " & FilterVar(UNIConvDate(Request("txtConFrDlvyDt")), "''", "S") & ""		
	End If		
	
	If Len(Request("txtConToDlvyDt")) Then
		iStrVal = iStrVal & " AND SC.DLVY_DT <=  " & FilterVar(UNIConvDate(Request("txtConToDlvyDt")), "''", "S") & ""		
	End If	
	...
	If Len(Request("txtConSoNo")) Then
		iStrVal = iStrVal & " AND SC.SO_NO =  " & FilterVar(Request("txtConSoNo"), "''", "S") & ""	
	End If
...
Sub SetHiddenQueryCon()
    ...
    Response.Write " .HFrPromiseDt.value = """ & Request("txtConFrPromiseDt") & """" & vbCr
    Response.Write " .HToPromiseDt.value = """ & Request("txtConToPromiseDt") & """" & vbCr
