...
Dim LngRow
Dim iCommand
Dim lgObjConn   '2008-04-17 10:55오전 :: hanc
...
Sub SubBizSaveMultiUpdate(arrColVal, s_mvmt_rcpt_no)
    ...
    '--------------------------------------------------------------------------------------------------------
            'lgStrSQL = "UPDATE T_IF_RCV_PART_OUT_KO441 SET "
            'lgStrSQL = lgStrSQL & " ERP_APPLY_FLAG   = 'Y',  "
            'lgStrSQL = lgStrSQL & " UPDT_USER_ID     = 'unierp', "
            'lgStrSQL = lgStrSQL & " UPDT_DT          = GetDate()," 
            'lgStrSQL = lgStrSQL & " GOODS_MV_NO     = " &  FilterVar(Trim(UCase(s_mvmt_rcpt_no)),"''","S")
            'lgStrSQL = lgStrSQL & " WHERE TRANS_TIME    = " &  FilterVar(Trim(UCase(arrColVal(1))),"''","S")
            'lgStrSQL = lgStrSQL & " AND   OUT_NO      = " &  FilterVar(Trim(UCase(arrColVal(2))),"''","S")
            'lgStrSQL = lgStrSQL & " AND   OUT_TYPE_SUB   = " &  FilterVar(Trim(UCase(arrColVal(3))),"''","S")
            'lgStrSQL = lgStrSQL & " AND   CREATE_TYPE   = " &  FilterVar(Trim(UCase(arrColVal(4))),"''","S")
...
'		Response.Write "Call parent.DbQueryOk" & vbCr
	...
	For LngRow = 0 To UBound(EG1_EXP_GRP,1)	
		...
		iArrCols(35) = ConvSPChars(EG1_EXP_GRP(LngRow,EA_s_dn_dtl_ext1_cd2))	' Customer LOT No. 

		iArrCols(36) = iLngLastRow + LngRow
...
Case CStr(UID_M0002)			'☜: 저장 요청을 받음 
	...
	'				내역 저장 
	'===================================================
    Dim itxtSpreadIns, itxtSpreadUpd, itxtSpreadDel, itxtSpreadKey
    Dim iIntIndex
    Dim itxtSpreadArr
    Dim iCCount, iUCount, iDCount, iKCount
    ...
    iKCount  = Request.Form("txtKSpread").Count
    ...
    ' key
    ReDim itxtSpreadArr(iKCount)
    For iIntIndex = 1 To iKCount
        itxtSpreadArr(iIntIndex) = Request.Form("txtKSpread")(iIntIndex)
    Next
    itxtSpreadKey = Join(itxtSpreadArr,"")
    ...
    If Trim(itxtSpreadIns + itxtSpreadUpd + itxtSpreadDel) <> "" Then
        ...
        Dim s_mvmt_rcpt_no
            ...
            s_mvmt_rcpt_no  =    UCase(ConvSPChars(iStrDnNo ))
    
        	arrRowVal = Split(itxtSpreadKey, gRowSep)                                 '☜: Split Row    data
