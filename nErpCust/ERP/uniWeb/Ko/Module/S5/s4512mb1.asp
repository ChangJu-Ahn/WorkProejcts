...
Case CStr(UID_M0001)														'☜: 현재 조회/Prev/Next 요청을 받음 
    '==================== 조회결과 Index ==================
	Const S5G221_RS_DN_REQ_NO = 0                ' 출하번호 
	Const S5G221_RS_SHIP_TO_PARTY = 1        ' 납품처 
	Const S5G221_RS_SHIP_TO_PARTY_NM = 2     ' 납품처명 
	Const S5G221_RS_MOV_TYPE = 7             ' 출하형태 
	Const S5G221_RS_MOV_TYPE_NM = 8          ' 출하형태명 
	Const S5G221_RS_DLVY_DT = 9				 ' 납기일 
	Const S5G221_RS_PROMISE_DT = 10          ' 출고예정일 
	Const S5G221_RS_ACTUAL_GI_DT = 11        ' 출고일 
	Const S5G221_RS_TRANS_METH = 15          ' 운송방법 
	Const S5G221_RS_TRANS_METH_NM = 16       ' 운송방법명 
	Const S5G221_RS_GOODS_MV_NO = 17         ' 수불번호(출고번호)
	Const S5G221_RS_CI_FLAG = 18             ' 통관필요여부 
	Const S5G221_RS_POST_FLAG = 19           ' 출고처리여부 
	Const S5G221_RS_SO_TYPE = 20             ' 수주형태 
	Const S5G221_RS_SO_TYPE_NM = 21          ' 수주형태명 
	Const S5G221_RS_SO_NO = 22               ' 수주번호 
	Const S5G221_RS_VAT_FLAG = 38            ' 세금계산서정보 동시생성여부 
	Const S5G221_RS_AR_FLAG = 39             ' 매출정보 동시생성여부 
	Const S5G221_RS_PLANT_CD = 72            ' 공장 
	Const S5G221_RS_PLANT_NM = 73            ' 공장명 
	Const S5G221_RS_INV_MGR = 74             ' 재고담당 
	Const S5G221_RS_INV_MGR_NM = 75          ' 재고담당자명 
	Const S5G221_RS_RET_ITEM_FLAG = 77       ' 반품여부 
	Const S5G221_RS_REL_BILL_FLAG = 78       ' 매출여부 
	Const S5G221_RS_EXPORT_FLAG = 79         ' 수출여부 
    ...
    Set iObjS5G2111 = Server.CreateObject("PS5G221.cLookUpSDnReqHdr")
    ...
    iArrSDnReqHdr = iObjS5G2111.LookUp(gStrGlobalCollection, iStrDnNo, "N")
	...
	Response.Write ".txtHDnNo.value				= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_DN_REQ_NO))				& """" & vbcr	' 출하번호 

	Response.Write ".txtSoNo.value				= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_SO_NO))				& """" & vbcr	' 수주번호 
	Response.Write ".txtShipToParty.value		= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_SHIP_TO_PARTY))		& """" & vbcr	' 납품처 
	Response.Write ".txtShipToPartyNm.value		= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_SHIP_TO_PARTY_NM))		& """" & vbcr	' 납품처명 
	Response.Write ".txtDnType.value			= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_MOV_TYPE))				& """" & vbcr	' 출하타입 
	Response.Write ".txtDnTypeNm.value			= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_MOV_TYPE_NM))			& """" & vbcr	' 출하타입명 
	Response.Write ".txtPlannedGIDt.value		= """ & UNIDateClientFormat(iArrSDnReqHdr(S5G221_RS_PROMISE_DT))	& """" & vbcr	' 출고예정일 
	Response.Write ".txtDlvyDt.value			= """ & UNIDateClientFormat(iArrSDnReqHdr(S5G221_RS_DLVY_DT))		& """" & vbcr	' 납기일 
''''	Response.Write ".txtGINo.value		= """ & Trim(ConvSPChars(iArrSDnReqHdr(S5G221_RS_GOODS_MV_NO)))	& """" & vbcr			' 출고번호 
	'--실제출고일--
''''	If Trim(iArrSDnReqHdr(S5G221_RS_GOODS_MV_NO)) <> "" Then
''''		Response.Write ".txtActualGIDt.Text			= """ & UNIDateClientFormat(iArrSDnReqHdr(S5G221_RS_ACTUAL_GI_DT))	& """" & vbcr
''''	End If

	' 매출채권 
''''	If UCase(iArrSDnReqHdr(S5G221_RS_AR_FLAG)) = "Y" Then
...
''''	End If
	...
	' 세금계산서 
''''	If UCase(iArrSDnReqHdr(S5G221_RS_VAT_FLAG)) = "Y" Then
...
''''	End If
	...
	Response.Write ".txtSoType.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_SO_TYPE))			& """" & vbcr	' 수주타입 
	Response.Write ".txtSoTypeNm.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_SO_TYPE_NM))		& """" & vbcr	' 수주타입명 
	Response.Write ".txtPlantCd.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_PLANT_CD))			& """" & vbcr	' 공장 
	Response.Write ".txtPlantNm.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_PLANT_NM))			& """" & vbcr	' 공장명 
''''	Response.Write ".txtInvMgr.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_INV_MGR))			& """" & vbcr	' 재고담당자 
''''	Response.Write ".txtInvMgrNm.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_INV_MGR_NM))		& """" & vbcr	' 재고담당자명 

	Response.Write ".txtHRetFlag.value	= """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_RET_ITEM_FLAG))	& """" & vbcr	' 반품여부 
	Response.Write ".txtRetBillFlag.value = """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_REL_BILL_FLAG))	& """" & vbcr	' 매출여부 
	Response.Write ".txtExportFlag.value  = """ & ConvSPChars(iArrSDnReqHdr(S5G221_RS_EXPORT_FLAG))	& """" & vbcr	' 수출여부 
    ...
    Set pS5G228 = Server.CreateObject("pS5G228.cSListDnReqDtl")
	...
	call pS5G228.S_LIST_DN_REQ_DTL(gStrGlobalCollection, C_SHEETMAXROWS_D, i1_s_dn_dtl, i2_s_dn_hdr, eg1_exp_grp)
	...
	For LngRow = 0 To UBound(EG1_EXP_GRP,1)	
		...
		iArrCols(44) = iLngLastRow + LngRow
...
Case CStr(UID_M0002)																'☜: 저장 요청을 받음 
	...
	Set pS5G521 = Server.CreateObject("pS5G521.cSDnReqDtlSvr")

	If CheckSYSTEMError(Err,True) Then Response.End       

	' Maintain(ByVal pvStrGlobalCollection As String, _
    '          ByVal pvStrExceptDnFlag As String, _
    '          ByVal pvStrDnNo As String, _
    '          ByVal pvstrBatchDel As String, _
    '          Optional ByVal pvStrDataToInsert As String, _
    '          Optional ByVal pvStrDataToUpdate As String, _
    '          Optional ByVal pvStrDataToDelete As String, _
    '          Optional ByRef prErrorPosition As Variant, _
	'          Optional ByVal pvCb As String, _
	'          Optional ByVal pvICustomXML As String, _
	'          Optional ByRef prOCustomXML As Variant)
	Call pS5G521.Maintain(gStrGlobalCollection, "N", Request("txtHDnNo"), "N", Request("txtSpreadIns"), Request("txtSpreadUpd"), Request("txtSpreadDel"), iErrorPosition, "F")
...
''''	i3_s_dn_hdr(S427_I3_actual_gi_dt) = UNIConvDate(Request("txtActualGIDt"))
	i3_s_dn_hdr(S427_I3_actual_gi_dt) = UNIConvDate("")
