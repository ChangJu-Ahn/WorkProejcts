...
'Call ServerMesgBox("SubBizQueryMulti " , vbInformation, I_MKSCRIPT)
...
'Call ServerMesgBox("SubBizQueryMulti : " & FilterVar(Trim(Request("txtPlantCd")),"''", "S") , vbInformation, I_MKSCRIPT)
'Call ServerMesgBox("SubBizQueryMulti : " & FilterVar(Trim(Request("txtFromDt")),"''", "S") , vbInformation, I_MKSCRIPT)
	
    '---------- Developer Coding part (Start) ---------------------------------------------------------------


    lgStrSQL	= " SELECT	TOP " & iSelCount & " OUT_NO,                " & vbcrlf
    ...
    lgStrSQL = lgStrSQL & "       OUT_TYPE_SUB,                    " & vbcrlf
    ...
    lgStrSQL = lgStrSQL & "       SL_CD,                       " & vbcrlf
    lgStrSQL = lgStrSQL & "       ITEM_NM,                     " & vbcrlf
    lgStrSQL = lgStrSQL & "       SPEC,                        " & vbcrlf
    lgStrSQL = lgStrSQL & "       [DBO].[ufn_GetVatType](SHIP_TO_PARTY) VAT_TYPE,                 " & vbcrlf
    lgStrSQL = lgStrSQL & "       [DBO].[ufn_GetVatTypeNm](SHIP_TO_PARTY)VAT_TYPE_NM,             " & vbcrlf
    lgStrSQL = lgStrSQL & "       [DBO].[ufn_GetVatIncFlag](SHIP_TO_PARTY) VAT_INC_FLAG,          " & vbcrlf
    lgStrSQL = lgStrSQL & "       [DBO].[ufn_GetVatIncFlagNM](SHIP_TO_PARTY) VAT_INC_FLAG_NM,     "   & vbcrlf
    lgStrSQL = lgStrSQL & "       TRANS_TIME,                                                     " & vbcrlf
    lgStrSQL = lgStrSQL & "       CREATE_TYPE,                                                    " & vbcrlf
    lgStrSQL = lgStrSQL & "       PRICE,                       " & vbcrlf
    lgStrSQL = lgStrSQL & "	   		FLAG  ,   CUST_LOT_NO                        " & vbcrlf
    ...
    lgStrSQL = lgStrSQL & " 		A.OUT_TYPE_SUB,                     " & vbcrlf
    ...
    lgStrSQL = lgStrSQL & " 		[DBO].[ufn_GetSlCd_issued](A.PLANT_CD, [DBO].[ufn_GetItemCd](A.MES_ITEM_CD)) SL_CD,                        " & vbcrlf
    lgStrSQL = lgStrSQL & " 		D.ITEM_NM,              " & vbcrlf
    lgStrSQL = lgStrSQL & " 		D.SPEC,                    " & vbcrlf
    lgStrSQL = lgStrSQL & " 		A.PRICE,                         " & vbcrlf
    lgStrSQL = lgStrSQL & " 		A.TRANS_TIME,                         " & vbcrlf
    lgStrSQL = lgStrSQL & " 		CREATE_TYPE,                         " & vbcrlf
    lgStrSQL = lgStrSQL & " 		'A' FLAG     ,A.CUST_LOT_NO                    " & vbcrlf
    ...
    lgStrSQL = lgStrSQL & "         (SELECT COUNT(*) CNT, OUT_NO, TRANS_TIME, OUT_TYPE_SUB                         " & vbcrlf
    lgStrSQL = lgStrSQL & "         FROM  T_IF_RCV_PART_OUT_KO441                                               " & vbcrlf                                                                                                            
    lgStrSQL = lgStrSQL & "         WHERE PLANT_CD = " & FilterVar(Trim(Request("txtPlantCd")),"''", "S") & " " & vbcrlf
    lgStrSQL = lgStrSQL & "                AND CONVERT(CHAR(10), ACTUAL_GI_DT, 120) BETWEEN " & FilterVar(Trim(Request("txtFromDt")),"''", "S") & " AND " & FilterVar(Trim(Request("txtToDt")),"''", "S") & "  " & vbcrlf
    lgStrSQL = lgStrSQL & "         GROUP BY OUT_NO, TRANS_TIME, OUT_TYPE_SUB  " & vbcrlf
    lgStrSQL = lgStrSQL & "         HAVING COUNT(*) = 1) J " & vbcrlf
    lgStrSQL = lgStrSQL & " WHERE  A.SHIP_TO_PARTY = B.BP_ALIAS_NM       " & vbcrlf
    lgStrSQL = lgStrSQL & " AND A.MES_ITEM_CD = D.CBM_DESCRIPTION   AND D.ITEM_ACCT IN ('10','20')     " & vbcrlf
    lgStrSQL = lgStrSQL & " AND A.OUT_TYPE = 'P01' " & vbcrlf
    LGSTRSQL = LGSTRSQL & " AND A.OUT_NO = J.OUT_NO            " & vbcrlf
    LGSTRSQL = LGSTRSQL & " AND A.TRANS_TIME = J.TRANS_TIME    " & vbcrlf
    LGSTRSQL = LGSTRSQL & " AND A.OUT_TYPE_SUB = J.OUT_TYPE_SUB        " & vbcrlf
    lgStrSQL = lgStrSQL & " AND A.OUT_TYPE = C.UD_MINOR_CD       " & vbcrlf
    lgStrSQL = lgStrSQL & " AND C.UD_MAJOR_CD = 'ZZ002'       " & vbcrlf
    lgStrSQL = lgStrSQL & " AND	CONVERT(CHAR(10), A.ACTUAL_GI_DT, 120) BETWEEN " & FilterVar(Trim(Request("txtFromDt")),"''", "S") & " AND " & FilterVar(Trim(Request("txtToDt")),"''", "S") & "  " & vbcrlf
    lgStrSQL = lgStrSQL & " AND		B.BP_CD LIKE " & FilterVar(Trim(Request("txtShipToParty"))&"%","''", "S") & " " & vbcrlf
    lgStrSQL = lgStrSQL & " AND		A.SHIP_TO_PARTY LIKE '%'            " & vbcrlf
    ...
    lgStrSQL = lgStrSQL & " 		A.TRANS_TIME,                         " & vbcrlf
    lgStrSQL = lgStrSQL & " 		CREATE_TYPE,                         " & vbcrlf
    lgStrSQL = lgStrSQL & " 		'B' FLAG , A.CUST_LOT_NO                        " & vbcrlf
    lgStrSQL = lgStrSQL & " FROM	T_IF_RCV_PART_OUT_KO441 A,       " & vbcrlf
    lgStrSQL = lgStrSQL & " 		B_BIZ_PARTNER B,       " & vbcrlf
    lgStrSQL = lgStrSQL & " 		B_USER_DEFINED_MINOR C,       " & vbcrlf
    lgStrSQL = lgStrSQL & " 		B_ITEM   D,    " & vbcrlf
...
'Call ServerMesgBox("SubBizQueryMulti lgStrSQL : " & lgStrSQL, vbInformation, I_MKSCRIPT)

    If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then                   'R(Read) X(CursorType) X(LockType) 
'Call ServerMesgBox("SubBizQueryMulti : here 400 " , vbInformation, I_MKSCRIPT)
        ...
        Exit Sub 
    Else    
'Call ServerMesgBox("SubBizQueryMulti : here 401 " , vbInformation, I_MKSCRIPT)
       ...
       Do While Not lgObjRs.EOF
          ...
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("VAT_TYPE_NM"))
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("VAT_INC_FLAG"))
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("VAT_INC_FLAG_NM"))
