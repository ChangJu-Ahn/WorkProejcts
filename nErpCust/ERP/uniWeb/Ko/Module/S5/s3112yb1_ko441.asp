...
Sub SubBizQueryMulti()
    ...
    lgStrSQL = lgStrSQL & " 		A.OUT_TYPE_SUB,                     "
    ...
    lgStrSQL = lgStrSQL & " 		[DBO].[ufn_GetISSUESlCd](A.PLANT_CD, [DBO].[ufn_GetItemCd](A.MES_ITEM_CD)) SL_CD,                        "
    lgStrSQL = lgStrSQL & " 		D.ITEM_NM,              "
    lgStrSQL = lgStrSQL & " 		D.SPEC,                    "
    lgStrSQL = lgStrSQL & " 		A.PRICE  ,                       "
    lgStrSQL = lgStrSQL & " 		[DBO].[ufn_GetVatType](B.BP_CD) VAT_TYPE,            "
    lgStrSQL = lgStrSQL & " 		[DBO].[ufn_GetVatTypeNm](B.BP_CD)VAT_TYPE_NM,          "
    lgStrSQL = lgStrSQL & " 		[DBO].[ufn_GetVatIncFlag](B.BP_CD) VAT_INC_FLAG,         "
    lgStrSQL = lgStrSQL & " 		[DBO].[ufn_GetVatIncFlagNM](B.BP_CD) VAT_INC_FLAG_NM,       "
    ...
    lgStrSQL = lgStrSQL & "         (SELECT COUNT(*) CNT, OUT_NO, TRANS_TIME, OUT_TYPE_SUB                         "                                                                                                           
    lgStrSQL = lgStrSQL & "         FROM  T_IF_RCV_PART_OUT_KO441                                               "                                                                                                                       
    lgStrSQL = lgStrSQL & "         WHERE PLANT_CD = " & FilterVar(Trim(Request("txtPlantCd")),"''", "S") & " "
    lgStrSQL = lgStrSQL & "                AND CONVERT(CHAR(10), ACTUAL_GI_DT, 120) BETWEEN " & FilterVar(Trim(Request("txtFromDt")),"''", "S") & " AND " & FilterVar(Trim(Request("txtToDt")),"''", "S") & "  "
    lgStrSQL = lgStrSQL & "         GROUP BY OUT_NO, TRANS_TIME, OUT_TYPE_SUB                                                                                                                                                 "
    lgStrSQL = lgStrSQL & "         HAVING COUNT(*) = 1) J "
    lgStrSQL = lgStrSQL & " WHERE  A.SHIP_TO_PARTY = B.BP_ALIAS_NM       "
    lgStrSQL = lgStrSQL & " AND A.MES_ITEM_CD = D.CBM_DESCRIPTION  AND D.ITEM_ACCT IN ('10','20')     "
    lgStrSQL = lgStrSQL & " AND A.OUT_TYPE = C.UD_MINOR_CD       "
    LGSTRSQL = LGSTRSQL & " AND A.OUT_NO = J.OUT_NO            "
    LGSTRSQL = LGSTRSQL & " AND A.TRANS_TIME = J.TRANS_TIME    "
    LGSTRSQL = LGSTRSQL & " AND A.OUT_TYPE_SUB = J.OUT_TYPE_SUB        "
    lgStrSQL = lgStrSQL & " AND C.UD_MAJOR_CD = 'ZZ002'       "
    lgStrSQL = lgStrSQL & " AND A.OUT_TYPE = 'P01' "
