...
Const BIZ_PGM_QRY_ID3   = "s3112ab3_KO441.asp"		    '20080220::hanc	
...
dim C_TRANS_TIME         
dim C_CREATE_TYPE        
Dim C_Select  '20110726 added by songth
'Dim C_OUT_NO
'Dim C_SHIP_TO_PARTY
...
Dim lgSelectAll '20110726 added by songth
...
Sub SetDefaultVal()
	...
	lgSelectAll = False ' 20110726 added by songth
...
Sub InitSpreadPosVariables(ByVal pvSpdNo)
	Select Case UCase(pvSpdNo)
		...
		Case "B"
            ...
            C_rcpt_lot_no             = 18
            C_CUST_LOT_NO             = 19
            C_pgm_name                = 20      '2008-06-16 6:17오후 :: hanc
            C_pgm_price               = 21      '2008-06-16 6:17오후 :: hanc
            C_TRANS_TIME              = 22
            C_CREATE_TYPE             = 23		    
'            C_OUT_NO                  = 1
'            C_BP_CD                   = 2
	...
	Select Case UCase(pvSpdNo)
		...
		Case "B"
        	...
        	With ggoSpread
        		...
        		.Source = vspdData2
        		.Spreadinit "V20030901",,PopupParent.gAllowDragDropSpread
        		'.Spreadinit "V20030314",, PopupParent.gAllowDragDropSpread    
        		...
        		Call AppendNumberPlace("7","5","0")
                .SSSetCheck 	C_Select                 , "",	2,,, 1
        		.SSSetEdit		C_OUT_NO                 , "출하(P/L NO)", 15, 0
        		...
        		.SSSetEdit		C_rcpt_lot_no            , "입고 LOT번호", 15, 0
        		.SSSetEdit		C_CUST_LOT_NO            , "고객 LOT번호", 15, 0
        	...
        	End With
    Case "C" 'added by thsong 20110725
           	Call initSpreadPosVariables(pvSpdNo)    
        	...
        	With ggoSpread
        		...
        		Call AppendNumberPlace("7","5","0")
        		.SSSetEdit		C3_shipdate  , "출고일",10, 0
        		.SSSetEdit		C3_ITEMCD    , "품목코드", 12, 0
        		.SSSetEdit		C3_ItemNm    , "품목명", 40, 0
        		.SSSetFloat		C3_nowQty    , "현재고량" ,13,PopupParent.ggQtyNo,ggStrIntegeralPart, ggStrDeciPointPart,PopupParent.gComNum1000,PopupParent.gComNumDec,,,"Z"       		
        		.SSSetFloat		C3_outQty    , "출고량" ,13,PopupParent.ggQtyNo,ggStrIntegeralPart, ggStrDeciPointPart,PopupParent.gComNum1000,PopupParent.gComNumDec,,,"Z"       		
        	
        		'Call .SSSetColHidden(C_ITEM_NM,C_SPEC1,True)
        		'Call .SSSetColHidden(C_TRANS_TIME,C_TRANS_TIME,True)
        		'Call .SSSetColHidden(C_CREATE_TYPE,C_CREATE_TYPE,True)
        		Call .SSSetColHidden(vspdData3.MaxCols,vspdData3.MaxCols,True)
...
Sub SetSpreadLock()
	...
	'added by thsong 20110725
    ggoSpread.Source = vspdData3
	ggoSpread.SpreadLockWithOddEvenRowColor()	

	vspdData1.OperationMode = 5	'Multi Select Mode
	vspdData2.OperationMode = 0	'single Select Mode   '20110728 changed by songth
	vspdData3.OperationMode = 5
End Sub
...
Function OKClick()
	...
	Dim intColCnt, intRowCnt, intInsRow, iLngSelectedRows,iStrChkStatus,iIntCnt, iSelectCnt
    ...
    If iLngSelectedRows > 0 Then 
			...
			.Redraw = False
			intInsRow = 0
			iSelectCnt = 0 '체크된 수량을 저장하는 버튼 arrReturn함수를 사용해서 몇개를 넘겨줘야 하는지갯수파악이필요
			
			For intRowCnt = 1 To vspdData2.MaxRows
			    Call vspdData2.GetText(1, intRowCnt, iStrChkStatus)
                If iStrChkStatus = "1" Then
                    iSelectCnt = iSelectCnt + 1
                end if
            NEXT
            
            Redim arrReturn(iSelectCnt, 37) '총 몇개를 넘겨줘야 하는지를 여기서 체크함
            
            For intRowCnt = 1 To vspdData2.MaxRows
				vspdData2.Row = intRowCnt
				vspdData1.Row = vspdData1.ActiveRow
				'20110726 added by songth
                Call vspdData2.GetText(1, intRowCnt, iStrChkStatus)

                If iStrChkStatus = "1" Then  '20110726 changed by songth
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       ...
       Case "B"
            ggoSpread.Source = vspdData2
            Call ggoSpread.GetSpreadColumnPos(iCurColumnPos)
            C_Select                 = iCurColumnPos(1)     ' c_select를 적용하기위해 쿼리문에다가 기본체크안되게 0값을 그냥가져오도록 추가해줘야함.   
            C_OUT_NO                 = iCurColumnPos(2)        
            C_BP_CD                  = iCurColumnPos(3)        
            C_BP_NM                  = iCurColumnPos(4)        
            C_ITEM_CD                = iCurColumnPos(5)        
            C_PLANT_CD               = iCurColumnPos(6)        
            C_SL_CD                  = iCurColumnPos(7)        
            C_OUT_TYPE               = iCurColumnPos(8)        
            C_UD_MINOR_NM            = iCurColumnPos(9)        
            C_GOOD_ON_HAND_QTY       = iCurColumnPos(10)        
            C_GI_QTY                 = iCurColumnPos(11)        
            C_GI_UNIT                = iCurColumnPos(12)        
            C_LOT_NO                 = iCurColumnPos(13)        
            C_LOT_SUB_NO             = iCurColumnPos(14)        
            C_ACTUAL_GI_DT           = iCurColumnPos(15)        
            C_ITEM_NM                = iCurColumnPos(16)        
            C_SPEC1                  = iCurColumnPos(17)        
            C_rcpt_lot_no            = iCurColumnPos(18)        
            C_CUST_LOT_NO            = iCurColumnPos(19)        
            C_pgm_name               = iCurColumnPos(20)        
            C_pgm_price              = iCurColumnPos(21)        
            C_TRANS_TIME             = iCurColumnPos(22)        
            C_CREATE_TYPE            = iCurColumnPos(23)     
...
'            C_CREATE_TYPE            = iCurColumnPos(22)  
       ...
       Case "C" 'added by thsong 20110725
            ...
            C3_Shipdate               = iCurColumnPos(1)        
            C3_ItemCd                 = iCurColumnPos(2)        
            C3_ItemNm                 = iCurColumnPos(3)        
            C3_nowQty                    = iCurColumnPos(4)        
            C3_outQty                    = iCurColumnPos(5)        
...
Sub Form_Load()
	...
	Call InitSpreadSheet("C")'added by thsong 20110725
...
Sub vspdData1_Click(ByVal Col , ByVal Row)
	...
	Else
        ...
        Call DbPreDtlQuery2()    '20110725 added by songth  
        
        
    End If

End Sub
'--20110725 added by thsong 마지막 spreed에 칼럼 클릭시 데이타가 자동정렬되도록 하는것
Sub vspdData2_Click(ByVal Col , ByVal Row)
	
	gMouseClickStatus = "SPC"					'SpreadSheet 대상명이 vspdData1일경우 
	Set gActiveSpdSheet = vspdData2
    Call SetPopupMenuItemInf("0000111111")
    '20110726 changed by songth
    'Call SetPopupMenuItemInf("0000110111")
    If vspdData2.MaxRows <= 0 Then Exit Sub
 
   	    
    If Row = 0 Then
		'vspdData2.OperationMode = 0
        'ggoSpread.Source = vspdData2
        If lgSortKey = 1 Then
            ggoSpread.SSSort Col
            lgSortKey = 2
        ...
        End If
	Else
	   '20110728 added by songth 체크박스를 활성화 하기 위해spreadunlock 함수 사용
	   ggoSpread.Source = vspdData2
	   ggoSpread.SpreadUnLock	C_Select, Row, C_Select, Row 

    End If
End Sub
'--20110725 added by thsong 2번째 spreed로 날짜별 수량 집계 보여주고 클릭시 마지막 spreed에 상세 내용보여줌
Sub vspdData3_Click(ByVal Col , ByVal Row)
	
	gMouseClickStatus = "SPC"					'SpreadSheet 대상명이 vspdData1일경우 
	Set gActiveSpdSheet = vspdData3
    Call SetPopupMenuItemInf("0000111111")

    
    If vspdData3.MaxRows <= 0 Then Exit Sub
   	    
    If Row = 0 Then
		vspdData3.OperationMode = 0
        ggoSpread.Source = vspdData3
        If lgSortKey = 1 Then
            ggoSpread.SSSort Col
            lgSortKey = 2
...
Function FncQuery()
	FncQuery = False
		...
		Else
			...
			Call DbPreDtlQuery2() '조회버튼 클릭시 grid1번실행
			Call DbPreDtlQuery()  '조회버튼 클릭시 grid2번실행 당연히 값이 없음.빈값보여주기위해 호출한것임
		End If
	FncQuery = False
End Function
...
'20110725::songth========================================================================================
Function DbPreDtlQuery2()											'☆: 조회 성공후 실행로직 
	lgStrPrevKey2 = ""   '2008-06-10 5:00오후 :: hanc
'	lgStrPrevKey4 = ""
'  	lgStrPrevKey5 = ""
	vspdData3.MaxRows = 0
	
	If DbDtlQuery2 = False Then	
		Call RestoreToolBar()
		Exit Function
	End If	
...
Function DbDtlQuery2() 
	...
	End If		
 	...
 	If lgIntFlgMode = PopupParent.OPMD_UMODE Then
 		strVal = BIZ_PGM_QRY_ID3 & "?txtMode=" & PopupParent.UID_M0001					
...
Function DbDtlQuery() 
    ...
    Dim lngRows
	DbDtlQuery = False   
	vspdData3.Row = vspdData3.ActiveRow
...
' 		strVal = strVal & "&txtSoNo=" & Trim(txtSoNo.value)
    ...
    vspdData3.Col = C3_ShipDate '20110725 added by songth
    strVal = strVal & "&txtShipDate=" & Trim(vspdData3.Text)
...
Sub PopRestoreSpreadColumnInf()
	...
	Elseif gActiveSpdSheet.Id = "B" Then
		ggoSpread.Source = vspdData2
		Call ggoSpread.ReOrderingSpreadData()
    Elseif gActiveSpdSheet.Id = "C" Then		
        ggoSpread.Source = vspdData3
		Call ggoSpread.ReOrderingSpreadData()
		Call InitData(1,1)
	End If
...
Function DbQueryOk()
	...
	Set gActiveElement = document.activeElement
'     vspdData3.MaxRows = 0 'changed by songth 20110725
	'vspdData2.MaxRows = 0
	If DbDtlQuery = False Then	
		Call RestoreToolBar()
		Exit Function
	...
	<TR>
		<TD WIDTH=100% HEIGHT=* valign=top>
			<TABLE <%=LR_SPACE_TYPE_20%>>
				<TR>
					<TD HEIGHT="100%">
						<script language =javascript src='./js/s3112aa1_ko441_vaSpread_vspdData3.js'></script>
					</TD>
				</TR>
			</TABLE>
		</TD>
			...
			<TABLE <%=LR_SPACE_TYPE_30%>>
				<TR>
					<TD WIDTH=70% NOWRAP>     <IMG SRC="../../../CShared/image/query_d.gif"  Style="CURSOR: hand" ALT="Search" NAME="Search" onMouseOut="javascript:MM_swapImgRestore()" onMouseOver="javascript:MM_swapImage(this.name,'','../../../CShared/image/Query.gif',1)"  ONCLICK="FncQuery()"   ></IMG>
					                          <IMG SRC="../../../CShared/image/select_all_d.gif"  Style="CURSOR: hand" ALT="Select All" NAME="Search" onMouseOut="javascript:MM_swapImgRestore()"   onMouseOver="javascript:MM_swapImage(this.name,'','../../../CShared/image/select_all.gif',1)" ONCLICK="SelectAll()"></IMG> </TD>
