...
if Request("txtFlgMode") = Request("OPMD_UMODE") Then
		...
		txtPromiseFrDt= Request("HtxtPromiseFrDt")
		txtPromiseToDt= Request("HtxtPromiseToDt")
		...
		txtPromiseFrDt= Request("txtPromiseFrDt")
		txtPromiseToDt= Request("txtPromiseToDt")
...
Sub FixUNISQLData()
    ...
    If Len(txtPromiseFrDt) Then    
			strVal = strVal & " AND A.PROMISE_DT >=  " & FilterVar(UNIConvDate(txtPromiseFrDt), "''", "S") & ""				
	End If		
	
	If Len(txtPromiseToDt) Then	
			strVal = strVal & " AND A.PROMISE_DT <=  " & FilterVar(UNIConvDate(txtPromiseToDt), "''", "S") & ""				
	End If

	If Trim(txtRadio) = "Y" Then
		strVal = strVal & " AND B.REQ_QTY <= J.SUM_GI_QTY"
	ElseIf Trim(txtRadio) = "N" Then
		strVal = strVal & " AND ISNULL(J.SUM_GI_QTY, 0) < B.REQ_QTY"
	End If
	...
	If "<%=lgDataExist%>" = "Yes" Then
       'Set condition data to hidden area
		...
		If "<%=lgPageNo%>" = "1" Then   ' "1" means that this query is first and next data exists
			...
			parent.frm1.HtxtPromiseFrDt.value = "<%=txtPromiseFrDt%>"
			parent.frm1.HtxtPromiseToDt.value = "<%=txtPromiseToDt%>"
