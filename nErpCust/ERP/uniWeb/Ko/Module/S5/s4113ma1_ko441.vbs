...
Const BIZ_PGM_ID = "s4113mb1_ko441.asp"            '☆: Head Query 비지니스 로직 ASP명 
...
Sub InitSpreadSheet()
	...
	With ggoSpread
		...
		frm1.vspdData.MaxCols = C_REF_GUBUN + 1                  '☜: 최대 Columns의 항상 1개 증가시킴 
		...
		.SSSetEdit C_CUST_LOT_NO, "고객사 Lot No.", 18
...
Function OpenMESIssueRef()
	...
	iCalledAspName = AskPRAspName("S3112BA1_ko441")     '20080219::hanc

	if Trim(iCalledAspName) = "" then
		IntRetCD = DisplayMsgBox("900040",parent.VB_INFORMATION, "S3112BA1_ko441", "x")
		gblnWinEvent = False
		exit Function
	end if
...
'========================================
Function SetMESIssueRef(pvArrRet)
On Error Resume Next
	...
	With frm1.vspdData
		...
		For iLngCnt = 0 to iLngLoopCnt - 1
			...
			.Col = C_RetTypeNm		:	.text = pvArrRet(iLngCnt, 27)     		'반품유형명 
			.Col = C_LotFlag		:	.text = pvArrRet(iLngCnt, 22)			'Lot 관리여부 
			.Col = C_LotNo		:	.text = pvArrRet(iLngCnt, 23)			'Lot 관리여부 
			.Col = C_LotSeq		:	.text = pvArrRet(iLngCnt, 24)			'Lot 관리여부 
			.Col = C_DnSeq			:	.Text = 0
			.Col = C_CIQty			:	.Text = 0

			.Col = C_OUT_NO		:	.text = pvArrRet(iLngCnt, 29)
			.Col = C_TRANS_TIME		:	.text = pvArrRet(iLngCnt, 31)
			.Col = C_OUT_TYPE_SUB		:	.text = pvArrRet(iLngCnt, 33)
			.Col = C_CREATE_TYPE		:	.text = pvArrRet(iLngCnt, 32)
			.Col = C_REF_GUBUN		:	.text = "1"
