...
Sub SetDefaultVal()
	...
	If lgSGCd <> "" then 
		Call ggoOper.SetReqAttr(frm1.txtSales_Grp, "Q") 
        	frm1.txtSales_Grp.value = lgSGCd
	End If
...
Sub InitSpreadSheet()
    ...
    With ggoSpread
        ...
        .Spreadinit "V20070601", , Parent.gAllowDragDropSpread
 
        frm1.vspdData.MaxCols = C_Tracking_no_PopUp + 1            '☜: 최대 Columns의 항상 1개 증가시킴
        ...
        .SSSetEdit C_Tracking_no, "Tracking No", 25, , , 25, 2
        .SSSetButton C_Tracking_no_Popup        
        ...
        Call .MakePairsColumn(C_Tracking_no, C_Tracking_no_Popup)
...
'=========================================
Sub SetSpreadColorByQuery(ByVal pvStartRow, ByVal pvEndRow)
    ...
    With ggoSpread
        ...
        .SSSetProtected C_Tracking_no, pvStartRow, pvEndRow
        .SSSetProtected C_Tracking_no_Popup, pvStartRow, pvEndRow
...
Sub SetSpreadColor(ByVal pvStartRow, ByVal pvEndRow)
    ...
    With ggoSpread
		...
		for curRow = pvStartRow to pvEndRow
		    Call ChangeTrackingSetField(curRow)
		Next    
...
Function ChangeTrackingSetField(ByVal IRow) ' 박정순 추가
	...
	With frm1.vspdData
		strPlantCd = Trim(frm1.txtPlant.Value)
		.Row = IRow
		.col = C_ItemCd        '품목코드
		strItemCd =  .text
	        ...
	        If Trim(strItemCd) = "" Then 
               ...
               ggoSpread.SSSetProtected C_Tracking_no, IRow, IRow
  	           ggoSpread.SSSetProtected C_Tracking_no_Popup, IRow, IRow 
	...
	If  strTrackingFlag = "Y" Then
 	    ggoSpread.SpreadUnLock  C_Tracking_no, IRow, C_Tracking_No, IRow
	    ggoSpread.SSSetRequired C_Tracking_no, IRow, IRow
	    ggoSpread.SpreadUnLock  C_Tracking_no_Popup, IRow, C_Tracking_No_Popup, IRow	    
	Else	 
...
Function OpenConDnNo()
    ...
    iCalledAspName = AskPRAspName("S4111PA2_ko441")
            
    If Trim(iCalledAspName) = "" Then
        IntRetCD = DisplayMsgBox("900040", Parent.VB_INFORMATION, "S4111PA2_ko441", "x")
        IsOpenPop = False
        Exit Function
    End If
...
Function OpenDNType()
    ...
    arrParam(3) = ""
    arrParam(4) = "A.minor_cd = B.mov_type And B.TRNS_TYPE = " & FilterVar("DI", "''", "S") & " AND usage_flag = 'Y' " & _
                     " and A.major_cd = " & FilterVar("I0001", "''", "S") & " and b.mov_type = c.mov_type and c.so_mgmt_flag = " & FilterVar("N", "''", "S") & _
					"  and c.EXPORT_FLAG = " & FilterVar("N", "''", "S") & "  and c.REL_DN_FLAG = " & FilterVar("Y", "''", "S") & "  and c.REL_BILL_FLAG = " & FilterVar("Y", "''", "S") & " "
    arrParam(5) = "출하형태"
...
Function OpenLotNoPopup(ByVal iWhere)
    ...
    With frm1.vspdData
        ...
        If Trim(arrRet(0)) <> "" Then
            ...
            .Col = C_Tracking_no: .Text = arrRet(2)
...
'========================================
Function OpenTrackingNoPopup(ByVal iWhere)
	Dim iArrRet
	Dim iArrParam(5), iArrField(6), iArrHeader(6)
    ...
    With frm1.vspdData
        ...
        If Trim(iArrRet(0)) <> "" Then
            .Col = C_Tracking_no: .Text = iArrRet(0)
            Call vspdData_Change(.Col, .Row)
            lgBlnFlgDtlChgValue = True
        End If
    End With
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
            ...
            C_Tracking_no = iCurColumnPos(35)
            C_Tracking_no_PopUp = iCurColumnPos(36)
    End Select
End Sub
...
Sub vspdData_ButtonClicked(ByVal Col, ByVal Row, ByVal ButtonDown)
    ...
    With frm1.vspdData
        ...
        Select Case Col
            ...
            Case C_Tracking_no_PopUp
                Call OpenTrackingNoPopup(Row)
    ...
    With frm1.vspdData
        ...
        Select Case Col
            ...
            Case C_ItemCd
				...
				Call ChangeTrackingSetField(Row)                    
...
Function FncCopy()
    With frm1.vspdData
    	...
    	call  ChangeTrackingSetField(.ActiveRow ) ' 2007-06-29 박정순 추가 
    ...
    With frm1.vspdData
        For iLngRow = 1 To .MaxRows
            ...
            ElseIf .Text <> "" Then
                ...
                .Col = C_Tracking_no:      iArrData(41) = Trim(.Text)                  ' Tracking no
