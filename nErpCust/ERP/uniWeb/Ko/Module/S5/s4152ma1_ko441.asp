 	    ...
 	    With frm1.vspdData
          ...
          Call GetSpreadColumnPos("A")
	       .ReDraw = false
		    ...
		    ggoSpread.SSSetEdit     C_Gi_Type_SUB,          "출고TYPE(SUB)", 15, 0  
		    ...
		    ggoSpread.SSSetEdit     C_Tester,               "Test모델", 14, 0
		    ggoSpread.SSSetEdit     C_Pgm_nm2,              "PGM명2", 14, 0
		    ggoSpread.SSSetEdit     C_Tester2,              "Test모델2", 14, 0
		    ggoSpread.SSSetEdit     C_MV_NO,                "재고이동번호", 14, 0
		    ggoSpread.SSSetEdit     C_MV_NO1,               "출고번호", 14, 0
    ...
    If pvSpdNo = "" OR pvSpdNo = "B" Then
	    ...
	    With frm1.vspdData2
	       ...
	       .ReDraw = false
		    ...
		    ggoSpread.SSSetEdit     C_Gi_Type_SUBD,          "출고TYPE(SUB)", 15, 0  
		    ...
		    ggoSpread.SSSetEdit     C_TesterD,               "Test모델", 14, 0
		    ggoSpread.SSSetEdit     C_Pgm_nm2D,              "PGM명2", 14, 0
		    ggoSpread.SSSetEdit     C_Tester2D,              "Test모델2", 14, 0
		    ggoSpread.SSSetEdit     C_MV_NOD,                "재고이동번호", 14, 0
		    ggoSpread.SSSetEdit     C_MV_NO1D,               "출고번호", 14, 0
...
'	ggoSpread.SSSetRequired		C_Price,  -1, C_Price
	ggoSpread.spreadlock		C_Po_No, -1, C_MV_NO1
...
Sub SetSpreadColor(ByVal pvStartRow,ByVal pvEndRow)
  ...
  '  ggoSpread.SSSetRequired    C_Item_Cd,              pvStartRow, pvEndRow
    ...
    ggoSpread.SSSetProtected    C_Gi_Type_SUB  ,         pvStartRow, pvEndRow
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			C_Gi_Type_SUB         = iCurColumnPos(i) : i = i + 1
	        ...
	        C_Tester              = iCurColumnPos(i) : i = i + 1
	        C_Pgm_nm2             = iCurColumnPos(i) : i = i + 1
	        C_Tester2             = iCurColumnPos(i) : i = i + 1
	        C_MV_NO               = iCurColumnPos(i) : i = i + 1
	        C_MV_NO1              = iCurColumnPos(i) : i = i + 1
       ...
       Case "B"
			...
			C_Gi_Type_SUBD        = iCurColumnPos(i) : i = i + 1
	        ...
	        C_TesterD             = iCurColumnPos(i) : i = i + 1
	        C_Pgm_nm2D            = iCurColumnPos(i) : i = i + 1
	        C_Tester2D            = iCurColumnPos(i) : i = i + 1
	        C_MV_NOD              = iCurColumnPos(i) : i = i + 1
	        C_MV_NO1D             = iCurColumnPos(i) : i = i + 1
...
Sub vspdData_Change(ByVal Col , ByVal Row )
   ...
   With frm1.vspdData
        ...
        Select Case Col
			Case C_Price1 
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt1: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price2 
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt2: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price3
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt3: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price4 
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt4: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price5 
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt5: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price6
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt6: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price7
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt7: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price8
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt8: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price9
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt9: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price10
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt10: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price11
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt11: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price12
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt12: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price13
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt13: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price14
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt14: .Text = iPrice * iDblQty
                 Call DoSum(Row)
			Case C_Price15
                 iPrice = UNICDbl(.Text): .Col = C_Gi_Qty:  iDblQty = UNICDbl(.Text) 
                 .Col = C_Amt15: .Text = iPrice * iDblQty
                 Call DoSum(Row)
...
Function DoSum(Row) 
   ...
   With frm1.vspdData
        ...
        .Col = C_Gi_Price: .Text = iDblPrice  'price 총액 설정
        ...
        .Col = C_Gi_Amt: .Text = iDblAmt  'amt 총액 설정
...
'========================================================================================================
 Function DbQuery()
  ...
  If lgIntFlgMode = Parent.OPMD_UMODE Then
   ...
   strVal = strVal & "&txtList_Type=" & Trim(frm1.txtHList_Type.value)   
   strVal = strVal & "&lgStrPrevKey=" & lgStrPrevKey
   strVal = strVal & "&txtMaxRows=" & frm1.vspdData.MaxRows  
     
  Else
  
    If frm1.rdoList_type(0).checked Then
        rdoList_type =   "1"
    ElseIf frm1.rdoList_type(1).checked Then
        rdoList_type = "2"
    Else
        rdoList_type = "3"
    End If
   ...
   strVal = strVal & "&txtList_Type=" & Trim(rdoList_type)  
   strVal = strVal & "&lgStrPrevKey=" & lgStrPrevKey
   strVal = strVal & "&txtMaxRows=" & frm1.vspdData.MaxRows
  End If
...
End Function
 ...
 Function DbQueryFail()             <% '☆: 조회 성공후 실행로직 %>
...
Sub ButtonDnAccept(Byval ButtonDown)
	...
	strWhere = " a.PLANT_CD in ('P01','P02','P09')    AND a.OUT_TYPE IN ('P01','P02','P09') AND c.DN_NO = b.DN_NO "
      ...
      <FIELDSET CLASS="CLSFLD">
       <TABLE <%=LR_SPACE_TYPE_40%>>
        ...
        <TR>
		 ...
		 <TD CLASS="TD6" NOWRAP><INPUT TYPE="RADIO" CLASS="Radio" NAME=rdoList_type ID=rdoList_type1  tag="11" Checked><LABEL FOR=rdoList_type1>전체</LABEL>&nbsp;&nbsp;
							    <INPUT TYPE="RADIO" CLASS="Radio" NAME=rdoList_type ID=rdoList_type2  tag="11"><LABEL FOR=rdoList_type2>출고처리</LABEL>&nbsp;&nbsp;
							    <INPUT TYPE="RADIO" CLASS="Radio" NAME=rdoList_type ID=rdoList_type3  tag="11"><LABEL FOR=rdoList_type3>미출고처리</LABEL></TD>
        </TR>
       </TABLE>
      </FIELDSET>
     </TD>
...
<INPUT TYPE=HIDDEN NAME="txtHList_Type" tag="24" TABINDEX = -1>
</FORM>
