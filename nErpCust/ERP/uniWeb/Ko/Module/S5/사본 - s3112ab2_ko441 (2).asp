...
Sub SubBizQueryMulti()
    ...
    Const C_SHEETMAXROWS_D  = 900                                          '☆: Server에서 한번에 fetch할 최대 데이타 건수        
    
    On Error Resume Next                                                                 '☜: Protect system from crashing
    Err.Clear                                                                            '☜: Clear Error status

'Call ServerMesgBox("HANC : 100" & "START"  , vbInformation, I_MKSCRIPT)
	iSelCount = C_SHEETMAXROWS_D + C_SHEETMAXROWS_D *  lgStrPrevKey + 1
	
    '---------- Developer Coding part (Start) ---------------------------------------------------------------

		lgStrSQL = lgStrSQL & " select TOP " & iSelCount & " b.OUT_NO, " & vbcrlf
		lgStrSQL = lgStrSQL & FilterVar(Request("txtShipToParty"),"''","S") & " as BP_CD " & vbcrlf
'		lgStrSQL = lgStrSQL & " ,(SELECT top 1 BP_CD FROM B_BIZ_PARTNER WHERE BP_ALIAS_NM=b.SHIP_TO_PARTY and USAGE_FLAG = 'y') BP_CD " & vbcrlf
		...
		lgStrSQL = lgStrSQL & " , rtrim(b.PGM_NAME) as pgm_name " & vbcrlf
		lgStrSQL = lgStrSQL & " , ISNULL([DBO].[ufn_GetPrice]("&FilterVar(Request("txtShipToParty"),"''","S")&", a.ITEM_CD, " & FilterVar(Request("txtSoNo"),"''","S") & ", b.GI_UNIT, b.PGM_NAME),0) pgm_price " & vbcrlf
		...
		lgStrSQL = lgStrSQL & " AND c.bp_cd =" & FilterVar(Request("txtShipToParty"),"''","S") & vbcrlf
		...
		lgStrSQL = lgStrSQL & " 		order by b.OUT_NO " & vbcrlf
