        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
					...
					lgStrSQL =            " SELECT TOP " & iSelCount  & " A.ITEM_CD , C.ITEM_NM , C.SPEC  , C.BASIC_UNIT, "
					lgStrSQL = lgStrSQL & "        GOOD_ON_HAND_QTY "
					lgStrSQL = lgStrSQL & "   FROM I_ONHAND_STOCK A(NOLOCK), "
					lgStrSQL = lgStrSQL & "        B_STORAGE_LOCATION B(NOLOCK), "
					lgStrSQL = lgStrSQL & "        B_ITEM C(NOLOCK) "
					lgStrSQL = lgStrSQL & "  WHERE A.ITEM_CD = C.ITEM_CD "
					lgStrSQL = lgStrSQL & "    AND A.SL_CD = B.SL_CD "
					lgStrSQL = lgStrSQL & "    AND (CASE WHEN B.BP_CD = " & FilterVar(lgKeyStream(0),"''", "S") & " THEN GOOD_ON_HAND_QTY "
					lgStrSQL = lgStrSQL & "              ELSE 1 END) > 0 "
					
					If lgkeystream(0) <> "" Then
						lgStrSQL = lgStrSQL & "            	AND	B.BP_CD = " & FilterVar(lgKeyStream(0),"''", "S")
					End If
					...
					If lgkeystream(2) <> "" Then
						lgStrSQL = lgStrSQL & "            	AND	A.tracking_no = " & FilterVar(lgKeyStream(2) ,"''", "S")
					End If
