...
Sub SubBizQueryCond()
	...
	If lgKeyStream(0) <> "" Then
		...
		If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then
		    ...
		    Call DisplayMsgBox("SCM002", vbInformation, "사업장", "", I_MKSCRIPT)
		    Call SetErrorStatus()
		Else
			Response.Write "<Script Language=VBScript>" & vbCrLf
	...
	If lgKeyStream(1) <> "" then 'AND lgErrorStatus <> "YES" Then
		...
		If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then
			...
			Response.Write "</Script>" & vbCrLf
		    Call DisplayMsgBox("970000", vbInformation, "매출형태", "", I_MKSCRIPT)
		    Call SetErrorStatus()
		Else
			Response.Write "<Script Language=VBScript>" & vbCrLf
				Response.Write "parent.frm1.txtivtypeNm.value = """ & ConvSPChars(lgObjRs("bill_type_nm")) & """" & vbCrLf
			Response.Write "</Script>" & vbCrLf
		End If
	Else
		Response.Write "<Script Language=VBScript>" & vbCrLf
...
Sub SubBizQueryMulti()
    ...
    Else
        ...
        Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & UNIDateClientFormat(lgObjRs("BILL_DT"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("BILL_type_NM"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("CUR"))
            ...
            lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs("VAT_RATE"), ggExchRate.DecPoint, 0)
			lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("VAT_TYPE"))
			lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("BILL_NO"))
    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
					...
					lgStrSQL	= "	select	TOP " & iSelCount  & "  A.BIZ_AREA , C.BIZ_AREA_NM , BILL_DT , A.BILL_TYPE , D.BILL_TYPE_NM , " _
								& "       CUR , A.BILL_AMT_LOC , A.VAT_AMT_LOC , (A.BILL_AMT_LOC + A.VAT_AMT_LOC ) TOT_AMT , " _
								& "       A.VAT_RATE , dbo.ufn_GetCodeName('B9001', A.VAT_TYPE) AS VAT_TYPE, A.BILL_NO " _
								& "  FROM S_BILL_HDR A(NOLOCK) INNER JOIN B_BIZ_AREA C(NOLOCK) " _
								& "    ON A.BIZ_AREA = C.BIZ_AREA_CD " _
								& "  INNER JOIN S_BILL_TYPE_CONFIG D(NOLOCK) ON A.BILL_TYPE = D.BILL_TYPE " _
								& "   AND A.POST_FLAG = " & FilterVar("Y","''", "S")
					
					If lgkeystream(0) <> "" Then
						lgStrSQL = lgStrSQL & "             and 	a.BIZ_AREA = " & FilterVar(lgKeyStream(0),"''", "S")
					End If
					If lgkeystream(1) <> "" Then
						lgStrSQL = lgStrSQL & "            	and 	a.BILL_type = " & FilterVar(lgKeyStream(1),"''", "S")
					End If
					If lgkeystream(2) <> "" Then
						lgStrSQL = lgStrSQL & "            	and 	A.BILL_TO_PARTY = " & FilterVar(lgKeyStream(2),"''", "S")
					End If
					If lgkeystream(3) <> "" Then
						lgStrSQL = lgStrSQL & "            	and 	a.BILL_DT >= " & FilterVar(UNIConvDate(lgKeyStream(3)),"''", "S")
					End If					
					If lgkeystream(4) <> "" Then
						lgStrSQL = lgStrSQL & "            	and 	a.BILL_DT <= " & FilterVar(UNIConvDate(lgKeyStream(4)),"''", "S")
					End If					
        ...
        Case "C"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "I"
                    lgStrSQL =            " select bill_type_nm From S_BILL_TYPE_CONFIG where bill_type = " & FilterVar(lgKeyStream(1) & "" ,"''", "S")
               Case "B"
