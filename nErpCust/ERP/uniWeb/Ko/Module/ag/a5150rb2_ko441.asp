...
Sub MakeSpreadSheetData()
    ...
    If strAllcAmt <> 0 Then
		Do While Not (Rs0.EOF Or Rs0.BOF)
			PAmt = strAllcAmt 
			strAllcAmt = strAllcAmt - UNIConvNum(Rs0(25) ,0)
			...
			For ColCnt = 0 To UBound(lgSelectListDT) - 1
   		    	...
   		    	Select Case ColCnt
			    Case 4,5,6,7
				If (ColCnt = 4 Or ColCnt = 6) And rs0(ColCnt) <> "" Then
					Call SubCreateCommandObject(lgObjComm)

					With lgObjComm
						.CommandText = "USP_A_MGNT_NAME"
						.CommandType = adCmdStoredProc
						lgObjComm.Parameters.Append lgObjComm.CreateParameter("@INCTRL_CD"  ,adVarWChar,adParamInput,Len(rs0(ColCnt)),rs0(ColCnt))
						lgObjComm.Parameters.Append lgObjComm.CreateParameter("@INCTRL_VAL" ,adVarWChar,adParamInput,Len(rs0(ColCnt+1)),rs0(ColCnt+1))
					...
					End With
   					...
   					If  Err.number = 0 Then
						...
						iRowStr = iRowStr & Chr(11) & FormatRsString(lgSelectListDT(ColCnt),rs0(ColCnt+1))	
						iRowStr = iRowStr & Chr(11) & FormatRsString(lgSelectListDT(ColCnt+1),lgObjComm.Parameters("@OUT_DATA_COLM_NM").Value)
					Else
						iRowStr = iRowStr & Chr(11) & FormatRsString(lgSelectListDT(ColCnt),"")	
						iRowStr = iRowStr & Chr(11) & FormatRsString(lgSelectListDT(ColCnt+1),"")	
...
Sub FixUNISQLData()
	...
	Select Case UCase(strOpenType)
		...
		Case "U6" '미결(신용카드)
			...
			UNIValue(5,0) = UCase(" " & FilterVar(strBpCd, "''", "S") & " " )
			UNIValue(6,0) = UCase(" " & FilterVar(strBizCd, "''", "S") & " " )			
		Case "U9" '미결(기타)
			UNISqlId(0) = "A5150RA206KO441"		
			UNISqlId(1) = "ADEPTNM"
			UNISqlId(2) = "AACCTNM"
			UNISqlId(3) = "CommonQry"
			UNISqlId(4) = "CommonQry"
...
Sub TrimData()
	...
	Select Case UCase(strOpenType)
		...
		Case "U9"
			...
			'>>AIR 사업장 조건추가
			If strBpCd <> "" Then
				strCond = strCond & " AND C.BP_CD = " & FilterVar(strBpCd , "''", "S") & ""
			End If
			If strBizCd <> "" Then
				strCond = strCond & " AND F.BIZ_AREA_CD =" & FilterVar(strBizCd , "''", "S") & ""
			End If	
