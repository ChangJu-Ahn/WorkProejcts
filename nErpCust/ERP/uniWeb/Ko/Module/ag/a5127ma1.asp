...
'*  2. Function Name        : 
'*  3. Program ID           : a5127ma1
'*  4. Program Name         : 결의전표 등록(copy)
...
'========================================================================================================
Const BIZ_PGM_ID          = "a5101mb1.asp"			'☆: 비지니스 로직 ASP명 
Const JUMP_PGM_ID_TAX_REP = "a5101ma1"

'========================================================================================================
Const C_GLINPUTTYPE = "TG"
Const MENU_NEW		= "1100000000011111"
Const MENU_CRT		= "1100111100111111"
Const MENU_UPD		= "1100111100111111"
...
Sub SetSpreadLock(Byval stsFg, Byval Index, ByVal lRow  , ByVal lRow2 )
    ...
    With frm1
		...
		Select Case Index
			...
			Case 1
				ggoSpread.SpreadLock C_ItemSeq, -1, C_AcctCd2 'Item Grid 전체 Lock설정 
				ggoSpread.SSSetProtected	.vspdData.MaxCols,-1,-1
		End Select
...
Sub SetSpreadColor(Byval stsFg, Byval Index, ByVal lRow, ByVal lRow2)
    With frm1
		...
		ggoSpread.SSSetRequired  C_DocCur, lRow, lRow2	   ' 통화 
...
Function OpenReftempgl()
	...
	iCalledAspName = AskPRAspName("a5101ra1_ko441")
	...
	With frm1
		Select Case iWhere
		     Case "0"
               ...
               .hInternalCd.value = Trim(arrRet(2))
...
Function PgmJumpChk(strPgmId)
	...
	'------------------------ 
    ggoSpread.Source = frm1.vspdData    
	If Not(frm1.hCongFg.value = "C" Or frm1.cboGlInputType.Value <> C_GLINPUTTYPE) Then   
	...
	If Trim(frm1.txtTempGlNo.value) <> "" Then
		WriteCookie "TEMP_GL_NO", Trim(frm1.txtTempGlNo.value)
	Else
		WriteCookie "TEMP_GL_NO", Trim(frm1.htxtTempGlNo.value)
	End If
...
Function FncSave() 
	...
	If lngItemCnt < 1 Then																'결의전표항목 데이터가 존재하지 않습니다.
		IntRetCD = DisplayMsgBox("114200", "X", "X", "X")
		Exit Function
	End If
...
'		strSelect = strSelect & " WHEN B.CR_FG = 'Y' AND 'CR'='" & Trim(tmpDrCrFG) & "' THEN 'Y' "  		
		...
		strWhere =			  " D.TEMP_GL_NO = " & FilterVar(.htxtTempGlNo.value, "''", "S") 	
...
' Function Desc : Copy시 관리항목을 모두 call한다. 
'=======================================================================================================
Function DbQueryAll(ByVal Row)
	...
	With frm1
		...
		strSelect =				" D.ITEM_SEQ, C.DTL_SEQ,  A.CTRL_CD, A.CTRL_NM , LTrim(ISNULL(C.CTRL_VAL,'')), '',"
		strSelect = strSelect & " CASE  WHEN A.COLM_DATA_TYPE = " & FilterVar("D", "''", "S") & "  Then " & FilterVar("(Format : YYYY-MM-DD)", "''", "S") & "  End , D.ITEM_SEQ, LTrim(ISNULL(A.TBL_ID,'')), LTrim(ISNULL(A.DATA_COLM_ID,'')), "
		strSelect = strSelect & " LTrim(ISNULL(A.DATA_COLM_NM,'')), LTrim(ISNULL(A.COLM_DATA_TYPE,'')), LTrim(ISNULL(A.DATA_LEN,'')), "
		strSelect = strSelect & " CASE WHEN B.DR_FG = " & FilterVar("Y", "''", "S") & "  AND  B.CR_FG = " & FilterVar("Y", "''", "S") & "  Then " & FilterVar("DC", "''", "S") & "  "
'		strSelect = strSelect & " WHEN B.DR_FG = 'Y' AND 'DR'='" & Trim(tmpDrCrFG) & "' THEN 'Y' "  
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gIf" align="center" CLASS="CLSMTABP"><font color=white>결의전표등록 (Copy)</font></td>
					...
					<TD WIDTH="100%" HEIGHT=* VALIGN=TOP >
						<TABLE <%=LR_SPACE_TYPE_60%>>					
							<TR>								
								...
								<TD CLASS=TD6 NOWRAP><SELECT NAME="cboGlType" tag="22" STYLE="WIDTH:82px:" ALT="전표형태"></SELECT></TD> 
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>부서</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtDeptCd" ALT="부서코드" MAXLENGTH="10" SIZE=10 STYLE="TEXT-ALIGN: left" tag="22XXXU"><IMG SRC="../../../CShared/image/btnPopup.gIf" NAME="btnCostCd" align=top TYPE="BUTTON" ONCLICK="vbscript:Call OpEndept(frm1.txtDeptCd.Value, 0)" tag="22">&nbsp;
		...
		<TD WIDTH="100%">
			<TABLE <%=LR_SPACE_TYPE_30%>>
				<TR>
					...
					<TD WIDTH="*" ALIGN=RIGHT><A HREF = "VBSCRIPT:PgmJumpChk(JUMP_PGM_ID_TAX_REP)">결의전표등록</A></TD>
					</TD>
				</TR>
			</TABLE>
		</TD>
