...
'*  2. Function Name        : 
'*  3. Program ID           : a5121oa1
'*  4. Program Name         : 회계전표출력 
'*  5. Program Desc         : 회계전표출력 
'*  6. Component List       : 
...
Sub txtDeptCD_OnChange()
	...
	If TRim(frm1.txtDeptCd.value) <>"" Then
		'----------------------------------------------------------------------------------------
			...
			strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(frm1.txtDateFr.Text, gDateFormat,""), "''", "S") & ")"
			strWhere = strWhere & " AND ORG_CHANGE_DT <= " 
			strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(frm1.txtDateTo.Text, gDateFormat,""), "''", "S") & ") "
...
Sub SetPrintCond(StrEbrFile, StrUrl)
	...
	Dim	VarDateFr, VarDateTo, VarDeptCd, VarBizAreaCd, VarBizAreaCd1, varGlNoFr, varGlNoTo, varGlPutType

	Dim strAuthCond

	StrEbrFile = "a5121ma1_1"
	...
	varGlNoFr = " "
	varGlNoTo = "ZZZZZZZZZZZZZZZZZZ"
	varGlPutType = "%"

	If Len(frm1.txtDeptCd.value) > 0 Then VarDeptCd = Trim(frm1.txtDeptCd.value)
	If Len(frm1.txtGlNoFr.value) > 0 Then varGlNoFr = Trim(frm1.txtGlNoFr.value)
	If Len(frm1.txtGlNoTo.value) > 0 Then varGlNoTo = Trim(frm1.txtGlNoTo.value)
	If Len(frm1.txtInputType.value) > 0 Then 
						...
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtDateFr" CLASS=FPDTYYYYMMDD tag="12X1" Title="FPDATETIME" ALT="시작회계일자" id=fpDateTime1></OBJECT>');</SCRIPT>&nbsp;~&nbsp;
													   <SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtDateTo" CLASS=FPDTYYYYMMDD tag="12X1" Title="FPDATETIME" ALT="종료회계일자" id=fpDateTime2></OBJECT>');</SCRIPT></TD>
							</TR>
							<TR>
								<TD CLASS="TD5" NOWRAP>입력부서</TD>
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtDeptCd" SIZE=10 MAXLENGTH=10 tag="11XXXU" ALT="입력부서코드"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnDeptCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenDeptOrgPopup()">
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtInputType" SIZE=10 MAXLENGTH=10 tag="11XXXU" ALT="전표입력경로코드"><IMG SRC="../../image/btnPopup.gif" NAME="btnBizAreaCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopup(frm1.txtInputType.Value,4)">
								                       <INPUT TYPE="Text" NAME="txtInputTypeNm" SIZE=25 tag="14X" ALT="전표입력경로명"></TD>
							</TR>							
							<TR>
								<TD CLASS="TD5" NOWRAP>전표번호</TD>
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtGlNoFr" SIZE=15 MAXLENGTH=18 tag="11XXXU" ALT="시작회계번호"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnTempGlNoFr" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopup(frm1.txtGlNoFr.Value,0)">&nbsp;~&nbsp;
													   <INPUT TYPE="Text" NAME="txtGlNoTo" SIZE=15 MAXLENGTH=18 tag="11XXXU" ALT="종료회계번호"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnTempGlNoTo" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopup(frm1.txtGlNoTo.Value,1)">
								</TD>
							</TR>
						</TABLE>
					</TD>
