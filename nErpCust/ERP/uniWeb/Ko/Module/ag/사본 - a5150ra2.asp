...
Sub SetDefaultVal()
	...
	frDt = UNIDateAdd("M", -1, arrParam(6),PopupParent.gServerDateFormat)
...
Sub InitSpreadSheet()
    ...
    Call SetZAdoSpreadSheet("a5150RA2","S","A","V20030215",PopupParent.C_SORT_DBAGENT,vspdData, C_MaxKey, "X","X")
    Call SetSpreadLock() 
End Sub
...
Function DbQuery() 
    ...
    If lgIntFlgMode  <> PopupParent.OPMD_UMODE Then   ' This means that it is first search
		...
		strVal = strVal & "&txtFrOpenDt="		& Trim(txtFrOpenDt.text)				
		strVal = strVal & "&txtToOpenDt="		& Trim(txtToOpenDt.text)				
