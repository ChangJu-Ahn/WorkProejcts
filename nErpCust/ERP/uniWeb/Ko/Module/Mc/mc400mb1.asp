...
Err.Clear
	...
	strDoDate = Request("txtDoDate")
	strDoTime = Request("cboDoTime")
	...
	IF Trim(strDoDate) = "" Then
	   ...
	   Response.End
	ELSE
	   strDoDate = FilterVar(UniConVDate(strDoDate), "''", "S")
	END IF
	
	IF Trim(strDoTime) = "" Then
	   Call DisplayMsgBox("900010", vbOKOnly, "", "", I_MKSCRIPT)
	   Response.End
	ELSE
	   strDoTime = FilterVar(strDoTime, "''", "S")
	END IF
	...
	If Cint(strQryMode) = Cint(OPMD_CMODE) Then
		...
		IF Request("txtWcCd") <> "" Then
			...
			Else
				%>
				<Script Language=vbscript>
					parent.frm1.txtWcCd.value = "<%=ConvSPChars(rs4("WC_CD"))%>"
				</Script>
				<%
				rs4.Close
				Set rs4 = Nothing
	...
	UNISqlId(0) = "mc400mb101"
	
	UNIValue(0, 0) = "^"
	UNIValue(0, 1) = "" & FilterVar("IS", "''", "S") & ""		'ISSUED
	UNIValue(0, 2) = strPlantCd
	UNIValue(0, 3) = strDoDate
	UNIValue(0, 4) = strDoTime 	
...
%>
	...
	Else
		...
		.frm1.hDoDate.value			= "<%=UNIDateClientFormat(Request("txtDoDate"))%>"
		.frm1.hDoTime.value			= "<%=UNIDateClientFormat(Request("txtDoTime"))%>"
