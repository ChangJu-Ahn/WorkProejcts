...
%>
		<Script Language=vbScript>
		With parent
			.txtDeptCd.value = "<%=Trim(rs1(0))%>"
			.txtDeptNm.value = "<%=Trim(rs1(1))%>"
		End With
		</Script>
<%
    End If
...
%>
		<Script Language=vbScript>
		With parent
			.txtBpCd.value = "<%=Trim(rs2(0))%>"
			.txtBpNm.value = "<%=Trim(rs2(1))%>"
		End With
		</Script>
<%
    End If
...
Sub TrimData()
		...
		strCond = strCond & " AND fp.PRRCPT_STS  = " & FilterVar("O", "''", "S") & "  AND fp.CONF_FG = " & FilterVar("C", "''", "S") & "  " 
		strCond = strCond & " AND fp.bal_amt <> 0 AND fp.Gl_NO <> " & FilterVar("","''","S") & " "
	...
	If lgAuthBizAreaCd <> "" Then
		strCond		= strCond & " AND fp.BIZ_AREA_CD = " & FilterVar(lgAuthBizAreaCd, "''", "S")
	End If
	
	If lgInternalCd <> "" Then
		strCond		= strCond & " AND fp.INTERNAL_CD = " & FilterVar(lgInternalCd, "''", "S")
	End If
	
	If lgSubInternalCd <> "" Then
		strCond		= strCond & " AND fp.INTERNAL_CD LIKE " & FilterVar(lgSubInternalCd & "%", "''", "S")
	End If
	
	If lgAuthUsrID <> "" Then
		strCond		= strCond & " AND fp.INSRT_USER_ID = " & FilterVar(lgAuthUsrID, "''", "S")
	End If  
End Sub 
...
<Script Language=vbscript>
    If "<%=lgDataExist%>" = "Yes" Then
       ...
       End If
			...
			Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,Parent.GetKeyPos("A",8),Parent.GetKeyPos("A",12),"A", "Q" ,"X","X")
			Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,Parent.GetKeyPos("A",8),Parent.GetKeyPos("A",14),"A", "Q" ,"X","X")
