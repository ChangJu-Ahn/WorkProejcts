...
'============================================================================================================
Sub  TrimData()
     strFrDt		= Trim(UNIConvDateToYYYYMMDD(Request("txtPrFrDt"),gDateFormat,""))
     strToDt		= Trim(UNIConvDateToYYYYMMDD(Request("txtPrToDt"),gDateFormat,""))
...
Sub  SubPreQueryBySp()
	lgStrSQL = ""
	lgStrSQL = lgStrSQL & " DELETE FROM A_TMP_PR_TRACE_LIST WHERE QUERY_ID < CONVERT(VARCHAR(8),  GETDATE(),112)"
    ...
    CONST CALLSPNAME = "A_USP_FROM_PR_TO_GL_TRACE"
...
Sub SubBizQueryMulti()
	...
	lgStrSQL = lgStrSQL & " SUM(CASE WHEN PR_FG=" & FilterVar("PR", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) PRRCPT_LOC_AMT , "
	lgStrSQL = lgStrSQL & " SUM(CASE WHEN PR_FG=" & FilterVar("BATCH", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) BATCH_LOC_AMT, "
	lgStrSQL = lgStrSQL & " SUM(CASE WHEN PR_FG=" & FilterVar("GL", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) GL_LOC_AMT, "
	lgStrSQL = lgStrSQL & " SUM(CASE WHEN PR_FG=" & FilterVar("PR", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) - SUM(CASE WHEN PR_FG=" & FilterVar("GL", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) Diff_LOC_AMT, "
	lgStrSQL = lgStrSQL & " SUM(CASE WHEN PR_FG=" & FilterVar("TEMP_GL", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) TEMP_GL_LOC_AMT "
	...
	End If
	lgStrSQL = lgStrSQL & " FROM A_TMP_PR_TRACE_LIST "
	...
	If UCase(Trim(Request("DispMeth"))) Then 
		lgStrSQL = lgStrSQL & " HAVING ISNULL(SUM(CASE WHEN PR_FG=" & FilterVar("PR", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END),0) "
		lgStrSQL = lgStrSQL & "  <> ISNULL(SUM(CASE WHEN PR_FG=" & FilterVar("GL", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END),0) "
	End If		
	...
	Else 
		...
		Do While Not lgObjRs.EOF
			...
			lgstrData = lgstrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(lgObjRs("PRRCPT_LOC_AMT"), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")
