...
Else
	If Trim(Request("rdoStatusflag")) = "R" Then
		UNIValue(0, 6) =  " ISNULL(D.TAX_BILL_NO, '') <> ''  "
	Else
		UNIValue(0, 6) =  " ISNULL(D.TAX_BILL_NO, '') = ''  "
	End If
...
<%		Dim iDx
		For iDx = 0 to rs0.RecordCount - 1 %>
			...
			strData = strData & Chr(11) & "<%=ConvSPChars(rs0("dem_reg_num"))%>"
			strData = strData & Chr(11) & "<%=ConvSPChars(rs0("dem_cmp_name"))%>"
