...
Const BIZ_PGM_ID      = "GB004MB1.asp"						           '☆: Biz Logic ASP Name
...
Function OpenCost(strCode, iWhere, Row)
	...
	Select Case iWhere
	    ...
	    Case 2
	       ...
	       arrParam(2) = Trim(strCode)	     	    <%' Code Condition%>
	       arrParam(3) = "" 		                <%' Name Cindition%>
	       arrParam(4) = "a.acct_cd = b.acct_cd and a.gp_cd = " & FilterVar(acct_gp, "''", "S") & " and (a.temp_fg_3 in (" & FilterVar("G2", "''", "S") & "," & FilterVar("G3", "''", "S") & "," & FilterVar("G4", "''", "S") & "," & FilterVar("G5", "''", "S") & "," & FilterVar("G6", "''", "S") & "," & FilterVar("G7", "''", "S") & ") or (a.temp_fg_3 = " & FilterVar("G1", "''", "S") & " and b.acct_type = " & FilterVar("T", "''", "S") & " )) and a.DEL_FG <> " & FilterVar("Y", "''", "S") & "  "          <%' Where Condition%>
	       arrParam(5) = "계정코드"
	     ...
	     Case 3
	    	...
	    	arrParam(3) = Trim(strCode)			            ' Name Cindition
	    	arrParam(4) = "BM.major_cd = " & FilterVar("G1004", "''", "S") & " AND   BM.MINOR_CD = BC.MINOR_CD AND   BC.SEQ_NO = 2 AND  REFERENCE  = " & FilterVar("Y", "''", "S") & " "' Where Condition
	   ...
	   .ReDraw = false
        ...
        select case CStr(frm1.txtCurrencyCode.value)
           	case "1"
				...
				ggoSpread.SSSetEdit     C_cost_nm         ,     "Cost Center명" ,22,,,20
				ggoSpread.SSSetEdit     C_acct_gp         ,     "계정그룹"      ,12,,,20,2
				ggoSpread.SSSetButton   c_code_gb
				ggoSpread.SSSetEdit     C_acct_gm         ,     "계정그룹명"    ,30,,,30
				ggoSpread.SSSetEdit     C_acct_cd         ,     "계정코드"      ,23,,,20,2
				ggoSpread.SSSetButton   c_code_pb
				ggoSpread.SSSetEdit     C_acct_nm         ,     "계정명"        ,30,,,30
				ggoSpread.SSSetEdit     C_alloc_base      ,     "배부기준"      ,15,,,50
				ggoSpread.SSSetButton   C_curr_PB
				ggoSpread.SSSetFloat    C_alloc_rate      ,     "배부기준율"     ,15,6,ggStrIntegeralPart, ggStrDeciPointPart,Parent.gComNum1000,Parent.gComNumDec',,,"P"
			...
			case "2"
				...
				ggoSpread.SSSetButton   C_ACCT_PB
				ggoSpread.SSSetEdit     C_cost_nm         ,     "Cost Center명" ,50,,,20
				
				ggoSpread.SSSetEdit     C_acct_gp         ,     "계정그룹"      ,12,,,20,2
				ggoSpread.SSSetButton   c_code_gb
				ggoSpread.SSSetEdit     C_acct_gm         ,     "계정그룹명"    ,24,,,30
				
				ggoSpread.SSSetEdit     C_acct_cd         ,     "계정코드"      ,13,,,20,2
				ggoSpread.SSSetButton   c_code_pb
				ggoSpread.SSSetEdit     C_acct_nm         ,     "계정명"        ,24,,,30
				
				ggoSpread.SSSetEdit     C_alloc_base      ,     "배부기준"      ,25,,,50
				ggoSpread.SSSetButton   C_curr_PB
				ggoSpread.SSSetFloat    C_alloc_rate      ,     "배부기준율"     ,25,6,ggStrIntegeralPart, ggStrDeciPointPart,Parent.gComNum1000,Parent.gComNumDec',,,"P"
	        ...
	        case "3"
				...
				ggoSpread.SSSetButton   c_code_pb
				ggoSpread.SSSetEdit     C_acct_nm         ,     "계정명"        ,21,,,30
				ggoSpread.SSSetEdit     C_alloc_base      ,     "배부기준"      ,12,,,50
				ggoSpread.SSSetButton   C_curr_PB
				ggoSpread.SSSetFloat    C_alloc_rate      ,     "배부기준율"     ,12,6,ggStrIntegeralPart, ggStrDeciPointPart,Parent.gComNum1000,Parent.gComNumDec',,,"P"
       ...
       For lRow = 1 To .vspdData.MaxRows
           ...
           Select Case .vspdData.Text
               ...
               Case ggoSpread.InsertFlag                                      '☜: Update
                                    		               		...
                                    		               		strval = strval & Trim("1") & Parent.gColSep '3
                     .vspdData.Col = C_alloc_from    	      : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep '4
                     .vspdData.Col = C_acct_cd       		  : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep '5
                     .vspdData.Col = C_acct_gp       		  : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep '6
                     .vspdData.Col = C_alloc_base_code 	      : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep '7
                     .vspdData.Col = C_alloc_rate  			  : strVal = strVal & Trim(.vspdData.value) & Parent.gRowSep '8
                    lGrpCnt = lGrpCnt + 1
               Case ggoSpread.UpdateFlag                                      '☜: Update
                    ...
                    .vspdData.Col = C_acct_gp       		  : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep '6
                    .vspdData.Col = C_alloc_base_code		  : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep '7
                    .vspdData.Col = C_alloc_rate  	 		  : strVal = strVal & Trim(.vspdData.value) & Parent.gColSep '8
                    .vspdData.Col = C_alloc_base_old 		  : strVal = strVal & Trim(.vspdData.value) & Parent.gRowSep '9
                    lGrpCnt = lGrpCnt + 1
               Case ggoSpread.DeleteFlag                                      '☜: Delete
                                                  				...
                                                  				strDel = strDel & strYYYYMM& Parent.gColSep '2
                                               			   		strDel = strDel & Trim("1") & Parent.gColSep '3
                    .vspdData.Col = C_alloc_from     		  : strDel = strDel & Trim(.vspdData.Text) & Parent.gColSep '4
                    .vspdData.Col = C_acct_cd  	     		  : strDel = strDel & Trim(.vspdData.Text) & Parent.gColSep '5
                    .vspdData.Col = C_acct_gp       		  : strDel = strDel & Trim(.vspdData.Text) & Parent.gColSep '6
                    .vspdData.Col = C_alloc_base_code		  : strDel = strDel & Trim(.vspdData.Text) & Parent.gColSep '7
                    .vspdData.Col = C_alloc_rate  	 		  : strDel = strDel & Trim(.vspdData.value) & Parent.gRowSep '8
                    lGrpCnt = lGrpCnt + 1
           End Select
       Next
...
'계정 그룹코드의 값이 선택되어야 계정코드의 값이 활성화 된다.
   ...
   '=============================배부기준 값 체크 시작 ==================================================
			...
			If alloc_base <>"" Then
			    IntRetCD = CommonQueryRs("BM.MINOR_NM,BM.MINOR_CD","b_minor BM,B_CONFIGURATION BC","BM.major_cd = " & FilterVar("G1004", "''", "S") & " AND   BM.MINOR_CD = BC.MINOR_CD AND   BC.SEQ_NO = 2 AND  REFERENCE  = " & FilterVar("Y", "''", "S") & "  and BM.MINOR_NM = " & FilterVar(alloc_base, "''", "S") & "",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
			    If IntRetCD = False Then
...
Function ExeReflect()
    ...
    If currency_code = "3" Then
        Call CommonQueryRs("count(*)","g_alloc_Base","yyyymm = " & FilterVar(strYYYYMM, "''", "S") & " and alloc_kinds = " & FilterVar("1", "''", "S") & "  and from_alloc <>" & FilterVar("*", "''", "S") & "  and acct_gp <>" & FilterVar("*", "''", "S") & " " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    ...
    Else
        Call CommonQueryRs("count(*)","g_alloc_Base","yyyymm = " & FilterVar(strYYYYMM, "''", "S") & " and alloc_kinds = " & FilterVar("1", "''", "S") & "  and acct_gp <>" & FilterVar("*", "''", "S") & "  and from_alloc = " & FilterVar("*", "''", "S") & " " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
            if Trim(Replace(lgF0,Chr(11),"")) <> 0 then
	            IntRetCD = DisplayMsgBox("GA0009",Parent.VB_YES_NO,"X","X")
            end if
    End If
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>공통비 배부기준</font></td>
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS=TD6 NOWRAP WIDTH=86%><script language =javascript src='./js/gb004ma1_fpDateTime3_fpdtWk_yymm.js'></script></TD>
				...
				<TR>
					<TD WIDTH=100% HEIGHT=* valign=top>
						<TABLE <%=LR_SPACE_TYPE_30%>>
							<TR>
								<TD HEIGHT="100%">
									<script language =javascript src='./js/gb004ma1_vaSpread_vspdData.js'></script>
								</TD>
							</TR>
						</TABLE>
					</TD>
