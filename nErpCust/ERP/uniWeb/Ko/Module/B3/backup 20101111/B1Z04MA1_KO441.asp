...
Sub SetDefaultVal()	
    ...
    Call ggoOper.FormatDate(frm1.txtPefixDt, Parent.gDateFormat, 3)

	frm1.txtPefixDt.text = ToDateOfDB
...
Sub InitComboBox()
    ...
    Call CommonQueryRs("     UD_MINOR_CD,UD_MINOR_NM "," b_user_defined_minor "," UD_MAJOR_CD = " & FilterVar("zz011", "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    Call SetCombo2(frm1.cboPrefix, lgF0, lgF1, Chr(11))

    Call CommonQueryRs("     UD_MINOR_CD,UD_MINOR_NM "," b_user_defined_minor "," UD_MAJOR_CD = " & FilterVar("zz012", "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    Call SetCombo2(frm1.cboPrefix2, lgF0, lgF1, Chr(11))

	Call CommonQueryRs("     UD_MINOR_CD,UD_MINOR_NM "," b_user_defined_minor "," UD_MAJOR_CD = " & FilterVar("zz013", "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    Call SetCombo2(frm1.cboPrefix3, lgF0, lgF1, Chr(11))
...
Sub GetItemCode()
	...
	strPreFix = "1" & frm1.cboPrefix.value & frm1.cboPrefix2.value & frm1.cboPrefix3.value & "-" & Right(frm1.txtPefixDt.text,2) 


    Call CommonQueryRs(" isnull(left(max(replace(item_cd,'" & strPreFix  & "','')),3),0) "," b_item "," item_cd like " & FilterVar(strPreFix&"%", "'%'", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) 
		
		iSeqNo = cdbl(replace(lgF0,chr(11),"")) + 1
		
		frm1.txtItemCd2.value = strPreFix & right("000" & iSeqNo ,3)   
...
Sub txtPefixDt_DblClick(Button)
    If Button = 1 Then
        frm1.txtPefixDt.Action = 7
        Call SetFocusToDocument("M")
		Frm1.txtPefixDt.Focus
    End If
End Sub
								...
								<TD WIDTH=50% valign=top>
									<FIELDSET>			
										<LEGEND>기본정보</LEGEND>
											<TABLE CLASS="TB2" CELLSPACING=0>
												<TR>
													<TD CLASS=TD5 NOWRAP>채번(Product)</TD>
													<TD CLASS=TD6 NOWRAP><SELECT NAME="cboPrefix" ALT="채번코드" STYLE="Width: 150px;" tag="23" OnChange="vbscript:GetItemCode()"></SELECT>
														&nbsp;<INPUT TYPE=TEXT NAME="txtItemCd2" CLASS=protected READONLY=true TABINDEX="-1" SIZE=30 tag="14"></TD>													
												</TR>
												<TR>
													<TD CLASS=TD5 NOWRAP>채번(Type)</TD>
													<TD CLASS=TD6 NOWRAP><SELECT NAME="cboPrefix2" ALT="채번(Type)" STYLE="Width: 150px;" tag="23" OnChange="vbscript:GetItemCode()"></SELECT></TD>													
												</TR>
												<TR>
													<TD CLASS=TD5 NOWRAP>채번(SIZE)</TD>
													<TD CLASS=TD6 NOWRAP><SELECT NAME="cboPrefix3" ALT="채번(SIZE)" STYLE="Width: 150px;" tag="23" OnChange="vbscript:GetItemCode()"></SELECT></TD>													
												</TR>
												<TR>
												<TD CLASS=TD5 NOWRAP>채번(발생연도)</TD>
												<TD CLASS=TD6 NOWRAP>
													<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtPefixDt name=txtPefixDt CLASS=FPDTYYYYMMDD title=FPDATETIME tag="22X1" ALT="채번연도"></OBJECT>');</SCRIPT>
												</TD>
												</TR>
												<TR>
