...
'..품목그룹별표준라우팅 생성 기능 추가...KBS...2010.10.26...시작...
Const BIZ_PGM_CRTROUTING_ID = "b1b11mb9_ko441.asp"	
...
'		Call ggoOper.SetReqAttr(frm1.txtMoveAvgPrice, "Q")		'회색 
	...
	ElseIf frm1.cboProcType.value = "P" Then	'구매품	
        frm1.cboLotSizing.value = "M"       '2008-06-09 7:49오후 :: hanc
	End If
	Call SetPrcCtrlInd
End Sub
...
'-- 품목그룹별표준라우팅 생성 기능 추가...KBS...2010.10.26...시작...
Sub btnCrtRouting_OnClick()
	Dim Answer	
	Dim lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6
	Dim iStrSelect, iStrWhere
	Dim iCodeArr, iNameArr

	If lgBlnFlgChgValue = True Then
		Answer = DisplayMsgBox("900017", parent.VB_YES_NO, "X", "X")			 '⊙: "Will you destory previous data" 
		If Answer = vbNo Then
			Exit Sub
		End If
	End If
	...
	Call CommonQueryRs(" ITEM_GROUP_CD1, ITEM_GROUP_CD2 ", " VIEW_B_ITEM_GROUP_TREE1_KO441 ", " ITEM_CD =  " & FilterVar(frm1.txtItemCd1.value, "''", "S") & " ", lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	...
	frm1.txtItemGrp1.Value = iCodeArr(0)
	frm1.txtItemGrp2.Value = iNameArr(0)


	iStrSelect = " a.ROUT_NO, (Select Count(*) From P_ROUTING_DETAIL b Where a.PLANT_CD = b.PLANT_CD and a.ITEM_CD = b.ITEM_CD and a.ROUT_NO = b.ROUT_NO) "
	iStrWhere  = ""
	iStrWhere  = iStrWhere & "     a.PLANT_CD = " & FilterVar(frm1.txtItemCd1.value, "''", "S")
	iStrWhere  = iStrWhere & " And a.ITEM_CD  = " & FilterVar(frm1.txtPlantCd.value, "''", "S")
	iStrWhere  = iStrWhere & " And a.ROUT_NO  = " & FilterVar(frm1.txtItemGrp2.value, "''", "S")

	Call CommonQueryRs(iStrSelect , " P_ROUTING_HEADER a ", iStrWhere,  lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	...
	If Len(lgF0) Then 
		Call DisplayMsgBox("181302", "X", trim(frm1.txtItemGrp2.value), "X")
	End If	


	iStrWhere  = ""
	iStrWhere  = iStrWhere & "     PLANT_CD = " & FilterVar(frm1.txtItemCd1.value, "''", "S")
	iStrWhere  = iStrWhere & " And ITEM_GROUP_CD = " & FilterVar(frm1.txtItemGrp2.value, "''", "S")

	Call CommonQueryRs(" Count(*) ", " P_ITEM_GROUP_STD_ROUTING_KO441 ", iStrWhere,  lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	...
	If Len(lgF0) Then
		...
		If Cdbl(iCodeArr(0)) > 0 Then
		   Call DisplayMsgBox("P99003", "X", trim(frm1.txtItemGrp2.value), "X")
		   Err.Clear 
		   Exit Sub
		End If
	End If
...
Function RunCrtRouting()
	...
	strVal = BIZ_PGM_CRTROUTING_ID & "?txtMode=" & "CrtRouting"								
	strVal = strVal & "&txtPlantCd=" & Trim(frm1.txtPlantCd.value)			
	strVal = strVal & "&txtItemCd=" & Trim(frm1.txtItemCd1.value)
	strVal = strVal & "&txtItemGrp1=" & Trim(frm1.txtItemGrp1.value)
	strVal = strVal & "&txtItemGrp2=" & Trim(frm1.txtItemGrp2.value)
...
Function RunCrtRoutingOk()
	frm1.btnCrtRouting.Disabled = True

	Call DisplayMsgBox("P99007", "X", trim(frm1.txtItemGrp2.value), "X")

End Function

Function RunCrtRoutingError()

End Function
'-- 품목그룹별표준라우팅 생성 기능 추가...KBS...2010.10.26...종료...
...
Function DbQueryOk()													 '☆: 조회 성공후 실행로직 
	...
	'라우팅자동생성을 위한 품목그룹 검색을 위해 추가...시작...2010.10.28..KBS...
	Call QueryItemGrp()	
	'라우팅자동생성을 위한 품목그룹 검색을 위해 추가...종료...2010.10.28..KBS...
...
Function QueryItemGrp()
	...
	If Len(lgF0) Then 
		...
		frm1.txtItemGrp2.Value = iNameArr(0)
	Else
		frm1.txtItemGrp1.Value = ""
		frm1.txtItemGrp2.Value = ""
		frm1.btnCrtRouting.Disabled = True
	End If

	iStrSelect = " a.ROUT_NO, (Select Count(*) From P_ROUTING_DETAIL b Where a.PLANT_CD = b.PLANT_CD and a.ITEM_CD = b.ITEM_CD and a.ROUT_NO = b.ROUT_NO) "
	iStrWhere  = ""
	iStrWhere  = iStrWhere & "     a.PLANT_CD = " & FilterVar(frm1.txtPlantCd.value, "''", "S")
	iStrWhere  = iStrWhere & " And a.ITEM_CD  = " & FilterVar(frm1.txtItemCd1.value, "''", "S")
	iStrWhere  = iStrWhere & " And a.ROUT_NO  = " & FilterVar(frm1.txtItemGrp2.value, "''", "S")
	...
	If Len(lgF0) Then 
		...
		If Trim(iCodeArr(0)) <> ""  or  Cdbl(iNameArr(0)) > 0 Then
			frm1.btnCrtRouting.Disabled = True
		End If
	Else
		frm1.btnCrtRouting.Disabled = False
	End If	

	iStrWhere  = ""
	iStrWhere  = iStrWhere & "     PLANT_CD = " & FilterVar(frm1.txtPlantCd.value, "''", "S")
	iStrWhere  = iStrWhere & " And ITEM_GROUP_CD = " & FilterVar(frm1.txtItemGrp2.value, "''", "S")
