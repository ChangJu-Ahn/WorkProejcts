...
Sub SubBizDelete()
	...
	' TB_A125
    lgStrSQL =            "DELETE TB_A125_1 WITH (ROWLOCK) " & vbCrLf
    ...
    lgStrSQL =            "DELETE TB_A125_2 WITH (ROWLOCK) " & vbCrLf
	lgStrSQL = lgStrSQL & " WHERE CO_CD = " & FilterVar(Trim(UCase(wgCO_CD)),"''","S") 	 & vbCrLf
	lgStrSQL = lgStrSQL & "		AND FISC_YEAR = " & FilterVar(Trim(UCase(sFISC_YEAR)),"''","S") 	 & vbCrLf
	lgStrSQL = lgStrSQL & "		AND REP_TYPE = " & FilterVar(Trim(UCase(sREP_TYPE)),"''","S") 	 & vbCrLf  & vbCrLf 
...
Sub SubBizQuery()
	...
	Else
		...
		iDx = 1
			...
			lgstrData = lgstrData & "	.frm1.txtW15.value = """ & ConvSPChars(lgObjRs("W15")) & """" & vbCrLf
			lgstrData = lgstrData & "	.frm1.txtW16.value = """ & ConvSPChars(lgObjRs("W16")) & """" & vbCrLf
			lgstrData = lgstrData & "	.frm1.txtW17.value = """ & ConvSPChars(lgObjRs("W17")) & """" & vbCrLf
			lgstrData = lgstrData & "	.frm1.txtW18.value = """ & ConvSPChars(lgObjRs("W18")) & """" & vbCrLf
			
			
			Do While Not lgObjRs.EOF
				If lgObjRs("W6") <> "" And iDx > 4 Then
				...
				lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W9 & ", """ & ChangevbCrLf(ConvSPChars(lgObjRs("W9"))) & """" & vbCrLf
				lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W10 & ", """ & ConvSPChars(lgObjRs("W10")) & """" & vbCrLf
				lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W11_1 & ", """ & ConvSPChars(lgObjRs("W11_1")) & """" & vbCrLf
				lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W11_2 & ", """ & ConvSPChars(lgObjRs("W11_2")) & """" & vbCrLf
				...
				lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W19 & ", """ & ConvSPChars(lgObjRs("W19")) & """" & vbCrLf
				lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W20 & ", """ & ConvSPChars(lgObjRs("W20")) & """" & vbCrLf
				
				If lgObjRs("W21") = "Y" Then
					lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W21 & ", ""예""" & vbCrLf
				Else
					lgstrData = lgstrData & "	.SetText4Grid .C_C" & Right("0"&iDx,2) & "," & C_W21 & ", ""아니오""" & vbCrLf
				End If
...
Sub SubMakeSQLStatements(pMode,pCode1, pCode2, pCode3)
    Select Case pMode 
      Case "R"
			lgStrSQL =			  " SELECT  "
            lgStrSQL = lgStrSQL & "     CO_CD, FISC_YEAR,	"            
			lgStrSQL = lgStrSQL & "     REP_TYPE, SEQ_NO, W4,W5,W6,COUNTRY_NM as w6_1,	"
			lgStrSQL = lgStrSQL & "     W7,W8,W9,	"
			lgStrSQL = lgStrSQL & "     W10,W11_1,W11_2,W12,FULL_DETAIL_NM as w12_1,W13,	"
			lgStrSQL = lgStrSQL & "     W14,	"
			lgStrSQL = lgStrSQL & "     CASE WHEN W15=0 THEN NULL ELSE W15 END W15,	"
			lgStrSQL = lgStrSQL & "     CASE WHEN W16=0 THEN NULL ELSE W16 END W16,	"
			lgStrSQL = lgStrSQL & "     CASE WHEN W17=0 THEN NULL ELSE W17 END W17,	"
			lgStrSQL = lgStrSQL & "     CASE WHEN W18=0 THEN NULL ELSE W18 END W18,	"
			lgStrSQL = lgStrSQL & "      W19,	"
			lgStrSQL = lgStrSQL & "     CASE WHEN W20=0 THEN NULL ELSE W20 END W20,	"
			lgStrSQL = lgStrSQL & "     W21"	
            lgStrSQL = lgStrSQL & " FROM TB_A125 A WITH (NOLOCK)  left join ufn_TB_COUNTRY(" & filterVar(request("txtTmp"),"","S")    & ") b on a.w6=COUNTRY_CD  "
...
Sub SubBizSaveMultiCreate(arrColVal)
	...
	lgStrSQL = "INSERT INTO TB_A125 WITH (ROWLOCK) ("
	lgStrSQL = lgStrSQL & " CO_CD, FISC_YEAR, REP_TYPE, SEQ_NO" 
	lgStrSQL = lgStrSQL & " , W4, W5, W6 , W7 ,  W8 , W9 , W10, W11_1, W11_2, W12, W13, W14, W15, W16, W17, W18, W19, W20, W21 "
	...
	lgStrSQL = lgStrSQL & FilterVar(Trim(UCase(arrColVal(6))),"NULL","S")     & ","
	lgStrSQL = lgStrSQL & FilterVar(Trim(UCase(arrColVal(7))),"NULL","S")     & ","
	lgStrSQL = lgStrSQL & FilterVar(Trim(UCase(arrColVal(8))),"NULL","S")     & ","
	...
	lgStrSQL = lgStrSQL & FilterVar(Trim(UCase(arrColVal(16))),"NULL","S")     & ","
	lgStrSQL = lgStrSQL & FilterVar(Trim(UCase(arrColVal(17))),"0","N")     & ","
...
Sub SubBizSaveMultiUpdate(arrColVal)
    ...
    lgStrSQL = "UPDATE  TB_A125 WITH (ROWLOCK) "
    
    lgStrSQL = lgStrSQL & " SET " 

	lgStrSQL = lgStrSQL & " W4     = " &  FilterVar(Trim(UCase(Request("txtW4"))),"''","S")     & "," & vbCrLf
	'lgStrSQL = lgStrSQL & " W5     = " &  FilterVar(Trim(UCase(Request("txtW5"))),"0","N")     & ","
    ...
    lgStrSQL = lgStrSQL & " W11_1  = " &  FilterVar(Trim(UCase(arrColVal(7))),"NULL","S")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W11_2  = " &  FilterVar(Trim(UCase(arrColVal(8))),"NULL","S")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W12    = " &  FilterVar(Trim(UCase(arrColVal(9))),"''","S")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W13    = " &  FilterVar(Trim(UCase(arrColVal(10))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W14    = " &  FilterVar(Trim(UCase(arrColVal(11))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W15    = " &  FilterVar(Trim(UCase(arrColVal(12))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W16    = " &  FilterVar(Trim(UCase(arrColVal(13))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W17    = " &  FilterVar(Trim(UCase(arrColVal(14))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W18    = " &  FilterVar(Trim(UCase(arrColVal(15))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W19    = " &  FilterVar(Trim(UCase(arrColVal(16))),"NULL","S")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W20    = " &  FilterVar(Trim(UCase(arrColVal(17))),"0","N")     & "," & vbCrLf
    lgStrSQL = lgStrSQL & " W21    = " &  FilterVar(Trim(UCase(arrColVal(18))),"''","S")     & "," & vbCrLf
