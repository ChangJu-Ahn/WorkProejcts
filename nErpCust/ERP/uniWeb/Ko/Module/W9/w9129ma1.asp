...
'*  1. Module Name          : 지사세 
'*  2. Function Name        : 해외지사명세서 
'*  3. Program ID           : w9129ma1
'*  4. Program Name         : w9129ma1.asp
'*  5. Program Desc         : 해외지사명세서 
'*  6. Modified date(First) : 2006/01/09
'*  7. Modified date(Last)  :  2006/02/14
...
Const BIZ_MNU_ID = "w9129ma1"
Const BIZ_PGM_ID = "w9129mb1.asp"											 '☆: 비지니스 로직 ASP명 
Const EBR_RPT_ID = "w9129OA1"		' -- 주의 : EBR이 3개라 순차적으로 뒤에 붙여줌 
...
Sub InitSpreadComboBox()
    ...
    Dim iRow, iCol
	' 시부인 구분 
	IntRetCD1 = CommonQueryRs("MINOR_CD,MINOR_NM"," B_MINOR "," MAJOR_CD = 'W1091' ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Sub SetColorGrid(Byval pCol, Byval pBoolean)
	...
	'For i = C_C01 To C_C12 Step 2
		If .ColHidden = False Then
			...
			End If
			ggoSpread.SSSetProtected pCol, C_W6_1, C_W6_1
			ggoSpread.SSSetProtected pCol, C_W12_1, C_W12_1
		End If
	'Next
...
Sub InitData()
	...
	With frm1.vspdData
		...
		' -- 셀 합침 
		For iRow = C_W6 To C_W20
			ret = .AddCellSpan(C_C_NM, iRow , 2, 1)
		Next
		
		ret = .AddCellSpan(C_C_NM, C_W21 , 1, 2)
		
		' -- 높이 재지정 
		.Rowheight(C_W7) = 20		' 해외지사명 
		.Rowheight(C_W11) = 30		' 해외지사소재지 
...
Sub InitData2()
	...
	With frm1.vspdData
		...
		.Col = C_C_NM	: .value = " (8)소재지국"
		iRow = iRow + 1 : .Row = iRow    :  .TypeVAlign = 2
		.Col = C_C_NM	: .value = "    소재지국명"
	  
	  
		iRow = iRow + 1 : .Row = iRow   :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (9)해외지사명"				
		...
		iRow = iRow + 1 : .Row = iRow     :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (10)해외지사고유번호"

		iRow = iRow + 1 : .Row = iRow    :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (11)설립형태"
        
		iRow = iRow + 1 : .Row = iRow    :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (12)설립일자"
        
        iRow = iRow + 1 : .Row = iRow     :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (13)해외지사소재지"	
		...
		iRow = iRow + 1 : .Row = iRow      :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (14)업종"
		iRow = iRow + 1 : .Row = iRow      :.TypeVAlign = 2
		.Col = C_C_NM	: .value = "     업종상세"
		
		iRow = iRow + 1 : .Row = iRow       :.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (15)직원수"
		
		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Col = C_C_NM	: .value = " (16)본사파견직원수"
	    
		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Col = C_C_NM	: .value = " 해외지사 경영현황"
		.foreColor = rgb(0,0,255)
		'200703 hidden
		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Rowhidden = true ': .Col = C_C_NM	: .value = " (18)법인세"

		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Rowhidden = true ':.Col = C_C_NM	: .value = " (19)당기순이익"

		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Rowhidden = true ':.Col = C_C_NM	: .value = " (20)자산"

		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Rowhidden = true ':.Col = C_C_NM	: .value = " (21)부채"

		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Rowhidden = true ':.Col = C_C_NM	: .value = " (22)본사지원경비"

		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Col = C_C_NM	:.TypeHAlign = 2	: .value = "폐쇄"
		.Col = C_C_NM2	: .value = " (23)폐쇄일"

		iRow = iRow + 1 : .Row = iRow		:.TypeVAlign = 2
		.Col = C_C_NM2	: .value = " (24)회수금액"
		...
		For iCol = C_C01 To C_C12 Step 2
			...
			ggoSpread.SSSetEdit		iCol,	"", 20, , C_W11, 70
			...
			ggoSpread.SSSetFloat	iCol, "", 20, "8", ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , , , , C_W15
			...
			ggoSpread.SSSetFloat	iCol, "", 20, "8", ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , , , , C_W19
			ggoSpread.SSSetFloat	iCol, "", 20, "8", ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , , , , C_W20
			ggoSpread.SSSetFloat	iCol, "", 20, "8", ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , , , , C_W22
			
			ggoSpread.SSSetDate		iCol,		"",	20,		2,		Parent.gDateFormat,	C_W21
...
Function  OpenPopUp2(Byval strCode, Byval iWhere, Byval iRow )
    ...
    if frm1.vspdData.text ="" then
		call DisplayMsgBox("971012","X", "소재지국","X")
	...
	iCalledAspName = AskPRAspName("w9129pa1")
	
	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040",parent.VB_INFORMATION,"w9129pa1","x")
		IsOpenPop = False
		Exit Function
	End If
...
Sub vspdData_Change(ByVal Col , ByVal Row )
		...
		Select Case ROW
			...
			Case C_W6			
			       ...
			       .Col = Col
			        ...
			        IntRetCD =  CommonQueryRs(" COUNTRY_CD   ","B_COUNTRY"," COUNTRY_CD = '" & Trim(frm1.vspdData.text) & "' " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)            
						If IntRetCD = False Then
						    Call  DisplayMsgBox("970000","X","소재지국","X")                         '☜ : 입력된자료가 있습니다.
						    .col = col
						    .row = row
						    frm1.vspdData.Text = ""
						    .row = row+1
			...
			Case C_W10, C_W21' -- 설립일자가 폐쇄일보다 크면 에러 
				If CompareDateByFormat(GetText4Grid(Col, C_W10), GetText4Grid(Col, C_W21), GetText4Grid(C_C_NM, C_W10) , GetText4Grid(C_C_NM2, C_W21) , _
	...
	Select Case Row
		Case C_W6, C_W7, C_W8, C_W9, C_W10, C_W11, C_W12
			' -- 필수입력 체크 
			blnData = False
		...
		If Not .ColHidden Then
			For iRow = C_W6 To C_W21
				...
				If .BackColor = ggoSpread.RequiredColor Then
					Select Case iRow
						Case C_W6, C_W6_1,C_W7, C_W8, C_W9, C_W10, C_W11, C_W12,C_W12_1
							' -- 필수입력 체크 
							If Trim(.Text) = "" Then
								.Col = C_C_NM
					...
					case C_W8 '현지 법인고유번호 체크 
						...
						if .text <> "" then
							...
							if instr("89", left(.text,1) ) >  0 then
							else
								UNIMsgBox "법인고유번호 첫글자는  8,9 중에 하나 이어야 합니다.", 48, "uniERPII"
								exit function
							end if
							...
							'	UNIMsgBox "법인고유번호 두번째이후 3글자는  소재지국값 이어야 합니다.", 48, "uniERPII"
							'		exit function
							'end if
		...
		For lCol = C_C01 To C_C12 Step 2
			...
			For lRow = 1 To .MaxRows
               .Row = lRow
				...
				Select Case lRow
					Case C_W10, C_W21	' -- 날짜형 
						strVal = strVal & Trim(.Text) &  Parent.gColSep 
					
					Case C_W9
						If Trim(.Text) = "지점" Then
					...
					Case C_W18
						strVal = strVal & Trim(frm1.txtW17_1.value) &  Parent.gColSep 			
						
					Case C_W22	
						strVal = strVal & Trim(.Value) &  Parent.gRowSep 
					Case Else	' -- 숫자/문자형 
...
function FncBtnPrint1(strPrintType)
	...
	if  CommonQueryRs("distinct  seq_no "," TB_A128 A ",sWhere,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) then
						...
						<TR>
						    ...
						    <TR>
								   <TD align=right>
									   ...
									   <TR>
									       <TD CLASS="TD51" width="20%" ALIGN=CENTER rowspan="2">(4)전기말 가동 지사수</TD>
									       <TD CLASS="TD51" width="40%" ALIGN=CENTER colspan="2">당기 설치 변동사항</TD>
									       <TD CLASS="TD51" width="20%" ALIGN=CENTER rowspan="2">(7)당기발가동지사수<BR>((4)+(5)-(6))</TD>
									  </TR>
									  <TR>
						
									       <TD CLASS="TD51" width="20%" ALIGN=CENTER>(5)신설지사수</TD>
									       <TD CLASS="TD51" width="20%" ALIGN=CENTER>(6)폐쇄  지사수</TD>
