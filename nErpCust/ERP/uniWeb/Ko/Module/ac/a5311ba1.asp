...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			C_JOB_CD 			= iCurColumnPos(4)    
			C_JOB_NM			= iCurColumnPos(5)
			C_ERR_CNT			= iCurColumnPos(6)
			C_ERR_PB 			= iCurColumnPos(7)    
    End Select    
End Sub
...
'	iRetCd = ChkExistVersion
    ...
    With frm1
        ...
        strVal = strVal     & "&txtVerCd="          & .txtVerCd.Value					'☜: Query Key
...
Function OpenVerCd()
	...
	arrParam(0) = "환평가작업버전"											<%' 팝업 명칭 %>
'	arrParam(1) = " (SELECT DISTINCT(VER_CD) VER_CD FROM A_EXCHANGE_JOB WHERE YYYYMM = " & FilterVar(strYYYYMM, "''", "S") & " ) A "	<%' TABLE 명칭 %>
	arrParam(1) = " (SELECT DISTINCT(VER_CD) VER_CD FROM A_EXCHANGE_VERSION) A  "	<%' TABLE 명칭 %>	
	arrParam(2) = ""															<%' Code Condition%>
	arrParam(3) = "" 		            										<%' Name Cindition%>
	arrParam(4) = " "				<%' Where Condition%>
	arrParam(5) = "환평가작업버전"

    arrField(0) = "ED08" & parent.gColSep & "VER_CD"	     			        <%' Field명(1)%>

    arrHeader(0) = "작업버전"			    								<%' Header명(0)%>

	arrRet = window.showModalDialog("../../comasp/CommonPopup.asp", Array(arrParam, arrField, arrHeader), _
	"dialogWidth=480px; dialogHeight=450px; center: Yes; help: No; resizable: Yes; status: No;")
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS="TD6" NOWRAP>
										<script language =javascript>ExternalWrite('<OBJECT classid=<%=gCLSIDFPDT%>  NAME="txtWork_dt" CLASS="FPDTYYYYMM" TAG="12X1" ALT="작업년월" Title="FPDATETIME"  id=fpDateTime></OBJECT></TD>')</script>
									<TD CLASS="TD5" NOWRAP>평가버젼</TD>
									<TD CLASS="TD6" NOWRAP><INPUT TYPE=TEXT NAME="txtVerCd" ALT="평가버전" SIZE=10 MAXLENGTH=10 tag="12XXXU" STYLE="TEXT-ALIGN: Left"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnAcctGp" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenVerCd()"></TD>
								</TR>
							</TABLE>
						</FIELDSET>
					</TD>
