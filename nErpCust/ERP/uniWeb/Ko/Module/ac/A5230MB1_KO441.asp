...
Sub SubBizBatch()
    ...
    Dim StrFrDt,StrToDt,StrAcct,StrFrBiz,StrToBiz,StrAcctVal,StrAcctVal2,StrAcctVal3,StrAcctVal4
    ...
    StrAcctVal2	= lgKeyStream(6)
    StrAcctVal3	= lgKeyStream(7)
    StrAcctVal4	= lgKeyStream(8)
    ...
    'Call SvrMsgBox(StrFrDt &"_"& StrToDt &"_"& StrAcct &"_"& StrFrBiz &"_"& StrToBiz &"_"& StrAcctVal &"_"& StrAcctVal2 &"_"& StrAcctVal3 &"_"& StrAcctVal4, vbInformation, I_MKSCRIPT)
    With lgObjComm
    
        .CommandText = "x_usp_balance_amt_of_acct_cd_by_ctrl_cd"
	    ...
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@acct_cd"		,adVarXChar,adParamInput,10, StrAcct)
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@ctrl_cd"		,adVarXChar,adParamInput,10, StrAcctVal)
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@ctrl_cd2"		,adVarXChar,adParamInput,10, StrAcctVal2)
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@ctrl_val"		,adVarXChar,adParamInput,30, StrAcctVal3)
...
Sub SubBizQueryMulti()
    ...
    lgStrSQL =            "    SELECT TOP " & iSelCount  & "  (CASE WHEN GROUPING(CTRL_VAL_NM) = 1 THEN '' ELSE CTRL_VAL END )CTRL_VAL, "
    lgStrSQL = lgStrSQL & "           (CASE WHEN GROUPING(CTRL_VAL_NM) = 1 THEN '¼Ò°è' ELSE CTRL_VAL_NM END )CTRL_VAL_NM, "
    lgStrSQL = lgStrSQL & "           CTRL_VAL2, "
    lgStrSQL = lgStrSQL & "           CTRL_VAL_NM2, "
    lgStrSQL = lgStrSQL & "           SUM(L_YR_BAL_LOC_AMT)L_YR_BAL_LOC_AMT, "
    lgStrSQL = lgStrSQL & "           SUM(T_PRD_INC_LOC_AMT)T_PRD_INC_LOC_AMT, "
    lgStrSQL = lgStrSQL & "           SUM(T_PRD_DEC_LOC_AMT)T_PRD_DEC_LOC_AMT, "
    lgStrSQL = lgStrSQL & "           SUM(T_PRD_BAL_LOC_AMT)T_PRD_BAL_LOC_AMT "
    lgStrSQL = lgStrSQL & "      FROM x_balance_amt_of_acct_cd_by_ctrl_cd "
    lgStrSQL = lgStrSQL & "     WHERE sp_id = '" & StrId & "' "
    lgStrSQL = lgStrSQL & "    group by CTRL_VAL,CTRL_VAL_NM,CTRL_VAL2,CTRL_VAL_NM2  with rollup  "
    lgStrSQL = lgStrSQL & "    HAVING  GROUPING(CTRL_VAL) <> 1 and GROUPING(CTRL_VAL_NM) = 1 or GROUPING(CTRL_VAL_NM2) = 0  "
    else
    lgStrSQL =            "    SELECT TOP " & iSelCount  & " * "
    lgStrSQL = lgStrSQL & "      FROM x_balance_amt_of_acct_cd_by_ctrl_cd(NOLOCK) "
    lgStrSQL = lgStrSQL & "     WHERE sp_id = '" & StrId & "' "
