...
With Parent.frm1
	...
	Else
		If "<%=ConvSPChars(Trim(EG4_q_configuration(E4_gr_insp_type)))%>" = "A" Then 
			If "<%=ConvSPChars(Trim(EG4_q_configuration(E4_st_yn_after_receipt)))%>" = "Y" Then
				If parent.UNICDbl("<%=UniNumClientFormat(EG3_q_inspection_result(E3_goods_qty), ggQty.DecPoint ,0)%>") > 0 Then
					.txtSlCdForGood.Value = "<%=ConvSPChars(Trim(EG2_q_inspection_request(E2_r_sl_cd)))%>"
					.txtSlNmForGood.Value = "<%=ConvSPChars(Trim(EG2_q_inspection_request(E2_r_sl_nm)))%>"
				End If
		
				If parent.UNICDbl("<%=UniNumClientFormat(EG3_q_inspection_result(E3_defectives_qty), ggQty.DecPoint ,0)%>") > 0 Then
					.txtSlCdForDefective.Value = "<%=ConvSPChars(Trim(EG2_q_inspection_request(E2_r_sl_cd)))%>"
					.txtSlNmForDefective.Value = "<%=ConvSPChars(Trim(EG2_q_inspection_request(E2_r_sl_nm)))%>"
				End If
			End If
		ElseIf "<%=ConvSPChars(Trim(EG4_q_configuration(E4_gr_insp_type)))%>" = "B" Then 
			If "<%=ConvSPChars(Trim(EG4_q_configuration(E4_pr_yn_before_receipt)))%>" = "Y" Then
				If parent.UNICDbl("<%=UniNumClientFormat(EG3_q_inspection_result(E3_goods_qty), ggQty.DecPoint ,0)%>") > 0 Then
					.txtSlCdForGood.Value = "<%=ConvSPChars(Trim(EG2_q_inspection_request(E2_r_sl_cd)))%>"
					.txtSlNmForGood.Value = "<%=ConvSPChars(Trim(EG2_q_inspection_request(E2_r_sl_nm)))%>"
				End If
...
With parent
	.lgMvmtMethod = "<%=ConvSPChars(EG4_q_configuration(E4_gr_insp_type))%>"
	.lgSTYNAfterGR = "<%=ConvSPChars(EG4_q_configuration(E4_st_yn_after_receipt))%>"
	.lgPRYNBeforeGR = "<%=ConvSPChars(EG4_q_configuration(E4_pr_yn_before_receipt))%>"
