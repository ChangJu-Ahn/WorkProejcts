...
'###검사분류별 변경부분 Start###
Const BIZ_PGM_QRY_ID = "Q2112MB1.asp"
Const BIZ_PGM_QRY2_ID = "Q2112MB3.asp"
Const BIZ_PGM_SAVE_ID = "Q2112MB2.asp"
'/* 전체 삭제 관련 - START */
Const BIZ_PGM_DEL_ID = "Q2112MB4.asp"
'/* 전체 삭제 관련 - END */
Const BIZ_PGM_JUMP1_ID = "Q2111MA1"
Const BIZ_PGM_JUMP2_ID = "Q2113MA1"
'/* 9월 정기패치: 판정으로의 Link 추가 - START */
Const BIZ_PGM_JUMP3_ID = "Q2114MA1"
	...
	With frm1
		...
		If lgIntFlgModeM = Parent.OPMD_UMODE Then 
			...
			strVal = BIZ_PGM_QRY2_ID & "?txtMode=" & Parent.UID_M0001 _
									 ...
									 & "&txtInspSeries=" & .hInspSeries.value _
									 & "&lgStrPrevKeyM=" & lgStrPrevKeyM(iRow - 1) _
									 & "&lglngHiddenRows=" & lglngHiddenRows(iRow - 1) _
									 & "&lRow=" & CStr(iRow) _
		...
		Else
			...
			strVal = BIZ_PGM_QRY2_ID & "?txtMode=" & Parent.UID_M0001 _
									 ...
									 & "&txtInspItemCd=" & GetSpreadText(.vspdData,C_InspItemCd,Row,"X","X") _
									 & "&txtInspSeries=" & GetSpreadText(.vspdData,C_InspSeries,Row,"X","X") _
									 & "&lgStrPrevKeyM=" & lgStrPrevKeyM(iRow - 1) _
									 & "&lglngHiddenRows=" & lglngHiddenRows(iRow - 1) _
									 & "&lRow=" & CStr(iRow) _
									 & "&txtMaxRows=" & .vspdData2.MaxRows
...
Function SortSheet()
    ...
    With frm1.vspdData2
        ...
        .SortKeyOrder(1) = 1 'SS_SORT_ORDER_ASCENDING	'0: 정렬None 1 :오름차순  2: 내림차순 
        .SortKeyOrder(2) = 1 'SS_SORT_ORDER_ASCENDING	'0: 정렬None 1 :오름차순  2: 내림차순 
...
Function FncSave() 
    ...
    '8월 정기패치: 화면에 보이는 우측 스프레드에 행추가 되었으나 Hidden 스프레드에 반영이 안된 것 체크 
...
Function FncCopy() 
	...
	With frm1
		...
		If CheckDataExist = False Then Exit function
	...
	with frm1
		...
		If lgIntFlgMode = Parent.OPMD_UMODE Then
			strVal = BIZ_PGM_QRY_ID & "?txtMode=" & Parent.UID_M0001 _
									& "&txtInspReqNo=" & Trim(.hInspReqNo.value) _
									& "&txtPlantCd=" & Trim(.hPlantCd.value) _
		...
		For lRow = 1 To .vspdData2.MaxRows
			...
			Select Case .vspdData2.Text
				...
				Case ggoSpread.InsertFlag				'☜: 신규 
					...
					strVal = "C" & iColSep _
								 & GetSpreadText(.vspdData,C_InspItemCd,lRow2,"X","X") & iColSep _
								 & GetSpreadText(.vspdData,C_InspSeries,lRow2,"X","X") & iColSep _
								 & UNIConvNum(GetSpreadText(.vspdData2,C_SampleNo,lRow,"X","X"), 0) & iColSep _
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0">
							<TR>
								...
								<TD BACKGROUND="../../../CShared/image/table/seltab_up_bg.gif" ALIGN="center" CLASS="CLSMTAB"><FONT COLOR=white>수입검사내역 등록</FONT></TD>
					...
					<TD HEIGHT=*  WIDTH=100% VALIGN=TOP>						
						<TABLE <%=LR_SPACE_TYPE_60%>>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP>
									<script language =javascript src='./js/q2112ma1_fpDoubleSingle1_txtLotSize.js'></script>
								</TD>
							</TR>
							<TR>
										...
										<TR HEIGHT="*">
												...
												<TABLE <%=LR_SPACE_TYPE_20%>>
													<TR HEIGHT="*" WIDTH="100%">
														<TD WIDTH="100%" Colspan=4>
															<script language =javascript src='./js/q2112ma1_I370280086_vspdData.js'></script>
														</TD>
													</TR>
													<TR HEIGHT="20" WIDTH="100%">
														<TD CLASS="TD5" NOWRAP>시료수</TD>            
														<TD CLASS="TD6" NOWRAP>
															<script language =javascript src='./js/q2112ma1_fpDoubleSingle1_txtSampleQty.js'></script>
														</TD>
														<TD CLASS="TD5" NOWRAP>불량수</TD>
														<TD CLASS="TD6" NOWRAP>
															<script language =javascript src='./js/q2112ma1_fpDoubleSingle2_txtDefectQty.js'></script>
														</TD>
													</TR>			
													<TR HEIGHT="20" WIDTH="100%">
											...
											<TD HEIGHT=100% WIDTH=35%>
												<TABLE <%=LR_SPACE_TYPE_20%>>
													<TR HEIGHT="*">
														<TD WIDTH="100%">
															<script language =javascript src='./js/q2112ma1_I185297518_vspdData2.js'></script>
														</TD>
													</TR>
												</TABLE>
											</TD>
    	...
    	<TD WIDTH=100%>
    		<TABLE <%=LR_SPACE_TYPE_30%>>
    			<TR>        				
    				...
    				<!-- '/* 9월 정기패치: 판정으로의 Link 추가 - START */ -->
    				<TD WIDTH=* ALIGN=RIGHT><A href="vbscript:LoadInspection">수입검사</A>&nbsp;|&nbsp;<A href="vbscript:LoadDefectType">불량유형</A>&nbsp;|&nbsp;<A href="vbscript:LoadDecision">판정</A></TD>
    				<!-- '/* 9월 정기패치: 판정으로의 Link 추가 - END */ -->
					<TD WIDTH=10>&nbsp;</TD>
    			</TR>
    		</TABLE>
