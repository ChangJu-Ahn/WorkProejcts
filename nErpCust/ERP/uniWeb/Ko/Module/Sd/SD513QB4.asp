...
Sub FixUNISQLData()
	...
	iStrVal = "WHERE BH.BILL_DT >=  " & FilterVar(lgConFromDt , "''", "S") & ""			' 시작일 
	
	If lgConToDt <> "" Then
		iStrVal = iStrVal  & "AND BH.BILL_DT <=  " & FilterVar(lgConToDt , "''", "S") & ""			' 종료일 
	End If
    ...
    If lgConSalesOrgCd <> "" Then		
		...
		iStrVal = iStrVal  & "AND BH.SALES_ORG =  " & FilterVar(lgConSalesOrgCd , "''", "S") & ""	' 영업조직 
	End If	
