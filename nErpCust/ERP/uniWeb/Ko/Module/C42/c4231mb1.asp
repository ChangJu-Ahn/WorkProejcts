...
Sub SubBizQueryA()
    ...
    With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4231MA1_T1"		' --  변경해야할 SP 명 
    ...
    If Not oRs is nothing Then
		...
		For iLngRow = 0 To 	iLngGrRowCnt		
					...
					For iLngCol =0 To iLngColCnt   
						iStrData = iStrData & Chr(11) & UniConvNumberDBToCompany( arrTemp(iLngCol, iLngRow),ggQty.DecPoint, ggQty.RndPolicy, ggQty.RndUnit, 0)	' -- 행, 열, 값(썸)
...
Sub SubBizQueryB()
    ...
    With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4231MA1_T2"		' --  변경해야할 SP 명 
	    .CommandType = adCmdStoredProc
    ...
    If Not oRs is nothing Then
		...
		iLngRowCnt	= UBound(arrDataSet, 2)	
		For iLngRow = 0 To 	iLngRowCnt 
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*3, CLng(arrDataSet(0, iLngRow))-1-tmpKey) = arrDataSet(2, iLngRow)	' -- 행, 열, 값(썸)
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*3+1, CLng(arrDataSet(0, iLngRow))-1-tmpKey) = arrDataSet(3, iLngRow)	' -- 행, 열, 값(썸)
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*3+2, CLng(arrDataSet(0, iLngRow))-1-tmpKey) = arrDataSet(4, iLngRow)	' -- 행, 열, 값(썸)			
		Next
...
Sub SubBizQueryC()
   ...
   With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4231MA1_T3"		' --  변경해야할 SP 명 
	    .CommandType = adCmdStoredProc
    ...
    If Not oRs is nothing Then
		If sNextKey="" then 
			...
			For i = 0 To   iLngRowCnt 
					...
					sTxt2 = sTxt2 & "기초재공수량" & gColSep 
					sTxt2 = sTxt2 & "기초재공금액" & gColSep 
					sTxt2 = sTxt2 &  "기말재공수량" & gColSep
					sTxt2 = sTxt2 &  "기말재공금액" & gColSep  
		...
		For iLngRow = 0 To 	iLngRowCnt 																		 
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*4,   CLng(arrDataSet(0, iLngRow))-1-tmpKey) = UniConvNumberDBToCompany(arrDataSet(2, iLngRow), ggQty.DecPoint, ggQty.RndPolicy, ggQty.RndUnit, 0)			' -- 행, 열, 값(썸)
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*4+1, CLng(arrDataSet(0, iLngRow))-1-tmpKey) = UniConvNumberDBToCompany(arrDataSet(3, iLngRow), ggAmtOfMoney.DecPoint, ggAmtOfMoney.RndPolicy, ggAmtOfMoney.RndUnit, 0)	' -- 행, 열, 값(썸)
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*4+2, CLng(arrDataSet(0, iLngRow))-1-tmpKey) = UniConvNumberDBToCompany(arrDataSet(4, iLngRow), ggQty.DecPoint, ggQty.RndPolicy, ggQty.RndUnit, 0)			' -- 행, 열, 값(썸)
			arrTemp(CLng(arrDataSet(1, iLngRow)-1)*4+3, CLng(arrDataSet(0, iLngRow))-1-tmpKey) = UniConvNumberDBToCompany(arrDataSet(5, iLngRow), ggAmtOfMoney.DecPoint, ggAmtOfMoney.RndPolicy, ggAmtOfMoney.RndUnit, 0)	' -- 행, 열, 값(썸)
		Next														
		...
		For iLngRow = 0 To 	iLngGrRowCnt			
					...
					iStrData = iStrData & Chr(11) & ConvSPChars(replace(arrRows(1, iLngRow),"%2","공정소계"))
					iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(2, iLngRow))					
					' -- 데이타그리드 출력(수정분)
					For iLngCol =0 To iLngColCnt   
					...
					If Trim(arrRows(0, iLngRow)) = "%1" Then	' -- 소계행을 구분함 
						sGrpTxt = sGrpTxt & arrRows(0, iLngRow) & gColSep & 1 & gColSep &  arrRows(4, iLngRow) & gRowSep		' -- 소계구분|행번호(배열의 위치에 주의)
				
					End If
					If Trim(arrRows(1, iLngRow)) = "%2" Then	' -- 소계행을 구분함 
						sGrpTxt = sGrpTxt & arrRows(1, iLngRow) & gColSep & 2 & gColSep &  arrRows(4, iLngRow) & gRowSep		' -- 소계구분|행번호(배열의 위치에 주의)
...
Sub SubBizQueryD()
  ...
  With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4231MA1_T4"		' --  변경해야할 SP 명 
	    .CommandType = adCmdStoredProc
