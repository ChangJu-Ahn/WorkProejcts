...
'*  3. Program ID           : c4227ma1
'*  4. Program Name         : 배부요소품목별매출이익분석조회 
'*  5. Program Desc         :
'*  6. Comproxy List        : 
'*  7. Modified date(First) : 2005/12/02
...
Sub SubBizQuery()
	...
	Dim sPlantCd,sProjectNo, sItemAcct, sItemCd, sSalesGrp, sBPCd, sTypeFlag, sGrid, arrTmp, TmpBuffer(), sType2
	Dim sKey1, sKey2
...
'    On Error Resume Next                                                             '☜: Protect system from crashing
	...
	sProjectNo	= Request("txtDSTB_FCTR_CD")	
	...
	sType2		= Request("txtTYPE2")
	sKey1	=Request("txtKey1")
	sKey2	=Request("txtKey2")
		
	If sStartDt = "" And sPlantCd= "" And sProjectNo = "" And sItemAcct = "" And sItemCd = "" Then
		Call DisplayMsgBox("900014", vbInformation, "", "", I_MKSCRIPT)      '☜ : No data is found. 
		Exit Sub
	End If
	
	If sPlantCd = "" Then sProjectNo = "%"
	If sProjectNo = "" Then sProjectNo = "%"
	If sItemAcct = "" Then sItemAcct = "%"
	If sItemCd = "" Then sItemCd = "%"
	If sSalesGrp = "" Then sSalesGrp = "%"
	If sBPCd = "" Then sBPCd = "%"
	
	If Instr(1, sPlantCd, "%") = 0 And Len(sPlantCd) < 4 And sType2 = "1" Then sPlantCd = sPlantCd & "%"
	If Instr(1, sProjectNo, "%") = 0 And Len(sProjectNo) < 25 And sType2 = "1" Then sProjectNo = sProjectNo & "%"
	If Instr(1, sItemAcct, "%") = 0 And Len(sItemAcct) < 2 And sType2 = "1" Then sItemAcct = sItemAcct & "%"
	If Instr(1, sItemCd, "%") = 0 And Len(sItemCd) < 18 And sType2 = "1" Then sItemCd = sItemCd & "%"
	If Instr(1, sSalesGrp, "%") = 0 And Len(sSalesGrp) < 4 And sType2 = "1" Then sSalesGrp = sSalesGrp & "%"
	If Instr(1, sBPCd, "%") = 0 And Len(sBPCd) < 10 And sType2 = "1" Then sBPCd = sBPCd & "%"
    ...
    With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4227MA1_TYPE" & sTypeFlag		' --  변경해야할 SP 명 
		...
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@PLANT_CD",	adVarXChar,	adParamInput, 25,Replace(sPlantCd, "'", "''"))
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@PROJECT_NO",	adVarXChar,	adParamInput, 25,Replace(sProjectNo, "'", "''"))
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@ITEM_ACCT",	adVarXChar,	adParamInput, 2,Replace(sItemAcct, "'", "''"))
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@ITEM_CD",	adVarXChar,	adParamInput, 10,Replace(sItemCd, "'", "''"))
		...
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@SKEY1",	adVarXChar,	adParamInput, 25,Replace(sKey1, "'", "''"))
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@SKEY2",	adVarXChar,	adParamInput, 18,Replace(sKey2, "'", "''"))
    ...
    If Not oRs.EOF Then
		...
		If sTypeFlag = "2" Then	' -- All 또는 2번 그리드 
			...
			For iLngRow = 0 To 	iLngGrRowCnt
				...
				iStrData = iStrData & Chr(11) & ConvLang(ConvSPChars(Trim(arrRows(8, iLngRow))), arrRows(iLngGrColCnt-2, iLngRow))	' -- ITEM_ACCT_NM
				...
				iStrData = iStrData & Chr(11) & ConvSPChars(Trim(arrRows(10, iLngRow)))	' -- Row_Seq
				'iStrData = iStrData & Chr(11) & iLngRow+1
				iStrData = iStrData & Chr(12)
		...
		Response.Write "	.frm1.hPROJECT_NO.value = """ & sProjectNo & """" & vbCr
...
Function ConvLang(Byval pLang, Byval GroupNo)
	...
	If GroupNo <> "0" Then
		...
		pTmp = Replace(pTmp , "%4", "영업그룹소계")
		pTmp = Replace(pTmp , "%5", "거래처소계")
	Else
		pTmp = pLang
	End If
	ConvLang = pTmp
