...
<!-- '======================================================================================================
'*  1. Module Name          : Cost
'*  2. Function Name        :자품목투입현황 
'*  3. Program ID           : c4236ma1.asp
'*  4. Program Name         : 자품목투입현황 
'*  5. Program Desc         : 자품목투입현황 
'*  6. Modified date(First) : 2005-12-27
...
Const BIZ_PGM_ID = "c4236mb1.asp"                               'Biz Logic ASP
...
Sub InitSpreadSheet(byVal pvSpd)
    ...
    If pvSpd = "" or pvSpd ="A" Then 
		...
		ggoSpread.SSSetEdit		C_CostCd,	"작업지시C/C",12,,,,1
		ggoSpread.SSSetEdit		C_CostNm,	"C/C명"	,15
		ggoSpread.SSSetEdit		C_OrderNo,	"오더번호"	,15,,,,1
		ggoSpread.SSSetEdit		C_OrderSeq,	"오더SEQ"	,15,,,,1
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
 	Dim iCurColumnPos
 		...
 		Case "A"
			...
			C_OrderSeq		  = iCurColumnPos(5)	
			C_WcCd		  = iCurColumnPos(6)	
			C_WcNm  = iCurColumnPos(7)	
			C_ItemAcct		  = iCurColumnPos(8)	
			C_AcctNm  = iCurColumnPos(9)	
			C_ItemCd		  = iCurColumnPos(10)	
			C_ItemNm  = iCurColumnPos(11)	
			C_TypeCd		  = iCurColumnPos(12)	
			C_TypeNm  = iCurColumnPos(13)	
			C_WipQty  = iCurColumnPos(14)	
			C_WipAmt  = iCurColumnPos(15)	
...
Function OpenPopUp(Byval iWhere)
	...
	Select Case iWhere
		...
		Case 4
			arrParam(0) = "작업장 팝업"
			arrParam(1) =" (SELECT WC_CD, WC_NM from P_WORK_CENTER UNION ALL SELECT PUR_GRP AS WC_CD, PUR_GRP_NM AS WC_NM FROM B_PUR_GRP ) A "
		...
		Case 5
			arrParam(0) = "오더번호 팝업"
			arrParam(1) = " (select distinct b.cost_cd,b.cost_nm,a.order_No "
			arrParam(1) = arrParam(1) & "	from c_bom_issue_by_opr_s a(nolock) "
			arrParam(1) = arrParam(1) & "	inner join b_cost_center b(nolock)"
			arrParam(1) = arrParam(1) & "	on a.cost_cd=b.cost_cd"
			arrParam(1) = arrParam(1) & "	 where a.yyyymm = " & filtervar ( strYYYYMM,"","S") &  ") A"
			arrParam(2) = Trim(.txtOrder_No.value)
			arrParam(3) = ""
			arrParam(4) = ""
			arrParam(5) = "오더번호" 

			arrField(0) ="HH" & Parent.gColSep & "order_No"
			arrField(1) ="ED10" & Parent.gColSep & "cost_cd"		
			arrField(2) = "ED20" & Parent.gColSep & "cost_nm"
			arrField(3) = "ED20" & Parent.gColSep & "order_No"		
    
			arrHeader(0) = "오더번호"
			arrHeader(1) = "작업지시C/C"
			arrHeader(2) = "작업지시C/C명"	
			arrHeader(3) = "오더번호"
		...
		Case 6
			...
			arrParam(1) = arrParam(1) & "	from b_minor a(nolock) join c_bom_issue_by_opr_s b(nolock) "
...
Function DbQuery() 
    ...
    With frm1
		...
		If lgIntFlgMode = parent.OPMD_UMODE Then
			...
			strVal = strVal & "&txtOrder_No=" & Trim(.hOrder_no.value)
		...
		Else
			...
			strVal = strVal & "&txtOrder_No=" & Trim(.txtOrder_No.value)
...
'check cost
	If Trim(frm1.txtCost_cd.value) <> "" Then		
		...
		strWhere = " cost_cd = " & FilterVar(frm1.txtCost_cd.value, "''", "S") & "  and cost_type='M' "
...
'check item acct	
	...
	If Trim(frm1.txtWC_Cd.value) <> "" Then
		strFrom = "( select wc_cd, wc_nm  from 	p_work_center a(nolock) union all "
		strFrom = strFrom & "  select pur_grp as wc_cd, pur_grp_nm as wc_nm from  b_pur_grp b(nolock) ) a "
		
		strWhere = " a.wc_cd  = " & FilterVar(frm1.txtWC_Cd.value, "''", "S") & " "			
		
		Call CommonQueryRs("  a.wc_nm ", strFrom , strWhere,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	...
	End If
'check order no
	If Trim(frm1.txtOrder_No.value) <> "" Then
		strFrom = "	c_bom_issue_by_opr_s a(nolock) inner join b_cost_center b(nolock) on a.cost_cd=b.cost_cd "
		
		strWhere = " a.order_no  = " & FilterVar(frm1.txtOrder_No.value, "''", "S") & " "			
		strWhere = strWhere & " and a.yyyymm="  & filterVar(sStartDt,"","S")	& " "
		Call CommonQueryRs(" distinct a.cost_cd, b.cost_nm , a.order_no ", strFrom , strWhere,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	
		IF Len(lgF0) < 1 Then 
			Call DisplayMsgBox("970000","X",frm1.txtOrder_No.alt,"X")			
			ChkKeyField = False
			frm1.txtOrder_No.focus
			Exit Function
		End If
	
		strDataNm = split(lgF0,chr(11))
		frm1.txtOrder_No.value = strDataNm(2)
	Else
		frm1.txtOrder_No.value=""
	End If
					...
					<TD HEIGHT=20 WIDTH=100%>
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								...
								<TR>
									<TD CLASS=TD5 NOWRAP>오더번호</TD>
									<TD CLASS=TD6 NOWRAP><INPUT TYPE=TEXT NAME="txtOrder_no" SIZE=18 MAXLENGTH=18 tag="11xxxU" ALT="제조오더번호"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnProdOrder" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenPopUp(5)"></TD>									
									<TD CLASS="TD5">수불유형</TD>
									<TD CLASS="TD6" NOWRAP><input NAME="txtMovType" TYPE="Text" MAXLENGTH="3" tag="11XXXU" size="10" ALT="수불유형"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnMovType" align=top TYPE="BUTTON" onclick="vbscript:Call OpenPopup(6)">
