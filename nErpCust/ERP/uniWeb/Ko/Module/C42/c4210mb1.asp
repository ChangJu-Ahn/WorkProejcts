...
Sub SubBizQuery()
	...
	sDstbOrder		= Request("txtDSTB_ORDER")	
	sSenderCostCd	= Request("txtSENDER_COST_CD")	
	...
	If sDstbOrder = "" Then sDstbOrder = "%"
	If sSenderCostCd = "" Then sSenderCostCd = "%"
    ...
    With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4210MA1_TYPE" & sType		' --  변경해야할 SP 명 
	    .CommandType = adCmdStoredProc
		'lgObjComm.Parameters.Refresh
		...
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@DSTB_ORDER",	adVarXChar,	adParamInput, 3,Replace(sDstbOrder, "'", "''"))
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@SENDER_COST_CD",	adVarXChar,	adParamInput, 10,Replace(sSenderCostCd, "'", "''"))
	...
	' -- 그룹바이 행으로 루핑 
		For iLngRow = 0 To 	iLngGrRowCnt
			...
			If sType = "1" Then
				iStrData = iStrData & Chr(11) & ConvLang(ConvSPChars(Trim(arrRows(0, iLngRow))), arrRows(iLngGrColCnt-2, iLngRow))								' -- DSTB_ORDER
				iStrData = iStrData & Chr(11) & ConvLang(ConvSPChars(Trim(arrRows(1, iLngRow))), arrRows(iLngGrColCnt-2, iLngRow))	' -- SENDER_COST_CD
				iStrData = iStrData & Chr(11) & ConvLang(ConvSPChars(Trim(arrRows(2, iLngRow))), "0")					' -- SENDER_COST_NM
				iStrData = iStrData & Chr(11) & Trim(arrRows(3, iLngRow))					' -- SEND_AMT
			...
			Else	 ' -- B 인 경우 
				iStrData = iStrData & Chr(11) & ConvLang2(ConvSPChars(Trim(arrRows(0, iLngRow))), arrRows(iLngGrColCnt-2, iLngRow))	' -- ACCT_CD
		...
		If sType = "1" Then
			...
			Response.Write "	.frm1.hSENDER_COST_CD.value = """ & sSenderCostCd & """" & vbCr 	
		...
		Else
			...
			Response.Write "	.frm1.hSENDER_COST_CD2.value = """ & sSenderCostCd & """" & vbCr
