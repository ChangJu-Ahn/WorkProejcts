...
<!-- '======================================================================================================
'*  1. Module Name          : Cost
'*  2. Function Name        : C/C별 배부내역조회 
'*  3. Program ID           : c4210ma1.asp
'*  4. Program Name         : C/C별 배부내역조회 
'*  5. Program Desc         : C/C별 배부내역조회 
...
Const BIZ_PGM_ID = "c4210mb1.asp"                               'Biz Logic ASP
...
Sub InitSpreadSheet(Byval pMaxCols)
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetEdit		1,	"배부순서" , 7,,,,1	
		ggoSpread.SSSetEdit		2,	"Sender C/C"	, 10,,,,1	
		ggoSpread.SSSetEdit		3,	"Sender C/C"	, 15
		ggoSpread.SSSetFloat		4,	"금액"	, 15,		Parent.ggAmtOfMoneyNo,		ggStrIntegeralPart,		ggStrDeciPointPart,		Parent.gComNum1000,		Parent.gComNumDec

		For i = 5 To pMaxCols -1			
...
Function OpenPopUp(Byval iWhere)
    ...
    With frm1
		...
		Select Case iWhere
			Case 0
				arrParam(0) = "배부순서 팝업"
				arrParam(1) = "dbo.c_mfc_alloc_by_cc_s"	
				arrParam(2) = Trim(.txtDSTB_ORDER.value)
				arrParam(3) = ""
				arrParam(4) = " yyyymm=" & FilterVar(sYear & sMon, "''", "S") & " and dstb_order <> 0"
				arrParam(5) = "배부순서" 

				arrField(0) = "DSTB_ORDER"	
				arrField(1) = ""
				arrField(2) = ""		
			...
			Case 1
				...
				arrParam(2) = Trim(.txtSENDER_COST_CD.value)
				arrParam(3) = ""
				arrParam(4) = " A.yyyymm=" & FilterVar(sYear & sMon, "''", "S") & " and A.dstb_order = 0"
...
Function SetPopUp(Byval arrRet, Byval iWhere)
	...
	With frm1
		...
		Select Case iWhere
			...
			Case 0
				.txtDSTB_ORDER.value		= arrRet(0)
				
			Case 1
				.txtSENDER_COST_CD.value	= arrRet(0)
				.txtSENDER_COST_NM.value	= arrRet(1)
...
Sub txtSENDER_COST_CD_onKeyPress()
	If window.event.keyCode = 13 Then
		Call FncQuery
	End If
End Sub
...
Function DbQuery() 
    ...
    With frm1
		...
		strVal = strVal & "&txtDSTB_ORDER=" & Trim(.txtDSTB_ORDER.value)
		strVal = strVal & "&txtSENDER_COST_CD=" & Trim(.txtSENDER_COST_CD.value)
...
Function DbQuery2(Byval pRow) 
    ...
    With frm1
		...
		strVal = strVal & "&txtDSTB_ORDER=" & GetText4Grid(frm1.vspdData, 1, pRow)
		strVal = strVal & "&txtSENDER_COST_CD=" & GetText4Grid(frm1.vspdData, 2, pRow)
		strVal = strVal & "&txtFROM_ACCT_CD=" & Trim(.txtFROM_ACCT_CD.value)
		strVal = strVal & "&txtTO_ACCT_CD=" & Trim(.txtTO_ACCT_CD.value)
...
'check CC
	If Trim(frm1.txtSENDER_COST_CD.value) <> "" Then
		strFrom = " dbo.c_mfc_alloc_by_cc_s a left outer join B_COST_CENTER b on a.recv_cost_cd = b.cost_cd  "
		strWhere = " a.recv_cost_cd  = " & FilterVar(frm1.txtSENDER_COST_CD.value, "''", "S") & " "	
		...
		IF Len(lgF0) < 1 Then 
			Call DisplayMsgBox("970000","X",frm1.txtSENDER_COST_CD.alt,"X")
			frm1.txtSENDER_COST_CD.focus 
			frm1.txtSENDER_COST_NM.value = ""
			ChkKeyField = False
			Exit Function
		End If
	
		strDataNm = split(lgF0,chr(11))
		frm1.txtSENDER_COST_NM.value = strDataNm(0)
	ELSE
		frm1.txtSENDER_COST_NM.value=""
	End If
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS="TD5" NOWRAP>배부순서</TD>
									<TD CLASS="TD6" NOWRAP><input NAME="txtDSTB_ORDER" TYPE="Text" MAXLENGTH="2" tag="15XXXU" size="10" ALT="배부순서"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCalType" align=top TYPE="BUTTON" onclick="vbscript:Call OpenPopup(0)">
									</TD>
								</TR>    
								<TR>
									<TD CLASS="TD5">Sender C/C</TD>
									<TD CLASS="TD6" NOWRAP><input NAME="txtSENDER_COST_CD" TYPE="Text" MAXLENGTH="10" tag="15XXXU" size="10" ALT="Sender C/C"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCalType" align=top TYPE="BUTTON" onclick="vbscript:Call OpenPopup(1)">
									<input NAME="txtSENDER_COST_NM" TYPE="TEXT"  tag="14XXX" size="40">
									</TD>
									<TD CLASS="TD5">계정</TD>
									<TD CLASS="TD6" valign=top><input NAME="txtFROM_ACCT_CD" TYPE="Text" MAXLENGTH="20" tag="15XXXU" size="10" ALT="시작 계정"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCalType" align=top TYPE="BUTTON" onclick="vbscript:Call OpenPopup(2)">
									<input NAME="txtFROM_ACCT_NM" TYPE="TEXT"  tag="14XXX" size="30">&nbsp;~<br>
									<input NAME="txtTO_ACCT_CD" TYPE="Text"  tag="15XXXU"MAXLENGTH="20" size="10" ALT="종료 계정"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCalType" align=top TYPE="BUTTON" onclick="vbscript:Call OpenPopup(3)">
...
<INPUT TYPE=HIDDEN NAME="hDSTB_ORDER" tag="24" TABINDEX= "-1">
<INPUT TYPE=HIDDEN NAME="hSENDER_COST_CD" tag="24" TABINDEX= "-1">
...
<INPUT TYPE=HIDDEN NAME="hSENDER_COST_CD2" tag="24" TABINDEX= "-1">
</FORM>
