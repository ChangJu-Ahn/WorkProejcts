...
Const BIZ_PGM_ID = "c4207mb1.asp"                               'Biz Logic ASP
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetEdit		6,	"공순"	, 5,,,,1
		ggoSpread.SSSetEdit		7,	"공정"	, 7,,,,1
		ggoSpread.SSSetEdit		8,	"공정명"	, 10				
...
Sub SetQuerySpreadColor(Byval pGrpRow)
	...
	For i = 0 to iLoopCnt -1
		...
		Select Case arrCol(0)
			Case "2"
				.Col = -1
			   ret = .AddCellSpan(6, .Row , 3, 6)
				.BackColor = RGB(250,250,210) 
				.ForeColor = vbBlack
			Case "3"
				.Col = -1
				ret = .AddCellSpan(4, .Row , 5, 6)
				.BackColor = RGB(204,255,153) 
				.ForeColor = vbBlack
			Case "4"
				.Col = -1
				ret = .AddCellSpan(3, .Row , 6, 6)
				.BackColor = RGB(204,255,255) 
				.ForeColor = vbBlack
			Case "5"  
				.Col = -1
				ret = .AddCellSpan(2, .Row, 7, 6)
				.BackColor = RGB(255,228,181) 
				.ForeColor = vbBlack
			Case "6" 
				ret = .AddCellSpan(1, .Row, 8, 6)
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS="TD6" valign=top><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> name=txtSTART_DT CLASS=FPDTYYYYMM title=FPDATETIME ALT="기준년월" tag="12" id=txtSTART_DT></OBJECT>');</SCRIPT></TD>
