...
Sub SubBizQuery()
	...
	Dim sPlantCd, sItemAcct, sProcType, sItemCd, sWcCd, sType, iBas, arrCol, iColDept, iiColSize, sOrderNo
	...
	sOrderNo	= Request("txtORDER_NO")
	sType		= Request("txtOptionFlag")				' -- 그리드 구분 
		
	If sStartDt = "" And sPlantCd = ""  And sItemAcct = "" And sProcType = "" And sItemCd = "" And sWcCd = "" And sOrderNo = "" And sType = "" Then
    ...
    With lgObjComm
		...
		.CommandText = "dbo.usp_C_4208MA1" 		' --  변경해야할 SP 명 
		...
		For iLngRow = 0 To 	iLngRowCnt
			...
			'If sOptionFlag = "F" Then	' 28개씩 진행 
				...
				For iColDept = 0 To 5	' -- 컬럼행뎁스가 6개 
					...
					iStrData = iStrData & Chr(11) & ConvLang(ConvSPChars(Trim(arrRows(9, iLngRow))), arrRows(iLngColCnt-1, iLngRow))	' -- 공정명 
...
Function ConvLang(Byval pLang, Byval GroupNo)
	...
	If GroupNo <> "0" Then
		...
		pTmp = Replace(pTmp , "%5", "오더소계")
		pTmp = Replace(pTmp , "%6", "공정소계")
	Else
		pTmp = pLang
	End If
	ConvLang = pTmp
