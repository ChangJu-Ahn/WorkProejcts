...
Sub SubBizQuery()
	...
	sOrderNo	= TRIM(Request("txtOrder_No"))
    ...
    With lgObjComm
		.CommandTimeout = 0		
		.CommandText = "dbo.usp_C_C4236MA1_LIST"		' --  변경해야할 SP 명 
		...
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@ORDER_NO",	adVarXChar,	adParamInput, 18,Replace(sOrderNo, "'", "''"))
		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@MOV_TYPE",	adVarXChar,	adParamInput, 3,Replace(sMovTYpe, "'", "''"))
			
 		lgObjComm.Parameters.Append lgObjComm.CreateParameter("@SHEETCNT",  adSmallInt, adParamInput,, 5000)	
    ...
    If Not oRs is nothing Then
		...
		For iLngRow = 0 To 	iLngRowCnt	
				If ConvSPChars(arrRows(0, iLngRow))="%1" then 
					tmpC1=tmpC1 & ConvSPChars(arrRows(0, iLngRow)) & gColsep &  0 & gColsep & arrRows(15, iLngRow) & gRowSep
				elseIf  ConvSPChars(arrRows(1, iLngRow))="%2" then 
					tmpC1=tmpC1 &  ConvSPChars(arrRows(1, iLngRow)) & gColsep & 1 & gColsep & arrRows(15, iLngRow) & gRowSep
				elseIf  ConvSPChars(arrRows(3, iLngRow))="%3" then 
					tmpC1=tmpC1 &  ConvSPChars(arrRows(3, iLngRow)) & gColsep & 3 & gColsep & arrRows(15, iLngRow) & gRowSep
				elseIf  ConvSPChars(arrRows(5, iLngRow))="%4" then 
					tmpC1=tmpC1 &  ConvSPChars(arrRows(5, iLngRow)) & gColsep & 5 & gColsep & arrRows(15, iLngRow) & gRowSep
				elseIf  ConvSPChars(arrRows(7, iLngRow))="%5" then 
					tmpC1=tmpC1 &  ConvSPChars(arrRows(7, iLngRow)) & gColsep & 7 & gColsep & arrRows(15, iLngRow) & gRowSep	
				elseIf  ConvSPChars(arrRows(9, iLngRow))="%6" then 
					tmpC1=tmpC1 &  ConvSPChars(arrRows(9, iLngRow)) & gColsep & 9 & gColsep & arrRows(15, iLngRow) & gRowSep
				End If
				iStrData = ""
				...
				iStrData = iStrData & Chr(11) & replace(ConvSPChars(arrRows(3, iLngRow)),"%3","오더소계")				
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(4, iLngRow))
				iStrData = iStrData & Chr(11) & replace(ConvSPChars(arrRows(5, iLngRow)),"%4","작업장소계")
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(6, iLngRow))
				iStrData = iStrData & Chr(11) & replace(ConvSPChars(arrRows(7, iLngRow)),"%5","계정소계")
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(8, iLngRow))
				iStrData = iStrData & Chr(11) &  replace(ConvSPChars(arrRows(9, iLngRow)),"%6","품목소계")
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(10, iLngRow))
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(11, iLngRow))
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(12, iLngRow))			
				
				iStrData = iStrData & Chr(11) & UniConvNumberDBToCompany(arrRows(13, iLngRow),ggQty.DecPoint, ggQty.RndPolicy, ggQty.RndUnit, 0)
				...
				iStrData = iStrData & Chr(11) & ConvSPChars(arrRows(15, iLngRow))		
	...
	Response.Write "	.frm1.hOrder_no.value=	""" & sOrderNo	& """" & vbcr
