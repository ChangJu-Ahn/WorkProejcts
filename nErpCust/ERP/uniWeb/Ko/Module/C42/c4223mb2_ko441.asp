...
'Call ServerMesgBox("sType==>" & sType, vbInformation, I_MKSCRIPT)
    ...
    With lgObjComm
		...
		.CommandText = "dbo.usp_C_C4223MA2_TYPE1_" & sType & "_Ko441"	' --  변경해야할 SP 명 
    ...
    If Not oRs.EOF Then
		...
		If sNextKey = "" Then	' -- 헤더 레코드셋 포함되어 옴 
			...
			For j = i To iLngRowCnt
				...
				'2010.10.18...kbs...Add...Start
				If sType = "B" Then
				   sTxt = sTxt & "MES" & gColSep
				   sTxt = sTxt & "MES" & gColSep
				   sTxt = sTxt & "MES" & gColSep
				   sTxt = sTxt & "MES" & gColSep
				End If
				'2010.10.18...kbs...Add...End
				...
				'2010.10.18...kbs...Add...Start
				If sType = "B" Then
				   sTxt2 = sTxt2 & "Loss" & gColSep
				   sTxt2 = sTxt2 & "Bonus" & gColSep
				   sTxt2 = sTxt2 & "Give" & gColSep
				   sTxt2 = sTxt2 & "Take" & gColSep
				End If
				'2010.10.18...kbs...Add...End
		...
		For iLngRow = 0 To 	iLngRowCnt			
			...
			If sType = "A" Then
			   ...
			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*2+1, CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(3, iLngRow)		' -- 열, 행, 값(썸)
			Else
			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*6  , CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(2, iLngRow)		' -- 열, 행, 값(썸)
			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*6+1, CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(3, iLngRow)		' -- 열, 행, 값(썸)
			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*6+2, CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(4, iLngRow)		' -- 열, 행, 값(썸)
			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*6+3, CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(5, iLngRow)		' -- 열, 행, 값(썸)

			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*6+4, CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(6, iLngRow)		' -- 열, 행, 값(썸)
			   arrTemp((CLng(arrDataSet(1, iLngRow))-1)*6+5, CLng(arrDataSet(0, iLngRow))-1) = arrDataSet(7, iLngRow)		' -- 열, 행, 값(썸)
			End If
			'2010.10.18...kbs...Change...End
		...
		Else
			...
			If sNextKey = "" Then
				...
				'2010.10.18...kbs...Change...Start
				Response.Write "	Call Parent.SetQuerySpreadColor2(" & iLngRowCnt & ")	" & vbCr 
				'2010.10.18...kbs...Change...End
