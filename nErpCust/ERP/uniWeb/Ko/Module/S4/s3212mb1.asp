...
Const S367_I1_lc_no = 0 
'Const S367_I2_lc_seq = 0
...
Select Case strMode
	...
	If Trim(Request("txtLCNo")) = "" Then
		Call ServerMesgBox("수주형태값이 비어있습니다!",vbInformation, I_MKSCRIPT)              
		Response.End 
	End If
...
<Script Language=VBScript>
	With parent.frm1
		...
		.txtHOpenDt.value = "<%=ConvSPChars(E26_s_lc_hdr(S357_E26_open_dt))%>"
...
<Script Language=VBScript>
	...
	With parent
		.frm1.txtTotItemAmt.value = "<%=UNINumClientFormat(E2_s_lc_dtl(S367_E2_lc_amt), ggAmtOfMoney.DecPoint, 0)%>"
		LngMaxRow = .frm1.vspdData.MaxRows										'Save previous Maxrow
<%
	For LngRow = 0 To intGroupCount
%>
        
        strData = strData & Chr(11) & "<%=ConvSPChars(EG1_exp_grp(LngRow,S367_EG1_E4_item_cd))%>"																				'1
        strData = strData & Chr(11) & "<%=ConvSPChars(EG1_exp_grp(LngRow,S367_EG1_E4_item_nm))%>"																				'2
        strData = strData & Chr(11) & "<%=ConvSPChars(EG1_exp_grp(LngRow,S367_EG1_E4_spec))%>"
        strData = strData & Chr(11) & "<%=ConvSPChars(EG1_exp_grp(LngRow,S367_EG1_E7_lc_unit))%>"																				'3
        
        strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_lc_qty), ggQty.DecPoint, 0)%>"										'3
        
'       strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_price), ggUnitCost.DecPoint, 0)%>"		'6
' 포맷적용 (2006-06-29 박정순)

	strData = strData & Chr(11) & "<%=UNINumClientFormatByCurrency(EG1_exp_grp(LngRow,S367_EG1_E7_price), lgCurrency, ggUnitCostNo)%>"        '6 단가 


		strData = strData & Chr(11) & "<%=UNINumClientFormatByCurrency(EG1_exp_grp(LngRow,S367_EG1_E7_lc_amt), lgCurrency, ggAmtOfMoneyNo)%>"

        strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_over_tolerance), ggExchRate.DecPoint, 0)%>"	'10
        strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_under_tolerance), ggExchRate.DecPoint, 0)%>"	'11
        ...
        strData = strData & Chr(11) & "<%=ConvSPChars(EG1_exp_grp(LngRow,S367_EG1_E6_so_seq))%>"																				'12
        strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_dn_qty), ggQty.DecPoint, 0)%>"									'13
        strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_cc_qty), ggQty.DecPoint, 0)%>"									'14
        strData = strData & Chr(11) & "<%=UNINumClientFormat(EG1_exp_grp(LngRow,S367_EG1_E7_bl_qty), ggQty.DecPoint, 0)%>"													'15
        ...
        strData = strData & Chr(11) & LngMaxRow + <%=LngRow%>																									'16
