...
'*  1. Module Name          : 영업관리																	*
'*  2. Function Name        :																			*
'*  3. Program ID           : s3221mb4.asp																*
'*  4. Program Name         : 데이터가져오기(Local L/C Amend등록 Local L/C참조에서)						*
'*  5. Program Desc         : 데이터가져오기(Local L/C Amend등록 Local L/C참조에서)						*
...
Select Case strMode
	Case CStr(UID_M0001)														
		...
		Dim iObjPS4G119																' Master L/C Header 조회용 Object
		Dim iCommandSent 
		...
		Set iObjPS4G119 = Server.CreateObject("PS4G119.cSLkLcHdrSvr")

        If CheckSYSTEMError(Err, True) = True Then
            Response.End
        End If
        
        Call iObjPS4G119.S_LOOKUP_LC_HDR_SVR(gStrGlobalCollection, iCommandSent, I1_s_lc_hdr, _
...
<Script Language=VBScript>
	With parent.frm1
		...
		'##### Rounding Logic #####
		'항상 거래화폐가 우선 
		.txtAtCurrency1.value			= "<%=ConvSPChars(E26_s_lc_hdr(S357_E26_cur))%>"
		.txtAtCurrency2.value			= "<%=ConvSPChars(E26_s_lc_hdr(S357_E26_cur))%>"		
		...
		.txtBeLocAmt.text = "<%=UNINumClientFormat(E26_s_lc_hdr(S357_E26_lc_loc_amt), ggAmtOfMoney.DecPoint, 0)%>"
		.txtAtXchRate.text= "<%=UNINumClientFormat(E26_s_lc_hdr(S357_E26_xch_rate), ggExchRate.DecPoint, 0)%>"
		.txtAtLocAmt.text = "<%=UNINumClientFormat(E26_s_lc_hdr(S357_E26_lc_loc_amt), ggAmtOfMoney.DecPoint, 0)%>"
		...
		.txtRef.value = "<%=ConvSPChars(E26_s_lc_hdr(S357_E26_remark))%>"
...
<%
		Set iObjPS4G119 = Nothing														'☜: Unload Comproxy
		Set pB17014 = Nothing
