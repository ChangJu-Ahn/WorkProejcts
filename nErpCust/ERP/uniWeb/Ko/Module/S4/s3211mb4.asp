...
'*  1. Module Name          : 영업관리																	*
'*  2. Function Name        :																			*
'*  3. Program ID           : s3211mb4.asp																*
'*  4. Program Name         : 데이터 가져오기(Local L/C 등록 수주참조에서)								*
'*  5. Program Desc         : 데이터 가져오기(Local L/C 등록 수주참조에서)								*
'*  7. Modified date(First) : 2000/04/10																*
'*  8. Modified date(Last)  : 2000/12/18																*
...
<Script Language=VBScript>
	...
	With parent.frm1
		...
		.txtXchRate.text = "<%=UNINumClientFormat(E1_s_so_hdr(E1_xchg_rate),ggExchRate.DecPoint,0)%>"
		...
		.txtLocAmt.text	= "<%=UNINumClientFormat(E1_s_so_hdr(E1_net_amt_loc), ggAmtOfMoney.DecPoint, 0)%>"	
		.txtPayDur.text = "<%=ConvSPChars(E1_s_so_hdr(E1_pay_dur))%>"

		If "<%=E1_s_so_hdr(E1_lc_flag)%>" = "L" Then
