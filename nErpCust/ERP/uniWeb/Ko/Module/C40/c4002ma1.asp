...
Dim C_RECV_COST_CD_LEVEL	' -- C/C 래벨 
...
Sub InitSpreadSheet()
    ...
    ggoSpread.SSSetFloat	C_DSTB_ORDER,		"배부순서"	, 7,		"6",		ggStrIntegeralPart,		ggStrDeciPointPart,		Parent.gComNum1000,		Parent.gComNumDec ,,,,"1","" 
    ...
    ggoSpread.SSSetEdit		C_SENDER_COST_NM	,"Sender" & vbCrLf & "C/C명",15
    ...
    ggoSpread.SSSetEdit		C_STD_DSTB_FCTR_NM		,"표준원가배부요소명",20
	...
	ggoSpread.SSSetEdit		C_SEQ_NO					,"번호",7,1
	ggoSpread.SSSetEdit		C_SUB_SEQ_NO				,"번호",7,1
    ...
    ggoSpread.SSSetEdit		C_RECV_COST_NM				,"Receiver" & vbCrLf & "C/C명"	,20
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			' -- 그리드1의 컬럼 정의 
			 ...
			 C_DSTB_ORDER				= iCurColumnPos(2)	' -- 배부순서 
			 C_COST_CD_LEVEL			= iCurColumnPos(3)	' -- C/C 래벨 
		...
		Case "B"
			...
			C_RECV_COST_CD_LEVEL		= iCurColumnPos(3)		' -- C/C 래벨 
...
Function OpenPopUp2(Byval iWhere, Byval strCode, Byval strCode1)
	...
	Select Case iWhere
		Case C_RECV_COST_CD_LEVEL_POP
			...
			arrHeader(0) = "C/C Level"	
			arrHeader(1) = "Sender C/C"
			arrHeader(2) = "Sender C/C명"
			
		Case C_RECV_COST_CD_POP
			arrParam(0) = "Receiver C/C 팝업"
		...
		For iRows = 1 to  iMaxRows
			...
			If iRows <> Row Then
				If .Text = "" Then
				   iTmp = 0
				Else
				   iTmp = UNICDbl(.value)	' -- intCol의 최대값 
				End If
...
Function CancelChildGrid2()
	...
	With frm1.vspdData2
		...
		For iRow = iMaxRows To 1 Step -1
			...
			If UNICDbl(.value) = iSeqNo Then	' 부모순번이 같은 행 
				.Col = 0 : sFlag = .Text
...
Function DeleteChildGrid2()
	...
	With frm1.vspdData2
		...
		For iRow = 1 To iMaxRows
			...
			If Instr(1, sSeqNo, sSeqNo2) > 0 Then	' 부모순번이 같은 행 
...
Sub vspdData2_Change(ByVal Col, ByVal Row)
	...
	With frm1.vspdData2
		...
		Select Case Col
			...
			Case C_RECV_COST_CD_LEVEL	' -- c/c 래벨 
				sFromSQL = sFromSQL & "('7')" 
				sWhereSQL = "LEVEL_CD = " & FilterVar(sVal, "''", "S")
...
Function CheckExists(Byval pFromSQL, Byval pWhereSQL)
	...
	If CommonQueryRs(" TOP 1 1 ", pFromSQL , pWhereSQL, lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6) Then
		If Trim(lgF0) = "" Then Exit Function	' -- 존재한다고 나왔는데, 데이타가 없다면 False로 나감 
	...
	<TR HEIGHT=20>
		<TD WIDTH=100%>
			<TABLE <%=LR_SPACE_TYPE_30%>>
				<TR>
					<TD WIDTH=10></TD>
					<TD WIDTH=10>&nbsp;<BUTTON NAME="bttnPreview"  CLASS="CLSSBTN" ONCLICK="vbscript:BtnPrint('VIEW')" Flag=1 style="width: 150">배부규칙 설정확인</BUTTON></TD>
					<TD WIDTH=*></TD>
					<TD WIDTH=10>&nbsp;</TD>
				</TR>
			</TABLE>
