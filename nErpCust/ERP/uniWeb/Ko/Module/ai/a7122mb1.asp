...
'-------------------------
	...
	Set iPAAG010 = Server.CreateObject("PAAG010.cAAcqLkUpSvr")
...
'-------------------------
    ...
    For iLngRow = 0 To UBound(EG1_exp_group,1) - 1
		Call ExtractDateFrom(EG1_exp_group(iLngRow,22),"YYYYMM","",strYear,strMonth,strDay)
		StrDate = UniConvYYYYMMDDtoDate(gAPDateFormat,strYear,strMonth,"01")

		iStrData = iStrData & Chr(11) & UNIDateClientFormat(EG1_exp_group(iLngRow,12))		'1  취득일자 
		...
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,2))				'5  계정코드 
		iStrData = iStrData & Chr(11) & ""													'6  계정pop
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,3))				'7	계정단명 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,4))				'8	자산번호 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,5))				'9	자산명 
		iStrData = iStrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(EG1_exp_group(iLngRow,6),lgCurrency,ggAmtOfMoneyNo, "X" , "X")	'10	취득금액 	
		iStrData = iStrData & Chr(11) &	UNIConvNumDBToCompanyByCurrency(EG1_exp_group(iLngRow,7),gCurrency,ggAmtOfMoneyNo,gLocRndPolicyNo,"X")'11	취득금액(자국)
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,21))				'12	상각누계 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,16))				'13	재고수량 
		iStrData = iStrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(EG1_exp_group(iLngRow,9),lgCurrency,ggAmtOfMoneyNo, "X" , "X")'14	잔존가액 
		iStrData = iStrData & Chr(11) & UNIMonthClientFormat(ConvSPChars(strDate))			'15 감가상각시작년월 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,18))				'16 내용년수 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,23))				'17 상각상태코드 
		iStrData = iStrData & Chr(11) & ""													'18 상각상태코드팝업 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,25))				'19	상각상태명 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,10))				'20 참조번호 
		iStrData = iStrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow,11))				'21 적요 
