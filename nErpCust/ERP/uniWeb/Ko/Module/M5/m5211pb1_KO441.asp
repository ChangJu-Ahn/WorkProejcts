...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "M5211QA001"  										' main query(spread sheet에 뿌려지는 query statement)
	...
	IF Len(Trim(Request("txtBeneficiary"))) THEN		'수출자 
	...
	IF Len(Trim(Request("txtFrDt"))) THEN 				'B/L 일자 
		strVal = strVal & " AND BL_ISSUE_DT >= " & FilterVar(UniconvDate(Trim(Request("txtFrDt"))), "''", "S") & " "		
	END IF
		
	IF Len(Trim(Request("txtToDt"))) THEN 
		strVal = strVal & " AND BL_ISSUE_DT <= " & FilterVar(UniconvDate(Trim(Request("txtToDt"))), "''", "S") & " "		
	END IF

     If Request("gBizArea") <> "" Then
        strVal = strVal & " AND BIZ_AREA=" & FilterVar(Request("gBizArea"),"''","S")
     End If
     If Request("gPurGrp") <> "" Then
        strVal = strVal & " AND PUR_GRP=" & FilterVar(Request("gPurGrp"),"''","S")
     End If
     If Request("gPurOrg") <> "" Then
        strVal = strVal & " AND PUR_ORG=" & FilterVar(Request("gPurOrg"),"''","S")
     End If

    UNIValue(0,1) = strVal   
    UNIValue(1,0) = arrVal(0)					'거래처코드 
    
'	UNIValue(0,UBound(UNIValue,2)) = "ORDER BY BL_NO DESC "
    ...
    With parent
		...
		If "<%=lgDataExist%>" = "Yes" Then
			If "<%=lgPageNo%>" = "1" Then   ' "1" means that this query is first and next data exists
				...
				.frm1.hdnFrDt.value 			= "<%=ConvSPChars(Request("txtFrDt"))%>"
				.frm1.hdnToDt.value 			= "<%=ConvSPChars(Request("txtToDt"))%>"
			End If    
