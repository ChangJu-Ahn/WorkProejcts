...
'*  3. Program ID           : q3312mb2.asp
'*  4. Program Name         : 공정불량유형 파레토표 
'*  5. Program Desc         : 공정불량유형 
'*  6. Comproxy List        : 
'                             
'*  7. Modified date(First) : 2000/06/07
...
<Script Language=vbscript>
	...
	TempstrRoutNo		= "<%=Request("txtRoutNo")%>"
	TempstrOprNo		= "<%=Request("txtOprNo")%>"
	...
	'라우팅명 불러오기 
	Call parent.CommonQueryRs("ROUT_NO,DESCRIPTION","P_ROUTING_HEADER","PLANT_CD =  " & parent.FilterVar(TempstrPlantCd , "''", "S") & " AND ITEM_CD =  " & parent.FilterVar(TempstrItemCd , "''", "S") & " AND ROUT_NO =  " & parent.FilterVar(TempstrRoutNo , "''", "S") & "",strVar1,strVar2,strVar3,strVar4,strVar5,strVar6,strVar7)
	strVar1 = Replace(strVar1,Chr(11), "")
	strVar2 = Replace(strVar2,Chr(11), "")
	Parent.frm1.txtRoutNoDesc.Value = strVar2
	
	'공정명 불러오기 
	If TempstrRoutNo <> "" Then
		Call parent.CommonQueryRs("A.OPR_NO,B.MINOR_NM","P_ROUTING_DETAIL A,B_MINOR B","A.JOB_CD = B.MINOR_CD AND B.MAJOR_CD = " & parent.FilterVar("P1006", "''", "S") & " AND PLANT_CD =  " & parent.FilterVar(TempstrPlantCd , "''", "S") & " AND A.ITEM_CD =  " & parent.FilterVar(TempstrItemCd , "''", "S") & " AND ROUT_NO =  " & parent.FilterVar(TempstrRoutNo , "''", "S") & " AND A.OPR_NO =  " & parent.FilterVar(TempstrOprNo , "''", "S") & "",strVar1,strVar2,strVar3,strVar4,strVar5,strVar6,strVar7)
		strVar1 = Replace(strVar1,Chr(11), "")
		strVar2 = Replace(strVar2,Chr(11), "")
		Parent.frm1.txtOprNoDesc.Value = strVar2
	Else
		Call parent.CommonQueryRs("A.OPR_NO,B.MINOR_NM","P_ROUTING_DETAIL A,B_MINOR B,P_ROUTING_HEADER C","A.JOB_CD = B.MINOR_CD AND A.PLANT_CD = C.PLANT_CD AND A.ITEM_CD = C.ITEM_CD AND A.ROUT_NO = C.ROUT_NO AND B.MAJOR_CD = " & parent.FilterVar("P1006", "''", "S") & " AND C.MAJOR_FLG = " & parent.FilterVar("Y", "''", "S") & "  AND A.PLANT_CD =  " & parent.FilterVar(TempstrPlantCd , "''", "S") & " AND A.ITEM_CD =  " & parent.FilterVar(TempstrItemCd , "''", "S") & " AND A.OPR_NO =  " & parent.FilterVar(TempstrOprNo , "''", "S") & "",strVar1,strVar2,strVar3,strVar4,strVar5,strVar6,strVar7)
		strVar1 = Replace(strVar1,Chr(11), "")
		strVar2 = Replace(strVar2,Chr(11), "")
		Parent.frm1.txtOprNoDesc.Value = strVar2
	End If
...
Function DataReceive()
	...
	TempRoutNo = Request("txtRoutNo")
	If strRoutNo = "" Then
		strRoutNo = "%"
	End if
	
	strOprNo = Request("txtOprNo")
	TempOprNo = Request("txtOprNo")
	If strOprNo = "" Then
		strOprNo = "%"
	End if
...
Function GetData()
	...
	'당월 
	strSql1 = "SELECT Q_DEFECT_TYPE.Defect_Type_Nm, Sum(Q_PROCESS_DEFECT_TYPE_TOTAL.DEFECT_QTY) FROM Q_PROCESS_DEFECT_TYPE_TOTAL Left Outer Join " &_
	              "Q_DEFECT_TYPE On Q_PROCESS_DEFECT_TYPE_TOTAL.Defect_Type_Cd = Q_DEFECT_TYPE.Defect_Type_Cd Where Q_DEFECT_TYPE.Insp_Class_Cd=" & FilterVar("P", "''", "S") & "  and " &_
	                  "Q_PROCESS_DEFECT_TYPE_TOTAL.PLANT_CD = " & FilterVar(strPlantCd, "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.YR =  " & FilterVar(strYr1 , "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.Mnth =  " & FilterVar(strMnth1 , "''", "S") & " and " &_
	                  "Q_PROCESS_DEFECT_TYPE_TOTAL.Item_Cd = " & FilterVar(strItemCd, "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.ROUT_NO Like " & FilterVar(strRoutNo, "''", "S") & _
	                  " and Q_PROCESS_DEFECT_TYPE_TOTAL.OPR_NO LIKE " & FilterVar(strOprNo, "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.INSP_ITEM_CD LIKE " & FilterVar(strInspItemCd, "''", "S") & "" &_
	                  " and Q_PROCESS_DEFECT_TYPE_TOTAL.DEFECT_QTY > 0   " &_
	                  "GROUP BY Q_DEFECT_TYPE.Defect_Type_Nm Order By Sum(Q_PROCESS_DEFECT_TYPE_TOTAL.DEFECT_QTY) Desc"
	'전월 
	strSql2 = "SELECT Q_DEFECT_TYPE.Defect_Type_Nm, Sum(Q_PROCESS_DEFECT_TYPE_TOTAL.DEFECT_QTY) FROM Q_PROCESS_DEFECT_TYPE_TOTAL Left Outer Join " &_
	              "Q_DEFECT_TYPE On Q_PROCESS_DEFECT_TYPE_TOTAL.Defect_Type_Cd = Q_DEFECT_TYPE.Defect_Type_Cd Where Q_DEFECT_TYPE.Insp_Class_Cd=" & FilterVar("P", "''", "S") & "  and " &_
	                  "Q_PROCESS_DEFECT_TYPE_TOTAL.PLANT_CD = " & FilterVar(strPlantCd, "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.YR =  " & FilterVar(strYr2 , "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.Mnth =  " & FilterVar(strMnth2 , "''", "S") & " and " &_
	                  "Q_PROCESS_DEFECT_TYPE_TOTAL.Item_Cd = " & FilterVar(strItemCd, "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.ROUT_NO Like " & FilterVar(strRoutNo, "''", "S") & _
	                  " and Q_PROCESS_DEFECT_TYPE_TOTAL.OPR_NO LIKE " & FilterVar(strOprNo, "''", "S") & " and Q_PROCESS_DEFECT_TYPE_TOTAL.INSP_ITEM_CD Like " & FilterVar(strInspItemCd, "''", "S") & "" &_
	                  " and Q_PROCESS_DEFECT_TYPE_TOTAL.DEFECT_QTY > 0   " &_
	                  "GROUP BY Q_DEFECT_TYPE.Defect_Type_Nm Order By Sum(Q_PROCESS_DEFECT_TYPE_TOTAL.DEFECT_QTY) Desc"
	
	'Call ServerMesgBox(strSql1, vbCritical, I_MKSCRIPT)
	'Call ServerMesgBox(strSql2, vbCritical, I_MKSCRIPT)
	'**********************추가 부분시작*************************************
	...
	'라우팅 
	If TempRoutNo <> "" Then
		strSql = "SELECT ROUT_NO " & _
				"FROM P_ROUTING_HEADER " & _
				"WHERE	PLANT_CD = " & FilterVar(strPlantCd, "''", "S") & " AND ITEM_CD = " & FilterVar(strItemCd, "''", "S") & _
				"AND ROUT_NO = " & FilterVar(TempRoutNo, "''", "S")
		...
		'레코드가 하나도 없다면 
		If RS.EOF or RS.BOF then
			Call DisplayMsgBox("181300", vbOKOnly, "", "", I_MKSCRIPT)	'라우팅이 존재하지 않습니다.
			RS.Close
			Set RS = Nothing												'☜: ComProxy Unload
			Conn.Close
			Set Conn = Nothing												'☜: ComProxy Unload
	...
	'공정 
	If TempOprNo <> "" and TempRoutNo <> "" Then
		strSql = "SELECT OPR_NO " & _
				"FROM P_ROUTING_DETAIL " & _
				"WHERE	PLANT_CD = " & FilterVar(strPlantCd, "''", "S") & " AND ITEM_CD = " & FilterVar(strItemCd, "''", "S") & _
				"AND ROUT_NO = " & FilterVar(TempRoutNo, "''", "S") & " AND OPR_NO = " & FilterVar(TempOprNo, "''", "S")
		...
		'레코드가 하나도 없다면 
		If RS.EOF or RS.BOF then
			Call DisplayMsgBox("181200", vbOKOnly, "", "", I_MKSCRIPT)	'라우팅에 공정이 존재하지 않습니다.
			RS.Close
			Set RS = Nothing												'☜: ComProxy Unload
			Conn.Close
			Set Conn = Nothing												'☜: ComProxy Unload
