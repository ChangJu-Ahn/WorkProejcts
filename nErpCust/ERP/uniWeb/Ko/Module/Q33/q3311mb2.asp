...
'*  4. Program Name         : 공정검사품질추이(월/분기별)
'*  5. Program Desc         : 공정검사품질추이를 조회한다.
'*  6. Comproxy List        : 
'                             
...
Function GetData()
	...
	If strRoutNo = "" and strOprNo = "" Then
		strSql1 =	" SELECT Mnth, Sum(PRODT_LOT_CNT), Sum(REJT_LOT_CNT), Sum(LOT_SIZE_QTY), Sum(INSP_QTY), Sum(DEFECT_QTY) " & _
		            " FROM Q_Process_Inspection_Total " & _
					" Where PLANT_CD= " & FilterVar(strPlantCd, "''", "S") & _
		            " and ITEM_CD= " & FilterVar(strItemCd, "''", "S") & _
		            " and YR = " & strYr & _
					" GROUP BY MNTH ORDER BY Mnth"
	Else
		strSql1 =	" SELECT Mnth, PRODT_LOT_CNT, REJT_LOT_CNT, LOT_SIZE_QTY, INSP_QTY, DEFECT_QTY " & _
		            " FROM Q_Process_Inspection_Total " & _
					" Where PLANT_CD=" & FilterVar(strPlantCd, "''", "S") & _
		            " and ITEM_CD=" & FilterVar(strItemCd, "''", "S") & _
		            " and YR = " & strYr
		...
		If strRoutNo <> "" and strOprNo = "" Then
			strsql1 = strsql1 & " and ROUT_NO = " & FilterVar(strRoutNo, "''", "S") & _
								" ORDER BY Mnth"
		ElseIf strRoutNo = "" and strOprNo <> "" Then
			strsql1 = strsql1 & " and OPR_NO = " & FilterVar(strOprNo, "''", "S") & _
								" ORDER BY Mnth"
		ElseIf strRoutNo <> "" and strOprNo <> "" Then
			strsql1 = strsql1 & " and ROUT_NO = " & FilterVar(strRoutNo, "''", "S") & _
								" and OPR_NO = " & FilterVar(strOprNo, "''", "S") & _
								" ORDER BY Mnth"
		End If
	End If
	...
	strSql2 = " SELECT a.DEFECT_RATIO_UNIT_CD,  b.PARAMETER " & _
			  ...
			  " AND a.INSP_CLASS_CD = " & FilterVar("P", "''", "S") & "  "
	...
	strSql4 = " SELECT Cast(MNTH AS INT), TARGET_VALUE " & _
			  ...
			  " AND INSP_CLASS_CD = " & FilterVar("P","''","S") & _
			  " ORDER BY Mnth "
	'/* [2005-10-24] 연월목표치 없을 경우 Null값 오류 발생관련 수정 - END */	          
