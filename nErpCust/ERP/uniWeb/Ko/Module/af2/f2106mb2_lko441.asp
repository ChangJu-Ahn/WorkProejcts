...
lgSelectList ="A.CUR_BDG_YYYYMM,SUM(A.BDG_GL_AMT)  BDG_GL_AMT,SUM(A.BDG_TMP_AMT) BDG_TMP_AMT,SUM(A.BDG_GL_AMT + A.BDG_TMP_AMT) ,1"
...
'	             (DEPT_CTRL_FLAG = 'Y' AND INTERNAL_CD LIKE SUBSTRING(@INTERNAL_CD, 1, Len(@INTERNAL_CD)-1)+'%'))
	...
	'2008/01/17 수정 (하위부서통제여부 추가로 하위부서의 실적도 추가한다...	
	strWhere = ""
	strWhere = strWhere & " WHERE A.bdg_cd =  " & FilterVar(strBdgCd, "''", "S") & " and A.bdg_yyyymm =  " & FilterVar(strBdgYymm, "''", "S") _
			           ' " AND A.internal_cd LIKE SUBSTRING ( " & FilterVar(strInternalCd, "''", "S") & ",1, Len(" & FilterVar(strInternalCd, "''", "S") & ")-1)+'%' " 
  
	strWhere = strWhere & " and A.internal_cd like  " & FilterVar(trim(strInternalCd)+"%", "''", "S") & "  "
	...
	strWhere	= strWhere	& lgBizAreaAuthSQL & lgInternalCdAuthSQL & lgSubInternalCdAuthSQL & lgAuthUsrIDAuthSQL	
        strWhere = strWhere	& " group by A.CUR_BDG_YYYYMM	"
