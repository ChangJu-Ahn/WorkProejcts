...
'*  2. Function Name        : 
'*  3. Program ID           : q3411mb2.asp
'*  4. Program Name         : 최종검사품질추이(월/분기별)
'*  5. Program Desc         : 최종검사품질추이(월/분기별)를 조회한다.
'*  6. Comproxy List        : 
'                             
...
Function GetData()
	...
	strSql1 = "SELECT Mnth, PRODT_LOT_CNT, REJT_LOT_CNT, LOT_SIZE_QTY, INSP_QTY, DEFECT_QTY " & _
			   " FROM Q_Final_Inspection_Total " & _
	...
	strSql3 = " SELECT A.TARGET_VALUE, B.PARAMETER " & _
			  " FROM Q_Yearly_Target A " & _
			  " inner join Q_DEFECT_RATIO_UNIT B ON A.DEFECT_RATIO_UNIT_CD = B.DEFECT_RATIO_UNIT_CD " & _
			   " Where A.PLANT_CD = " & FilterVar(strPlantCd, "''", "S") & _
			   " and A.YR = " & FilterVar(strYr,"''","S") & _
			   " and A.INSP_CLASS_CD = " & FilterVar("F", "''", "S")
			   
	strSql4 = " SELECT Cast(MNTH AS INT), TARGET_VALUE FROM Q_Monthly_Target " & _
			   " Where PLANT_CD = " & FilterVar(strPlantCd, "''", "S") & _
			   " and YR = " & FilterVar(strYr,"''","S") & _
			   " and INSP_CLASS_CD = " & FilterVar("F", "''", "S") &  _
			   " ORDER BY Mnth "		
