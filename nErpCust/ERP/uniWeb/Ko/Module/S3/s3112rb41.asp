...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "S3112ra411"									'* : 데이터 조회를 위한 SQL문 
	...
	If Len(Request("txtSoldtoParty")) Then
		strVal = strVal & " AND A.SOLD_TO_PARTY = " & FilterVar(Request("txtSoldtoParty"), "''", "S") & " "		
	End If
	arrVal(0) = FilterVar(Trim(Request("txtSoldtoParty")),"","S")

	If Len(Request("txtItem")) Then
		strVal = strVal & " AND B.ITEM_CD LIKE " & FilterVar("%" & Trim(Request("txtItem")) & "%", " ", "S") & " "		
	End If
	...
	If Len(Request("txtSoType")) Then
		strVal = strVal & " AND A.SO_TYPE = " & FilterVar(Request("txtSoType"), "''", "S") & " "		
	End If
...
<Script Language=vbscript>
    With parent
        ...
        Call Parent.ReFormatSpreadCellByCellByCurrency2(Parent.Frm1.vspdData,-1,-1,"<%=lgCurrency%>",Parent.GetKeyPos("A",9),"C", "Q" ,"X","X")
        Call Parent.ReFormatSpreadCellByCellByCurrency2(Parent.Frm1.vspdData,-1,-1,"<%=lgCurrency%>",Parent.GetKeyPos("A",10),"A", "Q" ,"X","X")
        Call Parent.ReFormatSpreadCellByCellByCurrency2(Parent.Frm1.vspdData,-1,-1,"<%=lgCurrency%>",Parent.GetKeyPos("A",14),"A", "Q" ,"X","X")
