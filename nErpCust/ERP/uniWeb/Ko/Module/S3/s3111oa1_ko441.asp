...
Function OpenConPop1()
	...
	If frm1.txtSales_Grp.className = "protected" Then Exit Function
...
Function BtnPrint() 
	...
	If lgBACd <> "" Then
		vargBizArea = " AND S_SO_HDR.BIZ_AREA =  " & FilterVar(Trim(UCase(lgBACd)), "" ,  "S")
	Else
		vargBizArea = ""
	End If
	If lgPLCd <> "" Then
		vargPlant = " AND S_SO_DTL.PLANT_CD = " & FilterVar(Trim(UCase(lgPLCd)), "" ,  "S")
	Else
		vargPlant = ""
	End If
	If lgSGCd <> "" Then
		vargSalesGrp = " AND S_SO_HDR.SALES_GRP = " & FilterVar(Trim(UCase(lgSGCd)), "" ,  "S")
	Else
		vargSalesGrp = ""
	End If
	If lgSOCd <> "" Then
		vargSalesOrg = " AND S_SO_HDR.SALES_ORG = " & FilterVar(Trim(UCase(lgSOCd)), "" ,  "S")
	Else
		vargSalesOrg = ""
	End If	
	'--출력조건을 지정하는 부분 수정 - 끝 
...
' Print 함수에서 호출 
'----------------------------------------------------------------
	ObjName = AskEBDocumentName("s3111oa1_ko441","ebr")
