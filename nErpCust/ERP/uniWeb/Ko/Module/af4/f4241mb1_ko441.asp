...
Sub SubBizQuery()
    ...
    Const EA_f_ln_mon_dfr_plan_amt2 = 27
    Const EA_f_ln_mon_dfr_plan_loc_amt2 = 28
    Const EA_f_ln_mon_dfr_last_int_pay_dt = 29
    Const EA_f_ln_mon_dfr_last_int_pay_dt2 = 30
    Const EA_f_ln_mon_dfr_day_mthd = 31
    Const EA_f_ln_mon_dfr_xchg_rate2 = 32
...
'    End If
	...
	Set PAFG421LOOKUP = Server.CreateObject("PAFG421_KO441.bFLkUpMonDfrSvr")
		...
		For iLngRow = 0 To UBound(EG1_export_group, 1)
			...
			Else
				If UniCDbl(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_int_cls_amt), 0) = 0 Then
					...
					iStrData = iStrData & Chr(11) & UNINumClientFormat(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_plan_amt2),ggAmtOfMoney.DecPoint	,0)
					lDecClsAmt(0) = lDecClsAmt(0) + UniCDbl(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_plan_amt2),0)
			...
			iStrData = iStrData & Chr(11) & UNINumClientFormat(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_plan_amt2),ggAmtOfMoney.DecPoint	,0)
			iStrData = iStrData & Chr(11) & UNINumClientFormat(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_plan_loc_amt2),ggAmtOfMoney.DecPoint	,0)
			...
			iStrData = iStrData & Chr(11) & UNINumClientFormat(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_xchg_rate2),ggExchRate.DecPoint	,0)
			...
			'네패스 김미희과장 요청으로 변경...2009.08.28...kbs...자료 조회시점에 결산일자기준으로 금액 재계산
			iStrData = iStrData & Chr(11) & UNIDateClientFormat(Trim(EG1_export_group(iLngRow,EA_f_ln_mon_dfr_last_int_pay_dt2)))
...
Sub SubBizSave()
    ...
    Set PAFG421CU = server.CreateObject("PAFG421_KO441.cMngMonDfrSvr")   
