...
'Dim strLoanFg
Dim cboConfFg, cboApSts, cboLoanFg, txtDocCur, txtLoanPlcFg, txtLoanPlcCd, txtLoanType
Dim txtLoanPlcNm, txtLoanTypeNm
Dim strLoanDtFr
Dim strLoanDtTo
Dim strIntDtFr
Dim strIntDtTo
...
Sub FixUNISQLData()
    ...
    strPaymDt = UNIConvDate(Trim(Request("txtPaymDt")))
    
    UNISqlId(0) = "F4213MA1"
    UNISqlId(1) = "F4213MA2"					'SUM
	...
	UNIValue(0,1)  = " " & FilterVar(strPaymDt, "''", "S") & ""
	UNIValue(0,2)  = " " & FilterVar(strPaymDt, "''", "S") & ""
	UNIValue(0,3)  = " " & FilterVar(strPaymDt, "''", "S") & ""
	UNIValue(0,4)  = UCase(Trim(strWhere))
	
	'rs1에 대한 Value값 setting(총계)
	
	UNIValue(1,0)  = " " & FilterVar(strPaymDt, "''", "S") & ""
	UNIValue(1,1)  = " " & FilterVar(strPaymDt, "''", "S") & ""
	UNIValue(1,2)  = " " & FilterVar(strPaymDt, "''", "S") & ""
	UNIValue(1,3)  = UCase(Trim(strWhere))
	If txtLoanPlcFg = "BK" Then
...
Sub TrimData()
	...
	strWhere = ""
	strWhere = strWhere & " LN.loan_dt >=  " & FilterVar( strLoanDtFr , "''", "S") & " and LN.loan_dt <=  " & FilterVar(strLoanDtTo , "''", "S") & " "
	strWhere = strWhere & " AND LN.loan_dt <=  " & FilterVar( strPaymDt , "''", "S") & " "
	...
	If cboApSts   = "Y" Then strWhere = strWhere & " AND (HI.CHG_DT IS NOT NULL OR ISNULL(PM.PAY_AMT, 0) + ISNULL(LN.BAS_RDP_AMT,0) >= ISNULL(LN.LOAN_AMT, 0)) "
	If cboApSts   = "N" Then strWhere = strWhere & " AND HI.CHG_DT IS NULL AND ISNULL(PM.PAY_AMT, 0)+ISNULL(LN.BAS_RDP_AMT,0) < ISNULL(LN.LOAN_AMT, 0) "
...
With parent
	If "<%=lgDataExist%>" = "Yes" Then
		If "<%=lgPageNo%>" = "1" Then   ' "1" means that this query is first and next data exists
			...
			.Frm1.hLoanFg.Value		= "<%=ConvSPChars(cboLoanFg)%>"                  'For Next Search
			...
			.Frm1.hPaymDt.Value		= "<%=strPaymDt%>"                  'For Next Search
