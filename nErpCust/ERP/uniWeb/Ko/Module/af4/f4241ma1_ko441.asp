...
*  3. Program ID           : f4241ma1
*  4. Program Name         : 미지급이자 월결산 
*  5. Program Desc         : 
*  6. Comproxy List        :
...
'========================================================================================================
Const BIZ_PGM_ID = "f4241mb1_ko441.asp"                                      'Biz Logic ASP 
'========================================================================================================
...
Dim	C_DOC_CUR			
Dim	C_XCH_RATE			
Dim	C_DFR_INT_ACCT_CD	
Dim	C_DFR_INT_ACCT_NM	
Dim	C_INT_CLS_DT		
Dim	C_INT_PAY_DT		
Dim	C_INT_PAY_AMT		
	...
	With frm1.vspdData
		If frm1.rdoGiNo.checked = true then
			For intRow = 1 To .MaxRows
				.Row = intRow
				.Col = C_int_FG		:   intIndex = .Value            ' .Value means that it is index of cell,not value in combo cell type
...
Sub initSpreadPosVariables()  
	...
	C_INT_EXP_ACCT_NM		=  9
	C_INT_CLS_AMT			= 10
	C_INT_CLS_LOC_AMT		= 11
	C_INT_CLS_PLAN_AMT		= 12
	C_INT_CLS_PLAN_LOC_AMT	= 13
	C_DOC_CUR				= 14
	C_XCH_RATE				= 15
	C_DFR_INT_ACCT_CD		= 16
	C_DFR_INT_ACCT_NM		= 17
	C_INT_CLS_DT			= 18
	C_INT_PAY_DT			= 19
	C_INT_PAY_AMT			= 20
	C_INT_PAY_LOC_AMT		= 21
	C_INT_RATE				= 22
	C_LOAN_DT				= 23
	C_DUE_DT				= 24
	C_TEMP_GL_NO			= 25
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
       ...
       Call GetSpreadColumnPos("A") 
		...
		ggoSpread.SSSetEdit    C_DFR_INT_ACCT_CD	,"미지급이자계정"	,13		,		,	, 20
		ggoSpread.SSSetEdit    C_DFR_INT_ACCT_NM	,"미지급이자계정명",18	,       ,	, 20     ,2
		ggoSpread.SSSetDate    C_INT_CLS_DT		,"결산일자"		,12		,2                  ,Parent.gDateFormat   ,-1
		ggoSpread.SSSetDate    C_INT_PAY_DT		,"이자지급예정일자" ,12		,2                  ,Parent.gDateFormat   ,-1
		ggoSpread.SSSetFloat   C_INT_PAY_AMT		,"이자지급예정액"	,15		, "A" ,ggStrIntegeralPart, ggStrDeciPointPart,Parent.gComNum1000,Parent.gComNumDec
		ggoSpread.SSSetFloat   C_INT_PAY_LOC_AMT	,"이자지급예정액(자국)"	,15	, Parent.ggAmtOfMoneyNo ,ggStrIntegeralPart, ggStrDeciPointPart,Parent.gComNum1000,Parent.gComNumDec
		...
		ggoSpread.SSSetEdit    C_PAY_NO			,"상환번호"		,15		,		,	,15		,2
	   ...
	   Call ggoSpread.SSSetColHidden(C_PAY_NO ,C_PAY_NO		,True)

	   '네패스 추가항목...kbs...2009.08.28
	   'Call ggoSpread.SSSetColHidden(C_PAY_NO+1 ,C_PAY_NO+1		,True)
	   Call ggoSpread.SSSetColHidden(C_LAST_PAY_DT+1 ,C_LAST_PAY_DT+1		,True)
	...
	With frm1
		...
		For RowCnt = 1 To .vspdData.MaxRows
			...
			If .vspdData.text = "N" Then
				...
				'네패스 추가항목...kbs...2009.08.28
				'ggoSpread.SpreadLock	C_INT_FG	, RowCnt	, C_PAY_NO	, RowCnt
			...
			Else
				...
				If Trim(.vspdData.text) = "C" Then
					ggoSpread.SpreadLock	C_CHOICE_FG	, RowCnt	, C_PAY_NO	, RowCnt
				Else
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			C_INT_EXP_ACCT_CD			= iCurColumnPos(7)
			C_INT_EXP_ACCT_BT			= iCurColumnPos(8)
			C_INT_EXP_ACCT_NM			= iCurColumnPos(9)
			C_INT_CLS_AMT				= iCurColumnPos(10)
			C_INT_CLS_LOC_AMT			= iCurColumnPos(11)
			C_INT_CLS_PLAN_AMT			= iCurColumnPos(12)
			C_INT_CLS_PLAN_LOC_AMT		= iCurColumnPos(13)
			C_DOC_CUR					= iCurColumnPos(14)
			C_XCH_RATE					= iCurColumnPos(15)
			C_DFR_INT_ACCT_CD			= iCurColumnPos(16)
			C_DFR_INT_ACCT_NM			= iCurColumnPos(17)
			C_INT_CLS_DT				= iCurColumnPos(18)
			C_INT_PAY_DT				= iCurColumnPos(19)
			C_INT_PAY_AMT				= iCurColumnPos(20)
			C_INT_PAY_LOC_AMT			= iCurColumnPos(21)
			C_INT_RATE					= iCurColumnPos(22)
			C_LOAN_DT					= iCurColumnPos(23)
			C_DUE_DT					= iCurColumnPos(24)
			C_TEMP_GL_NO				= iCurColumnPos(25)
			C_GL_NO						= iCurColumnPos(26)
			C_CLS_FG					= iCurColumnPos(27)
			C_CONF_FG					= iCurColumnPos(28)
			C_PAY_NO					= iCurColumnPos(29)

			'네패스 추가항목...kbs...2009.08.28
			C_LAST_PAY_DT					= iCurColumnPos(30)
           ...
           Select Case .Text
               ...
               Case ggoSpread.UpdateFlag 
					...
					If Trim(.value) = "N" Then
						...
						If Trim(.Text) <> "" Then
							If CommonQueryRs("A.ACCT_CD","A_Acct A, A_ACCT_GP B, A_JNL_ACCT_ASSN C","A.GP_CD=B.GP_CD AND A.DEL_FG <> " & FilterVar("Y", "''", "S") & "  AND A.Acct_cd=C.Acct_CD and C.trans_type = " & FilterVar("FI006", "''", "S") & "  and C.jnl_cd = " & FilterVar("PI", "''", "S") & " " & _
...
Sub PopRestoreSpreadColumnInf()
	...
	Call SetSpreadLock
	Call ReFormatSpreadCellByCellByCurrency(Frm1.vspdData,-1,-1,C_DOC_CUR,C_INT_CLS_AMT,"A" ,"I","X","X")
	Call ReFormatSpreadCellByCellByCurrency(Frm1.vspdData,-1,-1,C_DOC_CUR,C_INT_CLS_PLAN_AMT,"A" ,"I","X","X")
	Call ReFormatSpreadCellByCellByCurrency(Frm1.vspdData,-1,-1,C_DOC_CUR,C_INT_PAY_AMT,"A" ,"I","X","X")
End Sub
	           ...
	           .Col = 0
				Select Case .Text
				    Case ggoSpread.InsertFlag
				 			...
				 			.Col = C_INT_PAY_DT				: strVal = strVal & UNIConvDate(Trim(.Text))		& iRowSep
				    ...
				    Case ggoSpread.DeleteFlag
				 											  ...
				 											  strVal = strVal & UNIConvDate(Trim(frm1.txtBaseDt.text))	& iColSep
				 			.Col = C_LOAN_NO				: strVal = strVal & Trim(.Text)					& iColSep
				 			.Col = C_SEQ					: strVal = strVal & UNICDbl(Trim(.Text))		& iRowSep
       ...
       For lRow = 1 To .MaxRows
			...
			If Trim(.Text) = "1" Then
				...
				If Trim(.value) = "N" Then
					...
					.Col = C_LOAN_NO				: strVal = strVal & Trim(.Text)      & iColSep
					.Col = C_SEQ					: strVal = strVal & UNICDbl(Trim(.Text))	& iColSep
					.Col = C_INT_CLS_PAMT		: strVal = strVal & UNICDbl(Trim(.Text))     & iColSep
					.Col = C_INT_CLS_LOC_AMT	: strVal = strVal & UNICDbl(Trim(.Text))     & iColSep
				 	.Col = C_INT_EXP_ACCT_CD		: strVal = strVal & Trim(.Text)     & iColSep
				 	.Col = C_INT_PAY_DT				: strVal = strVal & UNIConvDate(Trim(.Text)) & iRowSep
...
Function OpenPopup(Byval strCode,Byval iWhere)
	...
	Case 1,2
		...
		arrParam(4) = "A.GP_CD=B.GP_CD AND A.DEL_FG <> " & FilterVar("Y", "''", "S") & "  AND A.Acct_cd=C.Acct_CD" & _
			" and C.trans_type = " & FilterVar("FI006", "''", "S") & "  and C.jnl_cd = " & FilterVar("PI", "''", "S") & " "
	...
	Case 3
		...
		arrParam(2) = strCode											' Code Condition
		arrParam(3) = ""												' Name Cindition
		arrParam(4) = "B.INT_PAY_STND = " & FilterVar("DI", "''", "S") & "  AND B.CONF_FG IN (" & FilterVar("C", "''", "S") & " ," & FilterVar("E", "''", "S") & " ) AND B.LOAN_DT <= " & FilterVar(UNICONVDATE(frm1.txtBaseDt.TEXT), "''", "S") & _
...
'		If Col <> C_CHOICE_FG AND Col <> C_INT_EXP_ACCT_CD Then
'		If Col <> C_INT_EXP_ACCT_CD Then
'			.Col = C_INT_EXP_ACCT_CD	: .Text = ""
'			.Col = C_CHOICE_FG			: .Value = "0"
'			ggoSpread.SSSetProtected	C_INT_EXP_ACCT_CD   , Row	, Row
'			ggoSpread.SSSetProtected	C_CHOICE_FG			, Row	, Row
...
Sub fpLoanDt_DblClick(Button)
	if Button = 1 then
		frm1.fpLoanDt.Action = 7
		Call SetFocusToDocument("M")
		frm1.fpLoanDt.Focus
	End if
End Sub
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTABP"><font color=white>미지급이자월결산</font></td>
						...
						<FIELDSET>
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS=TD6 NOWRAP><script language =javascript src='./js/f4241ma1_fpBaseDt_txtBaseDt.js'></script>&nbsp;
						...
						<FIELDSET>
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS="TD6" NOWRAP><INPUT NAME="txtIntExpAcctCd" MAXLENGTH="18" SIZE=15  ALT ="이자비용계정" tag="11XXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnDfrInt" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopup(frm1.txtIntExpAcctCd.value, 1)">
				...
				<TR>
					<TD WIDTH="100%" HEIGHT=* VALIGN=TOP>
						<TABLE <%=LR_SPACE_TYPE_20%>>
							<TR>
								<TD HEIGHT="100%">
									<script language =javascript src='./js/f4241ma1_vaSpread1_vspdData.js'></script>
								</TD>
							</TR>
						</TABLE>
					</TD>
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_20%>>
								<TR>
									...
									<TD CLASS=TD6 NOWRAP>
										<script language =javascript src='./js/f4241ma1_fpDoubleSingle11_txtAlcSum.js'></script>&nbsp;
										<script language =javascript src='./js/f4241ma1_fpDoubleSingle12_txtAlcLocSum.js'></script>&nbsp;
	                                </TD>
									<TD CLASS="TD5" NOWRAP>결산계획총액|자국</TD>
									<TD CLASS=TD6 NOWRAP>
										<script language =javascript src='./js/f4241ma1_fpDoubleSingle21_txtPlanSum.js'></script>&nbsp;
										<script language =javascript src='./js/f4241ma1_fpDoubleSingle22_txtPlanLocSum.js'></script>&nbsp;
	                                </TD>
								</TR>
							</TABLE>
						</FIELDSET>
