...
Function FncSave() 
	...
	If UCase(Trim(frm1.txtRcptType.value)) = "FI" Then
	   If frm1.vspddata.Maxrows < 1 Then 
	      MsgBox "유산스 차입내역을 선택하십시요."
	      Exit Function
	   End if
	End if
    '-----------------------
	...
	With frm1.vspdData
		For lngRows = 1 To .MaxRows
			...
			If .Text <>  ggoSpread.DeleteFlag Then
				...
				.Col = C_CLS_LOC_AMT	:	strVal = strVal & Trim(ROUND(UNIConvNum(.Text,0),0)) & parent.gColSep
...
Function SetRefOpen(Byval arrRet)
	...
	With frm1
		If Trim(.txtBpLoanCd.value) = "" then
			...
			For ii = 1 To .vspdData.MaxRows
				...
				If .vspdData.text <> ggoSpread.DeleteFlag Then
					...
					If iDrCrFg = "DR" Then
						...
						Else
							dblCrLocAmt = ROUND(dblCrLocAmt +  UNICDBL(Trim(.vspdData.Text)),0)
						End If	
					Else
						If .vspdData.Text = "" Then						
							dblDrLocAmt = dblDrLocAmt +  0
						Else
							dblDrLocAmt = ROUND(dblDrLocAmt +  UNICDBL(Trim(.vspdData.Text)),0)
						End If	
					End If
		...
		Select Case Col
			Case C_CLS_AMT
				...
				End If
   				...
   				.vspddata.Col = C_OPEN_NO
				open_no = .vspddata.Text

   				IF CommonQueryRs( "bal_amt,bal_loc_amt" , "a_open_ap  " , "ap_no = " & FilterVar(open_no, "''", "S") & " " , lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) Then         
   					IF CommonQueryRs( "bal_amt,bal_loc_amt" , "a_open_ap  " , "ap_no = " & FilterVar(open_no, "''", "S") & " " , lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) Then         
   					
   					
				  	if cdbl(split(lgF0 ,chr(11))(0)) = cdbl(ClsAmt) then
				    
						.vspddata.Col = C_CLS_LOC_AMT	
						.vspddata.value = cdbl(split(lgF1 ,chr(11))(0)) 
				  	end if
					...
					<DIV ID="TabDiv" SCROLL=no>
							<TABLE <%=LR_SPACE_TYPE_60%>>
								...
								<TR ID="OpenCondition1" style="display: none">
									...
									<TD CLASS="TD6" NOWRAP><INPUT CLASS="clstxt" TYPE=TEXT NAME="txtBpLoanCd" SIZE="10" MAXLENGTH=10 STYLE="TEXT-ALIGN: Left" tag="21X" ALT="거래처"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnBankLoanCd" align=top TYPE="BUTTON"ONCLICK="vbscript:CALL OpenBp(frm1.txtBpLoanCd.value , 0)">
