...
'*  3. Program ID           : f4218oa1
'*  4. Program Name         : 차입처별차입금상세출력 
...
'FristDate	 = UNIGetFirstDay(StartDate,parent.gServerDateFormat)
'%>
...
Function SetDefaultVal()
	...
	frm1.txtPaymDt.Text		= UNIConvDateAToB(StartDate, parent.gServerDateFormat, parent.gDateFormat)
...
Sub SetPrintCond(condvar, StrEbrFile)
    Dim VarLoanDtFr, VarLoanDtTo, VarPaymDt 
	...
	VarLoanDTFr = UniConvDateToYYYYMMDD(frm1.txtLoanDtFr.Text,parent.gDateFormat, parent.gServerDateType)
	VarLoanDtTo = UniConvDateToYYYYMMDD(frm1.txtLoanDTTo.Text,parent.gDateFormat, parent.gServerDateType)
	VarPaymDt   = UniConvDateToYYYYMMDD(frm1.txtPaymDT.Text,parent.gDateFormat, parent.gServerDateType)
...
'	If frm1.cboApSts.value <> "" Then VarRdpClsFg = frm1.cboApSts.value
	If frm1.cboApSts.value = "Y" Then
		VarRdpClsFg = "AND (F_LN_HIS.CHG_DT IS NOT NULL OR ISNULL(PM.PAY_AMT, 0) + ISNULL(F_LN_INFO.BAS_RDP_AMT,0) >= ISNULL(F_LN_INFO.LOAN_AMT, 0))"
	ElseIf frm1.cboApSts.value = "N" Then
		VarRdpClsFg = "AND (F_LN_HIS.CHG_DT IS NULL AND ISNULL(PM.PAY_AMT, 0) + ISNULL(F_LN_INFO.BAS_RDP_AMT,0) < ISNULL(F_LN_INFO.LOAN_AMT, 0))"
	Else
		VarRdpClsFg = ""
	End If
	...
	condvar = condvar & "|PaymDt|"			& VarPaymDt
					...
					<TD WIDTH=100% HEIGHT=*>
							<TABLE <%=LR_SPACE_TYPE_60%>>
								...
								<TR>
									...
									<TD CLASS=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtLoanDtFr name=txtLoanDtFr CLASS=FPDTYYYYMMDD title=FPDATETIME tag="12X1" ALT="시작차입일자"></OBJECT>');</SCRIPT>&nbsp;~&nbsp;
														 <SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtLoanDtTo name=txtLoanDtTo CLASS=FPDTYYYYMMDD title=FPDATETIME tag="12X1" ALT="종료차입일자"></OBJECT>');</SCRIPT>
									</TD>
                              	</TR>
								<TR>
									<TD CLASS=TD5 NOWRAP>기준일자</TD>
									<TD CLASS=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtPaymDt name=txtPaymDt CLASS=FPDTYYYYMMDD title=FPDATETIME tag="12X1" ALT="기준상환일자"></OBJECT>');</SCRIPT>&nbsp;
								    </TD>
			                  	</TR>
			                  	<TR>
