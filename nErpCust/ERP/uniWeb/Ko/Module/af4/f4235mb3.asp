...
Sub FixUNISQLData()
    ...
    Where01 = ""    
	Where01 = Where01 & "RO.LOAN_NO,LN.LOAN_NO,RO.BP_CD,BP.BP_NM,"
	...
	Where01 = Where01 & "LN.LOAN_BANK_CD,ME.MEAN_TYPE,ME.PAY_MEAN_ACCT_CD,LN.BANK_ACCT_NO,LN.BANK_CD,RO.ORG_CHANGE_ID,"
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    ...
    UNISqlId(0) = "F4235MA101"	'차입금마스터정보 
...
'%>	
	...
	With parent.frm1		
		...
		.txtBpRoCd.value			= "<%=ConvSPChars(rs0(2))%>"
		.txtBpRoNm.value			= "<%=ConvSPChars(rs0(3))%>"		
		...
		ElseIf "<%=ConvSPChars(rs0(48))%>" = "YN" Then
			.Rb_IntStart1.Checked =True
			.Rb_IntEnd2.Checked   =True
		ElseIf "<%=ConvSPChars(rs0(48))%>" = "NY" Then
...
'		.htxtBpLoanCd.value			= "<%=ConvSPChars(rs0(65))%>"		
...
'		Call ServerMesgBox("Nothing!", vbInformation, I_MKSCRIPT)	                               
%>			
	End With
'End With
	With parent	
