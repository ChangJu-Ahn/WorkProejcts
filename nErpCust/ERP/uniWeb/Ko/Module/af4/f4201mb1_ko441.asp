...
Sub SubBizQuery()
    ...
    Set iPAFG405list = server.CreateObject("PAFG405_KO441.bFLkUpLnSvr")

    If CheckSYSTEMError(Err,True) = True Then
		Exit Sub
    End If    
    Call iPAFG405list.F_LOOKUP_LN_SVR(gStrGlobalCollection, iCommandSent, I_f_ln_info, E_f_ln_info, E_PrevNext_Code, EG1_export_allc_info, I2_a_data_auth)
    If CheckSYSTEMError(Err,True) = True Then
    ...
    Response.Write " .txtLoanNm.value       = """ & ConvSPChars(E_f_ln_info(C_LOAN_NM)) & """              " & vbCr
    Response.Write " .txtLoanDt.text        = """ & UNIDateClientFormat(E_f_ln_info(C_LOAN_DT)) & """      " & vbCr
    Response.Write " .txtDueDt.text         = """ & UNIDateClientFormat(E_f_ln_info(C_DUE_DT)) & """       " & vbCr
    Response.Write " .txtDeptCd.value       = """ & ConvSPChars(E_f_ln_info(C_DEPT_CD)) & """              " & vbCr
    Response.Write " .txtDeptNm.value       = """ & ConvSPChars(E_f_ln_info(C_DEPT_NM)) & """              " & vbCr
    ...
    Response.Write " .txtIntRate.value      = """ & UNINumClientFormat(E_f_ln_info(C_LOAN_INT_RATE),		"6"	,0)			& """" & vbCr    
	...
	'Response.Write  "    Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData," & 1 & "," & iLngRow  & ",.C_DOC_CUR,.C_OPEN_AMT, ""A"" ,""I"",""X"",""X"")						 " & vbCr
	'Response.Write  "    Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData," & 1 & "," & iLngRow  & ",.C_DOC_CUR,.C_BAL_AMT,  ""A"" ,""I"",""X"",""X"")						 " & vbCr
	'Response.Write  "    Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData," & 1 & "," & iLngRow  & ",.C_DOC_CUR,.C_CLS_AMT,  ""A"" ,""I"",""X"",""X"")						 " & vbCr
	'Response.Write  "    Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData," & 1 & "," & iLngRow  & ",.C_DOC_CUR,.C_DC_AMT,   ""A"" ,""I"",""X"",""X"")						 " & vbCr
	'Response.Write  "    Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData," & 1 & "," & iLngRow  & ",.C_DOC_CUR,.C_XCH_RATE, ""D"" ,""I"",""X"",""X"")						 " & vbCr
...
'Call svrmsgbox(txtSpread, vbinformation, i_mkscript)
	...
	Redim I5_b_bank(C_LOAN_BANK_CD)
		I5_b_bank(C_LOAN_BANK_CD) = Trim(Request("txtBankLoanCd"))
    ...
    Set iPAFG405CU = server.CreateObject("PAFG405_KO441.cFMngLnSvr")   
    ...
    Select Case lgIntFlgMode   
		Case  OPMD_CMODE                                                             '☜ : Create	
			Call iPAFG405CU.F_MANAGE_LN_SVR(gStrGlobalCollection, "CREATE",	I1_f_ln_info,	I2_a_temp_amt,	I3_b_biz_partner,_
												...
												E1_b_auto_numbering,	E2_b_auto_numbering, txtSpread, I13_a_data_auth)
        ...
        Case  OPMD_UMODE           
			Call iPAFG405CU.F_MANAGE_LN_SVR(gStrGlobalCollection, "UPDATE",	I1_f_ln_info,	I2_a_temp_amt,	I3_b_biz_partner,_
												...
												I9_b_bank_acct, I10_f_ln_repay_mean,	I11_b_acct_dept,	I12_b_currency, _
												E1_b_auto_numbering, E2_b_auto_numbering, txtSpread, I13_a_data_auth)		
			strLoanNo = Trim(Request("txtLoanNo"))			
		Case Else				
...
Sub SubBizDelete()
	...
	Set iPAFG405D = server.CreateObject("PAFG405_KO441.cFMngLnSvr")   
...
Sub SubPlanExec()
    ...
    Set PAFG400EXE = Server.CreateObject("PAFG400_KO441.cFMngLnPlnSvr")

    If CheckSYSTEMError(Err, True) = True Then
		Exit Sub
    End If

    Call PAFG400EXE.F_MANAGE_LN_PLAN_SVR(gStrGloBalCollection, Request("txtLoanNo")	, _
															   UniConvDate(Request("txtDateFr")), _
															   UniConvDate(Request("txtDateTo")), _
															   EG1_export_group)
