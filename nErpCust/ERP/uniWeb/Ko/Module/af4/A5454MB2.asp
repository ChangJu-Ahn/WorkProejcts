...
Sub SubBizQueryMulti()
	...
	'차입번호 선택 	
		lgStrSel0 = ", LOAN_NO, SEQ, " & vbCr
		lgStrSel1 = " A.LOAN_NO, A.SEQ, " & vbCr
		lgStrGrpBy0 = " A.LOAN_NO, A.SEQ," & vbCr
    Else
		lgStrSel0 = ", '', '', " & vbCr
		lgStrSel1 = "" & vbCr
		lgStrGrpBy0 = ""    & vbCr
	...
	If lgStrPrevKey  =0 Then
		...
		lgStrSQL2 = lgStrSQL2 & " FROM (SELECT SUM(ISNULL(A.INT_CLS_LOC_AMT, 0)) - SUM(ISNULL(G.ITEM_LOC_AMT,0)) SUM_DIFF_AMT,  " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 		FROM F_LN_MON_DFR_INT A	LEFT JOIN F_LN_INFO B ON A.LOAN_NO = B.LOAN_NO " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 					LEFT JOIN A_ACCT C ON A.DFR_INT_ACCT_CD = C.ACCT_CD " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 								WHERE G1.GL_INPUT_TYPE = " & FilterVar("LD", "''", "S") & "  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 						) G ON A.gl_no = G.gl_no AND A.dfr_INT_ACCT_CD = G.ACCT_CD " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 								WHERE I1.GL_INPUT_TYPE = " & FilterVar("LD", "''", "S") & "  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 						) I ON A.temp_gl_no = I.temp_gl_no AND A.dfr_INT_ACCT_CD = I.ACCT_CD " & vbCr

		lgStrSQL2 = lgStrSQL2 & "		WHERE  A.INT_CLS_DT >= " & strFrDt  & " AND A.INT_CLS_DT <= " & strToDt  & vbCr
		lgStrSQL2 = lgStrSQL2 & "			AND A.CLS_FG = " & FilterVar("Y", "''", "S") & "  AND ISNULL(A.LOAN_NO, '') <> ''" & vbCr
		...
		If Trim(Request("txtAcctCd")) <> "" Then
			lgStrSQL2 = lgStrSQL2 & " AND A.DFR_INT_ACCT_CD = " & strAcctCd & vbCr
		End If	
		...
		lgStrSQL2 = lgStrSQL2 & " 	GROUP BY A.DFR_INT_ACCT_CD, C.ACCT_NM, " & vbCr
	...
	lgStrSQL = lgStrSQL & "			" & lgStrSel0 & vbCr
	...
	lgStrSQL = lgStrSQL & " 	FROM F_LN_MON_DFR_INT A	LEFT JOIN F_LN_INFO B ON A.LOAN_NO = B.LOAN_NO " & vbCr
	...
	lgStrSQL = lgStrSQL & " ORDER BY DFR_INT_ACCT_CD, INT_CLS_DT "	 & vbCr
	...
	Else
		...
		Do While Not lgObjRs.EOF
			...
			lgstrData = lgstrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(lgObjRs(6), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")		'SUM_DIFF_AMT
			lgstrData = lgstrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(lgObjRs(7), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")		'SUM_CLS_AMT
			lgstrData = lgstrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(lgObjRs(8), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")		'SUM_BATCH_ITEM_AMT
			lgstrData = lgstrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(lgObjRs(9), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")		'SUM_GL_ITEM_AMT
