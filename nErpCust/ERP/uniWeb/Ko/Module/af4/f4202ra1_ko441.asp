...
Const BIZ_PGM_ID        = "f4202rb1_ko441.asp"
...
'	arrParam = arrParent(0)
	...
	If  Trim("<%=Request("PGM")%>") = "F4204MA1"  Then
		top.document.title = "은행기초차입번호팝업"
	ElseIf Trim("<%=Request("PGM")%>") = "F4234MA1"  Then
		top.document.title = "만기연장번호팝업"
	ElseIf Trim("<%=Request("PGM")%>") = "F4201MA1"  Then
		top.document.title = "은행차입금번호팝업"
	ElseIf Trim("<%=Request("PGM")%>") = "F4207MA1"  Then
		top.document.title = "요청차입금번호팝업"
	Else
...
Sub InitSpreadSheet()
    ...
    Call SetZAdoSpreadSheet("F4202RA1","S","A","V20030407",PopupParent.C_SORT_DBAGENT,frm1.vspdData, C_MaxKey, "X","X")
    Call SetSpreadLock() 
End Sub
...
'			arrParam(4) = "A.CONF_FG IN ('C','E')"
			If Trim("<%=Request("PGM")%>") = "F4201MA1"  Then
				arrParam(4) = "A.LOAN_BASIC_FG = " & FilterVar("LN", "''", "S") & " "
				arrParam(4) = arrParam(4) & "AND A.LOAN_PLC_TYPE = " & FilterVar("BK", "''", "S") & " "
			ElseIf Trim("<%=Request("PGM")%>") = "F4204MA1"  Then
			...
			ElseIf Trim("<%=Request("PGM")%>") = "F4234MA1"  Then
				...
				arrParam(4) = arrParam(4) & "AND A.LOAN_PLC_TYPE = " & FilterVar("BK", "''", "S") & " "			
			ElseIf Trim("<%=Request("PGM")%>") = "F4201MA1"  Then
				arrParam(4) = "A.LOAN_BASIC_FG = " & FilterVar("LQ", "''", "S") & " "				
			Else
				arrParam(4) = " 1=1 "
			End If
			
			' 권한관리 추가 
			If lgAuthBizAreaCd	<> "" Then	arrParam(4) = arrParam(4) & " AND A.BIZ_AREA_CD = " & FilterVar(lgAuthBizAreaCd, "''", "S")
			If lgInternalCd		<> "" Then	arrParam(4) = arrParam(4) & " AND A.INTERNAL_CD = " & FilterVar(lgInternalCd, "''", "S")
			If lgSubInternalCd	<> "" Then	arrParam(4) = arrParam(4) & " AND A.INTERNAL_CD like " & FilterVar(lgSubInternalCd & "%", "''", "S")
			If lgAuthUsrID		<> "" Then	arrParam(4) = arrParam(4) & " AND A.INSRT_USER_ID = " & FilterVar(lgAuthUsrID, "''", "S")
		...
		Case 5		'차입은행 
			...
			arrHeader(0) = frm1.txtBankLoanCd.Alt
			arrHeader(1) = frm1.txtBankLoanNm.Alt
...
Dim IntRetCD
    ...
    If frm1.txtLoanFromDt.Text  = "" Then
	frm1.txtLoanFromDt.Text = "2001-01-01"
    End If

    If frm1.txtLoanToDt.Text  = "" Then
	frm1.txtLoanToDt.Text = "2999-12-31"
    End If
...
'		strVal = strVal & "&txtBankLoanCd=" & Trim(.hBankLoanCd.value)
'		strVal = strVal & "&txtBankLoanCd_Alt=" & Trim(.txtBankLoanCd.Alt)
'	Else 
		...
		strVal = strVal & "&txtBankLoanCd="		& Trim(.txtBankLoanCd.value)
		strVal = strVal & "&txtBankLoanCd_Alt="	& Trim(.txtBankLoanCd.Alt)		
			...
			<FIELDSET CLASS="CLSFLD">
				<TABLE <%=LR_SPACE_TYPE_40%>>					
					<TR>
						...
						<TD CLASS=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=fpLoanFrDt name=txtLoanFromDt CLASS=FPDTYYYYMMDD title=FPDATETIME tag="11" ALT="시작차입일자"></OBJECT>');</SCRIPT>&nbsp;~&nbsp;<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=fpLoanToDt name=txtLoanToDt CLASS=FPDTYYYYMMDD title=FPDATETIME tag="11" ALT="종료차입일자"></OBJECT>');</SCRIPT></TD>
					...
					<TR>
						...
						<TD CLASS="TD6" NOWRAP><INPUT NAME="txtBankLoanCd" ALT="차입은행" SIZE="10" MAXLENGTH="10"  tag="11XXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnBankLoanCd" align=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopUp(frm1.txtBankLoanCd.value, 5)">&nbsp;<INPUT NAME="txtBankLoanNm" ALT="차입은행명" STYLE="TEXT-ALIGN: Left" tag="24X"></TD>						
					</TR>
					<TR>
