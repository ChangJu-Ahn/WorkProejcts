...
'==========================================  1.2.1 Global 상수 선언  ======================================
'==========================================================================================================
Const BIZ_PGM_ID = "f4201mb1_test.asp"		 '☆: 비지니스 로직 ASP명 
...
'air	Dim iCalledAspName
'air	Dim IntRetCD
'air	Dim arrRet
'air	Dim arrParam(8)	                           '권한관리 추가 (3 -> 4)
'air
'air	If IsOpenPop = True Then Exit Function
'air	iCalledAspName = AskPRAspName("a5101ra1")
'air	If Trim(iCalledAspName) = "" Then
'air		IntRetCD = DisplayMsgBox("900040", parent.VB_INForMATION, "a5101ra1", "X")
'air		IsOpenPop = False
'air		Exit Function
'air	End If
'air
'air	IsOpenPop = True
'air
'air	Call CookiePage("TEMP_GL_POPUP")
'air
'air	arrParam(4)	= lgAuthorityFlag              '권한관리 추가	
'air	
'air	' 권한관리 추가
'air	arrParam(5) = lgAuthBizAreaCd
'air	arrParam(6) = lgInternalCd
'air	arrParam(7) = lgSubInternalCd
'air	arrParam(8) = lgAuthUsrID
'air	
'air	arrRet = window.showModalDialog(iCalledAspName, Array(window.parent, arrParam), _
'air		     "dialogWidth=660px; dialogHeight=450px; center: Yes; help: No; resizable: No; status: No;")
'air
'air	IsOpenPop = False
'air
'air	If arrRet(0) = ""  Then
'air		frm1.txttempGlNo.focus
'air		Exit Function
'air	Else
'air		Call SetRefTempGl(arrRet)
'air	End If
End Function
...
Sub txtRcptType_OnChange()
	...
	If UCase(Trim(strval)) = "FI" Then
		...
		Call ggoOper.SetReqAttr(frm1.txtBpLoanCd, "N")
	Else
		OpenCondition1.style.display = "none"
		Call ggoOper.SetReqAttr(frm1.txtBpLoanCd, "D")
	End If		
...
Function SetRefOpen(Byval arrRet)
	...
	With frm1
		...
		.txtBpLoanCd.value = arrRet(0,2)	'거래처
		.txtBpLoanNm.value = arrRet(0,3)	'거래처명

		.txtUserFld1.value = arrRet(0,14)	'발생전표번호
		.fpLoanAmt.value = arrRet(0,9)	'금액
