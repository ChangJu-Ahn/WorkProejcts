...
Const BIZ_PGM_ID 		= "f4232rb1_ko441.asp"                              '☆: Biz Logic ASP Name
...
Dim lgAuthUsrID, lgAuthUsrNm					' 개인 
	...
	Select Case Trim("<%=Request("PGM")%>")
		Case "F4232MA1"
			top.document.title = "유동성전환 차입금번호팝업"
		Case "F4233MA1"
			top.document.title = "유동성전환취소 차입금번호팝업"
		Case Else  
...
Sub InitComboBox()
	Select Case Trim("<%=Request("PGM")%>")
		Case "F4232MA1", "F4233MA1"
			Call CommonQueryRs(" MINOR_CD,MINOR_NM "," B_MINOR "," MAJOR_CD = " & FilterVar("F1020", "''", "S") & "  AND MINOR_CD LIKE " & FilterVar("L%", "''", "S") & "  ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Sub InitSpreadSheet()
    ...
    Call SetZAdoSpreadSheet("F4232RA1","S","A","V20030411",PopupParent.C_SORT_DBAGENT,frm1.vspdData, C_MaxKey, "X","X")
    Call SetSpreadLock() 
End Sub
...
'			arrParam(4) = arrParam(4) & ""
			...
			If Trim("<%=Request("PGM")%>") = "F4232MA1"  Then				
				arrParam(4) = " loan_fg = " & FilterVar("LL", "''", "S") & " "
			ElseIf Trim("<%=Request("PGM")%>") = "F4233MA1"  Then
				arrParam(4) = " 1=1 "
			ElseIf Trim("<%=Request("PGM")%>") = "F4220BA1"  Then				
				arrParam(4) = " 1=1 "
			ElseIf Trim("<%=Request("PGM")%>") = "F4223MA1"  Then				
				arrParam(4) = " 1=1 "
			ElseIf Trim("<%=Request("PGM")%>") = "F4231MA1"  Then				
				arrParam(4) = " Int_Votl = " & FilterVar("F", "''", "S") & "  "
			Else
				arrParam(4) = " 1=1 "
			End If
...
Dim IntRetCD
    ...
    If frm1.txtLoanFrDt.Text  = "" Then
	frm1.txtLoanFrDt.Text = "2001-01-01"
    End If
	...
	If frm1.txtDueFrDt.Text <> "" And frm1.txtDueToDt.Text <> "" Then	
		If CompareDateByFormat(frm1.txtDueFrDt.Text, frm1.txtDueToDt.Text, frm1.txtDueFrDt.Alt, frm1.txtDueToDt.Alt, _
				...
				Exit Function
		End if														'⊙: GL Date Compare Common Function
			...
			<FIELDSET CLASS="CLSFLD">
				<TABLE <%=LR_SPACE_TYPE_40%>>
					<TR>
						...
						<!--네패스 요청으로 필수항목에서 선택항목으로 변경...20090901...kbs	-->
						<TD CLASS=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=fpLoanFrDt name=txtLoanFrDt  CLASS=FPDTYYYYMMDD title=FPDATETIME tag="11" ALT="시작결산일자"></OBJECT>');</SCRIPT>&nbsp;~&nbsp;<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=fpLoanToDt name=txtLoanToDt CLASS=FPDTYYYYMMDD title=FPDATETIME tag="11" ALT="종료결산일자"></OBJECT>');</SCRIPT></TD>
					...
					<TR>
						...
						<TD CLASS="TD6" NOWRAP><SELECT NAME="cboLoanFg" ALT="장단기구분" STYLE="WIDTH: 135px" tag="11"><OPTION VALUE=""></OPTION></SELECT></TD>
					</TR>
					<TR>
						...
						<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtLoanType" SIZE=10 MAXLENGTH=10 tag="11XXXU" ALT="차입용도코드" ><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnBizAreaCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopup(frm1.txtLoanType.Value,1)">
											   <INPUT TYPE="Text" NAME="txtLoanTypeNm" SIZE=20 tag="14X" ALT="차입용도명"></TD>
					</TR>
					<TR>
						...
						<TD CLASS="TD6" NOWRAP><INPUT NAME="txtLoanPlcCd" ALT="차입처" SIZE="10" MAXLENGTH="18"  tag="11X"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnLoanPlcCd" align=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopUp(frm1.txtLoanPlcCd.Value, 0)">
