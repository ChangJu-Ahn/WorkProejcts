...
*  1. Module Name          : 경영손익 
*  2. Function Name        :
*  3. Program ID           : GC004MA1
*  4. Program Name         : 사업공통비 배부기준 Data 등록 
*  5. Program Desc         : 
*  6. Comproxy List        :
...
'========================================================================================================
Const BIZ_PGM_ID = "GC004MB1.asp"                                      'Biz Logic ASP
'========================================================================================================
...
Sub InitSpreadPosVariables()
	 C_center    = 1                                                 'Column ant for Spread Sheet
	 C_center_bt = 2
	 C_centernm  = 3
	 C_data      = 4
End Sub
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
       ...
       ggoSpread.SSSetEdit  C_center   , "Profit Center"      ,35,,, 35,2
       ggoSpread.SSSetButton C_center_bt
       ggoSpread.SSSetEdit  C_centernm , "Profit Center 명"   ,45,,, 45,2
       ggoSpread.SSSetFloat C_data     , "배부 Data"        ,30, Parent.ggAmtOfMoneyNo ,ggStrIntegeralPart, ggStrDeciPointPart,Parent.gComNum1000,Parent.gComNumDec,,,"Z"
...
Sub SetSpreadLock()
    ...
    .vspdData.ReDraw = False   
      ggoSpread.SpreadLock    C_center		, -1 , C_center
      ggoSpread.SpreadLock    C_center_bt	, -1 , C_center_bt
      ggoSpread.SpreadLock    C_centernm	, -1 , C_centernm
      ggoSpread.SpreadUnlock  C_data		, -1 , C_data
...
Sub SetSpreadYn_Lock()
    With frm1		
    .vspdData.ReDraw = False
      ggoSpread.SpreadLock    C_center		, -1 , C_center
      ggoSpread.SpreadLock    C_center_bt	, -1 , C_center_bt
      ggoSpread.SpreadLock    C_centernm	, -1 , C_centernm
      ggoSpread.SpreadLock    C_data		, -1 , C_data
      ggoSpread.SSSetProtected	.vspdData.MaxCols,-1,-1
    .vspdData.ReDraw = True
...
Function OpenCurrency()
	...
	arrParam(4) = arrParam(4) & "  AND  a.seq_no =3  and reference = " & FilterVar("Y", "''", "S") & "   "				<%' Where Condition%>
	arrParam(5) = "배부기준 코드"
...
Function OpenCost(strCode, iWhere, Row)
	...
	Select Case iWhere
	    Case 1
	    	...
	    	arrParam(4) = "cost_type <> " & FilterVar("C", "''", "S") & "  and cost_cd not in (select cost_cd from g_dstb_cc)"				   ' Where Condition
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			C_center_bt            		= iCurColumnPos(2)
			C_centernm          		= iCurColumnPos(3)    
			C_data                		= iCurColumnPos(4)
    End Select    
End Sub
'========================================================================================
       ...
       For lRow = 1 To .vspdData.MaxRows
           ...
           Select Case .vspdData.Text
               ...
               Case ggoSpread.DeleteFlag                                      '☜: Delete
												  ...
												  strVal = strVal & lRow & iColSep
                                                  strval = strval & ggoOper.RetFormat(.fpdtWk_yymm.text,"yyyyMM") & iColSep
                                                  strval = strval & Trim(.txtCurrencyCode.value) & iColSep
                    .vspdData.Col = C_center 	: strval = strVal & Trim(.vspdData.Text) & iRowSep
...
Sub vspdData_Change(Col , Row)
	...
	IntRetCD = CommonQueryRs(" cost_nm "," b_cost_center  "," cost_cd =  " & FilterVar(Input_alloc , "''", "S") & " and cost_type <> " & FilterVar("C", "''", "S") & "  and cost_cd not in (select cost_cd from g_dstb_cc)",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)	
...
Function ExeReflect()
    ...
    currency_code = Trim(Frm1.txtCurrencyCode.Value)
        ...
        Call CommonQueryRs("count(*)","g_alloc_data","yyyymm = " & FilterVar(strYYYYMM, "''", "S") & " and alloc_kinds = " & FilterVar("2", "''", "S") & "  and alloc_base = " & FilterVar(Trim(Frm1.txtCurrencyCode.value), "''", "S") & " " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
			 if Trim(Replace(lgF0,Chr(11),"")) <> 0 then
	            IntRetCD = DisplayMsgBox("GA0010",Parent.VB_YES_NO,"X","X")  '배부경로가 이미 존재합니다. 작업을 수행하시겠습니까?
			 end if
    ...
    End If
		...
		Call CommonQueryRs("count(*)","b_configuration","Major_cd = 'G1004' and Minor_cd = " & FilterVar(Trim(Frm1.txtCurrencyCode.value), "''", "S") & " and seq_no = 1 and reference = 'n' "  ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
			if Trim(Replace(lgF0,Chr(11),"")) = 0 then							'수동배부요소가 없으면 
	             Call DisplayMsgBox("GA1511","X","X","X")					'자동배분요소는 전월 카피 할 수 없습니다.
	             Exit Function
			end if
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>공통비 기준Data</font></td>
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS="TD6" NOWRAP>
										<script language =javascript src='./js/gc004ma1_fpDateTime3_fpdtWk_yymm.js'></script>
				...
				<TR>
					<TD WIDTH=100% HEIGHT=100% valign=top>
							<TABLE <%=LR_SPACE_TYPE_30%>>
								<TR>
									<TD HEIGHT="100%">
										<script language =javascript src='./js/gc004ma1_OBJECT1_vspdData.js'></script>
								</TD>
								</TR>
							</TABLE>
					</TD>
				...
				<TR>
					<TD>
							...
							<TABLE <%=LR_SPACE_TYPE_30%>>
										<TR HEIGHT=20>
											<TD WIDTH=10>&nbsp;</TD>
									        <TD><BUTTON NAME="bttnPreview"  CLASS="CLSSBTN" onclick="ExeReflect()" Flag=1>일괄생성</BUTTON></TD>
											
											<TD CLASS=TD6 NOWRAP></TD>
							    			<TD CLASS=TD6 NOWRAP></TD>
							    			<TD CLASS=TD5 NOWRAP>합계</TD>
							    			<TD CLASS=TD6 NOWRAP><script language =javascript src='./js/gc004ma1_fpDoubleSingle2_txtDataAmt.js'></script></TD>
										</TR>
							</TABLE>
							</FIELDSET>
					</TD>
