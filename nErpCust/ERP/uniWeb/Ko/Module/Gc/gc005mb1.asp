...
<%
	...
	Response.Write lgStrPrevKeyIndex
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "R"
                       ...
                       case "X"
                          ...
                          lgStrSQL = lgStrSQL & " where G.from_alloc  = B.cost_cd AND G.acct_cd = C.acct_cd "                                                                              
                          lgStrSQL = lgStrSQL & pCode
                          lgStrSQL = lgStrSQL & " AND   alloc_kinds   = " & FilterVar(2, "''", "S")
                          lgStrSQL = lgStrSQL & " AND   b.cost_type <> " & FilterVar("C", "''", "S") & "  and g.ctrl_cd = " & FilterVar("*", "''", "S") & "  and g.from_alloc <> g.to_alloc "
						  lgStrSQL = lgStrSQL & " group by G.from_alloc,B.cost_nm "
						  lgStrSQL = lgStrSQL & " order by G.from_alloc "
						
						  
                       case "5"
                          lgStrSQL = "Select sum(case when C.bal_fg = " & FilterVar("DR", "''", "S") & " then G.to_amount else -(G.to_amount)end) as A  "
                          lgStrSQL = lgStrSQL & " from g_alloc_result G, b_cost_center B, a_acct C "
                          lgStrSQL = lgStrSQL & " where G.from_alloc  = B.cost_cd AND G.acct_cd = C.acct_cd "                                                                              
                          lgStrSQL = lgStrSQL & pCode
                          lgStrSQL = lgStrSQL & " AND   alloc_kinds   = " & FilterVar(2, "''", "S")
				     ...
				     Select Case pCode1
 					 Case "X"
     					lgStrSQL = "select C.to_alloc, A.cost_nm, sum(case when D.bal_fg = " & FilterVar("DR", "''", "S") & " then C.to_amount else -(C.to_amount)end) as B   " 
						lgStrSQL = lgStrSQL & " FROM  b_cost_center A,g_alloc_result C , a_acct D,b_cost_center b "
						lgStrSQL = lgStrSQL & " where C.to_alloc = A.cost_cd and C.acct_cd = D.acct_cd and c.from_alloc = b.cost_cd"						
                        lgStrSQL = lgStrSQL & " AND   alloc_kinds   = " & FilterVar(2, "''", "S")                        
						lgStrSQL = lgStrSQL & pcode
						lgStrSQL = lgStrSQL & " AND   b.cost_type <> " & FilterVar("C", "''", "S") & "  AND   c.from_alloc <> c.to_alloc and c.ctrl_cd = " & FilterVar("*", "''", "S") & "  "
						lgStrSQL = lgStrSQL & " group by c.to_alloc,  A.cost_nm " 
						lgStrSQL = lgStrSQL & " order by to_alloc "

					 Case "22"				
     					lgStrSQL = "select sum(case when D.bal_fg = " & FilterVar("DR", "''", "S") & " then C.to_amount else -(C.to_amount)end) as B   " 
						lgStrSQL = lgStrSQL & " FROM  b_cost_center A,g_alloc_result C , a_acct D,b_cost_center b "
						lgStrSQL = lgStrSQL & " where C.to_alloc = A.cost_cd and C.acct_cd = D.acct_cd and c.from_alloc = b.cost_cd "						
                        lgStrSQL = lgStrSQL & " AND   alloc_kinds   = " & FilterVar(2, "''", "S")
						lgStrSQL = lgStrSQL & " AND   b.cost_type <> " & FilterVar("C", "''", "S") & "  AND   c.from_alloc <> c.to_alloc and c.ctrl_cd = " & FilterVar("*", "''", "S") & "  "
