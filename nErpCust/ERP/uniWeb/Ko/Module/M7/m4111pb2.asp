...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "M4111PA201"											' main query(spread sheet에 뿌려지는 query statement)
	...
	If Len(Request("txtMvmtType")) Then
	  strVal = strVal & " AND A.IO_TYPE_CD =  " & FilterVar(Request("txtMvmtType"), "''", "S") & " "
	ELSE	
	...
	IF UCase(Request("txtChkFlag")) = "Y" Then      '품질모듈 셋업.
	strVal = strVal & "AND A.INSPECT_STS = " & FilterVar("IF", "''", "S") & " "
	End IF
