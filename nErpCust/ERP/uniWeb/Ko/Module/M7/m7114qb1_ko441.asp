...
'*  2. Function Name        : 
'*  3. Program ID           : m7114qb1_ko441
'*  4. Program Name         : 발주대비입고현황조회
'*  5. Program Desc         : 발주대비입고현황조회
'*  6. Component List       : 
'*  7. Modified date(First) : 2007/12/13
'*  8. Modified date(Last)  : 2007/12/13
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    Redim UNIValue(6,5)                                                  '⊙: DB-Agent로 전송될 parameter를 위한 변수 
                                                                          '    parameter의 수에 따라 변경함 
     UNISqlId(0) = "M7114QA1KO441"                                        '20071213::hanc  m3111qa201  M7114QA1KO441
...
'--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
    ...
    If Len(Trim(Request("txtPlantCd"))) Then
		strSQL = strSQL & " AND M_PUR_ORD_DTL.PLANT_CD =  " & FilterVar(Trim(UCase(Request("txtPlantCd"))), " " , "S") & " "
		strSQL1 = strSQL1 & " AND B.PLANT_CD =  " & FilterVar(Trim(UCase(Request("txtPlantCd"))), " " , "S") & " "
    End If
     '---구매그룹 
    If Len(Trim(Request("txtPurGrpCd"))) Then
		strSQL = strSQL & " AND M_PUR_ORD_HDR.PUR_GRP LIKE " & FilterVar(Trim(UCase(Request("txtPurGrpCd")))& "%", " " , "S") & " "
		strSQL1 = strSQL1 & " AND A.PUR_GRP LIKE " & FilterVar(Trim(UCase(Request("txtPurGrpCd")))& "%", " " , "S") & " "
    End If
     '---공급처 
    If Len(Trim(Request("txtBpCd"))) Then
		strSQL = strSQL & " AND M_PUR_ORD_HDR.BP_CD LIKE " & FilterVar(Trim(UCase(Request("txtBpCd")))& "%", " " , "S") & " "
		strSQL1 = strSQL1 & " AND A.BP_CD LIKE " & FilterVar(Trim(UCase(Request("txtBpCd")))& "%", " " , "S") & " "
    End If
     '---발주일 
    If Len(Trim(Request("txtPoFrDt"))) Then
		strSQL = strSQL & " AND M_PUR_ORD_HDR.PO_DT >=  " & FilterVar(uniConvDate(Request("txtPoFrDt")), "''", "S") & ""
		strSQL1 = strSQL1 & " AND A.PO_DT >=  " & FilterVar(uniConvDate(Request("txtPoFrDt")), "''", "S") & ""
    Else
		strSQL = strSQL & " AND M_PUR_ORD_HDR.PO_DT >= " & FilterVar("1900/01/01", "''", "S") & ""
		strSQL1 = strSQL1 & " AND A.PO_DT >= " & FilterVar("1900/01/01", "''", "S") & ""
    End If

    If Len(Trim(Request("txtPoToDt"))) Then
		strSQL = strSQL & " AND M_PUR_ORD_HDR.PO_DT <=  " & FilterVar(uniConvDate(Request("txtPoToDt")), "''", "S") & ""
		strSQL1 = strSQL1 & " AND A.PO_DT <=  " & FilterVar(uniConvDate(Request("txtPoToDt")), "''", "S") & ""
    Else
		strSQL = strSQL & " AND M_PUR_ORD_HDR.PO_DT <= " & FilterVar("2999/12/30", "''", "S") & ""
		strSQL1 = strSQL1 & " AND A.PO_DT <= " & FilterVar("2999/12/30", "''", "S") & ""
    End If    
     '---발주형태 
    If Len(Trim(Request("txtPoType"))) Then
		strSQL = strSQL & " AND M_PUR_ORD_HDR.PO_TYPE_CD LIKE " & FilterVar(Trim(UCase(Request("txtPoType")))& "%", " " , "S") & " "
		strSQL1 = strSQL1 & " AND A.PO_TYPE_CD LIKE " & FilterVar(Trim(UCase(Request("txtPoType")))& "%", " " , "S") & " "
    End If
    '--선택
    If Len(Trim(Request("rdoClsFlg"))) Then
		strSQL2 = FilterVar(Trim(UCase(Request("rdoClsFlg"))), " " , "S") & " "
    End If
     ...
     UNIValue(0,3) = Trim(lgSelectList)		                              '☜: Select 절에서 Summary    필드 
	 UNIValue(0,4)  = strSQL1
	 UNIValue(0,5)  = strSQL2
...
'--------------- 개발자 coding part(실행로직,End)----------------------------------------------------
'20071213::hanc     UNIValue(0,UBound(UNIValue,2)    ) = Trim(lgTailList)	'---Order By 조건 
...
'20071213::hanc         Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.Frm1.vspdData,"<%=iFrPoint+1%>",parent.frm1.vspddata.maxrows,Parent.GetKeyPos("A",23), Parent.GetKeyPos("A",24),"C", "Q" ,"X","X")	'발주금액 
'20071213::hanc         Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.Frm1.vspdData,"<%=iFrPoint+1%>",parent.frm1.vspddata.maxrows,Parent.GetKeyPos("A",23), Parent.GetKeyPos("A",25),"A", "Q" ,"X","X")	'발주금액 
'20071213::hanc         Call Parent.ReFormatSpreadCellByCellByCurrency2(Parent.Frm1.vspdData,"<%=iFrPoint+1%>",parent.frm1.vspddata.maxrows, Parent.Parent.gCurrency , Parent.GetKeyPos("A",26),"A", "Q" ,"X","X")	'발주자국금액				'매입자국금액 
