...
<%
	...
	Err.Clear
														   '실행 오류가 발생할 때 오류가 발생한 문장 바로 다음에 실행이 계속될 수 있는 문으로 컨트롤을 옮길 수 있도록 지정합니다.				
...
Sub FixUNISQLData()
	...
	Redim UNIValue(2,2)													  '☜: 각각의 SQL ID와 입력될 where 조건의 쌍으로 된 2차원 배열 
	
	UNISqlId(0) = "M4111PA501"											' main query(spread sheet에 뿌려지는 query statement)
	UNISqlId(1) = "M4111PA302"											'입고유형(STO건으로 인한 수정(KJH:03-01-06)
	...
	UNIValue(1,0) = " AND A.IO_TYPE_CD =  " & FilterVar(FilterVar(Request("txtMvmtType")),"","SNM", "''", "S") & " "	'입고유형 
    
    UNIValue(2,0) = FilterVar(Trim(UCase(Request("txtSupplierCd"))), "''" , "S") 			    	'공급처 
