...
Dim pComp
    ...
    Select Case UCase(Trim(lgCurrentSpd))
        ...
        Case "B"
            pComp = ">="
            strWhere = " CONVERT(VARCHAR(4), DATEPART(year," & FilterVar(UNIConvDateCompanyToDB(lgKeyStream(5),NULL),"NULL", "S") & ")))"        
            strWhere = strWhere & " AND haa010t.entr_dt < " & FilterVar(UNIConvDateCompanyToDB(lgKeyStream(5),NULL),"NULL", "S")
        ...
        Case "C"  
            ...
            strWhere = strWhere & " AND haa010t.year_area_cd = hfa100t.year_area_cd"
            strWhere = strWhere & CQuery
            strWhere = strWhere & " AND hfa100t.own_rgst_no =  " & FilterVar(BlgObjRs("biz_own_rgst_no"), "''", "S")'by 20040210 cyc 
            strWhere = strWhere & " ORDER BY hfa100t.year_area_cd , hfa100t.own_rgst_no " 'by 20040210 cyc 
            Call SubMakeSQLStatements("MR",strWhere,"x",pComp)                              '☆: Make sql statements                   
...
Sub ASubBizQueryMulti()
    ...
    If FncOpenRs("R",lgObjConn,AlgObjRs,lgStrSQL,"X","X") = False Then
       Call SetErrorStatus("")
    Else        
		strWhere = " year_area_cd in ( select year_area_cd from haa010t )"
        Call CommonQueryRs("count(*) ","HFA100T",strWhere,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Sub BSubBizQueryMulti()
    ...
    Else    
        Do While Not BlgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(13) & Chr(10)
            IF Cdbl(BlgObjRs("com_no")) > 0 Then                
                lgCurrentSpd = "C"
...
Dim ConWhere
    ...
    ConWhere = " And haa010t.emp_no = " & FilterVar(ClgObjRs("haa010t_emp_no"), "''", "S")    
    ...
    Else
        ...
        Do While Not DlgObjRs.EOF
            ...
            If Trim(Oldres_no) = "" Then
               Oldres_no = DlgObjRs("res_no")
               Cwork_no = 1
            Else
               If Oldres_no = DlgObjRs("res_no") Then                            
                   Cwork_no = Cwork_no + 1
               Else
                   Oldres_no = DlgObjRs("res_no")
                   Cwork_no = 1
...
'Response.End         
                Case "B"                           
                    ...
                    lgStrSQL = lgStrSQL & " (SELECT emp_no, COUNT(*) emp_no_cnt"                                            '1차에서 바뀐 Query
                    lgStrSQL = lgStrSQL & " FROM hfa040t"                                                                   '1차에서 바뀐 Query                    
                    lgStrSQL = lgStrSQL & " WHERE year_yy = " & FilterVar(Year(UNIConvDateCompanyToDB(lgKeyStream(5),NULL)),"NULL", "S")                                                         '1차에서 바뀐 Query
                    lgStrSQL = lgStrSQL & " GROUP BY emp_no) AS t"                                                          '1차에서 바뀐 Query                                       
                    lgStrSQL = lgStrSQL & " WHERE haa010t.emp_no = hdf020t.emp_no"
                    lgStrSQL = lgStrSQL & " AND hdf020t.emp_no = hfa050t.emp_no"
                    lgStrSQL = lgStrSQL & " AND hfa050t.emp_no *= t.emp_no"                                                 '1차에서 바뀐 Query
