...
<%        
    Else
        ...
        Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ""
            lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs("EDU_AMT"), ggAmtOfMoney.DecPoint,0)
			...
			Select Case lgObjRs("FAMILY_TYPE")
			    Case "1"
			            amtSum1 = amtSum1 + cdbl(lgObjRs("EDU_AMT")) 			
			    Case "2"
			            amtSum2 = amtSum2 + cdbl(lgObjRs("EDU_AMT")) 
			    Case "3","4","5"
			            amtSum3 = amtSum3 + cdbl(lgObjRs("EDU_AMT")) 
			    Case "H"
						amtSum4 = amtSum4 + cdbl(lgObjRs("EDU_AMT")) 
			    Case "M"			    
			            amtSum5 = amtSum5 + cdbl(lgObjRs("EDU_AMT")) 
			End Select            
...
Sub SubBizSaveMultiUpdate(arrColVal)
    ...
    lgStrSQL =            "UPDATE HFA090T"
	lgStrSQL = lgStrSQL & "   SET FAMILY_TYPE = " & FilterVar(UCase(arrColVal(4)), "''", "S")   & ","
	lgStrSQL = lgStrSQL & "       EDU_AMT     = " & UNIConvNum(arrColVal(5),0)                   & ","
...
'대학생 교육비 및 인원수 
	...
	lgStrSQL = lgStrSQL & "    SET UNIV_EDU = EDU.EDU_SUM   , UNIV_EDU_CNT = EDU.EDU_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(edu_amt),0) EDU_SUM,  count(*) EDU_CNT"
	lgStrSQL = lgStrSQL & "				FROM hfa090t "
	lgStrSQL = lgStrSQL & " 			WHERE yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND family_type = '1') EDU "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
...
'초등고교육비 및 인원수 
    ...
    lgStrSQL =            " UPDATE HFA030T"
	lgStrSQL = lgStrSQL & "    SET FAM_EDU = EDU.EDU_SUM   , FAM_EDU_CNT = EDU.EDU_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(edu_amt),0) EDU_SUM,  count(*) EDU_CNT"
	lgStrSQL = lgStrSQL & "				FROM hfa090t "
	lgStrSQL = lgStrSQL & " 			WHERE yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND family_type = '2') EDU "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
'Response.Write "lgStrSQL:" & lgStrSQL
'Response.end
...
'유치원, 미취학아동, 영유아교육비 및 인원수	
    lgStrSQL =            " UPDATE HFA030T"
	lgStrSQL = lgStrSQL & "    SET KIND_EDU = EDU.EDU_SUM   , KIND_EDU_CNT = EDU.EDU_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(edu_amt),0) EDU_SUM,  count(*) EDU_CNT"
	lgStrSQL = lgStrSQL & "				FROM hfa090t "
	lgStrSQL = lgStrSQL & " 			WHERE yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND family_type IN ('3','4','5') ) EDU "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
'Response.Write "lgStrSQL:" & lgStrSQL
'Response.end
...
'장애인 교육비 및 인원수	
    lgStrSQL =            " UPDATE HFA030T"
	lgStrSQL = lgStrSQL & "    SET Disabled_edu_amt = EDU.EDU_SUM   , Disabled_edu_cnt = EDU.EDU_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(edu_amt),0) EDU_SUM,  count(*) EDU_CNT"
	lgStrSQL = lgStrSQL & "				FROM hfa090t "
	lgStrSQL = lgStrSQL & " 			WHERE yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND family_type ='H' ) EDU "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
'Response.Write "lgStrSQL:" & lgStrSQL
'Response.end
...
'본인 교육비 
    lgStrSQL =            " UPDATE HFA030T"
	lgStrSQL = lgStrSQL & "    SET PER_EDU = EDU.EDU_SUM , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(edu_amt),0) EDU_SUM "
	lgStrSQL = lgStrSQL & "				FROM hfa090t "
	lgStrSQL = lgStrSQL & " 			WHERE yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND family_type ='M' ) EDU "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
'Response.Write "lgStrSQL:" & lgStrSQL
'Response.end
    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
                     lgStrSQL = "Select   FAMILY_NAME, FAMILY_REL,dbo.ufn_GetCodeName('H0140',FAMILY_REL) FAMILY_REL_NM, FAMILY_TYPE, EDU_AMT,SUBMIT_FLAG,CASE WHEN SUBMIT_FLAG='Y' THEN '국세청자료' ELSE '그밖의자료' END SUBMIT_FLAGNM, YEAR_FLAG"
                     lgStrSQL = lgStrSQL & " From  HFA090T "
                     lgStrSQL = lgStrSQL & " WHERE YY     " & pComp1 & pCode1
