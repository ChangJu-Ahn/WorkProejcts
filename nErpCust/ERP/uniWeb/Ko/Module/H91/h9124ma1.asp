...
Sub InitData()
    ...
    For intRow = 1 To Frm1.vspdData.MaxRows			
	    Frm1.vspdData.Row = intRow
	     ...
	     " 		WHEN   2005 -  CONVERT(INT,case when SUBSTRING (replace(FAMILY_RES_NO,'-',''),7,1) in(1,2) then 1900 else 2000 end  +LEFT(FAMILY_RES_NO,2))  >= 65 THEN '경로자' "&_
	     " 		ELSE '' END ",_
	     "  HFA150T ",_
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetDate		C_MED_DT,			"지급일",   10,2, gDateFormat
        ggoSpread.SSSetEdit		C_MED_NAME,			"지급처상호", 15,,, 20,1   
        ggoSpread.SSSetEdit		C_MED_RGST_NO,      "지급처사업자번호", 14,,, 10,1   

        ggoSpread.SSSetEdit     C_FAMILY_NAME,		"가족성명",      10,,, 30,2
        ggoSpread.SSSetButton   C_FAMILY_NAME_POP

		ggoSpread.SSSetEdit		C_FAMILY_REL_CD,	"가족관계코드", 10     
		ggoSpread.SSSetEdit		C_FAMILY_REL_NM,	"가족관계", 10       
	    ggoSpread.SSSetEdit		C_FAMILY_RES_NO,	"주민번호" ,11,,, 14,2        		
		ggoSpread.SSSetEdit		C_FAMILY_TYPE_CD,	"대상자구분코드", 10     
		ggoSpread.SSSetEdit		C_FAMILY_TYPE_NM,	"대상자구분", 10         
		ggoSpread.SSSetFloat	C_MED_AMT,			"지급금액", 10,"2", ggStrIntegeralPart,  ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec,,,"Z"
		ggoSpread.SSSetFloat	C_PROV_CNT,			"지급건수", 10,"6", ggStrIntegeralPart,  ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec,,,"P"
        ggoSpread.SSSetEdit		C_MED_TEXT,			"의료비내용", 20,,, 50,2 
         ...
         Case  C_FAMILY_NAME       '가족명 
            ...
            Else
                ...
                If strRel = false then
	        		Call DisplayMsgBox("970029","X","부양가족공제자등록에 해당자가 의료비 체크가 되어있는지","X")	 
  	                Frm1.vspdData.Col = C_FAMILY_REL_CD
                    Frm1.vspdData.Text = ""
  	                Frm1.vspdData.Col = C_FAMILY_REL_NM
                    Frm1.vspdData.Text = ""
		...
		Select Case Frm1.vspdData.Text 
			Case  ggoSpread.InsertFlag
				...
				If Frm1.vspdData.Text = "" Then
					Call DisplayMsgBox("800489","X","가족사항의 주민번호","X")
				...
				If Frm1.vspdData.Text = "" Then
					Call DisplayMsgBox("800489","X","환경등록의 REFERENCE","X")
					Frm1.vspdData.action = 0   
					Exit Function
				End If
				...
				If cint(left(UniConvDateAToB(frm1.vspdData.Text,parent.gDateFormat,parent.gServerDateFormat),4)) <> cint(Frm1.txtYear.Year) Then
					Call DisplayMsgBox("970029","X","지급일 년도","X")
					Frm1.vspdData.action = 0   
					Exit Function
				End If			
...
<TABLE <%=LR_SPACE_TYPE_00%>>
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../Cshared/Image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>기초자료등록(의료비)</font></td>
					...
					<TD HEIGHT=20 WIDTH=100%>
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									<TD CLASS=TD5 NOWRAP>정산연도</TD>
									<TD CLASS=TD6 NOWRAP>
										<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtYear" CLASS=FPDTYYYY tag="12X1" Title="FPDATETIME" ALT="정산연도" id=fpDateTime1> </OBJECT>');</SCRIPT>
									</TD>	
	...
	<TR>
		<TD <%=HEIGHT_TYPE_01%> WIDTH=100%>&nbsp;&nbsp;&nbsp;※의료비는 부양가족공제자등록 화면에서 의료비로 체크된 사람만 입력가능합니다.</TD>
	</TR> 	
	<TR HEIGHT=55>
		<TD WIDTH=100%>
			<TABLE >
				<TR><TD WIDTH=10>&nbsp;</TD>
					<TD WIDTH=100>일반의료비:</TD>
