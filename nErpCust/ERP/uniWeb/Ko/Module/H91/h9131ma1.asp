...
'*  3. Program ID           : h9131ma1.asp
'*  4. Program Name         : h9131ma1.asp
'*  5. Program Desc         : 기초자료등록(신용카드)
...
Const BIZ_PGM_ID      = "h9131mb1.asp"						           '☆: Biz Logic ASP Name
...
Sub InitComboBox()
    ...
    Dim iWhere
	 ...
	 ggoSpread.SetCombo "1" & vbtab & "2" & vbtab & "3"   , C_TypeCd
     ggoSpread.SetCombo "신용카드" & vbtab & "현금영수증" & vbtab & "학원비지로납부"  ,C_TypeCdnm
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetCombo C_TypeCd    , "",5
		ggoSpread.SSSetCombo C_TypeCdNm    , "구분", 20
		ggoSpread.SSSetFloat C_USEAmt       , "금액", 15,"2", ggStrIntegeralPart,  ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec,,,"Z"
		...
		call ggoSpread.MakePairsColumn(C_TypeCd,C_TypeCdNm,"1")
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
			...
			C_TypeCd = iCurColumnPos(5)
			C_TypeCdNm = iCurColumnPos(6)
			C_USEAmt    = iCurColumnPos(7)
...
Sub SetSpreadLock()
    ...
    With frm1.vspdData
		...
		.ReDraw = False
		 ...
		 ggoSpread.SpreadLock    C_TypeCdNm, -1, C_TypeCdNm
		 ggoSpread.SpreadLock    C_subMitCdNM, -1, C_subMitCdNM
		 ggoSpread.SSSetRequired C_USEAmt,    -1, C_USEAmt
...
Sub SetSpreadColor(ByVal pvStartRow, ByVal pvEndRow)
	...
	With frm1
		...
		.vspdData.ReDraw = False
		 ...
		 ggoSpread.SSSetRequired		C_TypeCdNm, pvStartRow, pvEndRow
		 ggoSpread.SSSetRequired		C_subMitCdNM, pvStartRow, pvEndRow
		 ggoSpread.SSSetRequired		C_USEAmt, pvStartRow, pvEndRow
    ...
    Select Case Col
         Case  C_FamName
            ...
            Else
                ...
                strRel = CommonQueryRs(" FAMILY_RES_NO"," HFA150T "," CARD_YN = 'Y' AND FAMILY_NAME = " & FilterVar(iDx, "''", "S")  & " AND EMP_NO= " & FilterVar(frm1.txtEmp_no.value, "''", "S") & " AND YEAR_YY = " & FilterVar(frm1.txtYear.Year, "''", "S") ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
                ...
                Else
					...
					Call CommonQueryRs(" FAMILY_REL, dbo.ufn_GetCodeName('H0140',FAMILY_REL) " ,_
 					" HFA150T ",_
					" CARD_YN='Y' and EMP_NO =" & FilterVar(frm1.txtEmp_no.value, "''", "S")  & " AND YEAR_YY = " & FilterVar(frm1.txtYear.Year, "''", "S") & " AND FAMILY_NAME= " & FilterVar(family_nm, "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
         ...
         Case  C_TypeCdNm
                 ...
                 if  Frm1.vspdData.text ="2" then '현금영수증 
               

					 Frm1.vspdData.Col   = C_subMitCdnm
					 Frm1.vspdData.value = "Y"
					 ggoSpread.SSSetProtected	C_subMitCdnm, Row, Row
					 
					 call vspdData_ComboSelChange(C_subMitCdnm, Row)
					  		 
				 else
				 	
				 	 ggoSpread.SpreadUnLock C_subMitCdnm, Row, C_subMitCdnm, Row
				 	 ggoSpread.SSSetRequired C_subMitCdnm,    Row, Row	 
				 	 ' Frm1.vspdData.Col   = C_subMitCd
					' Frm1.vspdData.text = "N"
					 ' call vspdData_ComboSelChange(C_subMitCd, Row)
                 end if
...
Function OpenCode(Byval strCode, Byval iWhere, ByVal Row)
	...
	Select Case iWhere
	    ...
	    Case C_FamName_POP
	        ...
	        arrParam(4) = " CARD_YN='Y' and year_yy = " & FilterVar(frm1.txtYear.Year, "''", "S") & " and emp_no = " & FilterVar(frm1.txtEmp_no.value, "''", "S") 			' Where Condition
	...
	With frm1
		...
		Select Case iWhere
            ...
            '    if Trim(Replace(lgF0,Chr(11),""))<>"" then
		     ...
		     '  		Frm1.vspdData.Col = C_TypeCdNm 
		      ' 		Frm1.vspdData.Text = Trim(Replace(lgF1,Chr(11),""))
		       	
		       '		ggoSpread.SSSetProtected	C_TypeCdNm, Row, C_TypeCdNm		       
		       '	end if	
		     'else
				'ggoSpread.SpreadUnLock		C_TypeCdNm, Row, C_TypeCdNm,Row
				'ggoSpread.SSSetRequired		C_TypeCdNm, C_TypeCdNm, C_TypeCdNm			
		    ' end if  	
...
Function FncInsertRow(ByVal PvRowCnt) 
	...
	With frm1
        ...
        call vspdData_ComboSelChange(C_subMitCd,  .vspdData.ActiveRow)
         call vspdData_ComboSelChange(C_typeCd,  .vspdData.ActiveRow)
       .vspdData.ReDraw = True
    End With
	...
	For lRow = 1 To .vspdData.MaxRows
		...
		Select Case .vspdData.Text
			Case  ggoSpread.InsertFlag									  
                ...
                .vspdData.Col = C_TypeCd    : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep   '4
                .vspdData.Col = C_USEAmt       : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep   '5
            ...
            Case  ggoSpread.DeleteFlag									
                ...
                .vspdData.Col = C_TypeCd    : strDel = strDel & Trim(.vspdData.Text) & Parent.gRowSep   '4
...
<TABLE <%=LR_SPACE_TYPE_00%>>
	...
	<TR>
		<TD <%=HEIGHT_TYPE_01%> WIDTH=100%>&nbsp;&nbsp;&nbsp;※보험료는 부양가족공제자등록 화면에서 신용카드로 체크된 사람만 입력가능합니다.</TD>
	</TR> 		
	<TR HEIGHT=55>
		<TD WIDTH=100%>
			<TABLE >
				<TR><TD WIDTH=10>&nbsp;</TD>
					...
					<TD WIDTH=120>&nbsp;&nbsp;현금영수증:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum2 NAME="txtSum2" CLASS=FPDS140 tag="24X2Z" ALT="현금영수증" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
					
					<TD WIDTH=120>&nbsp;&nbsp;학원비지로납부:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum3 NAME="txtSum3" CLASS=FPDS140 tag="24X2Z" ALT="현금영수증" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
	...
	<TR>
		<TD WIDTH=100% HEIGHT=0><IFRAME NAME="MyBizASP" SRC="h9131mb1.asp" WIDTH=100% HEIGHT=1 FRAMEBORDER=0 SCROLLING=YES noresize framespacing=0></IFRAME>
		</TD>
	</TR>
</TABLE>
