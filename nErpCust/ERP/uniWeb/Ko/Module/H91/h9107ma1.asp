...
'*  3. Program ID           : h9107ma1.asp
'*  4. Program Name         : h9107ma1.asp
'*  5. Program Desc         : 기초자료등록(교육비)
...
Const BIZ_PGM_ID      = "h9107mb1.asp"						           '☆: Biz Logic ASP Name
...
Sub InitComboBox()
    ...
    Call  CommonQueryRs(" MINOR_CD,MINOR_NM "," B_MINOR "," MAJOR_CD = " & FilterVar("H0070", "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetFloat C_EduAmt       , "금액", 15,"2", ggStrIntegeralPart,  ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec,,,"Z"
...
Sub SetSpreadLock()
    ...
    With frm1.vspdData
		...
		.ReDraw = False
		 ...
		 ggoSpread.SSSetRequired C_EduAmt,    -1, C_EduAmt
...
Sub SetSpreadColor(ByVal pvStartRow, ByVal pvEndRow)
	...
	With frm1
		...
		.vspdData.ReDraw = False
		 ...
		 ggoSpread.SSSetRequired		C_EduAmt, pvStartRow, pvEndRow
    ...
    Select Case Col
         Case  C_FamName
            ...
            Else
                ...
                strRel = CommonQueryRs(" FAMILY_RES_NO"," HFA150T "," EDU_YN = 'Y' AND FAMILY_NAME = " & FilterVar(iDx, "''", "S")  & " AND EMP_NO= " & FilterVar(frm1.txtEmp_no.value, "''", "S") & " AND YEAR_YY = " & FilterVar(frm1.txtYear.Year, "''", "S") ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)

                If strRel = false then
	        		Call DisplayMsgBox("970029","X","부양가족공제자등록에 해당자가 교육비 체크가 되어있는지","X")	 
  	                Frm1.vspdData.Col = C_FamRelCd
                    Frm1.vspdData.Text = ""
  	                Frm1.vspdData.Col = C_FamRelNm
                    Frm1.vspdData.Text = ""
                ...
                Else
					...
					Call CommonQueryRs(" FAMILY_REL, dbo.ufn_GetCodeName('H0140',FAMILY_REL) " ,_
 					" HFA150T ",_
					" EDU_YN='Y' and EMP_NO =" & FilterVar(frm1.txtEmp_no.value, "''", "S")  & " AND YEAR_YY = " & FilterVar(frm1.txtYear.Year, "''", "S") & " AND FAMILY_NAME= " & FilterVar(family_nm, "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Function OpenCode(Byval strCode, Byval iWhere, ByVal Row)
	...
	Select Case iWhere
	    ...
	    Case C_FamName_POP
	        ...
	        arrParam(4) = " EDU_YN='Y' and year_yy = " & FilterVar(frm1.txtYear.Year, "''", "S") & " and emp_no = " & FilterVar(frm1.txtEmp_no.value, "''", "S") 			' Where Condition
	...
	With frm1
		...
		Select Case iWhere
		    Case C_FamName_POP
		    	...
		    	.vspdData.Col = C_FamRelNm
		    	.vspdData.text = arrRet(2) 
		    	 

				Call CommonQueryRs(" FAMILY_REL, dbo.ufn_GetCodeName('H0140',FAMILY_REL) , PARIA_YN ", " HFA150T ",_
				" EDU_YN='Y' and EMP_NO =" & FilterVar(frm1.txtEmp_no.value, "''", "S")  & " AND YEAR_YY = " & FilterVar(frm1.txtYear.Year, "''", "S") & " AND FAMILY_NAME= " & FilterVar(family_nm, "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
				...
				If Frm1.vspdData.Text = "0" Then
					...
					ggoSpread.SSSetProtected	C_FamTypeNm, iWhere, iWhere
					
				ElseIf Trim(Replace(lgF2,Chr(11),"")) = "Y" Then
		
		       		Frm1.vspdData.Col = C_FamTypeCd
					Frm1.vspdData.text = "H"
				...
				Else
					...
					ggoSpread.SpreadUnLock		C_FamTypeNm, iWhere, C_FamTypeNm,iWhere
					ggoSpread.SSSetRequired		C_FamTypeNm, iWhere, iWhere					
				End If
	...
	For lRow = 1 To .vspdData.MaxRows
		...
		Select Case .vspdData.Text
			Case  ggoSpread.InsertFlag									  
                ...
                .vspdData.Col = C_EduAmt       : strVal = strVal & Trim(.vspdData.Text) & Parent.gColSep   '5
...
<TABLE <%=LR_SPACE_TYPE_00%>>
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../Cshared/Image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>기초자료등록(교육비)</font></td>
	...
	<TR>
		<TD <%=HEIGHT_TYPE_01%> WIDTH=100%>&nbsp;&nbsp;&nbsp;※교육비는 부양가족공제자등록 화면에서 교육비로 체크된 사람만 입력가능합니다.</TD>
	</TR> 		
	<TR HEIGHT=55>
		<TD WIDTH=100%>
			<TABLE >
				<TR><TD WIDTH=10>&nbsp;</TD>
					<TD WIDTH=110>장애인교육비:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum4 NAME="txtSum4" CLASS=FPDS140 tag="24X2Z" ALT="대학교교육비" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
					<TD WIDTH=120>&nbsp;&nbsp;본인교육비:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum5 NAME="txtSum5" CLASS=FPDS140 tag="24X2Z" ALT="초등고교육비" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
					<TD WIDTH=*>&nbsp;</TD>
				</TR>
				<TR><TD WIDTH=10>&nbsp;</TD>
					<TD WIDTH=120>&nbsp;&nbsp;유치원교육비:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum3 NAME="txtSum3" CLASS=FPDS140 tag="24X2Z" ALT="유치원교육비" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
					<TD WIDTH=120>&nbsp;&nbsp;초등고교육비:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum2 NAME="txtSum2" CLASS=FPDS140 tag="24X2Z" ALT="초등고교육비" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
					<TD WIDTH=110>대학교교육비:</TD>
					<TD WIDTH=100><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> id=txtSum1 NAME="txtSum1" CLASS=FPDS140 tag="24X2Z" ALT="대학교교육비" Title="FPDOUBLESINGLE"></OBJECT>');</SCRIPT></TD>
					<TD WIDTH=*>&nbsp;</TD>
				</TR>								
				<TR><TD WIDTH=10>&nbsp;</TD>
	...
	<TR>
		<TD WIDTH=100% HEIGHT=0><IFRAME NAME="MyBizASP" SRC="h9107mb1.asp" WIDTH=100% HEIGHT=0 FRAMEBORDER=0 SCROLLING=YES noresize framespacing=0></IFRAME>
		</TD>
	</TR>
</TABLE>
