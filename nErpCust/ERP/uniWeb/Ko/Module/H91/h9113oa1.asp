...
'*  3. Program ID           : h9113oa1
'*  4. Program Name         : 연말정산징수부출력 
'*  5. Program Desc         : 연말정산징수부출력 
'*  6. Comproxy List        : +
...
Sub SetDefaultVal()
	...
	frm1.txtbas_yy.text	= strYear
	
	Call ggoOper.FormatDate(frm1.txtbas_yy,gDateFormat, 3)
	
	frm1.txtBas_dt.text	= UNIConvDateAToB("<%=GetSvrDate%>" ,parent.gServerDateFormat, parent.gDateFormat)

	if frm1.txtbas_yy.text <= "2004" then
		frm1.Format1.checked = true    ' 2002년 이전이면 2001년 양식 
	else
		frm1.Format2.checked = true
	end if
...
Function OpenDept(iWhere)
    ...
    arrParam(1) = UNIConvDate(frm1.txtBas_dt.Text)
	arrParam(2) = lgUsrIntCd                    ' 자료권한 Condition  
...
Function FncBtnPrint() 
    ...
    Call FuncGetTermDept(lgUsrIntCd,UNIConvDate(frm1.txtBas_dt.Text),strMin,strMax)
	...
	Dim bas_dt, bas_yy, biz_area_cd, emp_no, fr_dept_cd, ocpt_type, to_dept_cd
	Dim ObjName		
	With frm1
	    bas_yy = .txtbas_yy.text
	    biz_area_cd = .txtcust_cd.value 
	    bas_dt = .txtBas_dt.Year & Right("0" & .txtBas_dt.Month, 2) & Right("0" & .txtBas_dt.Day, 2) 
	...
	if frm1.Format1.checked = true then
		strEbrFile = strEbrFile & "_2004"
	else
		strEbrFile = strEbrFile & "_2005"
	end if   
...
'On Error Resume Next                                                    '☜: Protect system from crashing
    ...
    Else
        If strFrDept = "" then
            strFrDept = "00000000000"
        End if
        If strToDept = "" then
            strToDept = "zzzzzzzzzz"
        ElseIf strFrDept > strToDept then
	        Call DisplayMsgBox("800153","X","X","X")	'시작부서는 종료부서보다 작아야 합니다.
            frm1.txtFr_dept_cd.focus
            Set gActiveElement = document.activeElement
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>연말정산징수부</font></td>
					...
					<TD HEIGHT=20>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							  <TR>
									...
									<TD CLASS=TD6 NOWRAP COLSPAN=3><script language =javascript src='./js/h9113oa1_fpDateTime1_txtbas_yy.js'></script></TD>
								</TR>
							    <TR>
								...
								<TR>	
									...
									<TD CLASS=TD6 NOWRAP COLSPAN=3><script language =javascript src='./js/h9113oa1_bas_dt_txtBas_dt.js'></script></TD>
								</TR>
								<TR>	
									<TD CLASS=TD5 NOWRAP>신고양식</TD>
									<TD CLASS=TD6 NOWRAP COLSPAN=3><INPUT TYPE="RADIO" CLASS="RADIO" NAME="txtFormat" TAG="1X" VALUE="2002" ID="Format1"><LABEL FOR="Format1">2004년양식</LABEL>&nbsp;
								   							       <INPUT TYPE="RADIO" CLASS="RADIO" NAME="txtFormat" TAG="1X" VALUE="2004" ID="Format2"><LABEL FOR="Format2">2005년양식</LABEL>&nbsp;</TD>
								</TR> 								
								<TR>
								<TD CLASS="TD5" NOWRAP>대상자</TD>
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" ID = "txtEmp_no" NAME="txtEmp_no" SIZE=10 MAXLENGTH=13  tag="11XXXU" ALT="사번"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnAcctCdFR" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenEmpName (1)">
