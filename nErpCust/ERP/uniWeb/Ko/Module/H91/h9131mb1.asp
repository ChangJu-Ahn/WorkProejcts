...
<%        
    Else
        ...
        Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("USE_TYPENM"))
			...
			Select Case lgObjRs("USE_TYPE")
			    Case "1"
			            amtSum1 = amtSum1 + cdbl(lgObjRs("USE_AMT")) 
			     Case "2" 
			            amtSum2 = amtSum2 + cdbl(lgObjRs("USE_AMT"))         			
			    Case else 
			            amtSum3 = amtSum3 + cdbl(lgObjRs("USE_AMT")) 
...
Sub SubBizSaveMultiUpdate(arrColVal)
	...
	lgStrSQL = lgStrSQL & "       USE_AMT     = " & UNIConvNum(arrColVal(5),0)                   & ","
	'lgStrSQL = lgStrSQL & "       USE_TYPE     = " & FilterVar(UCase(arrColVal(4)), "''", "S")   & ","
    ...
    lgStrSQL = lgStrSQL & "   AND USE_TYPE     = " & FilterVar(UCase(arrColVal(4)), "''", "S")  
...
'신용카드 
	...
	lgStrSQL = lgStrSQL & "    SET CARD_USE_AMT = USE1.USE_SUM,   " ', Disabled_edu_cnt = USE.USE_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(USE_AMT),0) USE_SUM,  count(*) USE_CNT"
	lgStrSQL = lgStrSQL & "				FROM HFA180T "
	lgStrSQL = lgStrSQL & " 			WHERE YEAR_yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND USE_TYPE='1') USE1 "		
...
'현금영수증 
    ...
    lgStrSQL =            " UPDATE HFA030T"
	lgStrSQL = lgStrSQL & "    SET CARD2_USE_AMT = USE1.USE_SUM ,  "', UNIV_USE_CNT = USE.USE_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(USE_AMT),0) USE_SUM,  count(*) USE_CNT"
	lgStrSQL = lgStrSQL & "				FROM HFA180T "
	lgStrSQL = lgStrSQL & " 			WHERE YEAR_yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND USE_TYPE='2') USE1 "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
    lgObjConn.Execute lgStrSQL,,adCmdText + adExecuteNoRecords
	Call SubHandleError("MU",lgObjConn,lgObjRs,Err)
...
'학원비지로납부 
    ...
    lgStrSQL =            " UPDATE HFA030T"
	lgStrSQL = lgStrSQL & "    SET INSTITUTION_GIRO = USE1.USE_SUM ,  "', UNIV_USE_CNT = USE.USE_CNT , "
	lgStrSQL = lgStrSQL & "        UPDT_EMP_NO = " & FilterVar(gUsrId, "''", "S")   & ","
	lgStrSQL = lgStrSQL & "        UPDT_DT     = " & FilterVar(lgGetSvrDateTime,NULL,"S")	
	lgStrSQL = lgStrSQL & "   FROM  ( SELECT ISNULL(SUM(USE_AMT),0) USE_SUM,  count(*) USE_CNT"
	lgStrSQL = lgStrSQL & "				FROM HFA180T "
	lgStrSQL = lgStrSQL & " 			WHERE YEAR_yy = " & FilterVar(lgKeyStream(0), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND emp_no = " & FilterVar(lgKeyStream(1), "''", "S") 
	lgStrSQL = lgStrSQL & " 				AND USE_TYPE='3') USE1 "		
	lgStrSQL = lgStrSQL & " WHERE YY          = " & FilterVar(lgKeyStream(0), "''", "S") 
    lgStrSQL = lgStrSQL & "   AND EMP_NO      = " & FilterVar(lgKeyStream(1), "''", "S") 
    lgObjConn.Execute lgStrSQL,,adCmdText + adExecuteNoRecords
	Call SubHandleError("MU",lgObjConn,lgObjRs,Err)
    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
                     lgStrSQL = "Select   FAMILY_NAME, FAMILY_REL,dbo.ufn_GetCodeName('H0140',FAMILY_REL) FAMILY_REL_NM,  USE_AMT,USE_TYPE,CASE WHEN USE_TYPE='1' THEN '신용카드'  when use_type=2 then '현금영수증' else '학원비지로납부' END USE_TYPENM,SUBMIT_FLAG,CASE WHEN SUBMIT_FLAG='Y' THEN '국세청자료' ELSE '그밖의자료' END SUBMIT_FLAGNM, YEAR_FLAG"
                     lgStrSQL = lgStrSQL & " From  HFA180T "
