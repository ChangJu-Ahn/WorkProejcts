...
Sub SubBizQueryMulti(pCardCD)
    ...
    '---------- Developer Coding part (Start) ---------------------------------------------------------------
		...
		lgStrSQL = lgStrSQL & " WHERE A.NOTE_FG = " & FilterVar("CP", "''", "S") & "  "      
...
Sub SubBizSaveSingleCreate()
	...
	If Trim(Request("txtCardNoQry")) = "" Then 
		Call CardPayAutoNum()
	Else 
		strAutoCardNo = Trim(Request("txtCardNoQry"))

		If UCase(Left(Trim(strAutoCardNo),2)) = "CU" Then
          Call DisplayMsgBox("120714", vbInformation, "", "", I_MKSCRIPT)             '☜: No data is found. 
          lgErrorStatus = "YES" 
          Exit Sub
		End If

		'Parent.frm1.htxtCardNo.value = Trim(Request("txtCardNoQry")) 	
	End If	
    ...
    lgStrSQL = lgStrSQL & "  VALUES(" & FilterVar(strAutoCardNo	, "''", "S")  & ","
    lgStrSQL = lgStrSQL &					"" & FilterVar("CP", "''", "S") & " " & ","
               ...
               Case "R"
                        Select Case  lgPrevNext 
                             ...
                             Case "P"
									...
									lgStrSQL = lgStrSQL & " AND A.CARD_CO_CD = E.CARD_CO_CD				"
									lgStrSQL = lgStrSQL & " AND A.BP_CD = F.BP_CD								"
									lgStrSQL = lgStrSQL & " AND A.NOTE_FG = " & FilterVar("CP", "''", "S") & " 								"
									lgStrSQL = lgStrSQL & " AND A.NOTE_NO < " & FilterVar(lgKeyStream(0), "''", "S")  
