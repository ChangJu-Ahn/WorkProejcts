...
Sub SubBizQueryMulti()
	...
	'내부 TABLE 
	If Trim(Request("cboNoteFg")) = "D1" Then 
	' 받을어음 
		...
		lgStrSQL2 = lgStrSQL2 & " 											C2.ITEM_LOC_AMT, C2.ACCT_CD, C2.ITEM_SEQ,  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 											C1.BIZ_AREA_CD, dbo.ufn_GL_DTL_by_table(C2.gl_no, c2.item_seq, " & FilterVar("F_NOTE", "''", "S") & " , " & FilterVar("NOTE_NO", "''", "S") & " )  NOTE_NO " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 										AND C1.REF_NO LIKE " & FilterVar("FN%", "''", "S") & " " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 										AND B.GL_SEQ = C.ITEM_SEQ AND B.NOTE_NO = C.NOTE_NO " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 				AND D.ACCT_TYPE = " & FilterVar("D1", "''", "S") & " " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & "									AND B.TEMP_GL_SEQ = C.ITEM_SEQ AND B.NOTE_NO = C.NOTE_NO  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 						LEFT JOIN A_ACCT E ON B.NOTE_ACCT_CD = E.ACCT_CD  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				WHERE A.NOTE_STS = " & FilterVar("SM", "''", "S") & " " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND B.STS_DT >= " & strFrDt  & " AND B.STS_DT <= " & strToDt & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND E.ACCT_TYPE = " & FilterVar("D1", "''", "S") & "  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND ISNULL(C.ITEM_SEQ, '') <> '' " & vbCr
	...
	ElseIf Trim(Request("cboNoteFg")) = "D3" Then 
	' 지급어음		
		lgStrSQL2 = lgStrSQL2 & " 		FROM  (  SELECT  A.NOTE_NO, B.STS_DT, C.GL_NO,  C.GL_DT, B.NOTE_ACCT_CD, C.GL_INPUT_TYPE,  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 					SUM(ISNULL(A.STTL_AMT, 0)) SUM_STTL_AMT,  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 					SUM(ISNULL(C.ITEM_LOC_AMT, 0)) SUM_GL_AMT, " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 				AND B.STS_DT >= " & strFrDt  & " AND B.STS_DT <= " & strToDt  & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND ISNULL(C.ITEM_SEQ, '') <> ''  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND D.ACCT_TYPE = " & FilterVar("D3", "''", "S") & " " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				GROUP BY A.NOTE_NO, C.GL_NO, C.GL_DT, B.NOTE_ACCT_CD, C.GL_INPUT_TYPE, C.BIZ_AREA_CD, A.BP_CD, B.STS_DT) GL " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 		 LEFT JOIN (  SELECT 	A.NOTE_NO, B.STS_DT, C.TEMP_GL_NO, C.TEMP_GL_DT, B.NOTE_ACCT_CD, C.GL_INPUT_TYPE, " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 					SUM(ISNULL(A.STTL_AMT, 0)) SUM_STTL_AMT,  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 					SUM(ISNULL(C.ITEM_LOC_AMT, 0)) SUM_TEMP_AMT,  " & vbCr
		...
		lgStrSQL2 = lgStrSQL2 & " 				WHERE A.NOTE_STS = " & FilterVar("SM", "''", "S") & " " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND B.STS_DT >= " & strFrDt  & " AND B.STS_DT <= " & strToDt & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND E.ACCT_TYPE = " & FilterVar("D3", "''", "S") & "  " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				AND ISNULL(C.ITEM_SEQ, '') <> '' " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				GROUP BY A.NOTE_NO, C.TEMP_GL_NO, C.TEMP_GL_DT, B.NOTE_ACCT_CD, C.GL_INPUT_TYPE, C.BIZ_AREA_CD, A.BP_CD, B.STS_DT ) TMP " & vbCr
		lgStrSQL2 = lgStrSQL2 & " 				ON GL.NOTE_ACCT_CD = TMP.NOTE_ACCT_CD AND GL.BIZ_AREA_CD = TMP.BIZ_AREA_CD  AND GL.BP_CD = TMP.BP_CD AND GL.NOTE_NO = TMP.NOTE_NO " & vbCr
...
'	lgStrSQL2 = lgStrSQL2 & " ORDER BY ACCT_CD, NOTE_NO"	 & vbCr
	...
	'*********************************
	  ...
	  If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL2,"X","X") = True Then
			...
			lgTotTempGlLocAmt2 = UNIConvNumDBToCompanyByCurrency(lgObjRs("TEMP_GL_AMT"), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")
	...
	' 받을어음 
		...
		lgStrSQL = lgStrSQL & " 											C1.BIZ_AREA_CD, dbo.ufn_GL_DTL_by_table(C2.gl_no, c2.item_seq, " & FilterVar("F_NOTE", "''", "S") & " , " & FilterVar("NOTE_NO", "''", "S") & " )  NOTE_NO " & vbCr
		...
		lgStrSQL = lgStrSQL & " 										AND C1.REF_NO LIKE " & FilterVar("FN%", "''", "S") & " " & vbCr
		...
		lgStrSQL = lgStrSQL & " 										AND B.GL_SEQ = C.ITEM_SEQ AND B.NOTE_NO = C.NOTE_NO " & vbCr
		...
		lgStrSQL = lgStrSQL & " 								C1.BIZ_AREA_CD, dbo.ufn_GL_DTL_by_table(C2.TEMP_gl_no, c2.item_seq, " & FilterVar("F_NOTE", "''", "S") & " , " & FilterVar("NOTE_NO", "''", "S") & " )  NOTE_NO   " & vbCr
		...
		lgStrSQL = lgStrSQL & "									AND B.TEMP_GL_SEQ = C.ITEM_SEQ AND B.NOTE_NO = C.NOTE_NO  " & vbCr
