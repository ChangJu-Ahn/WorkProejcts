...
'----------------------------------------------------------------------------------------------------------
 Sub FixUNISQLData()
    ...
    Redim UNIValue(6,19)                                                  '⊙: DB-Agent로 전송될 parameter를 위한 변수 
                                                                          '    parameter의 수에 따라 변경함 
     UNISqlId(0) = "m6112qa201"     
     ...
     UNIValue(0,15) = UCase(Trim(strDistRefNoFrom))		'---배부참조번호 
     UNIValue(0,16) = UCase(Trim(strDistRefNo)) 
     UNIValue(0,17) = UCase(Trim(strDistTypeFrom))		'---배부유형 
    ...
    If  rs6.EOF And rs6.BOF Then
        ...
        Set rs6 = Nothing
         If Len(Request("txtDistType")) And FalsechkFlg = False Then
		   Call DisplayMsgBox("970000", vbInformation, "배부유형", "", I_MKSCRIPT)	'⊙: you must release this line if you change msg into code
	       FalsechkFlg = True	
		End If
    Else    
		arrRsVal(5) = rs6(1)
