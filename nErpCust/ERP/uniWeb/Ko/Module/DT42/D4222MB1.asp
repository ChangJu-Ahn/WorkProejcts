   ...
   If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then
    ...
    Else
       ...
       Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(replace(lgObjRs("DISUSE_REASON"),Chr(13) & Chr(10), ""))            
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("RETURN_CODE"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("RETURN_DESCRIPTION"))
    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
				...
				lgStrSQL = " "												                
                lgStrSQL = lgStrSQL & vbCrLf & "  Select distinct TOP " & iSelCount  & " A.CONVERSATION_ID, A.DTI_TYPE, "								
                lgStrSQL = lgStrSQL & vbCrLf & " 		 Y.MINOR_NM AS MINOR_NAME,	 "	
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.SEQ_ID,		 "									
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.DTI_WDATE ,		 "							
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.SUP_AMOUNT + A.TAX_AMOUNT AS TOTAL_AMOUNT, "	
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.SUP_AMOUNT,								 "	
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.TAX_AMOUNT,								 "	
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.BYR_COM_REGNO,							 "	
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.BYR_COM_NAME, "	
                lgStrSQL = lgStrSQL & vbCrLf & "         A.BYR_EMAIL,		 "							
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.REF_INVOICE_NUM,		 "						
                lgStrSQL = lgStrSQL & vbCrLf & "    	 F.DTI_STATUS, "	
                lgStrSQL = lgStrSQL & vbCrLf & "    	 B.MINOR_NM as s_minor_name,	 "									
                lgStrSQL = lgStrSQL & vbCrLf & " 		 isnull(F.SBDESCRIPTION,'')  AS DISUSE_REASON,	 "
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.AMEND_CODE ,	 "
                lgStrSQL = lgStrSQL & vbCrLf & " 		 O.MINOR_NM AS AMEND_CODE_NM,	 "
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.REMARK,	 "
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.REMARK2,	 "
                lgStrSQL = lgStrSQL & vbCrLf & " 		 A.REMARK3,	 "
                lgStrSQL = lgStrSQL & vbCrLf & "          CASE WHEN F.RETURN_CODE = '30000' THEN '' ELSE F.RETURN_CODE END RETURN_CODE,	 "
                lgStrSQL = lgStrSQL & vbCrLf & "          F.RETURN_DESCRIPTION	 "
                ...
                lgStrSQL = lgStrSQL & vbCrLf & "        AND A.SUPBUY_TYPE = F.SUPBUY_TYPE 	 "
                lgStrSQL = lgStrSQL & vbCrLf & "        AND A.DIRECTION = F.DIRECTION)	 "
                ...
                lgStrSQL = lgStrSQL & vbCrLf & "              ON (B.MAJOR_CD = 'DT409' 	 "
                lgStrSQL = lgStrSQL & vbCrLf & "             AND B.MINOR_CD = F.DTI_STATUS)	 "
                ...
                lgStrSQL = lgStrSQL & vbCrLf & "              ON (O.MAJOR_CD = 'DT408'	 " 
                lgStrSQL = lgStrSQL & vbCrLf & "             AND O.MINOR_CD = A.AMEND_CODE)	 "
                lgStrSQL = lgStrSQL & vbCrLf & " LEFT OUTER JOIN B_MINOR Y (NOLOCK) 	 "
                lgStrSQL = lgStrSQL & vbCrLf & "              ON (Y.MAJOR_CD = 'DT403' 	 "
                lgStrSQL = lgStrSQL & vbCrLf & "             AND Y.MINOR_CD = A.DTI_TYPE)	 "
