...
'*  4. Program Name         : 회계세금계산서관리(정발행)
'*  5. Program Desc         : 회계세금계산서관리(정발행)   
'*  6. Component List       : 
'*  7. Modified date(First) : 2011/05/30
...
'========================================================================================================
Const BIZ_PGM_ID  = "D4232MB1.asp"  'Main조회
Const BIZ_PGM_ID2 = "D4232MB2.asp"  'Dtl조회
Const BIZ_PGM_ID3 = "D4232MB3.asp"  'batch_id 채번후 발행취소
Const BIZ_PGM_ID4 = "D4232MB4.asp"  '발행취소 승인
Const BIZ_PGM_ID5 = "D4232MB5.asp"  '발행취소 거부
Const BIZ_PGM_ID6 = "D4232MB6.asp"  '발행취소 요청
Const BIZ_PGM_ID7 = "D4232MB7.asp"  '계산서 보기
...
Sub InitSpreadPosVariables()
	'add tab1 header datatable column
    	...
    	C1_remark               =  44     '비고     	
    	C1_sbdescription        =  45     '취소/거부 사유    	   	
    	C1_return_code          =  46     '오류코드
        C1_return_description   =  47     '오류설명
...
Sub InitSpreadPosVariables2()
	'add tab1 detail datatable column
	    ...
	    C2_tax_amount           =  8    '부가세금액
	    C2_total_amount         =  9    '합계금액
	    C2_remark               =  10   '비고              
	    C2_conversation_id      =  11   '전송관리번호
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData	
	  ...
	  Call ggoSpread.SSSetColHidden(C1_sbdescription, C1_sbdescription, True)		    
...
Sub InitSpreadSheet2()
	...
	With frm1.vspdData2	
		...
		ggoSpread.SSSetEdit  	C2_item_code, 		   "품목코드", 		30, ,,70
		...
		ggoSpread.SSSetEdit		C2_remark,			   "비고",			30, ,,50
		ggoSpread.SSSetEdit  	C2_conversation_id,	   "전송관리번호",	20, ,,50
		ggoSpread.SSSetEdit  	C2_dti_line_num, 	   "항목번호",      15, ,,18
																		
		
		Call ggoSpread.SSSetColHidden(C2_item_code, C2_item_code, True)
		Call ggoSpread.SSSetColHidden(C2_conversation_id, C2_conversation_id, True)
		Call ggoSpread.SSSetColHidden(C2_dti_line_num, C2_dti_line_num, True)
...
Sub SetSpreadLock_B()
	With frm1
        ...
        ggoSpread.SpreadLock	C2_item_md,	    -1, C2_item_md        
        ggoSpread.SpreadLock	C2_item_code,	-1, C2_item_code        
        ggoSpread.SpreadLock	C2_item_name,	-1, C2_item_name        
        ggoSpread.SpreadLock	C2_item_size,	-1, C2_item_size        
        ggoSpread.SpreadLock	C2_item_qty,	-1, C2_item_qty
        ggoSpread.SpreadLock	C2_unit_price,	-1, C2_unit_price
        ggoSpread.SpreadLock	C2_sup_amount,	-1, C2_sup_amount
        ggoSpread.SpreadLock	C2_tax_amount,	-1, C2_tax_amount
        ggoSpread.SpreadLock    C2_total_amount, 	-1, C2_total_amount		
        ggoSpread.SpreadLock    C2_remark, 		-1, C2_remark
        ggoSpread.SpreadLock    C2_conversation_id, -1, C2_conversation_id        
        ggoSpread.SpreadLock    C2_dti_line_num, -1, C2_dti_line_num
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
	...
	Select Case UCase(pvSpdNo)
		Case "A"
            ...
            C1_sbdescription        = iCurColumnPos(45)     '취소/거부사유                	       	    
    	    C1_return_code          = iCurColumnPos(46)     '오류코드
            C1_return_description   = iCurColumnPos(47)     '오류설명
...
Sub GetSpreadColumnPos2(ByVal pvSpdNo)
	...
	Select Case UCase(pvSpdNo)
		Case "A"
            ...
            C2_item_code            = iCurColumnPos(2)    '품목코드
	        C2_item_name            = iCurColumnPos(3)    '품목명
	        C2_item_size            = iCurColumnPos(4)    '규격    
	        C2_item_qty             = iCurColumnPos(5)    '수량
	        C2_unit_price           = iCurColumnPos(6)    '단가
	        C2_sup_amount           = iCurColumnPos(7)    '공급가액
	        C2_tax_amount           = iCurColumnPos(8)    '부가세금액
	        C2_total_amount         = iCurColumnPos(9)    '합계금액
	        C2_remark               = iCurColumnPos(10)   '비고              
	        C2_conversation_id      = iCurColumnPos(11)   '전송관리번호
	...
	With frm1
		...
		For lRow = 1 To .vspdData.MaxRows
			...
			If .vspdData.text = "1"  Then
				...
				If CommonQueryRs("  a.CONVERSATION_ID ", "  XXSB_DTI_MAIN  a inner join xxsb_dti_status b on a.conversation_id = b.conversation_id ", " a.SEQ_ID  = '" & strTaxBillNo & "' and b.DTI_STATUS <> 'O' ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) = True Then	
...
Function DbQueryOk()																		'☆: 조회 성공후 실행로직 
    ...
    With frm1.vspdData
        ...
        If (strStatus <> "I") and (strStatus <> "C") and (strStatus <> "N")   then    
   ...
   With frm1 
      Select Case UCase(strTaxDocStatus)
            ...
            Case "I"
                ...
                .btnAccept.disabled = true
                .btnReject.disabled = true
                .btnEmailPublish.disabled = false
                .CancelRequest.disabled = true
                .btnPrint.disabled = true
...
Function Check2()
	...
	End With
	 ...
	 if Count = frm1.vspdData2.maxrows then
		...
		if (NumTotalAmt2 <> NumTotalAmt) or (NumNetAmt2 <> NumNetAmt) or  (NumVatAmt2 <> NumVatAmt) then
