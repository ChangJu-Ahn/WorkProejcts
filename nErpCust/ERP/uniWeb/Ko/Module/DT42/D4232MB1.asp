...
UNIValue(0, 4) = FilterVar(UniConvDate(Request("txtIssuedFromDt")), "''", "S")
UNIValue(0, 5) = FilterVar(UniConvDate(Request("txtIssuedToDt")), "''", "S")


If Request("txtSupplierCd") = "" Then
	UNIValue(0, 6) = "|"
Else
	UNIValue(0, 6) = FilterVar(UCase(Request("txtSupplierCd")), "''", "S")
End If
...
If Request("txtBizAreaCd") = "" Then
	UNIValue(0, 7) = "%"
Else
	UNIValue(0, 7) = FilterVar(UCase(Request("txtBizAreaCd")), "''", "S")
End If
...
Else
    If Request("cboBillStatus") = "X" Then
	    UNIValue(0, 8) = " is null "
    Else
	    UNIValue(0, 8) = "=" & FilterVar(UCase(Request("cboBillStatus")), "''", "S")
    End If
End IF    
...
If Request("txtTaxDocNo") = "" Then
	UNIValue(0, 10) = "|"
Else
	UNIValue(0, 10) = FilterVar(UCase(Request("txtTaxDocNo")) & "%", "''", "S") 
End If

If Request("txtTaxBillNo") = "" Then
	UNIValue(0, 11) = "|"
Else
	UNIValue(0, 11) = FilterVar(UCase(Request("txtTaxBillNo")) & "%", "''", "S")
End If

If Request("txtTempGLNo") = "" Then
	UNIValue(0, 12) = "|"
Else
	UNIValue(0, 12) = FilterVar(UCase(Request("txtTempGLNo")) & "%", "''", "S")
End If

If Request("cboAmendCode") = "" Then
	UNIValue(0, 13) = "|"
Else
	UNIValue(0, 13) = FilterVar(UCase(Request("cboAmendCode")) & "%", "''", "S")
End If
...
<%		Dim iDx
		For iDx = 0 to rs0.RecordCount - 1 %>
            ...
            strData = strData & Chr(11) & "<%=ConvSPChars(rs0("sbdescription"))%>" 
