...
'*  8. Modified date(Last)	: 2002/02/25
...
Err.Clear
	...
	If strFlag <> "" Then
		...
		ElseIf strFlag = "ERROR_GROUP" Then
			...
			%>
			<Script Language=vbscript>
			parent.frm1.txtItemGroupCd.Focus()
			</Script>	
			<%
			Response.End
	...
	Else
		UNIValue(0, 3) = FilterVar(UCase(Trim(Request("txtChildItemCd"))),"''","S")
	End IF 
	...
	Else
		UNIValue(0, 5) = FilterVar(UCase(Trim(Request("txtTrackingNo"))),"''","S")
	End IF 
	...
	IF Request("cboInvMgr") = "" Then
		UNIValue(0, 7) = FilterVar("%", "''", "S") & " or isNULL(h.inv_mgr,'') = '' "
	Else
		UNIValue(0, 7) = FilterVar(UCase(Request("cboInvMgr")), "''", "S")
	End IF 
	
	IF Request("cboIssueMthd") = "" Then
		UNIValue(0, 8) = FilterVar("%","''","S")
	Else
		UNIValue(0, 8) = FilterVar(UCase(Request("cboIssueMthd")),"''","S")
	End IF 
	
	IF Trim(Request("txtWcCd")) = "" Then
		UNIValue(0, 9) = FilterVar("%", "''", "S")
	Else
		UNIValue(0, 9) = FilterVar(UCase(Request("txtWcCd")), "''", "S")
	End IF 
	
	IF Trim(Request("cboJobCd")) = "" Then
		UNIValue(0, 10) = FilterVar("%", "''", "S") & " or isNULL(c.job_cd,'') = '' "
	Else
		UNIValue(0, 10) = FilterVar(UCase(Request("cboJobCd")), "''", "S")
	End IF 
	
	IF Trim(Request("txtItemCd")) = "" Then
		UNIValue(0, 11) = FilterVar("%", "''", "S")
	Else
		UNIValue(0, 11) = FilterVar(UCase(Request("txtItemCd")), "''", "S")
	End IF 
	
	IF Trim(Request("txtItemGroupCd")) = "" Then
		UNIValue(0, 12) = "" 
	Else
		UNIValue(0, 12) = " and d.item_group_cd in (select item_group_cd from ufn_P_ListItemGrp(" & FilterVar(Trim(Request("txtItemGroupCd"))	, "''", "S") & " )) "
	End IF 
	...
	Select Case strQryMode
		Case CStr(OPMD_CMODE)
			...
			UNIValue(0, 14) = FilterVar(UCase(Request("txtProdtOrderNo")),"''","S")
		...
		Case CStr(OPMD_UMODE)
			...
			UNIValue(0, 14) = FilterVar(UCase(Request("lgStrPrevKey")),"''","S")
			UNIValue(0, 15) = FilterVar(UCase(Request("lgStrPrevKey1")),"''","S")
			UNIValue(0, 16) = FilterVar(UCase(Request("lgStrPrevKey")),"''","S")
			UNIValue(0, 17) = FilterVar(UCase(Request("lgStrPrevKey1")),"''","S")
			UNIValue(0, 18) = FilterVar(UCase(Request("lgStrPrevKey2")),"''","S")
			
	End Select
	
	If Request("rdoIssueFlag") = "Y" Then
		UNIValue(0, 19) = " and (a.req_qty - a.issued_qty) <= 0 "
	ElseIf Request("rdoIssueFlag") = "N" Then
		UNIValue(0, 19) = " and (a.req_qty - a.issued_qty) > 0 "
	Else
		UNIValue(0, 19) = " "
	End If	
...
%>
			...
			strData = strData & Chr(11) & "<%=Trim(ConvSPChars(rs0("issue_mthd")))%>"
...
%>
		...
		.frm1.hCompntCd.value = "<%=ConvSPChars(Request("txtChildItemCd"))%>"
		...
		.frm1.hProdMgr.value = "<%=ConvSPChars(UCase(Trim(Request("cboProdMgr"))))%>"
		.frm1.hInvMgr.value = "<%=ConvSPChars(UCase(Trim(Request("cboInvMgr"))))%>"
