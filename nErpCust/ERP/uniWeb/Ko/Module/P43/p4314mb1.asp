...
'*  3. Program ID           : RESERVATION LIST BY ORDER
'*  4. Program Name         : p4314mb1
'*  5. Program Desc         :
...
%>
			...
			strData = strData & Chr(11) & "<%=UniConvNumberDBToCompany(rs1("prodt_order_qty"), ggQty.DecPoint, ggQty.RndPolicy, ggQty.RndUnit, 0)%>"
			strData = strData & Chr(11) & "<%=ConvSPChars(rs1("prodt_order_unit"))%>"
...
%>
		...
		.frm1.hFromDate.value = "<%=Request("txtStartDt")%>"
		.frm1.hToDate.value = "<%=Request("txtEndDt")%>"		
<%			
		rs1.Close
		Set rs1 = Nothing
%>
