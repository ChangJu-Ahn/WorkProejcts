...
'*  1. Module Name          : 금형관리 
'*  2. Function Name        :
'*  3. Program ID           : P6215mb1
'*  4. Program Name         :
'*  5. Program Desc         :
'*  6. Comproxy List        : 
'*  7. Modified date(First) : 2004/01/27
...
Sub SubBizQueryMulti()
	...
	iLngMaxRow2	     = CDbl(lgMaxCount) * CDbl(lgPageNo_C) + 1
...
Sub FixUNISQLData()
	...
	IF Request("txtCastCd") = "" Then
	   strCast_Cd = "|"
	ELSE
	   strCast_Cd = FilterVar(Ucase(Trim(Request("txtCastCd"))),"''","S")
	END IF
	...
	IF Request("txtCarKind") = "" Then
	   strCar_kind = "|"
	ELSE
	   strCar_kind = FilterVar(Ucase(Trim(Request("txtCarKind"))),"''","S")
	END IF
	...
	UNIValue(1, 0) = FilterVar(Ucase(Trim(Request("txtCastCd"))),"''","S")

	UNIValue(2, 0) = strWork_Dt
	UNIValue(2, 1) = strPlantCd
	UNIValue(2, 2) = strCar_Kind
...
Sub QueryData()
	...
	Elseif Request("txtType") = "B" Then
		...
		UNIValue(0, 3) = strCar_Kind
		UNIValue(0, 4) = strCast_Cd

		UNISqlId(0) = "y6110y650"
...
Sub SubBizSaveMulti()
	...
	Set pPY6G215 = Server.CreateObject("PY6G215.CsF_Cast_PlanMultiSvr")
	...
	Call pPY6G215.Py6_MAINT_Y_FAC_CAST_MULTI_SVR(gStrGlobalCollection, trim(reqtxtSpread), iErrorPosition)

	If CheckSYSTEMError2(Err, True, iErrorPosition & "행","","","","") = True Then
	   Set pPY6G215 = Nothing
	   Exit Sub
	End If
