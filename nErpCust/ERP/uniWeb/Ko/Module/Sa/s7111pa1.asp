...
'*  3. Program ID           : S7111PA1
'*  4. Program Name         : Nego관리번호 Popup
'*  5. Program Desc         : 
'*  6. Comproxy List        : 
...
Const BIZ_PGM_ID 		= "s7111pb1.asp"                              '☆: Biz Logic ASP Name

Const C_MaxKey          = 3                                           '☆: key count of SpreadSheet
Const C_PopApplicant	= 1
Const C_PopNegoBank		= 2
...
'========================================================================================================
	Sub InitSpreadSheet()
		Call SetZAdoSpreadSheet("S7111PA1","S","A","V20030320", PopupParent.C_SORT_DBAGENT, frm1.vspdData, C_MaxKey, "X", "X" )
	    Call SetSpreadLock 	  	  
	End Sub
...
Function OpenConPopup(ByVal pvIntWhere)
	...
	Case C_PopNegoBank
		iArrParam(1) = "dbo.b_bank BK"
		iArrParam(2) = Trim(frm1.txtNegoBank.value)
		iArrParam(3) = ""
		iArrParam(4) = ""
		iArrParam(5) = frm1.txtNegoBank.alt
			
		iArrField(0) = "ED15" & PopupParent.gColSep & "BK.bank_cd"	<%' Field명(0)%>
		iArrField(1) = "ED30" & PopupParent.gColSep & "BK.bank_nm"	<%' Field명(1)%>

		iArrHeader(0) = "매입은행"
		iArrHeader(1) = "매입은행명"

		frm1.txtNegoBank.focus 
	Case C_PopSalesGrp												
...
Function SetConPopup(Byval pvArrRet,ByVal pvIntWhere)
	...
	Case C_PopNegoBank
		frm1.txtNegoBank.value = pvArrRet(0) 
		frm1.txtNegoBankNm.value = pvArrRet(1)   
	Case C_PopSalesGrp
...
'   Event Desc : 매입은행 
'==========================================================================================%>
Function txtNegoBank_OnChange1()
	Dim iStrCode
	
	With frm1
		iStrCode = Trim(.txtNegoBank.value)
		If iStrCode <> "" Then
			iStrCode = " " & FilterVar(iStrCode, "''", "S") & ""
			If Not GetCodeName(iStrCode, "default", "default", "default", "default", "" & FilterVar("BK", "''", "S") & "", C_PopNegoBank) Then
				.txtNegoBank.value = ""
				.txtNegoBankNm.value = ""
				.txtNegoBank.focus
			ELSE
...
'   Event Desc : 매입은행 
'==========================================================================================%>
Function txtNegoBank_OnKeyDown()
	lgBlnNegoBankChg = True
	lgBlnFlgChgValue = True
End Function
...
Function ChkValidityQueryCon()
	...
	If lgBlnNegoBankChg Then
		iStrCode = Trim(frm1.txtNegoBank.value)
		If iStrCode <> "" Then
			iStrCode = " " & FilterVar(iStrCode, "''", "S") & ""
			If Not GetCodeName(iStrCode, "default", "default", "default", "default", "" & FilterVar("BK", "''", "S") & "", C_PopNegoBank) Then
				Call DisplayMsgBox("970000", "X", frm1.txtNegoBank.alt, "X")
				frm1.txtNegoBank.focus
				ChkValidityQueryCon = False
				Exit Function
			End If
		Else
			frm1.txtNegoBankNm.value = ""
		End If
		lgBlnNegoBankChg = False
	End If
...
Function DbQuery() 
    ...
    With frm1
		...
		If lgIntFlgMode = PopupParent.OPMD_UMODE Then
			...
			strVal = strVal & "&txtNegoBank=" & .txtHNegoBank.value
		...
		Else
			...
			strVal = strVal & "&txtNegoBank=" & Trim(.txtNegoBank.value)
				...
				<FIELDSET CLASS="CLSFLD">
					<TABLE <%=LR_SPACE_TYPE_40%>>
						<TR>
							...
							<TD CLASS=TD5>NEGO일</TD>
							<TD CLASS=TD6>
								<script language =javascript src='./js/s7111pa1_fpDateTime1_txtFromDt.js'></script>&nbsp;~&nbsp;
								<script language =javascript src='./js/s7111pa1_fpDateTime2_txtToDt.js'></script>
							</TD>	
						</TR>	
						<TR>
							<TD CLASS=TD5>매입은행</TD>
							<TD CLASS=TD6><INPUT TYPE=TEXT NAME="txtNegoBank" ALT="매입은행" SIZE=10 MAXLENGTH=20 TAG="11XXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnNegoBank" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenConPopUp C_PopNegoBank">&nbsp;<INPUT TYPE=TEXT NAME="txtNegoBankNm" SIZE=20 TAG="14"></TD>
				...
				<TABLE <%=LR_SPACE_TYPE_20%>>
					<TR>
						<TD HEIGHT="100%" NOWRAP>
							<script language =javascript src='./js/s7111pa1_vaSpread_vspdData.js'></script>
						</TD>
					</TR>
				</TABLE>
			</TD>
...
<INPUT TYPE=HIDDEN NAME="txtHNegoBank" tag="24">
