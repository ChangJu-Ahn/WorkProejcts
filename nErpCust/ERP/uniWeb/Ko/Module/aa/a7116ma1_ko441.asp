...
* 11. Comment              : 
* 2005.05.04  Joo,  Sungho	  기간(월)별 감가상각 금액과 사업부 조회 조건 추가. 미리보기 추가
'********************************************************************************************** -->
<HTML>
<HEAD>
...
Const BIZ_PGM_ID = "a7116mb1_ko441.asp"			'☆: 비지니스 로직 ASP명 
...
Sub SetDefaultVal()
    ...
    Call ggoOper.FormatDate(frm1.txtRFr_dt, gDateFormat, 1)
    Call ggoOper.FormatDate(frm1.txtRTo_dt, gDateFormat, 1)
	
	frm1.txtFr_dt.focus
	frm1.txtFr_dt.Year		= strYear 		'년월 default value setting
	frm1.txtFr_dt.Month		= "01"

	frm1.txtTo_dt.Year		= strYear 	
	frm1.txtTo_dt.Month		= strMonth 	

	frm1.txtRFr_dt.Year		= strYear 		'년월 default value setting
	frm1.txtRFr_dt.Month		= "01"
        frm1.txtRFr_dt.Day              = "01"

	frm1.txtRTo_dt.Year		= strYear 	
	frm1.txtRTo_dt.Month		= strMonth 	
        frm1.txtRTo_dt.Day              = strDay
    ...
    '--------- Developer Coding Part (Start) ----------------------------------------------------------
	    If lgIntFlgMode <> parent.OPMD_UMODE Then
			...
			strVal = strVal & "&DurYrsFg="                  & DurYrsFg
                        strVal = strVal & "&txtRFryymmdd="               & Trim(.txtRFr_dt.text)
                        strVal = strVal & "&txtRToyymmdd="               & Trim(.txtRTo_dt.text)
...
Sub txtRFr_dt_DblClick(Button) 
    If Button = 1 Then
		Call SetFocusToDocument("M")    
        frm1.txtRFr_dt.Action = 7
        frm1.txtRFr_dt.focus
    End If
    lgBlnFlgChgValue = True
End Sub

Sub txtRFr_dt_Change()
    lgBlnFlgChgValue = True
End Sub

Sub txtRFr_dt_Keypress(KeyAscii)
    If KeyAscii = 13 Then
        Call MainQuery()
    End If
End Sub
...
Sub txtRTo_dt_DblClick(Button) 
    If Button = 1 Then
		Call SetFocusToDocument("M")    
        frm1.txtRTo_dt.Action = 7
        frm1.txtRTo_dt.focus
    End If
    lgBlnFlgChgValue = True
End Sub

Sub txtRTo_dt_Change()
    lgBlnFlgChgValue = True
  
End Sub

Sub txtRTo_dt_Keypress(KeyAscii)
    If KeyAscii = 13 Then
        Call MainQuery()
    End If
End Sub
...
Function BtnPrint(ByVal pvStrPrint) 
	...
	if (lgF0 = "") or Left(lgF0, Len(lgF0)-1) = "0" then 
		IntRetCD = DisplayMsgBox("800506","X","X","X")					'조건에 해당하는 데이터가 없습니다.(해당데이터가 있는지 확인하는 과정)
		Exit Function
	end if
	...
	iStrUrl = iStrUrl & "|ToDt|"		& varToDt
        iStrUrl = iStrUrl & "|RFrDt|"           & varRFrDt
        iStrUrl = iStrUrl & "|RToDt|"           & varRToDt
	
	iStrUrl = iStrUrl & "|strAuthCond|"	& strAuthCond

	OBjName = AskEBDocumentName("a7116oa1_ko441","ebr")    
						...
						<FIELDSET CLASS="CLSFLD">
						  <TABLE <%=LR_SPACE_TYPE_40%>>
							<TR>
			            				...
			            				<TD CLASS="TD6"><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtFr_dt name=txtFr_dt title=FPDATETIME tag="12X1" ALT="시작월" CLASS=FPDTYYYYMM></OBJECT>');</SCRIPT>&nbsp;~&nbsp;
           									<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtTo_dt name=txtTo_dt title=FPDATETIME tag="12X1" ALT="종료월" CLASS=FPDTYYYYMM></OBJECT>');</SCRIPT></TD>
								<TD CLASS="TD5" NOWRAP></TD>
								<TD CLASS="TD6" NOWRAP></TD>
							</TR>
							<TR>
			            				<TD CLASS="TD5" NOWRAP>취득일자</TD>
			            				<TD CLASS="TD6"><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtRFr_dt name=txtRFr_dt title=FPDATETIME tag="11X1" ALT="시작월" CLASS=FPDTYYYYMMDD></OBJECT>');</SCRIPT>&nbsp;~&nbsp;
           									<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> id=txtRTo_dt name=txtRTo_dt title=FPDATETIME tag="11X1" ALT="종료월" CLASS=FPDTYYYYMMDD></OBJECT>');</SCRIPT></TD>
<TD CLASS="TD5" NOWRAP>내용년수구분</TD>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtAcctCd" SIZE=10 MAXLENGTH=15 tag="11XXXU" ALT="계정코드"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnAcctCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenAcctPopup(frm1.txtAcctCd.Value, '')"> <INPUT TYPE="Text" NAME="txtAcctNm" SIZE=25 MAXLENGTH=30 tag="14" ALT="계정명"></TD>
							</TR>
							<TR>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtBizUnitCd" SIZE=15 MAXLENGTH=15 tag="11XXXU" ALT="사업부코드"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnBizUnitCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenBizUnitPopup(frm1.txtBizUnitCd.Value, '')"> <INPUT TYPE="Text" NAME="txtBizUnitNm" SIZE=25 MAXLENGTH=30 tag="14" ALT="사업부명"></TD>
								<TD CLASS="TD5" NOWRAP></TD> 
						...
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR HEIGHT =100%>
							 ...
							 <TR>
								<TD CLASS="TD5" NOWRAP></TD>
								<TD CLASS="TD6" NOWRAP></TD>
								<TD CLASS="TD5" NOWRAP>당기상각액</TD>
								<TD CLASS="TD6" NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> name=txtSum1 style="HEIGHT: 20px; RIGHT: 0px; TOP: 0px; WIDTH:180px" title="FPDOUBLESINGLE" ALT="당기상각액" tag="24X2" id=OBJECT1></OBJECT>');</SCRIPT></TD>
							</TR>
						</TABLE>
					</TD>
	...
	<TR HEIGHT=20>
		<TD WIDTH=100%>
			<TABLE  CLASS="BasicTB" CELLSPACING=0>
				<TR>
					<TD WIDTH=10>&nbsp;</TD>
					<TD><BUTTON NAME="btnPreview" CLASS="CLSSBTN"  Flag=1>미리보기</BUTTON>&nbsp;
