...
'추가내역
    ...
    Const A311_EG2_E2_acct_cd = 2		'계정코드
    Const A311_EG2_E2_acct_nm = 3		'계정단명
    Const A311_EG2_E3_asst_no = 4		'자산번호
    Const A311_EG2_E3_asst_nm = 5		'자산명
    Const A311_EG2_E3_acq_amt = 6		'취득금액
    Const A311_EG2_E3_acq_loc_amt = 7	'취득금액(자국)
    Const A311_EG2_E3_acq_qty = 8		'취득수량
    Const A311_EG2_E3_res_amt = 9		'잔존가액(자국)
    Const A311_EG2_E3_ref_no = 10		'참조번호
    Const A311_EG2_E3_asset_desc = 11	'적요

    Const A311_EG2_E3_reg_dt = 12		'자산등록일자
    Const A311_EG2_E3_spec = 13			'용도/규격
    Const A311_EG2_E3_doc_cur = 14		'거래통화
    Const A311_EG2_E3_xch_rate = 15		'환율
    Const A311_EG2_E3_inv_qty = 16		'재고수량
    Const A311_EG2_E3_tax_dur_yrs = 17	'세법기준내용년수
    Const A311_EG2_E3_cas_dur_yrs = 18	'기업회계기준내용년수
    Const A311_EG2_E3_gl_no = 19		'전표번호
    Const A311_EG2_E3_temp_gl_no = 20	'결의전표번호
...
'-------------------------
	...
	Set iPAAG010 = Server.CreateObject("PAAG010_test.cAAcqLkUpSvr")
...
'-------------------------
    ...
    For iLngRow = 0 To UBound(EG1_exp_group,1) - 1
        For iLngCol = 0 To UBound(EG1_exp_group,2)
        	...
        	elseif iLngCol = A311_EG2_E2_acct_cd then		'계정코드 2		
        		iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol)) & gColSep
        	elseif iLngCol = A311_EG2_E2_acct_nm then       '계정단명 3		
        		iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol))
        	elseif iLngCol = A311_EG2_E3_asst_no then       '자산번호 4		
        		iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol))
        	elseif iLngCol = A311_EG2_E3_asst_nm then       '자산명   5		
        		iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol))
        	elseif iLngCol = A311_EG2_E3_acq_amt then       '취득금액 6		
        		iStrData = iStrData & gColSep & UNIConvNumDBToCompanyByCurrency(EG1_exp_group(iLngRow,iLngCol),lgCurrency,ggAmtOfMoneyNo, "X" , "X")
			elseif iLngCol = A311_EG2_E3_acq_loc_amt then	'취득금액(자국) 7  
			...
			elseif iLngCol = A311_EG2_E3_acq_qty then       '취득수량 8       
				iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol))
			elseif iLngCol = A311_EG2_E3_res_amt then       '잔존가액(자국) 9 
				iStrData = iStrData & gColSep & UNIConvNumDBToCompanyByCurrency(EG1_exp_group(iLngRow,iLngCol),lgCurrency,ggAmtOfMoneyNo, "X" , "X")			
			elseif iLngCol = A311_EG2_E3_ref_no then        '참조번호 10	
				iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol))
			elseif iLngCol = A311_EG2_E3_asset_desc then    '적요 11		
				iStrData = iStrData & gColSep & ConvSPChars(EG1_exp_group(iLngRow,iLngCol))
        	end if
        	'iLngCol & "-" & 
		next
