...
Sub SubBizQueryMulti()
	...
	Const A352_EG1_mgnt_fg                      = 18
'	Const A352_EG1_bal_fg                       = 19

	Dim PAUG015_cALkUpClsTmpGlSvr
	Dim iStrData
	Dim iStrData1
'	Dim iStrData2
    Dim iexportData
    ...
    Set PAUG015_cALkUpClsTmpGlSvr = Server.CreateObject("PAUG015.cALkUpClsTmpGlSvr")
    
    If CheckSYSTEMError(Err, True) = True Then
		Exit Sub
    End If

	Call PAUG015_cALkUpClsTmpGlSvr.A_LOOKUP_CLS_TEMP_GL_SVR(gStrGlobalCollection, iStrTempGlNo, iexportData, iexportData1)

	If CheckSYSTEMError(Err, True) = True Then					
		Set PAUG015_cALkUpClsTmpGlSvr = Nothing        
		Exit Sub
    End If    
...
Sub SubBizSaveMulti()
	...
	Set PAUG015_cAMngClsTmpGlSvr = Server.CreateObject("PAUG015.cAMngClsTmpGlSvr")
	
	If CheckSYSTEMError(Err, True) = True Then
		Exit Sub
    End If
    
	iStrRetTempGlNo = PAUG015_cAMngClsTmpGlSvr.A_MANAGE_CLS_TEMP_GL_SVR(gStrGlobalCollection, iCommandSent, I1_b_currency, I2_a_gl, txtSpread, txtSpread3,I3_a_data_auth) 	
	
	If CheckSYSTEMError(Err, True) = True Then		
		Set PAUG015_cAMngClsTmpGlSvr = Nothing
		Exit Sub
    End If
