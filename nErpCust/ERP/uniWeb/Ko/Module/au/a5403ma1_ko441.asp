...
'⊙: 비지니스 로직 ASP명 
Const BIZ_PGM_ID      = "a5403mb1_ko441.asp"			'☆: 비지니스 로직 ASP명 

'==========================================  1.2.1 Global 상수 선언  ======================================
'==========================================================================================================
'⊙: Grid Columns

'@Grid_Column vspdData
	...
	With frm1
		Select Case    UCase   (Trim(pvSpdNo))
			...
			Case    "B"	
				...
				Call ggoSpread.SSSetColHidden(C_ItemSeq_2,C_ItemSeq_2,True )
				Call ggoSpread.SSSetColHidden(C_DrCrFg_2,C_DrCrFg_2,True )
				Call ggoSpread.SSSetColHidden(C_BalAmt_2,C_BalAmt_2,True)
				Call ggoSpread.SSSetColHidden(C_IsLAmtChange_2,C_IsLAmtChange_2,True )
				...
				Call ggoSpread.SSSetColHidden(C_AcctCd2_2,C_AcctCd2_2,True )
    ...
    With frm1
		...
		Select Case    Index
			Case    0			
		        ...
		        ggoSpread.SpreadLock C_deptnm, lRow, C_deptnm
				ggoSpread.SSSetProtected	.vspdData.MaxCols,-1,-1
				
			Case    1
				ggoSpread.SpreadLock C_ItemSeq, lRow, C_AcctCd2	'Item Grid 전체 Lock설정 
				ggoSpread.SSSetProtected	.vspdData.MaxCols,-1,-1
		End Select
...
Sub SetSpreadColor(Byval stsFg, Byval Index, ByVal pvStartRow, ByVal pvEndRow)
    With frm1
		...
		'ggoSpread.SSSetProtected C_deptPopup,	lRow, lRow2
...
Function OpenRefTempGl()
	...
	iCalledAspName = AskPRAspName("a5403ra1")

	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", parent.VB_INFORMATION, "a5403ra1", "X")
		IsOpenPop = False
		Exit Function
	End If
	...
	If arrRet(0) = ""  Then
		...
		Exit Function
	Else		
		frm1.txttempGlNo.value = UCase(Trim(ArrRet(0)))       
		frm1.txttempGlNo.focus
	End If
End Function
...
Function OpenPopupOA()
	...
	iCalledAspName = AskPRAspName("a5403ra2_ko441")
	
	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", parent.VB_INFORMATION, "a5403ra2_ko441", "X")
		IsOpenPop = False
		Exit Function
	End If
	...
	arrParam(7) = lgSubInternalCd
	arrParam(8) = lgAuthUsrID
	arrRet = window.showModalDialog(iCalledAspName, Array(window.parent,iStrParm,iStrParm2, iStrParm3,arrParam), _
...
Function ClickTab1()
	...
	Else
       Call SetToolbar(C_MENU_NEW_TAB1)										'⊙: 버튼 툴바 제어 
	End If
...
'	Call CookiePage("FORM_LOAD") 
	...
	frm1.txtDrLocAmt2.allownull = False
	frm1.txtCrLocAmt2.allownull = False
...
Sub vspdData4_ButtonClicked(ByVal Col, ByVal Row, Byval ButtonDown)
	...
	With frm1
		...
		If Row > 0 And Col = C_AcctPopup_2 Then
			.vspdData4.Col = Col - 1
			.vspdData4.Row = Row			
...
'==========================================================================================
Sub vspdData_ComboSelChange(ByVal pCol, ByVal pRow)
...
Function FncCopy() 
	...
	Call vspdData4_Change(C_AcctCd_2, frm1.vspddata4.activerow)    
	Set gActiveElement = document.ActiveElement 
End Function
		...
		With frm1.vspdData
		    ...
		    If .Text = ggoSpread.InsertFlag Then            
		        ...
		        If Len(.Text) > 0 Then
			        ...
			        Call DbQuery3(.ActiveRow, C_VSPDDATA1)
		        End If
...
'=======================================================================================================
Function DbQuery3(ByVal Row, ByVal pVspdData)
	Dim strVal	
	Dim lngRows
	...
	With frm1
		...
		If iStrOpenGlNo <> "" And iStrOpenGlItemSeq <> "" Then
			...
			strWhere =			  " D.GL_NO = " & FilterVar(UCase(iStrOpenGlNo), "''", "S")   
			strWhere = strWhere & " AND D.ITEM_SEQ	= " & iStrOpenGlItemSeq & " "
			strWhere = strWhere & " AND D.GL_NO		=  C.GL_NO  "
			strWhere = strWhere & " AND D.ITEM_SEQ  =  C.ITEM_SEQ "
			strWhere = strWhere & "	AND D.ACCT_CD	*= B.ACCT_CD "
		...
		Else
			...
			strWhere =			  " D.TEMP_GL_NO = " & FilterVar(UCase   (Trim(iStrTempGlNo)), "''", "S")   
			strWhere = strWhere & " AND D.ITEM_SEQ		= " & iStrTempGlItemSeq & " "
			strWhere = strWhere & " AND D.TEMP_GL_NO	=  C.TEMP_GL_NO  "
			strWhere = strWhere & " AND D.ITEM_SEQ		=  C.ITEM_SEQ "
			strWhere = strWhere & "	AND D.ACCT_CD		*= B.ACCT_CD "
...
Function DbSave() 
    ...
    With frm1.vspdData4
		...
		For lngRows = 1 To .MaxRows
			...
			If .Text <> ggoSpread.DeleteFlag Then
			    ...
			    '.Col = C_internalcd  internal_cd
			    strVal = strVal & Trim(.Text) & iColSep	 
			    
			    '.Col = C_costcd      cost_cd 
			    strVal = strVal & Trim(.Text) & iRowSep	 
	...
	With frm1
		.txtSpread3.value  = .txtSpread3.value & strVal						'Spread Sheet 내용을 저장    
		'권한관리추가 start
...
Function DbDelete()
    ...
    strVal = strVal & "&txtTempGlNo=" & UCase   (Trim(frm1.htxtTempGlNo.value))	'☜: 삭제 조건 데이타 
...
'	Name : CurFormatNumSprSheet()-- 스프레드에 멀티통화가 올 수 있으므로 사용하면 안됨 
					...
					<DIV ID="TabDiv" STYLE="FLOAT: left; HEIGHT: 100%; OVERFLOW: auto; WIDTH: 100%" SCROLL="no">
						<TABLE <%=LR_SPACE_TYPE_60%>>					
							<TR>								
								...
								<TD CLASS=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txttempGLDt" CLASS=FPDTYYYYMMDD title=FPDATETIME ALT="결의일자" tag="22" id=fpDateTime1></OBJECT>');</SCRIPT></TD>
...
<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT TABINDEX="-1" TYPE=HIDDEN CLASSID=<%=gCLSIDFPSPD%> NAME=vspdData3 WIDTH=100% HEIGHT=100% tag="23" TITLE="SPREAD" id=vaSpread3><PARAM NAME="MaxCols" VALUE="0"><PARAM NAME="MaxRows" VALUE="0"></OBJECT>');</SCRIPT>
