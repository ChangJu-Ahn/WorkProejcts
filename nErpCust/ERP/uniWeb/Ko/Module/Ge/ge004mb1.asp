        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "R"
                       ...
                       select case pCode1
                       case "X"
                          lgStrSQL = "Select TOP " & iSelCount  & "  A.from_alloc, case when B.item_group_nm is null then a.from_alloc else b.item_group_nm end item_group_nm,  sum(case when C.bal_fg = " & FilterVar("DR", "''", "S") & " then A.to_amount else -(A.to_amount) end) as A "
                          lgStrSQL = lgStrSQL & " From  g_alloc_result A, b_item_group B, a_acct C"
                          lgStrSQL = lgStrSQL & " Where  A.from_alloc *= B.item_group_cd and A.acct_cd = C.acct_cd "                                                                              
                          lgStrSQL = lgStrSQL & pCode
                          lgStrSQL = lgStrSQL & " AND   A.alloc_kinds   = " & FilterVar(5, "''", "S")                          
						  lgStrSQL = lgStrSQL & " group by  A.from_alloc, B.item_group_nm "
				     ...
				     Select Case pCode1
					 Case "X"
					    lgStrSQL = "select TOP " & iSelCount  & "   to_alloc, B.item_nm, sum(case when C.bal_fg = " & FilterVar("DR", "''", "S") & " then A.to_amount else -(A.to_amount) end) as B " 
						lgStrSQL = lgStrSQL & " FROM   g_alloc_result A, b_item B, a_acct C    "
						lgStrSQL = lgStrSQL & " where   A.to_alloc = B.item_cd and A.acct_cd = C.acct_cd  "						
                        lgStrSQL = lgStrSQL & " AND   alloc_kinds   = " & FilterVar(5, "''", "S")                        
						lgStrSQL = lgStrSQL & pcode
						lgStrSQL = lgStrSQL & " group by  to_alloc, b.item_nm " 
