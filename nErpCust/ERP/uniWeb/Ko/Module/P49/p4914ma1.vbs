...
'==========================================  1.2.1 Global 상수 선언  ======================================
'==========================================================================================================
Const BIZ_PGM_QRY0_ID						= "p4914mb0.asp"		'☆: Master
Const BIZ_PGM_QRY1_ID						= "p4914mb1.asp"		'☆: order 작업현황 
Const BIZ_PGM_QRY2_ID						= "p4914mb2.asp"		'☆: 작업현황 detail
Const BIZ_PGM_QRY3_ID						= "p4914mb3.asp"		'☆: 유실현황 

Const BIZ_PGM_QRY4_ID						= "p4914mb4.asp"		'☆: 간접공수 
Const BIZ_PGM_QRY5_ID						= "p4914mb5.asp"		'☆: 사고현황 

Const BIZ_PGM_SAVE_ID						= "p4914mb01.asp"		'☆: SAVE

Const BIZ_PGM_JUMPORDERRUN_ID				= "p4912ma1"

'Const BIZ_PGM_SAVE_ID						= "p4914mb4.asp"		'☆: Cancel Release Production Order ASP명 
...
'			ggoSpread.SSSetTime		C_EtcTime,					"기타공수", 13, 2, 1, 1
'			ggoSpread.SSSetTime		C_WkTime,					"작업공수", 13, 2, 1, 1
'			ggoSpread.SSSetTime		C_WkLossTime,				"유실공수", 13, 2, 1, 1
'			ggoSpread.SSSetTime		C_RealTime,					"실동공수", 13, 2, 1, 1
...
'			ggoSpread.SSSetButton	C_WkLossPopup3
...
'			ggoSpread.SSSetEdit		C_ProdtOrderNo4,"제조오더번호", 20
...
'			ggoSpread.SSSetEdit		C_ProdtOrderNo5,"제조오더번호", 20
...
'				ggoSpread.SpreadUnLock		C_BpCd,	pvStartRow, C_BpPopup, pvEndRow
		...
		Case "C"
			...
			If InOutType = "N" Then
				...
				ggoSpread.SSSetProtected	C_WkLossDesc3,		pvStartRow, pvEndRow
...
'		-- TAB2 --
		Case "D"
			...
			If InOutType = "N" Then
				...
				ggoSpread.SSSetRequired		C_ProdtOrderNo4,	pvStartRow, pvEndRow
		...
		Case "E"
			...
			If InOutType = "N" Then
				...
				ggoSpread.SSSetRequired		C_ProdtOrderNo5,	pvStartRow, pvEndRow
...
'			C_WkLossPopup3		= iCurColumnPos(14)
...
'			C_ProdtOrderNo4		= iCurColumnPos(4)
...
'			C_ProdtOrderNo5		= iCurColumnPos(4)
...
Function OpenProdOrderNo()
	...
	arrParam(2) = ""'frm1.txtprodDt.Text
	arrParam(3) = "RL"
	arrParam(4) = "RL"
...
'        Call OpenDept(Trim(.Text), "vspdData3")
        Call OpenDept(1)
...
'					.vspdData4.Col  = C_ProdtOrderNo4
'					.vspdData4.Text = Trim(frm1.KeyProdtOrderNo2.value)
...
'					.vspdData5.Col  = C_ProdtOrderNo5
'					.vspdData5.Text = Trim(frm1.KeyProdtOrderNo2.value)
...
'	lgIntFlgMode = parent.OPMD_UMODE												'⊙: Indicates that current mode is Update mode
'	Call SetActiveCell(frm1.vspdData2,1,1,"M","X","X")
'	Set gActiveElement = document.activeElement
'	lgBlnFlgChgValue = False
...
'	strVal = strVal & "&txtWcCd=" & Trim(frm1.txtWcCd.value)
'	strVal = strVal & "&txtprodDt=" & Trim(frm1.txtprodDt.Text)
'	strVal = strVal & "&txtProdOrderNo=" & Trim(frm1.KeyProdtOrderNo2.value)
'	strVal = strVal & "&txtResourceCd=" & Trim(frm1.KeyResourceCd3.value)
'
'    Call RunMyBizASP(MyBizASP, strVal)
'
'    DbQuery3 = True
...
'		strVal = strVal & Trim(.txtprodDt.Text) & iColSep
'		strVal = strVal & UCase(Trim(.txtWcCd.Value)) & iColSep

		strVal = strVal & Trim(.fpDoubleSingle1.Value) & iColSep
		strVal = strVal & Trim(.fpDoubleSingle5.Value) & iColSep
		strVal = strVal & Trim(.fpDoubleSingle9.Value) & iColSep
		strVal = strVal & Trim(.fpDoubleSingle12.Value) & iColSep
...
'					.Col = C_ItemCd							' 6
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_ProdtOrderQty					' 7
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_ProdtOrderUnit					' 8
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_BadQty							' 9
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_ExiProdQtyInOrderUnit			' 10
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_ProdQtyInOrderUnit				' 11
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_ProdQtyInOrderSum				' 12
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_ExiGoodQtyInOrderUnit			' 13
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_GoodQty						' 14
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_GoodQtySum						' 15
...
'					strVal1 = strVal1 & Trim(.Text) & iColSep
'					.Col = C_RealTime						' 22
'					strVal1 = strVal1 & Trim(.Text) & iRowSep
'					.Col = C_RealTime						' 22
'					strVal1 = strVal1 & Trim(.Text) & iColSep
...
Sub fpDoubleSingle1_Change()	' 재적시간 
	lgBlnFlgChgValue = True
'	If Trim(frm1.fpDoubleSingle1.Value) <> "" Then
'		Call SetCalcForm()
'	End If
End Sub
...
Sub fpDoubleSingle5_Change()
	lgBlnFlgChgValue = True
'	If Trim(frm1.fpDoubleSingle5.Value) <> "" Then
'		Call SetCalcForm()
'	End If
End Sub
...
Sub fpDoubleSingle9_Change()
	lgBlnFlgChgValue = True
'	If Trim(frm1.fpDoubleSingle9.Value) <> "" Then
'		Call SetCalcForm()
'	End If
End Sub
...
Sub fpDoubleSingle12_Change()
	lgBlnFlgChgValue = True
'	If Trim(frm1.fpDoubleSingle12.Value) <> "" Then
'		Call SetCalcForm()
'	End If
End Sub
...
'' Desc :
''========================================================================================================
'Function SetCalcForm()
'	Dim strValue
'
''  - ①잔업공수 = ①잔업시간 x ①잔업인원   (②③ 잔업공수도 동일 처리)
''  - 작업공수 = 재적시간 - 휴업시간 + ①잔업공수 + ②잔업공수 + ③잔업공수 + 지원시간(+) - 지원시간(-)
''  - 실동공수 = 작업공수 - 총유실공수 
'
'	With frm1
'	' 작업공수 
'	'				50 - 10 + 지원시간(+) - 지원시간(-)
'		strValue = .fpDoubleSingle5.Value - .fpDoubleSingle12.Value + ConvToSec(.fpDoubleSingle11.Value) + ConvToSec(.fpDoubleSingle6.Value) - ConvToSec(.fpDoubleSingle10.Value)
'		msgbox strValue
'		.fpDoubleSingle4.Value = ConvToTimeFormat(strValue)
'	' 실동공수 
'		.fpDoubleSingle15.Value = ConvToTimeFormat(strValue - ConvToSec(.fpDoubleSingle8.Value))
'	End With
'
'End Function
...
'	WriteCookie "txtProdOrderNo", Trim(frm1.txtProdOrderNo.value)
	WriteCookie "txtPGMID", "p4914mA1"
