Const BIZ_PGM_QRY_ID					= "p1401mb1_ko441.asp"		'☆: 전체 조회 
Const BIZ_PGM_HDRSAVE_ID				= "p1401mb4_ko441.asp"		'☆: 모품목 저장(최상위 노드)
Const BIZ_PGM_DTLSAVE_ID				= "p1401mb5_ko441.asp"		'☆: 자품목 입력,수정,삭제 
Const BIZ_PGM_HDRDEL_ID					= "p1401mb6_ko441.asp"		'☆: 전체 BOM 삭제 
...
Sub LookUpDtlOk()
	If frm1.hBomType.value = "E" Or lgStrHeaderFlg = 1 Then
...
Sub uniTree1_NodeClick(ByVal Node)
	...
	With frm1
		...
		If Not NodX Is Nothing Then													' 선택된 폴더가 있으면 
			...
			Else
'20071204::hanc		        txtPrntItemCd = Trim(PrntNode.Text)
                txtPrntItemCd = Trim(Mid(Trim(PrntNode.Text), 1, InStr(1, Trim(PrntNode.Text), "(")-1))
...
'20071204::hanc				txtChildItemCd  = Trim(NodX.Text)
				txtChildItemCd = Trim(Mid(Trim(NodX.Text), 1, InStr(1, Trim(NodX.Text), "(")-1))
