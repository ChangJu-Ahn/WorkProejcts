...
'*  2. Function Name        : 
'*  3. Program ID           : P1412MA1.asp
'*  4. Program Name         : 자품목 일괄변경 
'*  5. Program Desc         : 
'*  6. Comproxy List        : 
'*  7. Modified date(First) : 2003-03-17
...
'**********************************************************************************************************
Const BIZ_PGM_QRY_ID			= "p1412mb1_ko441.asp"							'☆: 비지니스 로직 ASP명 
Const BIZ_PGM_SAVE_ID			= "p1412mb2_ko441.asp"							'☆: 비지니스 로직 ASP명 
Const BIZ_PGM_LOOKUP_ECN_INFO	= "p1412mb3_ko441.asp"
...
Dim lgSelectAll '2008.02.21::air
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
		Case "A"
			...
			C_ChildItemQty	= iCurColumnPos(9)
			C_ChildUnit		= iCurColumnPos(10)
			C_ChildUnitPopup= iCurColumnPos(11)
			C_PrntItemQty	= iCurColumnPos(12)
			C_PrntUnit		= iCurColumnPos(13)
			C_PrntUnitPopup	= iCurColumnPos(14)
			C_SafetyLt		= iCurColumnPos(15)
			C_LossRate		= iCurColumnPos(16)
			C_SupplyType	= iCurColumnPos(17)
			C_SupplyTypeNm	= iCurColumnPos(18)
			C_ValidFromDt	= iCurColumnPos(19)
			C_ValidToDt		= iCurColumnPos(20)
			C_ECNNo			= iCurColumnPos(21)
			C_ECNNoPopup	= iCurColumnPos(22)
			C_ECNDesc		= iCurColumnPos(23)
			C_ReasonCd		= iCurColumnPos(24)
			C_ReasonCdPopup = iCurColumnPos(25)
...
'------------------------------------------  OpenMassChangeRef()  ----------------------------------------------
'	Name : OpenMassChangeRef()
'	Description : 
'---------------------------------------------------------------------------------------------------------
Function OpenMassChangeRef()
	Dim arrRet
	Dim arrParam(2)
	Dim iCalledAspName
	...
	iCalledAspName = AskPRAspName("P1412RA1")
	
	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", parent.VB_INFORMATION, "P1412RA1", "X")
		IsOpenPop = False
	...
	arrParam(1) = frm1.hItemCd.value	'Child Item Cd
...
'------------------------------------------  SetMassChange(arrRet)  --------------------------------------
'	Name : SetMassChange(arrRet)
'	Description : Open Mess Change Reference에서 Return되는 값 setting
'---------------------------------------------------------------------------------------------------------
Sub SetMassChange(arrRet)
	Dim iRow
	With frm1.vspdData
		.ReDraw = False		
		For iRow = 1 To .MaxRows
			...
			.value = 1
			.Col = C_ChildItemQty
			.text = udf_AlterNumber(.text, arrRet(7))
			.Col = C_ChildUnit
			.value = udf_AlterValue(.text, arrRet(8))		'User Defined Function
			.Col = C_PrntItemQty
			.text = udf_AlterNumber(.text, arrRet(9))
			.Col = C_PrntUnit
			.value = udf_AlterValue(.text, arrRet(10))
			.Col = C_SafetyLt
			.text = udf_AlterNumber(.text, arrRet(11))
			.Col = C_LossRate
			.text = udf_AlterNumber(.text, arrRet(12))
			.Col = C_SupplyType
			.text = udf_AlterValue(.text, arrRet(6))
			.Col = C_ValidFromDt
			.text = udf_AlterValue(.text, arrRet(13))
			.Col = C_ValidToDt
			...
			.value = arrRet(18)
			.Col = C_Remark
			.value = udf_AlterValue(.text, arrRet(19))
...
Sub LookUpEcnInfo(ByVal pEcnNo,ByVal pReasonCd,ByVal pTarget)
	...
	strVal = BIZ_PGM_LOOKUP_ECN_INFO & "?txtMode=" & parent.UID_M0001
    ...
    For IntRows = 1 To .vspdData.MaxRows
		...
		Select Case .vspdData.Text
            Case ggoSpread.UpdateFlag
                ...
                .vspdData.Col = C_ChildSeq		'3
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ChildItemQty	'4
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ChildUnit		'5
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_PrntItemQty	'6
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_PrntUnit		'7
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_SafetyLt		'8
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_LossRate		'9
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_SupplyType	'10
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ValidFromDt	'11
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ValidToDt		'12
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ECNNo			'13
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ECNDesc		'14
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_ReasonCd		'15
                strVal = strVal & Trim(.vspdData.Text) & iColSep
                .vspdData.Col = C_Remark		'16
...
Function udf_AlterNumber(ByVal pOld, ByVal pChkVal)
	If UniCDbl(pChkVal) > 0 Then
		udf_AlterNumber = pChkVal
	Else
		udf_AlterNumber = pOld
	End If
End Function
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>자품목일괄변경</font></td>
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="right"><img src="../../../CShared/image/table/seltab_up_right.gif" width="10" height="23"></td>
						    </TR>
						</TABLE>
					</TD>
					<TD WIDTH=* align=right><A href="vbscript:OpenMassChangeRef()">일괄변경</A></TD>
					<TD WIDTH=10>&nbsp;</TD>
				</TR>
			</TABLE>
		</TD>
...
<!--	<TR>
	...
	<TR><TD HEIGHT=30>
		<TABLE CLASS="basicTB" CELLSPACING=0>
			<TR>
				<TD WIDTH=70% NOWRAP>&nbsp;&nbsp;
					<IMG SRC="../../../CShared/image/select_all_d.gif"  Style="CURSOR: hand" ALT="Select All" NAME="Search" ONCLICK="SelectAll()"  onMouseOver="javascript:MM_swapImage(this.name,'','../../../CShared/image/select_all.gif',1)"></IMG>
				</TD>
			</TR>
		</TABLE>
	</TD></TR>	
