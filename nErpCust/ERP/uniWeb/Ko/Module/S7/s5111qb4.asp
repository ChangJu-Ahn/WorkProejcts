...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
	...
	UNIValue(0,5) = Trim(Request("txtSalesOrgLvl"))
	
	If Len(Trim(Request("txtSalesOrg"))) Then
		UNIValue(0,6) = " " & FilterVar(Request("txtSalesOrg"), "''", "S") & ""
	Else
		UNIValue(0,6) = "Default"
	End If
	UNIValue(0,7) = "Default"											' Usage Flag	
	UNIValue(0,8) = " " & FilterVar(Request("txtRootKey"), "''", "S") & ""				' Root Org.
	UNIValue(0,9) = " " & FilterVar(Request("txtOrgSuffix"), "''", "S") & ""			' Org. Suffix
	UNIValue(0,10) = " " & FilterVar(Request("txtGrpSuffix"), "''", "S") & ""			' Grp. Suffix
	UNIValue(0,11) = "" & FilterVar("N", "''", "S") & " "												' Rate Flag
