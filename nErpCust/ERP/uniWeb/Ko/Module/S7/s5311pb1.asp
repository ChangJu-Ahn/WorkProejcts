...
'*  4. Program Name         : 세금계산서관리번호 Popup
'*  5. Program Desc         : 
'*  6. Comproxy List        : 
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    If Len(Trim(Request("txtBillToParty"))) Then
		UNIValue(0,1) = " " & FilterVar(Request("txtBillToParty"), "''", "S") & ""                      '주문처 
	Else
		UNIValue(0,1) = "Null"
	End If
    If Len(Trim(Request("txtSalesGrp"))) Then
		UNIValue(0,2) = " " & FilterVar(Request("txtSalesGrp"), "''", "S") & ""                      '주문처 
	Else
		UNIValue(0,2) = "Null"
	End If

    If Len(Trim(Request("txtVatType"))) Then
		UNIValue(0,3) = " " & FilterVar(Request("txtVatType"), "''", "S") & ""                      '주문처 
	Else
		UNIValue(0,3) = "Null"
	End If
    If Len(Trim(Request("txtTaxBizArea"))) Then
		UNIValue(0,4) = " " & FilterVar(Request("txtTaxBizArea"), "''", "S") & ""                      '주문처 
	Else
		UNIValue(0,4) = "Null"
	End If
    If Len(Trim(Request("txtFromDt"))) Then
		UNIValue(0,5) = " " & FilterVar(UNIConvDate(Request("txtFromDt")), "''", "S") & ""                             '시작일 
	Else
		UNIValue(0,5) = "Null"
	End If

    If Len(Trim(Request("txtToDt"))) Then
		UNIValue(0,6) = " " & FilterVar(UNIConvDate(Request("txtToDt")), "''", "S") & ""									'종료일 
	Else
		UNIValue(0,6) = "Null"
	End If
	
    If Len(Trim(Request("txtPostFlag"))) Then
		UNIValue(0,7) = " " & FilterVar(Request("txtPostFlag"), "''", "S") & ""                      '주문처 
	Else
		UNIValue(0,7) = "Null"
	End If
'--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
