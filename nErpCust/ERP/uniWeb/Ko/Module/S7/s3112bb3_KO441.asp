...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
	...
	Else
		UNIValue(0,13) = "Null"
	End If
    UNIValue(0,14) = " " & FilterVar(Request("txtSoldToParty"), "''", "S") & ""                    '주문처

	If Len(Trim(Request("txtLotNo"))) Then	
			UNIValue(0,15)	= " AND LOT_NO LIKE " & FilterVar(Request("txtLotNo")&"%","''","S")
	End If
...
<Script Language=vbscript>
    With parent
		...
		.frm1.txtHLotNo.value	= "<%=Request("txtLotNo")%>"
		<%End If%>
