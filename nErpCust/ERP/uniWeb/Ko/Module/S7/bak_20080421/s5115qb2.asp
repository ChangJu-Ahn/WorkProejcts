...
Sub FixUNISQLData()
	...
	If Len(lgBizArea) Then
		...
		UNIValue(1,0)	= FilterVar(lgBizArea, "''", "S")
		iStrVal = iStrVal & " AND BIZ_AREA =  " & FilterVar(lgBizArea , "''", "S") & ""				
	End If
	
	'매출채권형태명===========================================================================================
    If Len(lgBillType) Then		
		UNISqlId(2)		= "s0000qa011"	
		UNIValue(2,0)	= FilterVar(lgBillType, "''", "S")
		iStrVal = iStrVal & " AND BILL_TYPE =  " & FilterVar(lgBillType , "''", "S") & ""				
	End If		
   
    
	UNISqlId(0)	= "S5115QA201"					
...
Sub QueryData()
	...
	If lgBillType <> "" Then
		If rs2.EOF And rs2.BOF Then
			rs2.Close
			Set rs2 = Nothing			
			Call ConNotFound("txtConBillType")			
			Exit Sub
		Else	
