...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    ...
    UNISqlId(0) = "S3112BA201"
'--------------- 개발자 coding part(실행로직,End)------------------------------------------------------

    UNIValue(0,0) = Trim(lgSelectList)                                      '☜: Select list
    
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    UNIValue(0,1) = " " & FilterVar(Request("txtApplicant"), "''", "S") & ""                     '개설의뢰인 
    UNIValue(0,2) = " " & FilterVar(Request("txtCurrency"), "''", "S") & ""                      '화폐 
	...
	UNIValue(0,7) = " " & FilterVar(Request("txtVatCalcType"), "''", "S") & ""                   '부가세적용기준 
	UNIValue(0,8) = " " & FilterVar(Request("txtVatType"), "''", "S") & ""                       '부가세유형 
	UNIValue(0,9) = " " & FilterVar(Request("txtVatIncFlag"), "''", "S") & ""                    '부가세포함구분 
	UNIValue(0,10) = " " & FilterVar(Request("txtPayTermsCd"), "''", "S") & ""                   '결제방법 
	UNIValue(0,11) = UNIConvNum(Request("txtVatRate"), 0)
	UNIValue(0,12) = " " & FilterVar(Request("txtBillTypeCD"), "''", "S") & ""                   '매출채권유형 
    If Len(Trim(Request("txtItemCd"))) Then
		UNIValue(0,13) = " " & FilterVar(Request("txtItemCd"), "''", "S") & ""                   '품목코드 
	Else
		UNIValue(0,13) = "Null"
	End If

	If Len(Trim(Request("txtTrackingNo"))) Then		
		UNIValue(0,14) = " " & FilterVar(Request("txtTrackingNo"), "''", "S") & ""    		
	Else
		UNIValue(0,14) = "Null"
	End If
...
<Script Language=vbscript>
    With parent
		...
		Call .ReFormatSpreadCellByCellByCurrency2(.frm1.vspdData,"<%=iFrPoint+1%>",.frm1.vspdData.MaxRows,"<%=Request("txtCurrency")%>",.GetKeyPos("A",8),"C","I","X","X")
		Call .ReFormatSpreadCellByCellByCurrency2(.frm1.vspdData,"<%=iFrPoint+1%>",.frm1.vspdData.MaxRows,"<%=Request("txtCurrency")%>",.GetKeyPos("A",9),"A","I","X","X")
