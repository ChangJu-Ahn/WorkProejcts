...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    ...
    If Len(Trim(Request("txtSalesGrp"))) Then
	    UNIValue(0,3) = " " & FilterVar(Request("txtSalesGrp"), "''", "S") & ""                   '영업그룹 
	Else
		UNIValue(0,3) = "Null"
	End If
    ...
    If Len(Trim(Request("txtBillNo"))) Then
		UNIValue(0,6) = " " & FilterVar(Request("txtBillNo"), "''", "S") & ""						'채권번호 
	Else
		UNIValue(0,6) = "Null"
	End If
	UNIValue(0,7) = " " & FilterVar(Request("txtVatType"), "''", "S") & ""							'VAT 유형 
	UNIValue(0,8) = " " & FilterVar(Request("txtVatIncFlag"), "''", "S") & ""						'VAT 포함여부 

    If Len(Trim(Request("txtItemCd"))) Then
		UNIValue(0,9) = " " & FilterVar(Request("txtItemCd"), "''", "S") & ""						'품목코드 
	Else
		UNIValue(0,9) = "Null"
	End If
'--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
