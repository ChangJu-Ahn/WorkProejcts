...
'========================================================================================================
Const BIZ_PGM_ID 		= "a3109rb1.asp"                              '☆: Biz Logic ASP Name
...
Sub InitVariables()
	...
	If UBound(arrParam) > 6 Then
		lgAuthBizAreaCd	= arrParam(6)
		lgInternalCd	= arrParam(7)
		lgSubInternalCd	= arrParam(8)
		lgAuthUsrID		= arrParam(9)  
	End If	
End Sub
...
' SetReqAttr(Object, Option) ; N : Required, Q : Protect, D : Default
	...
	ELSEIF txtDocCur.value = "" Then	
		...
		Call ggoOper.SetReqAttr(txtDocCur,   "D")			
	End If			
	txtPrDt.Text = UNIDateAdd("M", -1, arrParam(4),PopupParent.gDateFormat)
	txtToPrDt.Text = arrParam(4) 

	strSelect = "distinct ORG_CHANGE_ID"
	strFrom =  " B_ACCT_DEPT "
	strWhere = " ORG_CHANGE_DT >= "
	strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(txtPrDt.Text, gDateFormat,""), "''", "S") & ")"
	strWhere = strWhere & " AND ORG_CHANGE_DT <= " 
	strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(txtToPrDt.Text, gDateFormat,""), "''", "S") & ") "
		

	Call CommonQueryRs(strSelect,strFrom,strWhere,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
					
	 hOrgChangeId.value= Trim(Replace(lgF0,Chr(11),""))
...
Sub InitSpreadSheet()
    ...
    Call SetZAdoSpreadSheet("a3109ra110","S","A","V20021211",PopupParent.C_SORT_DBAGENT,vspdData, C_MaxKey, "X","X")
    ggoSpread.Source = vspdData
	Call ggoSpread.SSSetColHidden(GetKeyPos("A",14),GetKeyPos("A",14),True)
    Call SetSpreadLock() 
End Sub
...
Function ChkQueryDate()
	...
	If CompareDateByFormat(txtPrDt.text,htxtAllcDt.Value,txtPrDt.Alt,htxtAllcAlt.value, _
   	           "970025",txtPrDt.UserDefinedFormat,PopupParent.gComDateType, true) = False Then
	   chkQueryDate= False
	   txtPrDt.focus
	   Exit Function
	...
	If CompareDateByFormat(txtToPrDt.text,htxtAllcDt.Value,txtToPrDt.Alt, htxtAllcAlt.value,_
   	           "970025",txtToPrDt.UserDefinedFormat,PopupParent.gComDateType, true) = False Then
	   chkQueryDate= False
	   txtToPrDt.focus
	   Exit Function
	End If
			...
			<FIELDSET CLASS="CLSFLD">
				<TABLE <%=LR_SPACE_TYPE_40%>>
					<TR>				
						...
						<TD CLASS=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtPrDt" CLASS=FPDTYYYYMMDD tag="12" Title="FPDATETIME" ALT="선수금시작일자" ></OBJECT>');</SCRIPT>								
							    &nbsp;~&nbsp;<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtToPrDt" CLASS=FPDTYYYYMMDD tag="12" Title="FPDATETIME" ALT="선수금종료일자" ></OBJECT>');</SCRIPT></TD>												
					...
					<TR>
						...
						<TD CLASS=TD6 NOWRAP><INPUT TYPE=TEXT NAME="txtDeptCd" SIZE=10 MAXLENGTH=10 STYLE="TEXT-ALIGN: Left" tag=11NXXU" ALT="부서코드"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCostCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenDeptCd()"> <INPUT TYPE=TEXT NAME="txtDeptNm" SIZE=20 MAXLENGTH=20 STYLE="TEXT-ALIGN: left" tag="14" ALT="부서명"></TD>					
					</TR>
				</TABLE>
			</FIELDSET>
		</TD>
