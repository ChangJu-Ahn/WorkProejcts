...
Sub SubBizQueryMulti()
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & " , A.DEAL_BP_CD,H.BP_NM" 

	lgStrSQL = lgStrSQL & " FROM (" 
	lgStrSQL = lgStrSQL & "		SELECT A.CLS_AR_NO ALLC_NO, A.ACCT_CD, A.CLS_DT ALLC_DT,  SUM(A.CLS_LOC_AMT)  ALLC_LOC_AMT,A.GL_NO, A.TEMP_GL_NO,A.GL_INPUT_TYPE" 
	
	If Trim(Request("txtShowBiz")) = "Y" Then	lgStrSQL = lgStrSQL & " , A.BIZ_AREA_CD" 
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & ",A.DEAL_BP_CD" 
	
	lgStrSQL = lgStrSQL & "			FROM (  SELECT A.AR_NO, A.CLS_AR_NO, A.CLS_DT, A.ACCT_CD, A.CLS_LOC_AMT, A.GL_NO, A.TEMP_GL_NO,  A.GL_INPUT_TYPE" 
	
	If Trim(Request("txtShowBiz")) = "Y" Then	lgStrSQL = lgStrSQL & " , B.BIZ_AREA_CD" 
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & " , B.DEAL_BP_CD" 
	
	lgStrSQL = lgStrSQL & "					FROM (SELECT A.AR_NO, CLS_AR_NO, A.CLS_DT, A.ACCT_CD ACCT_CD, A.CLS_LOC_AMT+A.DC_LOC_AMT CLS_LOC_AMT, B.GL_NO, B.TEMP_GL_NO,B.GL_INPUT_TYPE" 
	lgStrSQL = lgStrSQL & "						  FROM A_CLS_AR A JOIN (SELECT ALLC_NO, GL_NO,TEMP_GL_NO, A_ALLC_RCPT.REF_NO," 
	lgStrSQL = lgStrSQL & "													CASE WHEN ALLC_TYPE = " & FilterVar("X", "''", "S") & "  THEN " & FilterVar("RX", "''", "S") & " WHEN ALLC_TYPE = " & FilterVar("P", "''", "S") & "  THEN " & FilterVar("CP", "''", "S") & " " 
	lgStrSQL = lgStrSQL & "														 WHEN ALLC_TYPE = " & FilterVar("A", "''", "S") & "  THEN " & FilterVar("CS", "''", "S") & "  WHEN ALLC_TYPE IN (" & FilterVar("M", "''", "S") & " , " & FilterVar("D", "''", "S") & " ) THEN " & FilterVar("CR", "''", "S") & "  END GL_INPUT_TYPE" 
	lgStrSQL = lgStrSQL & "												FROM A_ALLC_RCPT UNION" 
	lgStrSQL = lgStrSQL & "												SELECT PAYM_NO, GL_NO, TEMP_GL_NO, '' , " & FilterVar("LR", "''", "S") & "  GL_INPUT_TYPE FROM A_ALLC_PAYM UNION" 
	lgStrSQL = lgStrSQL & "												SELECT CLEAR_NO, GL_NO, '', '', " & FilterVar("CL", "''", "S") & "  GL_INPUT_TYPE FROM A_CLEAR_AP_AR" 
	lgStrSQL = lgStrSQL & "												) B ON B.ALLC_NO = A.CLS_AR_NO"
	lgStrSQL = lgStrSQL & "												where A.CLS_DT <= " & strToDt & "  AND A.CLS_DT >=" & strFrDt
	
	lgStrSQL = lgStrSQL & "						UNION ALL" 
	lgStrSQL = lgStrSQL & "						SELECT A.AR_NO, A.ADJUST_NO, A.ADJUST_DT, B.ACCT_CD, A.ADJUST_LOC_AMT,  A.GL_NO, A.TEMP_GL_NO, " & FilterVar("JR", "''", "S") & " GL_INPUT_TYPE" 
	lgStrSQL = lgStrSQL & "						FROM A_AR_ADJUST A JOIN A_GL_ITEM B ON B.GL_NO = A.GL_NO AND B.DR_CR_FG = " & FilterVar("CR", "''", "S") & " " 
	
	lgStrSQL = lgStrSQL & "						where A.ADJUST_DT <= " & strToDt & "  AND A.ADJUST_DT >=" & strFrDt 
	...
	lgStrSQL = lgStrSQL & "				GROUP BY A.CLS_AR_NO, A.ACCT_CD,A.CLS_DT,A.GL_INPUT_TYPE,A.GL_NO, A.TEMP_GL_NO" 
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & " C.DEAL_BP_CD, " 
	...
	lgStrSQL = lgStrSQL & "		LEFT JOIN A_OPEN_AR  C ON C.AR_NO=A_BATCH_GL_ITEM.KEY_VAL1"
	...
	lgStrSQL = lgStrSQL & "												where acct_cd in(select acct_cd from a_acct where acct_type LIKE " & FilterVar("%I%", "''", "S") & " ))"
	...
	If Trim(Request("txtShowBp")) = "Y" Then lgStrSQL = lgStrSQL & " AND E.DEAL_BP_CD= A.DEAL_BP_CD"
	...
	lgStrSQL = lgStrSQL & "			   WHERE  A_GL.GL_NO=A_GL_ITEM.GL_NO AND A_GL_ITEM.ACCT_CD IN (select acct_cd from a_acct where acct_type LIKE " & FilterVar("%I%", "''", "S") & " )"
	...
	If Trim(Request("txtShowBp")) = "Y" Then lgStrSQL = lgStrSQL & " AND D.BP_CD= A.DEAL_BP_CD"
	...
	If Trim(Request("txtShowBp")) = "Y" Then lgStrSQL = lgStrSQL & " LEFT JOIN A_TEMP_GL_DTL G ON A_TEMP_GL_ITEM.TEMP_GL_NO=G.TEMP_GL_NO AND A_TEMP_GL_ITEM.ITEM_SEQ=G.ITEM_SEQ AND G.CTRL_CD in ( Select CTRL_CD From A_CTRL_ITEM Where  TBL_ID = " & FilterVar("B_BIZ_PARTNER", "''", "S") & " )"
	...
	lgStrSQL = lgStrSQL & "					AND A_TEMP_GL_ITEM.ACCT_CD IN (select acct_cd from a_acct where acct_type LIKE " & FilterVar("%I%", "''", "S") & " )"
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & " AND F. CTRL_VAL= A.DEAL_BP_CD"
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & "	LEFT JOIN B_BIZ_PARTNER H ON A.DEAL_BP_CD=H.BP_CD"
...
'	lgStrSQL = lgStrSQL & "	 AND  A.ALLC_DT <=  " & strToDt & "  AND  A.ALLC_DT >= " & strFrDt 
	...
	lgStrSQL = lgStrSQL & "    A.ALLC_NO >= " & FilterVar(lgStrPrevKey, "''", "S")
	...
	lgStrSQL2 = lgStrSQL2 & "		SELECT A.CLS_AR_NO ALLC_NO, A.ACCT_CD, A.CLS_DT ALLC_DT,  SUM(A.CLS_LOC_AMT)  ALLC_LOC_AMT,A.GL_NO, A.TEMP_GL_NO,A.GL_INPUT_TYPE" 
	
	If Trim(Request("txtShowBiz")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & " , A.BIZ_AREA_CD" 
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & ",A.DEAL_BP_CD" 
	
	lgStrSQL2 = lgStrSQL2 & "			FROM (  SELECT A.AR_NO, A.CLS_AR_NO, A.CLS_DT, A.ACCT_CD, A.CLS_LOC_AMT, A.GL_NO, A.TEMP_GL_NO,  A.GL_INPUT_TYPE" 
	
	If Trim(Request("txtShowBiz")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & " , B.BIZ_AREA_CD" 
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & " , B.DEAL_BP_CD" 
	
	lgStrSQL2 = lgStrSQL2 & "					FROM (SELECT A.AR_NO, CLS_AR_NO, A.CLS_DT, A.ACCT_CD ACCT_CD, A.CLS_LOC_AMT+A.DC_LOC_AMT CLS_LOC_AMT, B.GL_NO, B.TEMP_GL_NO,B.GL_INPUT_TYPE" 
	lgStrSQL2 = lgStrSQL2 & "						  FROM A_CLS_AR A JOIN (SELECT ALLC_NO, GL_NO,TEMP_GL_NO, A_ALLC_RCPT.REF_NO," 
	lgStrSQL2 = lgStrSQL2 & "													CASE WHEN ALLC_TYPE = " & FilterVar("X", "''", "S") & "  THEN " & FilterVar("RX", "''", "S") & " WHEN ALLC_TYPE = " & FilterVar("P", "''", "S") & "  THEN " & FilterVar("CP", "''", "S") & " " 
	lgStrSQL2 = lgStrSQL2 & "														 WHEN ALLC_TYPE = " & FilterVar("A", "''", "S") & "  THEN " & FilterVar("CS", "''", "S") & "  WHEN ALLC_TYPE IN (" & FilterVar("M", "''", "S") & " , " & FilterVar("D", "''", "S") & " ) THEN " & FilterVar("CR", "''", "S") & "  END GL_INPUT_TYPE" 
	lgStrSQL2 = lgStrSQL2 & "												FROM A_ALLC_RCPT UNION" 
	lgStrSQL2 = lgStrSQL2 & "												SELECT PAYM_NO, GL_NO, TEMP_GL_NO, '' , " & FilterVar("LR", "''", "S") & "  GL_INPUT_TYPE FROM A_ALLC_PAYM UNION" 
	lgStrSQL2 = lgStrSQL2 & "												SELECT CLEAR_NO, GL_NO, '', '', " & FilterVar("CL", "''", "S") & "  GL_INPUT_TYPE FROM A_CLEAR_AP_AR" 
	lgStrSQL2 = lgStrSQL2 & "												) B ON B.ALLC_NO = A.CLS_AR_NO" 
	lgStrSQL2 = lgStrSQL2 & "												where A.CLS_DT <= " & strToDt & "  AND A.CLS_DT >=" & strFrDt
	lgStrSQL2 = lgStrSQL2 & "						UNION ALL" 
	lgStrSQL2 = lgStrSQL2 & "						SELECT A.AR_NO, A.ADJUST_NO, A.ADJUST_DT, B.ACCT_CD, A.ADJUST_LOC_AMT,  A.GL_NO, A.TEMP_GL_NO, " & FilterVar("JR", "''", "S") & " GL_INPUT_TYPE" 
	lgStrSQL2 = lgStrSQL2 & "						FROM A_AR_ADJUST A JOIN A_GL_ITEM B ON B.GL_NO = A.GL_NO AND B.DR_CR_FG = " & FilterVar("CR", "''", "S") & " " 
	lgStrSQL2 = lgStrSQL2 & "						where A.ADJUST_DT <= " & strToDt & "  AND A.ADJUST_DT >=" & strFrDt 
	lgStrSQL2 = lgStrSQL2 & "						)A INNER JOIN A_OPEN_AR B ON A.AR_NO=B.AR_NO" 
	lgStrSQL2 = lgStrSQL2 & "				)A "
	If Trim(Request("txtShowBp")) = "Y" Then  lgStrSQL2 = lgStrSQL2 & " LEFT JOIN  B_BIZ_PARTNER E ON A.DEAL_BP_CD = E.BP_CD"
	
	lgStrSQL2 = lgStrSQL2 & "				WHERE A.CLS_DT <= " & strToDt & "  AND A.CLS_DT >=" & strFrDt 
	lgStrSQL2 = lgStrSQL2 & "					AND A.ACCT_CD IN (SELECT ACCT_CD FROM A_ACCT WHERE ACCT_TYPE LIKE " & FilterVar("%I%", "''", "S") & " )" 
	lgStrSQL2 = lgStrSQL2 & "				GROUP BY A.CLS_AR_NO, A.ACCT_CD,A.CLS_DT,A.GL_INPUT_TYPE,A.GL_NO, A.TEMP_GL_NO" 
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & " C.DEAL_BP_CD, " 
	...
	lgStrSQL2 = lgStrSQL2 & "		LEFT JOIN A_OPEN_AR  C ON C.AR_NO=A_BATCH_GL_ITEM.KEY_VAL1"
	...
	lgStrSQL2 = lgStrSQL2 & "												where acct_cd in(select acct_cd from a_acct where acct_type LIKE " & FilterVar("%I%", "''", "S") & " ))"
	...
	If Trim(Request("txtShowBp")) = "Y" Then lgStrSQL2 = lgStrSQL2 & " AND E.DEAL_BP_CD= A.DEAL_BP_CD"
	...
	lgStrSQL2 = lgStrSQL2 & "			   WHERE  A_GL.GL_NO=A_GL_ITEM.GL_NO AND A_GL_ITEM.ACCT_CD IN (select acct_cd from a_acct where acct_type LIKE " & FilterVar("%I%", "''", "S") & " )"
	...
	If Trim(Request("txtShowBp")) = "Y" Then lgStrSQL2 = lgStrSQL2 & " AND D.BP_CD= A.DEAL_BP_CD"
	...
	If Trim(Request("txtShowBp")) = "Y" Then lgStrSQL2 = lgStrSQL2 & " LEFT JOIN A_TEMP_GL_DTL G ON A_TEMP_GL_ITEM.TEMP_GL_NO=G.TEMP_GL_NO AND A_TEMP_GL_ITEM.ITEM_SEQ=G.ITEM_SEQ AND G.CTRL_CD=" & FilterVar("BP", "''", "S") & " "
	...
	lgStrSQL2 = lgStrSQL2 & "					AND A_TEMP_GL_ITEM.ACCT_CD IN (select acct_cd from a_acct where acct_type LIKE " & FilterVar("%I%", "''", "S") & " )"
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & " AND F. CTRL_VAL= A.DEAL_BP_CD"
	...
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL2 = lgStrSQL2 & "	LEFT JOIN B_BIZ_PARTNER H ON A.DEAL_BP_CD=H.BP_CD"
	
	lgStrSQL2 = lgStrSQL2 & "	LEFT JOIN A_GL I ON A.GL_NO=I.GL_NO"
	lgStrSQL2 = lgStrSQL2 & "	WHERE  "
	lgStrSQL2 = lgStrSQL2 & "   A.ALLC_NO >= " & FilterVar(lgStrPrevKey, "''", "S")
