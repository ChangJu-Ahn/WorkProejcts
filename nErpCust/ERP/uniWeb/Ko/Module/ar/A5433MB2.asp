...
Sub SubBizQueryMulti()
	...
	lgStrSQL = lgStrSQL & " ALLC_LOC_AMT,GL_LOC_AMT,BATCH_LOC_AMT,Diff_LOC_AMT, TEMP_GL_LOC_AMT,GL_INPUT_TYPE, MINOR_NM" 
	...
	lgStrSQL = lgStrSQL & " FROM ( " 
	'*****************************************A_cls_AP에서 가져오기 시작******************************************			
	lgStrSQL = lgStrSQL & " SELECT A.ACCT_CD,B.ACCT_NM, SUM(A.CLS_LOC_AMT)  ITEM_LOC_AMT," & FilterVar("AR", "''", "S") & "  AR_FG, A.GL_INPUT_TYPE, C.MINOR_NM" 
	...
	lgStrSQL = lgStrSQL & " FROM (" 
	lgStrSQL = lgStrSQL & "			SELECT A.AR_NO, A.CLS_AR_NO, A.CLS_DT, A.ACCT_CD, A.CLS_AMT, A.CLS_LOC_AMT, A.GL_NO, A.TEMP_GL_NO, B.BIZ_AREA_CD, B.DEAL_BP_CD, A.GL_INPUT_TYPE" 
	lgStrSQL = lgStrSQL & "			FROM (SELECT A.AR_NO, CLS_AR_NO, A.CLS_DT, A.ACCT_CD ACCT_CD, A.CLS_AMT+A.DC_AMT CLS_AMT, A.CLS_LOC_AMT+A.DC_LOC_AMT CLS_LOC_AMT, B.GL_NO, B.TEMP_GL_NO,B.ALLC_TYPE GL_INPUT_TYPE" 
	lgStrSQL = lgStrSQL & "				   FROM A_CLS_AR A JOIN (SELECT ALLC_NO, GL_NO,TEMP_GL_NO, REF_NO, " 
	lgStrSQL = lgStrSQL & "												CASE WHEN ALLC_TYPE = " & FilterVar("X", "''", "S") & "  THEN " & FilterVar("RX", "''", "S") & " WHEN ALLC_TYPE = " & FilterVar("P", "''", "S") & "  THEN " & FilterVar("CP", "''", "S") & " "
	lgStrSQL = lgStrSQL & "													 WHEN ALLC_TYPE = " & FilterVar("A", "''", "S") & "  THEN " & FilterVar("CS", "''", "S") & "  WHEN ALLC_TYPE IN (" & FilterVar("M", "''", "S") & " , " & FilterVar("D", "''", "S") & " ) THEN " & FilterVar("CR", "''", "S") & "  END ALLC_TYPE"
	lgStrSQL = lgStrSQL & "										 FROM A_ALLC_RCPT UNION" 
	lgStrSQL = lgStrSQL & "										 SELECT PAYM_NO, GL_NO, TEMP_GL_NO, '' , " & FilterVar("LR", "''", "S") & " ALLC_TYPE FROM A_ALLC_PAYM UNION" 
	lgStrSQL = lgStrSQL & "										 SELECT CLEAR_NO, GL_NO, '', '', " & FilterVar("CL", "''", "S") & " ALLC_TYPE FROM A_CLEAR_AP_AR"
	lgStrSQL = lgStrSQL & "										) B ON B.ALLC_NO = A.CLS_AR_NO " 
	lgStrSQL = lgStrSQL & "				UNION ALL" 
	lgStrSQL = lgStrSQL & "				SELECT A.AR_NO, A.ADJUST_NO, A.ADJUST_DT, B.ACCT_CD,A.ADJUST_AMT, A.ADJUST_LOC_AMT,  A.GL_NO, A.TEMP_GL_NO, " & FilterVar("JR", "''", "S") & " ALLC_TYPE" 
...
'************************A_BATCH_GL_ITEM중에서  가져오기 시작*******************		
	...
	lgStrSQL = lgStrSQL & " " & FilterVar("BATCH", "''", "S") & "  AR_FG, B.GL_INPUT_TYPE, G.MINOR_NM" 
...
'********************************************A_GL_ITEM중에서 가져오기 시작***********************************		
	...
	lgStrSQL = lgStrSQL & " SELECT  A.ACCT_CD,F.ACCT_NM,ISNULL(SUM( A.ITEM_LOC_AMT),0) ITEM_LOC_AMT, " & FilterVar("GL", "''", "S") & "  AR_FG, B.GL_INPUT_TYPE, G.MINOR_NM " 
	...
	'If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & " LEFT JOIN B_BIZ_PARTNER E ON A.BP_CD=E.BP_CD" 
...
'********************************************A_Temp_GL_ITEM중에서 가져오기 시작***********************************
	...
	lgStrSQL = lgStrSQL & " SELECT A.ACCT_CD,F.ACCT_NM,ISNULL(SUM( A.ITEM_LOC_AMT),0) ITEM_LOC_AMT, " & FilterVar("TEMP_GL", "''", "S") & "  AR_FG, B.GL_INPUT_TYPE, H.MINOR_NM" 
	
	If Trim(Request("txtShowBiz")) = "Y" Then	lgStrSQL = lgStrSQL & " ,B.BIZ_AREA_CD,D.BIZ_AREA_NM" 
	If Trim(Request("txtShowBp")) = "Y" Then	lgStrSQL = lgStrSQL & " ,G.CTRL_VAL,E.BP_NM" 
		
	lgStrSQL = lgStrSQL & " FROM A_TEMP_GL_ITEM A  " 
	lgStrSQL = lgStrSQL & "			INNER JOIN  (SELECT A_TEMP_GL.TEMP_GL_NO,A_TEMP_GL.REF_NO, A_TEMP_GL.BIZ_AREA_CD, A_TEMP_GL.GL_INPUT_TYPE, A_TEMP_GL.CONF_FG" 
