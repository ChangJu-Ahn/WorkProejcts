...
Sub  TrimData()
     strFrDt		= Trim(UNIConvDateToYYYYMMDD(Request("txtArFrDt"),gDateformat,""))
     strToDt		= Trim(UNIConvDateToYYYYMMDD(Request("txtArToDt"),gDateformat,""))
...
Sub  SubPreQueryBySp()
	lgStrSQL = ""
	lgStrSQL = lgStrSQL & " DELETE FROM A_TMP_AR_TRACE_LIST WHERE QUERY_ID < CONVERT(VARCHAR(8),  GETDATE(),112)"
    ...
    CONST CALLSPNAME = "A_USP_FROM_AR_TO_GL_TRACE"
...
Sub SubBizQueryMulti()
	...
	lgStrSQL = lgStrSQL & " SUM(CASE WHEN AR_FG=" & FilterVar("AR", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END) AR_LOC_AMT , "
	...
	End If
	lgStrSQL = lgStrSQL & " FROM A_TMP_AR_TRACE_LIST (NoLock) "
	...
	If UCase(Trim(Request("DispMeth"))) Then 
		lgStrSQL = lgStrSQL & " HAVING ISNULL(SUM(CASE WHEN AR_FG=" & FilterVar("AR", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END),0) "
		lgStrSQL = lgStrSQL & "  <> ISNULL(SUM(CASE WHEN AR_FG=" & FilterVar("GL", "''", "S") & "  THEN ITEM_LOC_AMT ELSE 0 END),0) "
	End If		
