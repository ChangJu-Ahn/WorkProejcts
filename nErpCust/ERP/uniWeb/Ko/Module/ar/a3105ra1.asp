...
======================================================================================================
*  1. Module Name          : 회계 
*  2. Function Name        : a3105ra1.asp
*  3. Program ID           : 
*  4. Program Name         : 입금반제팝업
*  5. Program Desc         : 
*  6. Comproxy List        :
...
Const BIZ_PGM_ID 		= "a3105rb1.asp"                              '☆: Biz Logic ASP Name
...
Sub SetDefaultVal()
	...
	frm1.txtFrRcptDt.Text = UniConvDateAToB(frDt ,PopupParent.gServerDateFormat,PopupParent.gDateFormat) 
	frm1.txtToRcptDt.Text = UniConvDateAToB("<%=GetSvrDate%>" ,PopupParent.gServerDateFormat,PopupParent.gDateFormat)
...
Function OpenDeptOrgPopup()
	...
	arrParam(0) = frm1.txtFrRcptDt.text								'  Code Condition
   	arrParam(1) = frm1.txtToRcptDt.Text
...
Function SetDept(Byval arrRet)	
		...
		frm1.txtFrRcptDt.text = arrRet(4)
		frm1.txtToRcptDt.text = arrRet(5)
		frm1.txtDeptCd.focus		
End Function
'========================================================================================================
' Name : LoadInfTB19029()	
...
Sub InitSpreadSheet()
    ...
    Call SetZAdoSpreadSheet("a3105RA1","S","A","V20021211",PopupParent.C_SORT_DBAGENT,frm1.vspdData, C_MaxKey, "X","X")
    Call SetSpreadLock() 
End Sub
...
Function FncQuery() 
	...
	If CompareDateByFormat(frm1.txtFrRcptDt.text,frm1.txtToRcptDt.text,frm1.txtFrRcptDt.Alt,frm1.txtToRcptDt.Alt, _
        	               "970025",frm1.txtFrRcptDt.UserDefinedFormat,PopupParent.gComDateType, true) = False Then
	   frm1.txtFrRcptDt.focus
	   Exit Function
	End If
	
	IF NOT CheckOrgChangeId Then
		  IntRetCD = DisplayMsgBox("800600","X",frm1.txtFrRcptDt.alt,"X")            '⊙: Display Message(There is no changed data.)
		Exit Function
	End if
...
Function DbQuery() 
    ...
    '--------- Developer Coding Part (Start) ----------------------------------------------------------
        If lgIntFlgMode  <> PopupParent.OPMD_UMODE Then   ' This means that it is first search
			...
			strVal = strVal & "&txtFrRcptNo="   & Trim(.txtFrRcptNo.value)
			strVal = strVal & "&txtToRcptNo="   & Trim(.txtToRcptNo.value)
			strVal = strVal & "&txtdeptcd="     & Trim(.txtdeptcd.value)  
			strVal = strVal & "&txtDeptCd_Alt=" & Trim(.txtdeptcd.alt)
        Else
			strVal = strVal & "?txtFrRcptDt="   & Trim(.htxtFrRcptDt.value)
			strVal = strVal & "&txtToRcptDt="	& Trim(.htxtToRcptDt.value)
			strVal = strVal & "&txtFrRcptNo="   & Trim(.htxtFrRcptNo.value)
			strVal = strVal & "&txtToRcptNo="   & Trim(.htxtToRcptNo.value)
...
Function CheckOrgChangeId()
	...
	With frm1
		...
		If LTrim(RTrim(.txtDeptCd.value)) <> "" Then
			...
			strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(.txtFrRcptDt.Text, popupparent.gDateFormat,""), "''", "S") & ")"
			strWhere = strWhere & " AND ORG_CHANGE_DT <= " 
			strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(.txtToRcptDt.Text, popupparent.gDateFormat,""), "''", "S") & ") "
			...
			<FIELDSET CLASS="CLSFLD">
				<TABLE <%=LR_SPACE_TYPE_40%>>
					<TR>				
						...
						<TD CLASS=TD6 NOWRAP>
							<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT title=FPDATETIME CLASS=FPDTYYYYMMDD name=txtFrRcptDt CLASSID=<%=gCLSIDFPDT%> ALT="시작일자" tag="12"> </OBJECT>');</SCRIPT>&nbsp;~&nbsp;
							<SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT title=FPDATETIME CLASS=FPDTYYYYMMDD name=txtToRcptDt CLASSID=<%=gCLSIDFPDT%> ALT="종료일자" tag="12"> </OBJECT>');</SCRIPT>
						</TD>												
						<TD CLASS=TD5 NOWRAP>입금번호</TD>				
						<TD CLASS=TD6 NOWRAP>
						<INPUT TYPE="Text" NAME="txtFrRcptNo" SIZE=15 MAXLENGTH=18 STYLE="TEXT-ALIGN: Left" tag="1XXXXU" ALT="입금번호">&nbsp;~&nbsp;
						<INPUT TYPE="Text" NAME="txtToRcptNo" SIZE=15 MAXLENGTH=18 STYLE="TEXT-ALIGN: Left" tag="1XXXXU" ALT="입금번호">
						</TD>
					</TR>
					<TR>				
...
<INPUT TYPE=HIDDEN NAME="htxtFrRcptDt"		tag="24">
<INPUT TYPE=HIDDEN NAME="htxtToRcptDt"		tag="24">
<INPUT TYPE=HIDDEN NAME="htxtFrRcptNo"		tag="24">
<INPUT TYPE=HIDDEN NAME="htxtToRcptNo"		tag="24">
