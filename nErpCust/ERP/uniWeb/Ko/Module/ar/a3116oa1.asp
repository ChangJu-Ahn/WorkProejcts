...
'*  2. Function Name        : A/R
'*  3. Program ID           : a3116ma1
'*  4. Program Name         : 채권CheckList출력 
'*  5. Program Desc         : Report of A/R CheckList
'*  6. Comproxy List        : 
...
'========================================================================================================= 
Sub SetDefaultVal()
	frm1.fpDateFr.Text = StartDate
	frm1.fpDateTo.Text = EndDate
	...
	Select Case iWhere
		...
		Case 3, 4
			If frm1.rdoPayBp.checked = False then
				...
				IF frm1.txtDateFr.Text<>"" THEN 	arrParam(1) = arrParam(1) & " AND AR_DT >= " & FilterVar(UNIConvDate(frm1.txtDateFr.Text), "''", "S") & ""
				IF frm1.txtDateTo.Text<>"" THEN arrParam(1) = arrParam(1) & " AND AR_DT <= " & FilterVar(UNIConvDate(frm1.txtDateTo.Text), "''", "S") & ""
...
Sub SetPrintCond(StrEbrFile, StrUrl)
	...
	Dim	VarBizAreaCd, VarBizAreaCd1, VarDateFr, VarDateTo, VarDealBpCdFr, VarDealBpCdTo, VarBpLabel
	Dim	strAuthCond
	...
	VarDateFr		= UniConvDateToYYYYMMDD(frm1.fpDateFr.Text, parent.gDateFormat, parent.gServerDateType)
	VarDateTo		= UniConvDateToYYYYMMDD(frm1.fpDateTo.Text, parent.gDateFormat, parent.gServerDateType)
	...
	If lgAuthBizAreaCd <> "" Then			
		strAuthCond		= strAuthCond	& " AND A_OPEN_AR.BIZ_AREA_CD = " & FilterVar(lgAuthBizAreaCd, "''", "S")  		
	End If			

	If lgInternalCd <> "" Then			
		strAuthCond		= strAuthCond	& " AND A_OPEN_AR.INTERNAL_CD = " & FilterVar(lgInternalCd, "''", "S")  		
	End If			

	If lgSubInternalCd <> "" Then	
		strAuthCond		= strAuthCond	& " AND A_OPEN_AR.INTERNAL_CD LIKE " & FilterVar(lgSubInternalCd & "%", "''", "S")  
	End If	

	If lgAuthUsrID <> "" Then	
		strAuthCond		= strAuthCond	& " AND A_OPEN_AR.INSRT_USER_ID = " & FilterVar(lgAuthUsrID, "''", "S")  
	End If	
...
Function FncBtnPrint() 
	...
	If CompareDateByFormat(frm1.fpDateFr.text,frm1.fpDateTo.text,frm1.fpDateFr.Alt,frm1.fpDateTo.Alt, _
			"970025",frm1.fpDateFr.UserDefinedFormat,parent.gComDateType, true) = False Then
		frm1.fpDateFr.focus
		Exit Function
	End If
					...
					<TD WIDTH=100% HEIGHT=100% valign=top>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtDateFr" CLASS=FPDTYYYYMMDD tag="12X1" Title="FPDATETIME" ALT="시작채권일자" id=fpDateFr></OBJECT>');</SCRIPT>&nbsp;~&nbsp;
													   <SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtDateTo" CLASS=FPDTYYYYMMDD tag="12X1" Title="FPDATETIME" ALT="종료채권일자" id=fpDateTo></OBJECT>');</SCRIPT>
								</TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>거래처기준</TD>
								<TD CLASS=TD6 NOWRAP><INPUT TYPE=RADIO CLASS="RADIO" NAME="rdoBpLabel" ID="rdoDelBp" VALUE="S" TAG="11" Checked><LABEL FOR="rdoReport1">주문처</LABEL>&nbsp;&nbsp
