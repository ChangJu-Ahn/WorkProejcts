...
'*  3. Program ID           : S4212RB8
'*  4. Program Name         : 통관내역참조 
'*  5. Program Desc         : 
'*  6. Comproxy List        : 
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
	...
	IF Request("txtRefFlag") = "M" Then
		UNISqlId(0) = "S4212RA802"
	Else
		UNISqlId(0) = "S4212RA801"
	End If	
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    ...
    iStrWhere1 = iStrWhere1 & " AND CH.applicant =  " & FilterVar(Request("txtApplicant"), "''", "S") & ""			'수입자 
    iStrWhere1 = iStrWhere1 & " AND CH.cur =  " & FilterVar(Request("txtCurrency"), "''", "S") & ""					'화폐단위 
    iStrWhere1 = iStrWhere1 & " AND CH.sales_grp =  " & FilterVar(Request("txtSalesGrpCd"), "''", "S") & ""			'영업그룹 
    
    If Len(Trim(Request("txtFromDt"))) Then
		iStrWhere1 = iStrWhere1 & " AND CH.iv_dt >=  " & FilterVar(UNIConvDate(Request("txtFromDt")), "''", "S") & ""					'시작일 
	End If

    iStrWhere1 = iStrWhere1 & " AND CH.iv_dt <=  " & FilterVar(UNIConvDate(Request("txtToDt")), "''", "S") & ""							'종료일 

    iStrWhere1 = iStrWhere1 & " AND CH.incoterms =  " & FilterVar(Request("txtIncoterms"), "''", "S") & ""			'가격조건 

    If Len(Trim(Request("txtPayTermsCd"))) Then
	    iStrWhere1 = iStrWhere1 & " AND CH.pay_meth =  " & FilterVar(Request("txtPayTermsCd"), "''", "S") & ""			'결제방법 
	End If

    If Len(Trim(Request("txtItemCd"))) Then
	    iStrWhere1 = iStrWhere1 & " AND CD.item_cd =  " & FilterVar(Request("txtItemCd"), "''", "S") & ""			'품목코드 
	End If

	If Len(Trim(Request("txtSoNo"))) Then
	    iStrWhere1 = iStrWhere1 & " AND CD.so_no =  " & FilterVar(Request("txtSoNo"), "''", "S") & ""				'S/O번호 
	End If
	...
	If Len(Request("gBizArea")) Then
		iStrWhere1 = iStrWhere1 & " AND CH.BIZ_AREA = " & FilterVar(Request("gBizArea"), "''", "S") & " "			
	End If

	If Len(Request("gPlant")) Then
		iStrWhere1 = iStrWhere1 & " AND CD.PLANT_CD = " & FilterVar(Request("gPlant"), "''", "S") & " "			
	End If

	If Len(Request("gSalesGrp")) Then
		iStrWhere1 = iStrWhere1 & " AND CH.SALES_GRP = " & FilterVar(Request("gSalesGrp"), "''", "S") & " "			
	End If

	If Len(Request("gSalesOrg")) Then
		iStrWhere1 = iStrWhere1 & " AND CH.SALES_ORG = " & FilterVar(Request("gSalesOrg"), "''", "S") & " "			
	End If
