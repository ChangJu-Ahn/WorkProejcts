...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    ...
    iStrWhere1 = iStrWhere1 & " AND SH.sold_to_party =  " & FilterVar(Request("txtApplicant"), "''", "S") & ""		'수입자 
    iStrWhere1 = iStrWhere1 & " AND SH.cur =  " & FilterVar(Request("txtCurrency"), "''", "S") & ""					'화폐단위 
    iStrWhere1 = iStrWhere1 & " AND SH.sales_grp =  " & FilterVar(Request("txtSalesGrpCd"), "''", "S") & ""			'영업그룹 
    iStrWhere1 = iStrWhere1 & " AND SH.incoterms =  " & FilterVar(Request("txtIncoterms"), "''", "S") & ""			'가격조건 
    iStrWhere1 = iStrWhere1 & " AND SH.pay_meth =  " & FilterVar(Request("txtPayTermsCd"), "''", "S") & ""			'결제방법 
	iStrWhere1 = iStrWhere1 & " AND ST.trans_type =  " & FilterVar(Request("txtBillTypeCD"), "''", "S") & ""		'매출채권유형 
    
    If Len(Trim(Request("txtFromDt"))) Then
		iStrWhere1 = iStrWhere1 & " AND SH.so_dt >=  " & FilterVar(UNIConvDate(Request("txtFromDt")), "''", "S") & ""					'시작일 
	End If

    iStrWhere1 = iStrWhere1 & " AND SH.so_dt <=  " & FilterVar(UNIConvDate(Request("txtToDt")), "''", "S") & ""							'종료일 

    If Len(Trim(Request("txtItemCd"))) Then
	    iStrWhere1 = iStrWhere1 & " AND SD.item_cd =  " & FilterVar(Request("txtItemCd"), "''", "S") & ""			'품목코드 
	End If

	If Len(Trim(Request("txtSoNo"))) Then
	    iStrWhere1 = iStrWhere1 & " AND SH.so_no =  " & FilterVar(Request("txtSoNo"), "''", "S") & ""				'S/O번호 
	End If
	...
	If Len(Request("gBizArea")) Then
		iStrWhere1 = iStrWhere1 & " AND SH.BIZ_AREA = " & FilterVar(Request("gBizArea"), "''", "S") & " "			
	End If

	If Len(Request("gPlant")) Then
		iStrWhere1 = iStrWhere1 & " AND SD.PLANT_CD = " & FilterVar(Request("gPlant"), "''", "S") & " "			
	End If

	If Len(Request("gSalesGrp")) Then
		iStrWhere1 = iStrWhere1 & " AND SH.SALES_GRP = " & FilterVar(Request("gSalesGrp"), "''", "S") & " "			
	End If

	If Len(Request("gSalesOrg")) Then
		iStrWhere1 = iStrWhere1 & " AND SH.SALES_ORG = " & FilterVar(Request("gSalesOrg"), "''", "S") & " "			
	End If
