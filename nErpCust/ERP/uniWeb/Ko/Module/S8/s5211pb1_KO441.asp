...
'*  3. Program ID           : s5211pb1
'*  4. Program Name         : B/L관리번호 Popup
'*  5. Program Desc         : 
'*  6. Comproxy List        : 
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
    ...
    If Len(Trim(Request("txtPostFlag"))) Then
		iStrWhere1 = iStrWhere1 & " AND BH.post_flag =  " & FilterVar(Request("txtPostFlag"), "''", "S") & ""				'확정여부 
	End If
...
<Script Language=vbscript>
    With parent.frm1
		...
		If "<%=lgDataExist%>" = "Yes" Then
			...
			.txtHForwarder.value	= "<%=Request("txtForwarder")%>"
			.txtHSalesGrp.value		= "<%=Request("txtSalesGrp")%>"
			.txtHPostFlag.value		= "<%=Request("txtPostFlag")%>"
