...
'-------------------------------------------------------------------------------------------------------------------------    
 Sub bulk_disposal()
  ...
  '  strWhere_in = "and from_alloc <> " & FilterVar("*", "''", "S") & "  and acct_gp = " & FilterVar("*", "''", "S") & " "
    ...
    Call CommonQueryRs("count(*)","g_alloc_data","yyyymm = " & FilterVar(YYYYMM, "''", "S") & " and alloc_kinds = " & FilterVar("3", "''", "S") & "  and alloc_base = " & FilterVar(Currency_code, "''", "S") & " " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Sub SubBizQueryMulti()
    ...
    If Trim(lgKeyStream(1)) <> "" Then
        ...
        Call CommonQueryRs(" b.minor_nm "," b_configuration a, b_minor b "," a.minor_cd = b.minor_cd and a.major_cd = b.major_cd and   b.major_cd = " & FilterVar("G1004", "''", "S") & " AND  a.seq_no =4  and reference = " & FilterVar("Y", "''", "S") & "   and a.minor_cd =  " & FilterVar(lgKeyStream(1), "''", "S") & " ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
    ...
    Else
        ...
        Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("sales_grp_nm")) 'grm nm 명 
...
Sub SubBizSaveMultiCreate(arrColVal)
    ...
    lgStrSQL = lgStrSQL & " ALLOC_BASE    ,TO_SALES_GRP,TO_ITEM_GROUP,"
    ...
    lgStrSQL = lgStrSQL & "" & FilterVar("3", "''", "S") & ", "
    lgStrSQL = lgStrSQL & FilterVar(UCase(arrColVal(2)), "''", "S")     & "," & FilterVar("*", "''", "S") & " ," & FilterVar("*", "''", "S") & " ,"
    ...
    Select Case Mid(pDataType,1,1)
        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "R"
                       lgStrSQL = "Select TOP " & iSelCount  & "  G.to_alloc, B.sales_grp_nm, IsNull(SUM(G.to_data),0) as A "
                       lgStrSQL = lgStrSQL & " From  g_alloc_data G, b_sales_grp B"
                       lgStrSQL = lgStrSQL & " Where  G.to_alloc = B.sales_grp "
                       lgStrSQL = lgStrSQL & " and  g.alloc_kinds = " & FilterVar(3, "''", "S")                       
                       lgStrSQL = lgStrSQL & " and  g.yyyymm = " & pCode                     
                       lgStrSQL = lgStrSQL & " group by  G.to_alloc, B.sales_grp_nm"
			   ...
			   Case "I"
					   ...
					   lgStrSQL = lgStrSQL & " and	a.alloc_kinds = '3'	 "   
					   lgStrSQL = lgStrSQL & " and alloc_base = " & FilterVar(Currency_code, "''", "S") 
           End Select
