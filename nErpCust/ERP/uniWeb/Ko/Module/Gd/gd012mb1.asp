...
Sub FixUNISQLData()
   ...
   IF Trim(lgKeyStream(2)) <> "" Then                 '사업부 
	...
	IF Trim(lgKeyStream(4)) <> "" Then                 '거래처 
  		Call CommonQueryRs("BP_NM","B_BIZ_PARTNER","BP_CD = " & FilterVar(lgKeyStream(4), "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
		If Trim(Replace(lgF0,Chr(11),"")) = "X" then
	...
	UNISqlId(0) = "Gd012MA01"      '품목별 손익비교 
	UNISqlId(1) = "Gd012MA02"      '총계합 
	...
	IF Trim(lgKeyStream(4)) = "" Then					 ' 거래처 
		lgCodeCond =  lgCodeCond & "" 
	ELSE
		lgCodeCond =  lgCodeCond & " and a.bp_cd = " & FilterVar(lgKeyStream(4), "''", "S")
	END IF
