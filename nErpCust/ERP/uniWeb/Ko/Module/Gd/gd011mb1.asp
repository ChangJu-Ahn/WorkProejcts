...
Sub FixUNISQLData()
   ...
   IF Trim(lgKeyStream(2)) <> "" Then                 '사업부 
	...
	IF Trim(lgKeyStream(5)) <> "" Then                 '거래처 
  		Call CommonQueryRs("BP_NM","B_BIZ_PARTNER","BP_CD = " & FilterVar(lgKeyStream(5), "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
		If Trim(Replace(lgF0,Chr(11),"")) = "X" then
	...
	IF Trim(lgKeyStream(3))  <> "" Then					'Cost Center
  		Call CommonQueryRs("COST_NM","B_COST_CENTER","COST_TYPE = " & FilterVar("S", "''", "S") & "  AND COST_CD= " & FilterVar(lgKeyStream(4), "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
		If Trim(Replace(lgF0,Chr(11),"")) = "X" then
	...
	UNISqlId(0) = "GD011MA01"      '품목별 손익비교 
	UNISqlId(1) = "GD011MA02"      '총계합 
	...
	IF Trim(lgKeyStream(5)) = "" Then					 ' 거래처 
		lgCodeCond =  lgCodeCond & "" 
	ELSE
		lgCodeCond =  lgCodeCond & " and a.bp_cd = " & FilterVar(lgKeyStream(5), "''", "S")
	END IF
