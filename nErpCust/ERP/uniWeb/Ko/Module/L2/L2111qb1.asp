...
Sub FixUNISQLData()
	...
	lgSelectList = Replace(lgSelectList, "?", _
					"CASE WHEN ISD.STS = 0 THEN " & FilterVar("주문마감", "''", "S") & " " & _
						" WHEN ISD.STS = 1 THEN " & FilterVar("주문접수", "''", "S") & " " & _
						" WHEN ISD.STS = 2 THEN " & FilterVar("수주등록", "''", "S") & " " & _
						" ELSE CASE " & _
								"	WHEN SD.CLOSE_FLAG = " & FilterVar("Y", "''", "S") & "  THEN " & FilterVar("수주마감", "''", "S") & " " & _
								"	WHEN SD.REQ_QTY = 0 THEN '수주확정' " & _
								"	WHEN SD.BILL_QTY > 0 AND SD.SO_QTY <= SD.BILL_QTY THEN " & FilterVar("매출완료", "''", "S") & " " & _
								"	WHEN SD.BILL_QTY > 0 AND SD.SO_QTY > SD.BILL_QTY THEN '매출진행' " & _
								"	WHEN SD.GI_QTY > 0 AND SD.SO_QTY <= SD.GI_QTY THEN " & FilterVar("출고완료", "''", "S") & " " & _
								"	WHEN SD.GI_QTY > 0 AND SD.SO_QTY > SD.GI_QTY THEN '출고진행' " & _
								"	WHEN SD.SO_QTY <= SD.REQ_QTY THEN " & FilterVar("출고요청완료", "''", "S") & " " & _
								"	ELSE '출고요청진행' " & _
							  " END  " & _
					    " END ")
