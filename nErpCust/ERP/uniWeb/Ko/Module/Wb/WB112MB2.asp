...
Sub SubMakeSQLStatements(pMode,pCode1, pCode2, pCode3)
    Select Case pMode 
      ...
      Case "D"
		    ...
		    lgStrSQL = lgStrSQL & " (Case when match_cd = '10' then (select minor_nm from b_minor where  major_cd = 'W1056' and  minor_cd =a.ACCT_GP_CD) "
			lgStrSQL = lgStrSQL & "		  when match_cd = '06' then (select minor_nm from b_minor where  major_cd = 'W1084' and  minor_cd =a.ACCT_GP_CD)"
			lgStrSQL = lgStrSQL & "		  when match_cd = '07' then (select minor_nm from b_minor where  major_cd = 'W1085' and  minor_cd =a.ACCT_GP_CD)"
			lgStrSQL = lgStrSQL & "		  when match_cd = '34' then (select minor_nm from b_minor where  major_cd = 'W1086' and  minor_cd =a.ACCT_GP_CD)" 	 
			lgStrSQL = lgStrSQL & "		 else '' end 	    )  ACCT_GP_NM  "
            lgStrSQL = lgStrSQL & " FROM  TB_ACCT_MATCH A"
         	lgStrSQL = lgStrSQL & " WHERE A.CO_CD = " & pCode1 	 & vbCrLf
			lgStrSQL = lgStrSQL & "		AND A.FISC_YEAR = " & pCode2 	 & vbCrLf
			lgStrSQL = lgStrSQL & "		AND A.REP_TYPE = " & pCode3 	 & vbCrLf
			lgStrSQL = lgStrSQL & "		AND A.match_cd = " & request("sMAP_CD") 	 & vbCrLf
