...
Sub SubBizSaveSingleUpdate()
    ...
    if  Cint(Request("txtbas_strt_dd")) < 10 then
        lgStrSQL = lgStrSQL & "       bas_strt_dd = '0" & Request("txtbas_strt_dd") & "',"
    else
        lgStrSQL = lgStrSQL & "       bas_strt_dd = " & FilterVar(Request("txtbas_strt_dd"),"'00'","S") & ","
    end if
    lgStrSQL = lgStrSQL & "       bas_end_mm = " & FilterVar(Request("cboBas_end_mm"),"''","S") & ","

    if  Cint(Request("txtbas_end_dd")) < 10 then
        lgStrSQL = lgStrSQL & "       bas_end_dd = '0" & Request("txtbas_end_dd") & "',"
    else
        lgStrSQL = lgStrSQL & "       bas_end_dd = " & FilterVar(Request("txtbas_end_dd"),"'00'","S") & ","
    end if
    ...
    if  Cint(Request("txtpay_prov_dd")) < 10 then
        lgStrSQL = lgStrSQL & "       pay_prov_dd = '0" & Request("txtpay_prov_dd") & "',"
    else
        lgStrSQL = lgStrSQL & "       pay_prov_dd = " & FilterVar(Request("txtpay_prov_dd"),"'00'","S") & ","
    end if

    if  Cint(Request("txtpay_bas_dd")) < 10 then
        lgStrSQL = lgStrSQL & "       pay_bas_dd = '0" & Request("txtpay_bas_dd") & "',"
    else
        lgStrSQL = lgStrSQL & "       pay_bas_dd = " & FilterVar(Request("txtpay_bas_dd"),"'00'","S") & ","
    end if

    if  Cint(Request("txtdilig_dd")) < 10 then
        lgStrSQL = lgStrSQL & "       dilig_dd = '0" & Request("txtdilig_dd") & "'"
    else
        lgStrSQL = lgStrSQL & "       dilig_dd = " & FilterVar(Request("txtdilig_dd"),"'00'","S") 
    end if
...
Sub SubMakeSQLStatements(pMode,pCode)
    Select Case pMode 
      Case "R"
            ...
            lgStrSQL = lgStrSQL & " dilig_dd,          insur_dt " 
