...
Sub SubMakeSQLStatements(pMode,pCode1, pCode2, pCode3)
    ...
    Select Case pMode 
      Case "R"
			...
			lgStrSQL = lgStrSQL & "		,	@FISC_START_DT	datetime " & vbCrLf
			lgStrSQL = lgStrSQL & "		,	@FISC_END_DT	datetime " & vbCrLf
			lgStrSQL = lgStrSQL & "		,	@IGAP		NUMERIC(15) " & vbCrLf
			
			lgStrSQL = lgStrSQL & " SELECT  "	' -- 신고구분별 일수계산 
			If pCode3 = "'2'" Then ' -- 중간예납 
				lgStrSQL = lgStrSQL & "	@IGAP = DateDiff(day, FISC_START_DT, DateAdd(month, 6, FISC_START_DT) - 1)+1 " & vbCrLf
			Else
				lgStrSQL = lgStrSQL & "	@IGAP = DateDiff(day, FISC_START_DT, FISC_END_DT)+1 " & vbCrLf
			End If
			lgStrSQL = lgStrSQL & " FROM TB_COMPANY_HISTORY A WITH (NOLOCK) "
			lgStrSQL = lgStrSQL & " WHERE A.CO_CD = " & pCode1 	 & vbCrLf
			lgStrSQL = lgStrSQL & "		AND A.FISC_YEAR = " & pCode2 	 & vbCrLf
			lgStrSQL = lgStrSQL & "		AND A.REP_TYPE = " & pCode3 	 & vbCrLf & vbCrLf
			...
			lgStrSQL = lgStrSQL & " SELECT dbo.ufn_TB_16_2_GetRef(" & pCode1 & ", " & pCode2 & ", " & pCode3 & ", A.COMPANY_TYPE , A.STOCK_RATE) W16 " & vbCrLf ' -- 입금불산입율 표시분	' -- 익금불산입 
            ...
            lgStrSQL = lgStrSQL & " , A.DOC_AMT * (dbo.ufn_TB_16_2_GetRef(" & pCode1 & ", " & pCode2 & ", " & pCode3 & ", A.COMPANY_TYPE, A.STOCK_RATE) / 100) W17 "& vbCrLf
            lgStrSQL = lgStrSQL & " , @TB_26_AMT * ( (A.JUKSU * (dbo.ufn_TB_16_2_GetRef(" & pCode1 & ", " & pCode2 & ", " & pCode3 & ", A.COMPANY_TYPE, A.STOCK_RATE) /100)) / (@TB_3_2_AMT * @IGAP )	) W19 " & vbCrLf ' -- 지급이자*(취득가액*보유기간 /자산합계)
