...
'========================================================================================================
Const BIZ_PGM_ID 		= "w3131rb1.asp"                              '☆: Biz Logic ASP Name
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetEdit		C_W2,	"계정과목",	10,,,100,1
		ggoSpread.SSSetFloat	C_W3,	"금액",	10,		PopupParent.ggAmtOfMoneyNo,		ggStrIntegeralPart,		ggStrDeciPointPart,		PopupParent.gComNum1000,		PopupParent.gComNumDec ,,,,"0","" 
		ggoSpread.SSSetEdit		C_W4,	"대표계정코드",	15,,,100,1
		ggoSpread.SSSetEdit		C_W5,	"대표계정",	10,,,100,1
		ggoSpread.SSSetEdit		C_W6,	"매출/영업이익",	10,,,100,1
			 
		'Call ggoSpread.SSSetColHidden(.MaxCols, .MaxCols, True)
		'Call ggoSpread.SSSetColHidden(C_W4,C_W4,True)
		.Col = C_W4													'☆: 사용자 별 Hidden Column
		.ColHidden = True 
		.Col = C_W6													'☆: 사용자 별 Hidden Column
		.ColHidden = True 
...
Function OKClick()
	...
	Dim intColCnt, intRowCnt, intInsRow, iLngSelectedRows, sW4, dblAmt, Idx, arrTemp, iDx2
		...
		If iLngSelectedRows > 0 Then 
			...
			For intRowCnt = 1 To .MaxRows
				...
				If .SelModeSelected Then
					...
					Else
						.Col = C_W1		: arrTemp(iDx2, 0) = .Value
						.Col = C_W2		: arrTemp(iDx2, 1) = .Value
						.Col = C_W3		: arrTemp(iDx2, 2) = arrTemp(iDx2, 2) + UNICDbl(.Value)
						.Col = C_W4		: arrTemp(iDx2, 3) = .Value
						.Col = C_W5		: arrTemp(iDx2, 4) = .Value
						.Col = C_W6		: arrTemp(iDx2, 5) = .Value
...
Function CheckSavedW4(Byref arrRet2, Byval pW4)	' -- 저장된 W4를 검색한다..
...
Function GetGPCount(Byref arrRet2)	' -- 배열의 갯수 체크 
	Dim iRow, iMaxRows, sW4, iGPCnt
	...
	<TR HEIGHT=100%>
		<TD WIDTH=100% HEIGHT=* VALIGN=TOP>
			<TABLE <%=LR_SPACE_TYPE_20%>>
				<TR HEIGHT=100%>
					<TD WIDTH=100%>
						<script language =javascript src='./js/w3131ra1_OBJECT1_vspdData.js'></script>
					</TD>
				</TR>
			</TABLE>
		</TD>
