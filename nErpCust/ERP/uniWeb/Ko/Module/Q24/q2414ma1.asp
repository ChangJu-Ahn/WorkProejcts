...
Const BIZ_PGM_QRY_ID = "Q2414MB1.asp"								'☆: 비지니스 로직 ASP명 
Const BIZ_PGM_QRY2_ID = "Q2414MB3.asp"								'☆: 비지니스 로직 ASP명 
Const BIZ_PGM_SAVE_ID = "Q2414MB2.asp"								'☆: 비지니스 로직 ASP명 
'/* 전체 삭제 관련 - START */
Const BIZ_PGM_DEL_ID = "Q2414MB4.asp"
'/* 전체 삭제 관련 - END */
Const BIZ_PGM_JUMP1_ID = "Q2411MA1"
Const BIZ_PGM_JUMP2_ID = "Q2415MA1"
...
Dim C_InspItemNm '= 2
Dim C_InspSeries '= 3
Dim C_DefectTypeCd '= 4
Dim C_DefectTypeNm '= 5
Dim C_DefectQty1 '= 6

Dim C_DefectCauseCd '= 1										'☆: Spread Sheet의 Column별 상수 
Dim C_DefectCausePopup '= 2
Dim C_DefectCauseNm '= 3
Dim C_DefectQty2 '= 4
Dim C_ParentRowNo '= 5
Dim C_Flag '= 6
...
'=======================================================================================================
Sub InitSpreadSheet()	'###그리드 컨버전 주의부분###
	...
	With frm1
		...
		ggoSpread.SSSetEdit C_InspItemNm, "검사항목명", 14, 0, -1, 40
		ggoSpread.SSSetFloat C_InspSeries, "차수", 6, "6", ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec
		ggoSpread.SSSetEdit C_DefectTypeCd, "불량유형코드",12, 0, -1, 5, 2
		ggoSpread.SSSetEdit C_DefectTypeNm, "불량유형명", 14, 0, -1, 40
		ggoSpread.SSSetFloat C_DefectQty1, "불량수", 10, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec
...
Sub InitSpreadSheet2()
	...
	With frm1
		...
		ggoSpread.SSSetEdit C_DefectCauseCd, "불량원인코드", 12, 0, -1, 3, 2			'☆: Spread Sheet의 Column별 상수 
		ggoSpread.SSSetButton C_DefectCausePopup
		ggoSpread.SSSetEdit C_DefectCauseNm, "불량원인명", 14, 0, -1, 40
		ggoSpread.SSSetFloat C_DefectQty2, "불량수", 10, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , "Z"
	    ggoSpread.SSSetEdit C_ParentRowNo , "C_ParentRowNo", 5
	    ggoSpread.SSSetEdit C_Flag , "C_Flag", 5
	    
	    Call ggoSpread.MakePairsColumn(C_DefectCauseCd, C_DefectCausePopup)		    	
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
 	...
 	Select Case UCase(pvSpdNo)
 		Case "A"
			...
			C_DefectTypeCd = iCurColumnPos(4)
			C_DefectTypeNm = iCurColumnPos(5)
			C_DefectQty1 = iCurColumnPos(6)
		Case "B"
 			ggoSpread.Source = frm1.vspdData2
 			Call ggoSpread.GetSpreadColumnPos(iCurColumnPos)

			C_DefectCauseCd = iCurColumnPos(1)
			C_DefectCausePopup = iCurColumnPos(2)
			C_DefectCauseNm = iCurColumnPos(3)
...
Sub SetSpread2ColorRange(ByVal Row, Byval Row2, Byval Mode)
	...
	With ggoSpread
		If Mode = "I" Then
			.SpreadUnLock C_DefectCauseCd, Row, C_DefectCauseCd, Row2
			.SSSetRequired C_DefectCauseCd, Row, Row2
		Else			 
			.SSSetProtected C_DefectCauseCd, Row, Row2
			.SSSetProtected C_DefectCausePopup, Row, Row2
		End If
		
		If Mode <> "I" Then
			'For i = Row To Row + Row2
			For i = Row To Row2
				frm1.vspdData2.Col = 0	'###그리드 컨버전 주의부분###
				frm1.vspdData2.Row = i
				If frm1.vspdData2.Text = ggoSpread.InsertFlag Then
					ggoSpread.SpreadUnLock C_DefectCauseCd, i, C_DefectCauseCd, i
					ggoSpread.SSSetRequired C_DefectCauseCd, i, i
					
					ggoSpread.SpreadUnLock C_DefectCausePopup, i, C_DefectCausePopup, i
				End If
			Next		
		End If		
		
		.SSSetProtected C_DefectCauseNm, Row, Row2
...
End Function
 ...
 '------------------------------------------  OpenDefectCause()  -------------------------------------------------
'	Name : OpenDefectCause()
'	Description : DefectCause PopUp
'--------------------------------------------------------------------------------------------------------- 
Function OpenDefectCause(Byval strCode)
	Dim arrRet
	Dim arrParam(5), arrField(6), arrHeader(6)
	...
	IsOpenPop = True
'###검사분류별 변경부분 Start###
	arrParam(0) = "불량원인팝업"									' 팝업 명칭 
	arrParam(1) = "Q_DEFECT_CAUSE"									' TABLE 명칭 
	arrParam(2) = strCode										' Code Condition
	arrParam(3) = ""											' Name Cindition
	arrParam(4) = "PLANT_CD =  " & FilterVar(frm1.txtPlantCd.value, "''", "S") & "" & _
	                    " AND INSP_CLASS_CD =  " & FilterVar(strInspClass , "''", "S") & ""
	arrParam(5) = "불량원인"										' 조건필드의 라벨 명칭	
	
    arrField(0) = "DEFECT_CAUSE_CD"									' Field명(0)
    arrField(1) = "DEFECT_CAUSE_NM"									' Field명(1)
    
    arrHeader(0) = "불량원인코드"									' Header명(0)
    arrHeader(1) = "불량원인"									' Header명(1)
'###검사분류별 변경부분 End###      
	arrRet = window.showModalDialog("../../comasp/CommonPopup.asp", Array(arrParam, arrField, arrHeader), _
		"dialogWidth=420px; dialogHeight=450px; center: Yes; help: No; resizable: No; status: No;")
	...
	Call SetActiveCell(frm1.vspdData2,C_DefectCauseCd,frm1.vspdData2.ActiveRow,"M","X","X")
	If arrRet(0) = "" Then
		Exit Function
	Else
		frm1.vspdData2.Col = C_DefectCauseCd
		frm1.vspdData2.Text = Trim(arrRet(0))
		frm1.vspdData2.Col = C_DefectCauseNm
...
'			'Spread Color 처리	'@@@
		...
		If lgIntFlgModeM = Parent.OPMD_UMODE Then
			...
			strVal = strVal & "&txtDefectTypeCd=" & .hDefectTypeCd.value			'☆: 조회 조건 
		...
		Else
			...
			.vspdData.Col = C_DefectTypeCd
			strVal = strVal & "&txtDefectTypeCd=" & .vspdData.Text			'☆: 조회 조건 
			strVal = strVal & "&lgStrPrevKeyM=" & lgStrPrevKeyM(iRow - 1)
			strVal = strVal & "&lglngHiddenRows=" & lglngHiddenRows(iRow - 1)
			strVal = strVal & "&lRow=" & CStr(iRow)
			strVal = strVal & "&txtMaxRows=" & .vspdData2.MaxRows
...
'   Event Desc :
'=======================================================================================================
Sub ParentGetFocusCell(ByVal ParentRow, ByVal strDefectCause, Byval strFlag)
	Dim CheckField1		
	Dim CheckField2
	Dim i 
	Dim lngStart
	...
	With frm1.vspdData2
		...
		For i = lngStart To lngEnd
			...
			If CheckField1 = strDefectCause And CheckField2 = strFlag Then
				Exit For
			End If
		Next
...
Function ShowFromData(Byval Row, Byval lngShowingRows)
	...
	With frm1.vspdData2
		...
		If .MaxRows < 1 Then Exit Function	'###그리드 컨버전 주의부분###
		...
		If lngStartRow > 0 Then
			...
			If lngStartRow > 1 Then
				ggoSpread.SpreadLock 1, 1, .MaxCols, lngStartRow - 1	'@@@변경 
				.BlockMode = True
				.Row = 1
				.Row2 = lngStartRow - 1
				.RowHidden = True
			...
			If lngStartRow < .MaxRows Then
				If lngStartRow + lngShowingRows <= .MaxRows Then
					ggoSpread.SpreadLock 1, lngStartRow + lngShowingRows, .MaxCols, .MaxRows	'@@@변경 
					.BlockMode = True
...
Sub vspdData2_Click(ByVal Col, ByVal Row)
	...
	iActiveRow2 = frm1.vspdData2.ActiveRow
 	If frm1.vspdData2.IsVisible(C_DefectCauseCd, iActiveRow2, True) = False AND frm1.vspdData2.IsVisible(C_DefectCausePopup, iActiveRow2, True) = False AND frm1.vspdData2.IsVisible(C_DefectCauseNm, iActiveRow2, True) = False AND frm1.vspdData2.IsVisible(C_DefectQty2, iActiveRow2, True) = False Then
...
Sub Form_Load()
	...
	Call InitSpreadSheet2                                                       'Setup the Spread sheet
    Call SetDefaultVal
    Call InitVariables
    '/* 전체 삭제 관련 - START */
...
Sub Sub_vspdData_ScriptLeaveCell(ByVal Col, ByVal Row, ByVal NewCol, ByVal NewRow, Cancel)
	...
	If Row <> NewRow And NewRow > 0 Then
		With frm1
			...
			.vspdData.Col = C_DefectTypeCd
			.hDefectTypeCd.value = .vspdData.Text
...
Sub vspdData2_ButtonClicked(ByVal Col, ByVal Row, Byval ButtonDown)
	ggoSpread.Source = frm1.vspdData2	
	With frm1.vspdData2 
		If Row > 0 And Col = C_DefectCausePopUp Then
    		.Col = C_DefectCauseCd
			.Row = Row
    		Call OpenDefectCause(.Text)
    	End If
    End With
End Sub
	...
	With frm1.vspdData2
		For iRows = 1 To .MaxRows
			...
			If UNICDbl(.text) = 0 Then
				...
				Call ParentGetFocusCell(iParentRowNo, strDefectCause, strFlag)
				Exit Function
			End If
		Next
	End With
...
Function FncCopy() 
	...
	With frm1
		...
		.vspdData2.Col = C_DefectCauseCd
		.vspdData2.Text = ""
		
		.vspdData2.Col = C_DefectCauseNm
		.vspdData2.Text = ""
...
Function FncCancel() 
	...
	ggoSpread.Source = frm1.vspdData2	
	With frm1.vspdData2
		'범위가 보이지 않는 곳까지 넘어갔을 경우에 대한 재 블록 처리 - START	 
		...
		.BlockMode = False
		'범위가 보이지 않는 곳까지 넘어갔을 경우에 대한 재 블록 처리 - END
		...
		If lngRangeFrom <> 0 Then
			.Action = 19	'SS_ACTION_COPY_RANGE	'###그리드 컨버전 주의부분###
		End If
		.BlockMode = False
...
Function DbQuery() 
	...
	with frm1
		...
		If lgIntFlgMode = Parent.OPMD_UMODE Then
			...
			strVal = strVal & "&txtinspReqNo=" & .hInspReqNo.value				'조회 조건 데이타 
...
Function DbSave() 
	...
	With frm1
		For lRow = 1 To .vspdData2.MaxRows
			...
			iParentNum = CLng(.vspdData2.Text)
			For lRow2 = 1 To .vspdData.MaxRows
				.vspdData.Row = lRow2
				.vspdData.Col = .vspdData.MaxCols
				If iParentNum = CLng(.vspdData.Text) Then
			...
			Select Case .vspdData2.Text
				Case ggoSpread.InsertFlag				'☜: 신규 
					...
					.vspdData.Col = C_DefectTypeCd
					strDefectTypeCd = Trim(.vspdData.Text)
					.vspdData2.Col = C_DefectCauseCd
					strDefectCauseCd = Trim(.vspdData2.Text)
					.vspdData2.Col = C_DefectQty2
					...
					strVal = strVal & "C" & Parent.gColSep & _
							 ...
							 strDefectCauseCd & Parent.gColSep & _
				...
				Case ggoSpread.DeleteFlag				'☜: 삭제 
					...
					.vspdData.Col = C_DefectTypeCd			'3
					strDel = strDel & Trim(.vspdData.Text) & Parent.gColSep
					.vspdData2.Col = C_DefectCauseCd		'4
					strDel = strDel & Trim(.vspdData2.Text) & Parent.gColSep
					strDel = strDel & CStr(lRow) & Parent.gRowSep	'5
					lGrpCnt = lGrpCnt + 1
			End Select
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<TD BACKGROUND="../../../CShared/image/table/seltab_up_bg.gif" ALIGN="center" CLASS="CLSMTAB"><FONT COLOR=white>출하검사 불량원인</FONT></TD>
					...
					<TD HEIGHT=*  WIDTH=100% VALIGN=TOP>						
						<TABLE <%=LR_SPACE_TYPE_60%>>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP>
									<script language =javascript src='./js/q2414ma1_fpDoubleSingle1_txtLotSize.js'></script>
								</TD>
							</TR>
							<TR>
								<TD HEIGHT=100% WIDTH=100% Colspan=4>
									<TABLE WIDTH="100%" HEIGHT="100%" CELLSPACING=0 CELLPADDING=0>
										<TR HEIGHT="*">
											<TD WIDTH="62%">
												<script language =javascript src='./js/q2414ma1_I188005423_vspdData.js'></script>
											</TD>
											<TD WIDTH=10>&nbsp;</TD>
											<TD WIDTH="38%">
												<script language =javascript src='./js/q2414ma1_I651811450_vspdData2.js'></script>
											</TD>
										</TR>
									</TABLE>
								</TD>
	...
	<TR HEIGHT=20>
      		<TD WIDTH=100%>
      			<TABLE <%=LR_SPACE_TYPE_30%>>
        			<TR>        				
        				<TD WIDTH=10>&nbsp;</TD>
        				<TD WIDTH=* ALIGN=RIGHT><A href="vbscript:LoadInspection">출하검사</A>&nbsp;|&nbsp;<A href="vbscript:LoadDecision">판정</A></TD>
        				<TD WIDTH=10>&nbsp;</TD>
       				</TR>
      			</TABLE>
      		</TD>
...
<INPUT TYPE=HIDDEN NAME="hDefectTypeCd" tag="24" tabindex=-1 >
