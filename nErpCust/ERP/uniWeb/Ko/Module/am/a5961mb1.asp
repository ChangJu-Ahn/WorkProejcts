...
<%
    ...
    Err.Clear
    Call LoadBasisGlobalInf()                                                                       '☜: Clear Error status                                                            '☜: Clear Error status
...
Sub SubBizBatch()
    ...
    With lgObjComm
        ...
        .CommandText = "dbo.A_USP_A5961MB"
        .CommandType = adCmdStoredProc

        lgObjComm.Parameters.Append lgObjComm.CreateParameter("RETURN_VALUE",adInteger,adParamReturnValue)
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@yyyy"     ,adVarWChar,adParamInput,8, lgKeyStream(0))
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@pay_type"     ,adVarWChar,adParamInput,1, lgKeyStream(1))
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@biz_area_cd"     ,adVarWChar,adParamInput,5, lgKeyStream(2))
	    lgObjComm.Parameters.Append lgObjComm.CreateParameter("@user_id"     ,adVarWChar,adParamInput,13, gUsrID)
...
Sub SubBizQueryMulti()
    ...
    Else
        ...
        Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("acct_type"))
            lgstrData = lgstrData & Chr(11) & lgObjRs("pay_amt")
            lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs("amt"), ggQty.DecPoint, 0)
	'------ Developer Coding part (End   ) ------------------------------------------------------------------
        ...
        If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then
            ...
            Call SetErrorStatus()
        Else
            txtExpectAmt = UNINumClientFormat(lgObjRs("amt"), ggQty.DecPoint, 0)
        End If
    End If
...
Sub SubBizSaveMultiCreate(arrColVal)
   ...
   ' 'Err.Clear                                                                        '☜: Clear Error status
	...
	lgStrSQL = lgStrSQL & " from	a_expected_year_bonus			  "
	lgStrSQL = lgStrSQL & " where	YYYY	=	"	& FilterVar(lgKeyStream(0), "''", "S")
	lgStrSQL = lgStrSQL & " and		PAY_TYPE =	"	& FilterVar(arrColVal(2), "''", "S")
	lgStrSQL = lgStrSQL & " and		DEPT_CD =	"	& FilterVar(arrColVal(3), "''", "S")
	lgStrSQL = lgStrSQL & " and		BIZ_AREA_CD =	"	& FilterVar(arrColVal(4), "''", "S")
	lgStrSQL = lgStrSQL & " and		ORG_CHANGE_ID =	"	& FilterVar(arrColVal(5), "''", "S")
	lgStrSQL = lgStrSQL & " and		INTERNAL_CD =	"	& FilterVar(arrColVal(6), "''", "S")
	lgStrSQL = lgStrSQL & " and		ACCT_TYPE =	"	& FilterVar(arrColVal(7), "''", "S")
	...
	End if	
		...
		lgStrSQL = "INSERT INTO a_expected_year_bonus ("
...
Sub SubBizSaveMultiUpdate(arrColVal)
    ...
    '--------------------------------------------------------------------------------------------------------
    lgStrSQL = "UPDATE  a_expected_year_bonus "
    lgStrSQL = lgStrSQL & " SET DEPT_CD = " & FilterVar(UCase(arrColVal(3)), "''", "S") & ","
    lgStrSQL = lgStrSQL & "     BIZ_AREA_CD = " & FilterVar(UCase(arrColVal(4)), "''", "S") & ","
    lgStrSQL = lgStrSQL & "     ORG_CHANGE_ID = " & FilterVar(UCase(arrColVal(5)), "''", "S") & ","
    lgStrSQL = lgStrSQL & "     INTERNAL_CD = " & FilterVar(UCase(arrColVal(6)), "''", "S") & ","
    lgStrSQL = lgStrSQL & "     ACCT_TYPE = " & FilterVar(UCase(arrColVal(7)), "''", "S") & ","
    lgStrSQL = lgStrSQL & "     AMT = " & UNIConvNum(arrColVal(8),0)
    lgStrSQL = lgStrSQL & " WHERE  YYYY = " & FilterVar(lgKeyStream(0), "''", "S")
    lgStrSQL = lgStrSQL & "  AND  PAY_TYPE = " & FilterVar(UCase(arrColVal(2)), "''", "S")
    lgStrSQL = lgStrSQL & "  AND  DEPT_CD = " & FilterVar(UCase(arrColVal(9)), "''", "S")
    lgStrSQL = lgStrSQL & "  AND  ACCT_TYPE = " & FilterVar(UCase(arrColVal(10)), "''", "S")
...
Sub SubBizSaveMultiDelete(arrColVal)
    ...
    'A developer must define field to update record
    '--------------------------------------------------------------------------------------------------------
    lgStrSQL = "DELETE  a_expected_year_bonus "
           ...
           iSelCount = C_SHEETMAXROWS_D + C_SHEETMAXROWS_D *  lgStrPrevKeyIndex + 1
            ...
            Select Case Mid(pDataType,2,1)
                Case "R"
                    ...
                    lgStrSQL = lgStrSQL & "                   isnull(a.amt,0) as amt, "
                    lgStrSQL = lgStrSQL & "                   isnull(sum(d.pay_amt),0) as pay_amt "
                    lgStrSQL = lgStrSQL & "             from  a_expected_year_bonus a, "
                    lgStrSQL = lgStrSQL & "                   b_minor b, "
                    lgStrSQL = lgStrSQL & "                   b_acct_dept c, "
                    lgStrSQL = lgStrSQL & "                   a_paid_year_bonus d "
                    ...
                    lgStrSQL = lgStrSQL & "              and  a.yyyy *= d.yyyy "
                    lgStrSQL = lgStrSQL & "              and  a.pay_type *= d.pay_type "
                    lgStrSQL = lgStrSQL & "              and  a.dept_cd *= d.dept_cd "
                    lgStrSQL = lgStrSQL & "              and  a.acct_type *= d.acct_type "
                    ...
                    lgStrSQL = lgStrSQL & "                   a.org_change_id, "
                    lgStrSQL = lgStrSQL & "                   a.internal_cd, "
                    lgStrSQL = lgStrSQL & "                   isnull(a.amt,0) "
                Case "T"
                    lgStrSQL = "Select                    isnull(sum(a.amt),0) as amt "
                    lgStrSQL = lgStrSQL & "         from  a_expected_year_bonus a, "
                    lgStrSQL = lgStrSQL & "               b_minor b, "
                    lgStrSQL = lgStrSQL & "               b_acct_dept c "
                    lgStrSQL = lgStrSQL & "        where  a.acct_type = b.minor_cd "
       ...
       Case "<%=UID_M0001%>"                                                         '☜ : Query
          If Trim("<%=lgErrorStatus%>") = "NO" Then
              With Parent
                ...
                .frm1.txtExpectAmt.text = "<%=txtExpectAmt%>"
                .DBQueryOk
	         End with
          End If
       Case "<%=UID_M0002%>"                                                         '☜ : Save
