...
Const BIZ_PGM_ID    = "a5955mb1_ko441.asp"
...
Function SpreadWorkingChk()
    ...
    Dim strBizArea
    Dim strJobFg, strProgreessFg

    Call ExtractDateFrom(frm1.txtWork_dt.Text,frm1.txtWork_dt.UserDefinedFormat,Parent.gComDateType,strYear,strMonth,strDay)
    strYYYYMM  = strYear & strMonth
    strBizArea = Trim(frm1.txtBizAreaCd.value)
    ...
    with frm1.vspdData
	    For iRows = 1 to .MaxRows
	        ...
	        IntRetCd = CommonQueryRs("ISNULL(C.JOB_FG,'0') AS JOB_FG, UPPER(ISNULL(C.PROGRESS_FG,'N')) AS PROGRESS_FG, Case When ISNULL(C.JOB_FG,'0') = '1'  and UPPER(ISNULL(C.PROGRESS_FG,'N' )) = 'Y'  Then 'Y'  Else 'N'  End AS PROGRESS_FG1 "," B_MINOR A (nolock), A_JOB_RESULT C (nolock) ","  A.MINOR_CD *= C.JOB_CD AND A.MAJOR_CD = 'A1032' AND A.MINOR_CD = '" & strJobCd & "' AND C.YYYYMM = '" & strYYYYMM & "' AND C.BIZ_AREA_CD = '" & strBizArea  & "' ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	        ...
	        If IntRetCd = False Then
	            ...
	            Exit Function
	        Else
	            strJobFg       = Trim(Replace(lgF0,Chr(11),""))
	            strProgreessFg = Trim(Replace(lgF1,Chr(11),""))
	        End If

		if strProgreessFg = "Y" AND run_fg = 1  Then 
		    IntRetCD = DisplayMsgBox("XA0001","X","X","X")  '기작업구분이 Y 인 단계는 작업실행을 할 수 없습니다. 작업취소한 후 재작업하세요
		    Exit Function
		End If

		if strProgreessFg = "N" AND cancel_fg = 1  Then 
		    IntRetCD = DisplayMsgBox("XA0001","X","X","X")  '기작업구분이 N 인 단계는 작업취소를 할 수 없습니다
		    Exit Function
		End If
					...
					<TD NOWRAP  CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<TD NOWRAP  background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>월차결산작업(KO441)</font></td>
