...
Const BIZ_PGM_ID    = "a5956mb1_ko441.asp"
...
Dim IntRetCD1
	...
	IntRetCD1= CommonQueryRs("B_MINOR.MINOR_CD,B_MINOR.MINOR_NM","B_MINOR,B_CONFIGURATION ","B_MINOR.MINOR_CD = B_CONFIGURATION.MINOR_CD AND " & _
					"B_MINOR.MAJOR_CD = 'A1006' AND B_CONFIGURATION.SEQ_NO = 2 AND B_CONFIGURATION.REFERENCE = 'PP' "  _
...
Sub vspddata_Change(ByVal Col, ByVal Row)
	...
	Select Case Col
		  ...
		  If RcptType <> "" Then	
			IntRetCD  =  CommonQueryRs( "REFERENCE" , "B_CONFIGURATION  " , "MAJOR_CD = 'A1006' AND MINOR_CD = '" & Ucase(RcptType) & "' AND SEQ_NO = 4 ", lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) 
			...
			Else
				...
				IntRetCD = CommonQueryRs( "minor_nm" , "B_MINOR a, B_CONFIGURATION b " , "a.minor_cd = b.minor_cd and a.major_cd = 'A1006' and b.seq_no = 2 and b.reference = 'PP' AND a.MINOR_CD = '" & Ucase(RcptType) & "' ", lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) 
...
Sub subVspdSettingChange(ByVal Col , ByVal Row,  ByVal Row2, Byval varData)	
	...
	For CurRow = Row To Row2
		...
		If CommonQueryRs( "REFERENCE" , "B_CONFIGURATION  " , "MAJOR_CD = 'A1006' AND MINOR_CD = '" & strval & "' AND SEQ_NO = 4 ", lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) Then 
...
Sub txtDocCur_OnChangeASP()
    ...
    IF CommonQueryRs( "CURRENCY_DESC" , "B_CURRENCY" ,  " CURRENCY = '" & frm1.txtTradeCur.value & "'" , lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) Then
	...
	Select Case iWhere
	    Case 1
	    	...
	    	arrParam(3) = ""									' Name Cindition
   	        arrParam(4) = "a.minor_cd = b.minor_cd and a.major_cd = 'A1006' and b.seq_no = 2 and b.reference = 'PP' "          <%' Where Condition%>	       
...
'	Call InitComboBox()
	Call SetToolbar("1110100000111111")                                                     '☆: Developer must customize
...
Function FncNew()
	...
	Call SetToolbar("1110100000111111")
...
'	End If	
    ...
    strSelect = strSelect & "isnull(case t.loc_cur when " & FilterVar(ucase(trim(frm1.txtTradeCur.value)),"''", "S") & " Then 1 "
    strSelect = strSelect & "   Else  Case t.xch_rate_fg " 
    strSelect = strSelect & "         When 'M' Then ( SELECT isnull(STD_RATE,0) "
    strSelect = strSelect & "                FROM    b_monthly_exchange_rate (nolock) "
    strSelect = strSelect & "                WHERE    apprl_yrmnth     = CONVERT (varchar(06), " & FilterVar(FrDt,"'29991231'", "S") & ", 112) "
    strSelect = strSelect & "                and    from_currency    = " & FilterVar(ucase(trim(frm1.txtTradeCur.value)),"''", "S") 
    ...
    strSelect = strSelect & "                WHERE    apprl_dt        = " & FilterVar(FrDt,"'29991231'", "S")
    strSelect = strSelect & "                and    from_currency    = " & FilterVar(ucase(trim(frm1.txtTradeCur.value)),"''", "S") 
    strSelect = strSelect & "                and    to_currency    = t.loc_cur ) "
...
Function DbQuery()
    ...
    strVal = strVal     & "&txtMaxRows="         & 0         '☜: Max fetched data
...
Function BtnPopUp(Inobj,iRequired)
	...
	Case 7			
		...
		arrParam(3) = ""
		arrParam(4) = "A.COST_CD = B.COST_CD AND A.ORG_CHANGE_ID = (SELECT CUR_ORG_CHANGE_ID from B_COMPANY) AND B.BIZ_AREA_CD =  " & FilterVar(Trim(frm1.txtCostCd.value),"''","S") 
		arrParam(5) = "귀속부서"								
...
Function OpenBankAcct(byVal strCode,byVal strCard)
	...
	arrParam(4) = arrParam(4) & "AND (C.DPST_FG = 'SV' OR C.DPST_FG ='ET') " 
	arrParam(4) = arrParam(4) & "AND C.DPST_TYPE IN ('D1','D2','D3') " 
...
Function OpenNoteNo(byVal strCode, byVal strCard)
	...
	IF Ucase(strCard) = "CP"	Then
		...
		arrParam(4) = "A.NOTE_STS = 'BG' AND A.NOTE_FG = 'CP' AND A.BP_CD = C.BP_CD AND A.BANK_CD = B.BANK_CD"				
		arrParam(5) = "지불구매카드번호"			
	...
	Elseif  Ucase(strCard) = "NP"	Then
		...
		arrParam(4) = "A.NOTE_STS = 'BG' AND A.NOTE_FG = 'D3' AND A.BP_CD = C.BP_CD AND A.BANK_CD = B.BANK_CD"				
		arrParam(5) = "지급어음번호"			
	...
	Else
		...
		arrParam(4) = "A.NOTE_STS = 'ED' AND A.NOTE_FG = 'D1' AND A.BP_CD = C.BP_CD AND A.BANK_CD = B.BANK_CD"				
		arrParam(5) = "배서어음번호"			
...
Function txtDept1_Onchange()
	...
	strWhere	= strWhere & " from b_acct_dept where org_change_dt <='" & UniConvDateToYYYYMMDD(.fpDateTime.Text, parent.gDateFormat,"") & "'))"			
...
Function txtInsuerTp_Onchange()
    ...
    Else
        IntRetCD= CommonQueryRs(" MINOR_NM "," B_MINOR","  MAJOR_CD = 'A1030' AND MINOR_CD = " & FilterVar(Trim(frm1.txtInsuerTp.value),"''","S") ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) 
...
Function txtCustomCd_Onchange()
    ...
    Else
        IntRetCD= CommonQueryRs(" BP_NM "," B_BIZ_PARTNER ","  BP_TYPE <= 'CS' AND  BP_CD = " & FilterVar(Trim(frm1.txtCustomCd.value),"''","S") ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) 
...
Function txtTradeCur_Onchange()
    Dim IntRetCd
		...
		Else
		    ...
		    IntRetCD= CommonQueryRs(" CURRENCY_DESC "," B_CURRENCY "," CURRENCY = " & FilterVar(Trim(frm1.txtTradeCur.value),"''","S") ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Function txtInsureAcct_Onchange()
    Dim IntRetCd
		...
		Else
		    IntRetCD= CommonQueryRs(" ACCT_CD,ACCT_NM "," A_ACCT "," ACCT_CD = " & FilterVar(Trim(frm1.txtInsureAcct.value),"''","S")  & " and DEL_FG <> 'Y'" ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6) 		    
...
Dim IntRetCd, IntRetCD1
	...
	IntRetCD= CommonQueryRs(" CURRENCY "," B_BIZ_PARTNER ","  BP_TYPE <= 'CS' AND BP_CD =  " & FilterVar(Trim(frm1.txtCustomCd.value),"''","S") ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
...
Dim strSelect, strFrom, strWhere
	...
	Select Case Ucase(InType)
		...
		Case Else
			...
			Else
				IntRetCD= CommonQueryRs(" LOC_CUR "," B_COMPANY "," CO_CD LIKE '%' " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)				
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0 onclick="ClickTab1()" >
							<TR>
								...
								<td background="../../image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>보험정보수정(KO441)</font></td>
				...
				<TR>
					<TD WIDTH=100% VALIGN=TOP>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								<TD CLASS=TD5 NOWRAP>보험코드</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtInsuerCd1" TYPE=TEXT SIZE="20" MAXLENGTH="20"   TAG="24XXXU" ALT="보험코드"></TD>
								<TD CLASS=TD5 NOWRAP>보험명</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtInsuerNm1" TYPE=TEXT SIZE="30" MAXLENGTH="30"   TAG="24XXXU" ALT="보험명"></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>보험종류</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtInsuerTp" TYPE=TEXT SIZE=10  MAXLENGTH="2" TAG="24XXXU" ALT="보험종류"><IMG SRC="../../image/btnPopup.gif" NAME="btnInsuerTp" align=top TYPE="BUTTON" ONCLICK ="vbscript:Call BtnPopUp(frm1.txtInsuerTp,1)">&nbsp;<INPUT TYPE=TEXT NAME="txtInsuerTpNm"  SIZE="20" MAXLENGTH="50" TAG="24"></TD>
								<TD CLASS=TD5 NOWRAP>보험료계정</TD>
								<TD CLASS=TD6 NOWRAP><INPUT TYPE=TEXT NAME="txtInsureAcct" SIZE="10"  MAXLENGTH="20" TAG="24XXXU" ALT="보험료계정" OnChange="txtInsureAcct_Onchange()"><IMG SRC="../../image/btnPopup.gif" NAME="btnInsureAcct" align=top TYPE="BUTTON" ONCLICK ="vbscript:Call BtnPopUp(frm1.txtInsureAcct,8)">&nbsp;<INPUT TYPE=TEXT NAME="txtInsureAcctNm" SIZE="20" MAXLENGTH="50" TAG="24"></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>거래처</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtCustomCd" TYPE=TEXT SIZE=10 MAXLENGTH="10" TAG="24XXXU" ALT="거래처"><IMG SRC="../../image/btnPopup.gif" NAME="btnCustomCd" align=top TYPE="BUTTON" ONCLICK ="vbscript:Call OpenBp(frm1.txtCustomCd.value,3)">&nbsp;<INPUT TYPE=TEXT NAME="txtCustomNm"  SIZE="20" MAXLENGTH="50" TAG="24"></TD>
								<TD CLASS=TD5 NOWRAP>전표일자</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDT%> id=fpDateTime1 NAME="fpDateTime" style="HEIGHT: 20px; WIDTH: 100px" tag="24X1X" Title="FPDATETIME" ALT="전표일자"></OBJECT></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>거래통화</TD>
								<TD CLASS=TD6 NOWRAP><INPUT TYPE=TEXT NAME="txtTradeCur" SIZE="10"  MAXLENGTH="3" TAG="24XXXU" ALT="거래통화"><IMG SRC="../../image/btnPopup.gif" NAME="btnTradeCur" align=top TYPE="BUTTON" ONCLICK ="vbscript:Call BtnPopUp(frm1.txtTradeCur,2)">&nbsp;<INPUT TYPE=TEXT NAME="txtTradeCurNm" SIZE="20" MAXLENGTH="50" TAG="24"></TD>
								<TD CLASS=TD5 NOWRAP>환율</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDS%> id=txtExRate1 name=txtExRate style="HEIGHT: 20px; RIGHT: 0px; TOP: 0px; WIDTH: 150px" title="FPDOUBLESINGLE" ALT="환율" tag="24X5"></OBJECT></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>발의부서</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtDept1" TYPE=TEXT SIZE=10  MAXLENGTH="10" TAG="24XXXU" ALT="발의부서"><IMG SRC="../../image/btnPopup.gif" NAME="btnDept1" align=top TYPE="BUTTON" ONCLICK ="vbscript:Call OpenDept(frm1.txtDept1.value,1)">&nbsp;<INPUT TYPE=TEXT NAME="txtDept1Nm"  SIZE="20" MAXLENGTH="50" TAG="24"></TD>
							...
							<TR>
								...
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDS%> id=txtCntAmt1 name=txtCntAmt style="HEIGHT: 20px; RIGHT: 0px; TOP: 0px; WIDTH: 150px" title="FPDOUBLESINGLE" ALT="계약금액" tag="24X2"></OBJECT></TD>
								<TD CLASS=TD5 NOWRAP>계약액(자국)</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDS%> id=txtLocCntAmt1 name=txtLocCntAmt style="HEIGHT: 20px; RIGHT: 0px; TOP: 0px; WIDTH: 150px" title="FPDOUBLESINGLE" ALT="계약액(자국)" tag="24X2"></OBJECT></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>보험료</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDS%> id=txtAmt1 name=txtAmt style="HEIGHT: 20px; RIGHT: 0px; TOP: 0px; WIDTH: 150px" title="FPDOUBLESINGLE" ALT="보험료" tag="24X2"></OBJECT></TD>
								<TD CLASS=TD5 NOWRAP>보험료(자국)</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDS%> id=txtLocAmt1 name=txtLocAmt style="HEIGHT: 20px; RIGHT: 0px; TOP: 0px; WIDTH: 150px" title="FPDOUBLESINGLE" ALT="보험료(자국)" tag="24X2"></OBJECT></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>계약기간</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDT%> id=txtCntFrom1 NAME="txtCntFrom" style="HEIGHT: 20px; WIDTH: 100px" tag="24X1X" Title="FPDATETIME" ALT="계약기간시작"></OBJECT>~
									<OBJECT classid=<%=gCLSIDFPDT%> id=txtCntTo1 NAME="txtCntTo" style="HEIGHT: 20px; WIDTH: 100px" tag="24X1X" Title="FPDATETIME" ALT="계약기간종료"></OBJECT></TD>
								<TD CLASS=TD5 NOWRAP>적용기간(년월)</TD>
								<TD CLASS=TD6 NOWRAP><OBJECT classid=<%=gCLSIDFPDT%> id=txtFromDt NAME="txtFromDt" style="HEIGHT: 20px; WIDTH: 70px" tag="24X1X" Title="FPDATETIME" ALT="적용기간(년월)시작"></OBJECT>~
									<OBJECT classid=<%=gCLSIDFPDT%> id=txtToDt NAME="txtToDt" style="HEIGHT: 20px; WIDTH: 70px" tag="24X1X" Title="FPDATETIME" ALT="적용기간(년월)종료"></OBJECT></TD>
							</TR>
							<TR>
							...
							<TR>
								...
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtRefNo" TYPE=TEXT SIZE="30" MAXLENGTH="20"   TAG="24XXXU" ALT="관리번호"></TD>
							</TR>
						</TABLE>
					</TD>
				</TR>
