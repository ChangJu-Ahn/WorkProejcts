...
Const BIZ_PGM_ID = "b1261mb1_ko441.asp"												'☆: Head Query 비지니스 로직 ASP명 

Const BIZ_PGM_JUMP_ID1 = "b1263ma1"	'사업자이력등록 
Const BIZ_PGM_JUMP_ID2 = "b1261ma8_KO441"	'거래처조회 
...
Sub SetDefaultVal()
	...
	With frm1
	    ...
	    '2008-03-25 8:47오후 :: hanc
		<% '매출대상/이동 %>
		.rdoReserveMove_N.checked = True
		.txtRadioDepositMove.value = .rdoReserveMove_N.value 
...
Function OpenMinor(ByVal iMinor)
	...
	Case 90												<%' 사업부문 %>
		...
		arrParam(1) = "B_MINOR"							<%' TABLE 명칭 %>
		arrParam(2) = Trim(frm1.txtInd_AREA.value)		<%' Code Condition%>
		arrParam(3) = ""								<%' Name Cindition%>
		arrParam(4) = "MAJOR_CD=" & FilterVar("ZZ003", "''", "S") & " "				<%' Where Condition%>
		arrParam(5) = "사업부문"						<%' TextBox 명칭 %>
		
	    arrField(0) = "MINOR_CD"						<%' Field명(0)%>
	    arrField(1) = "MINOR_NM"						<%' Field명(1)%>
	    
	    arrHeader(0) = "코드"					<%' Header명(0)%>
	    arrHeader(1) = "명"						<%' Header명(1)%>

		frm1.txtInd_AREA.focus 
	Case 1												<%' 업종 %>
		If lgIntFlgMode = parent.OPMD_UMODE Then
			IsOpenPop = False
			Exit Function
	...
	Case 10												<%' 결제방법(구매)%>
		...
		arrParam(1) = "       (		  SELECT A.BP_CD,B.BP_NM "
		arrParam(1) = arrParam(1) & "   FROM "
		arrParam(1) = arrParam(1) & " 	   (SELECT BP_CD = MAX(BP_CD) "
		arrParam(1) = arrParam(1) & " 	      FROM B_BIZ_PARTNER "
		arrParam(1) = arrParam(1) & " 	     WHERE LEN(BP_CD) = 10 "
		arrParam(1) = arrParam(1) & " 	     GROUP BY LEFT(BP_CD,1)) A "
		arrParam(1) = arrParam(1) & " INNER JOIN B_BIZ_PARTNER B ON A.BP_CD = B.BP_CD )  A "
		
		arrParam(2) = ""		<%' Code Condition%>
		arrParam(3) = ""								<%' Name Cindition%>
		arrParam(4) = ""
		arrParam(5) = "거래처코드"					<%' TextBox 명칭 %>
		
	    arrField(0) = "A.BP_CD"						<%' Field명(0)%>
	    arrField(1) = "A.BP_NM"
	    
	    arrHeader(0) = "최종입력거래처"					<%' Header명(0)%>
	    arrHeader(1) = "최종입력거래처명"					<%' Header명(0)%>
...
Function OpenBankCo()
	...
	arrParam(4) = "MAJOR_CD='ZZ008'"							<%' Where Condition%>
...
If arrRet(0) <> "" Then 
	...
	Case 90												<%' 사업부문 %>
		frm1.txtInd_AREA.value = arrRet(0)
		frm1.txtInd_AREANm.value = arrRet(1)
	Case 1												<%' 업종 %>
...
'2008-03-25 8:47오후 :: hanc
Sub rdoReserveMove_Y_OnClick()
	lgBlnFlgChgValue = True
End Sub
'2008-03-25 8:47오후 :: hanc
Sub rdoReserveMove_N_OnClick()
	lgBlnFlgChgValue = True
End Sub
...
Function FncSave() 
    ...
    If Trim(UCase(frm1.txtContry_cd.value)) = "KR" AND Trim(UCase(frm1.txtHConBp_cd.value)) = "" Then	
		If Check_INDI_RGST(Trim(frm1.txtRepre_Rgst.value)) = False  AND UCase(parent.gCountry) = "KR"  Then
		    Dim Check_INDI
		    Check_INDI = DisplaMsgBox("126139", parent.VB_YES_NO, "X", "X")
	...
	With frm1
        ...
        '2008-03-25 8:48오후 :: hanc
		<% '매출대상/이동 %>
		If .rdoReserveMove_Y.checked = True Then
			.txtRadioDepositMove.value = .rdoReserveMove_Y.value 
		ElseIf .rdoReserveMove_N.checked = True Then
			.txtRadioDepositMove.value = .rdoReserveMove_N.value 
		End IF
...
Function DbQueryOk()														<%'☆: 조회 성공후 실행로직 %>
    ...
    if Trim(frm1.txtBankCo.value)  <>  "" then
	    Call CommonQueryRs(" MINOR_NM", "B_MINOR", " major_cd = 'ZZ008' AND MINOR_CD =  " & FilterVar(frm1.txtBankCo.value, "''", "S") & " ", lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
        iCodeArr = Split(lgF0, Chr(11))
        frm1.txtBankCoNm.value = iCodeArr(0)
    End if		
	frm1.txtConBp_cd.focus
					...
					<TD WIDTH=100% VALIGN=TOP>
						...
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								...
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtBp_cd" ALT="거래처코드" TYPE="Text" MAXLENGTH="10" SIZE=10 tag="23XXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnBp_Cd" align=top TYPE="BUTTON"ONCLICK="vbscript:OpenMinor(10)"></TD>								
							...
							<TR>
								...
								<TD CLASS=TD5 NOWRAP>거래처코드(MES)</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtBp_alias_nm" TYPE="Text" ALT="거래처별칭" MAXLENGTH="15" SIZE=30 style="background:#FFE5CB"  tag="23XXX"></TD>
							</TR>
							<TR>
								<TD CLASS=TD5 NOWRAP>사업부문</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtInd_AREA" ALT="사업부문" TYPE="Text" MAXLENGTH="5" SIZE=6 tag="23XXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnBp_Cd" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenMinor 90">&nbsp;<INPUT NAME="txtInd_AREANm" TYPE="Text" MAXLENGTH="30" SIZE=30 tag="24"></TD>							
								<TD CLASS=TD5 NOWRAP>매출대상/이동(MES)</TD>
								<TD CLASS=TD6 NOWRAP>
									<input type=radio CLASS="RADIO" name="rdoReserve_Move_type" id="rdoReserveMove_Y" value="1" tag = "21">
										<label for="rdoReserveMove_Y">이동</label>&nbsp;&nbsp;&nbsp;&nbsp;
									<input type=radio CLASS = "RADIO" name="rdoReserve_Move_type" id="rdoReserveMove_N" value="2" tag = "21" checked>
										<label for="rdoReserveMove_N">매출대상</label></TD>
							</TR>
						...
						<TABLE <%=LR_SPACE_TYPE_60%>>
							...
							<TR>
								...
								<TD CLASS=TD5 NOWRAP>수취인</TD>
								<TD CLASS=TD6 NOWRAP><INPUT NAME="txtCardMemNo" TYPE="Text" ALT="수취인" MAXLENGTH="30" SIZE=30 tag="21XXX"></TD>
							</TR>
							<TR>
							...
							<TR>
								...
								<TD CLASS=TD6 NOWRAP><TEXTAREA NAME="txtBankInfo" tag="21XXXU" ROWS='3' COLS='25' WRAP='hard' Style="BORDER-RIGHT: #3d4373 1px solid;BORDER-TOP: #3d4373 1px solid;BORDER-LEFT: #3d4373 1px solid;BORDER-BOTTOM: #3d4373 1px solid;"></TEXTAREA></TD>
								<TD CLASS=TD5 NOWRAP>&nbsp;</TD>
...
<INPUT TYPE=HIDDEN NAME="txtRadioDepositMove" tag="24" TABINDEX="-1">
