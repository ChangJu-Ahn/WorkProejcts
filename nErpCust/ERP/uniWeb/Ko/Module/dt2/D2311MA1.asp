...
Sub InitSpreadSheet()
	...
	With frm1.vspdData	
		...
		' uniGrid1 setting
		ggoSpread.SSSetCheck		C1_send_check,			"선택",     					 8,  -10, "", True, -1
		ggoSpread.SSSetEdit  	C1_issue_dt_flag, 	"계산서발행여부", 			15, 2,,18
		ggoSpread.SSSetEdit  	C1_success_flag, 		"전송여부", 					15, 2,,18
		ggoSpread.SSSetDate  	C1_process_date,		"전송일",     				13, 2, parent.gDateFormat
		ggoSpread.SSSetEdit		C1_dt_inv_no,			"전송관리번호",				30, ,,50
		ggoSpread.SSSetEdit  	C1_inv_no,				"매입번호",					20, ,,30
		ggoSpread.SSSetCombo		C1_change_reason_cd,	"수정사유",					15
		ggoSpread.SSSetCombo		C1_change_reason_nm,	"수정사유",					15
		ggoSpread.SSSetEdit		C1_change_remark,		"비고1",						20, ,,20
		ggoSpread.SSSetEdit		C1_change_remark2,	"비고2",						20, ,,20
		ggoSpread.SSSetEdit		C1_change_remark3,	"비고3",						20, ,,20
		ggoSpread.SSSetEdit  	C1_posted_flg,       "Posting 여부",				10, 2,,2
		ggoSpread.SSSetEdit  	C1_build_cd,			"발행처",   					15, 2,,18
		ggoSpread.SSSetEdit  	C1_build_nm,			"발행처명",   				15, ,,18
	
		ggoSpread.SSSetDate  	C1_issued_dt,       	"발행일",     				13, 2, parent.gDateFormat
		ggoSpread.SSSetEdit  	C1_vat_inc_flag_nm,	"VAT포함구분",     		15,  ,,15
		...
		ggoSpread.SSSetEdit		C1_tax_biz_area,		"세금신고사업장",			15, 2,,10
	...
	With frm1
		...
		For lRow = 1 To .vspdData.MaxRows
			...
			If .vspdData.text = "1" Then
				...
				If lGrpCnt > 20 Then
					Call DisplayMsgBox("205925", parent.VB_OK, .vspdData.text, "X")   '☜ 바뀐부분 
				End If
			
				.vspdData.Col = C1_posted_flg
				If .vspdData.text = "Y" Then
					.vspdData.Col = C1_inv_no
            	RetFlag = DisplayMsgBox("205924", parent.VB_YES_NO, .vspdData.text, "X")   '☜ 바뀐부분 
								...
								<FIELDSET CLASS="CLSFLD">
									<TABLE <%=LR_SPACE_TYPE_40%>>
										...
										<TR>
											...
											<TD CLASS=TD6 NOWRAP>
												<INPUT CLASS="clstxt" TYPE=TEXT NAME="txtSendMngNo" SIZE=40 MAXLENGTH=40 STYLE="TEXT-ALIGN: Left" tag="11XXXU" ALT="전송관리번호">
											</TD>
			...
			<TR HEIGHT="20">
				<TD WIDTH="100%" >
  					<TABLE <%=LR_SPACE_TYPE_30%>>
						<TR>
							<TD WIDTH=10>&nbsp</TD>
							<TD><BUTTON NAME="btnRetransfer" CLASS="CLSSBTN" OnClick="VBScript:Call fnPublish()">발행</TD>
							<TD WIDTH=10>&nbsp</TD>
						</TR>
  					</TABLE>
				</TD>
