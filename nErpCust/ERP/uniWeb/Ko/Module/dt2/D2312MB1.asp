...
If Request("rdoBillStatus") = "%" Then
	UNIValue(0, 6) = "|"
Else
	UNIValue(0, 6) = FilterVar(UCase(Request("rdoBillStatus")), "''", "S")
End If

If Request("rdoTransferStatus") = "%" Then
	UNIValue(0, 7) = "|"
Else
	UNIValue(0, 7) = FilterVar(UCase(Request("rdoTransferStatus")), "''", "S")
End If

If Request("txtSendMngNo") = "%" Then
	UNIValue(0, 8) = "|"
Else
	UNIValue(0, 8) = FilterVar("%" & UCase(Request("txtSendMngNo")) & "%", "''", "S")
End If

UNIValue(0, 9) = "(I.SUCCESS_FLAG IS NOT NULL OR i.success_flag = 'Y') AND i.dt_inv_no IS NOT NULL"
...
<%		Dim iDx
		For iDx = 0 to rs0.RecordCount - 1 %>
			...
			strData = strData & Chr(11) & "<%=ConvSPChars(rs0("process_date"))%>"
			...
			strData = strData & Chr(11) & "<%=ConvSPChars(rs0("build_nm"))%>"
