...
'		end if
		...
		strWhere = strWhere & "  Where  a.FND_BP_CD = '" & FilterVar(Request("BizPartnerID"), "", "SNM") & "'"

        
		if Request("txtUsrNm2") <> "" then
		strWhere = strWhere & "  and  A.FND_USER_NAME Like  '" & FilterVar(Request("txtUsrNm2"), "", "SNM") & "%'"
		end if
    ...
    If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then
        lgStrPrevKey = ""
           ...
           <Script Language = VBScript>
			  parent.frm1.txtHBpCd.Value = "<%=ConvSPChars(Request("BizPartnerID"))%>"
			  parent.dbqueryNotOk
			</Script>
           <%                      
           Call SetErrorStatus()
    Else
       ...
       Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("user_seq"))                        
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("reg_no"))            
...
Sub SubBizSaveMultiCreate(arrColVal)
    ...
    Err.Clear                   
        ...
        lgStrSQL = ""
		lgStrSQL = lgStrSQL & " INSERT INTO XXSB_DTI_BP_USER("	
		lgStrSQL = lgStrSQL & " FND_BP_CD, " 		
		lgStrSQL = lgStrSQL & " FND_USER_SEQ, " 		
		...
		lgStrSQL = lgStrSQL & UNIConvNum(arrColVal(9),0)     & ","				'Seq				
		lgStrSQL = lgStrSQL & FilterVar(arrColVal(10), "''", "S")     & ","	    'RegNo
...
Sub SubBizSaveMultiUpdate(arrColVal)
   ...
   Err.Clear      
    ...
    lgStrSQL = "UPDATE  XXSB_DTI_BP_USER"    
    lgStrSQL = lgStrSQL & " SET "         
    lgStrSQL = lgStrSQL & " FND_USER_NAME  = " & FilterVar(Trim(arrColVal(2)),"''","S")     & ","    
    lgStrSQL = lgStrSQL & " FND_USER_DEPT_NAME  = " & FilterVar(Trim(arrColVal(3)),"''","S")     & ","    
    lgStrSQL = lgStrSQL & " FND_USER_TEL_NUM  = " & FilterVar(Trim(arrColVal(4)),"''","S")     & ","    
    lgStrSQL = lgStrSQL & " FND_USER_EMAIL  = " & FilterVar(Trim(arrColVal(5)),"''","S")     & ","    
    lgStrSQL = lgStrSQL & " SMART_ID  = " & FilterVar(Trim(arrColVal(6)),"''","S")     & ","                    
    lgStrSQL = lgStrSQL & " FND_REGNO  = " & FilterVar(Trim(arrColVal(10)),"''","S")     & ","        
    lgStrSQL = lgStrSQL & " REMARK  = " & FilterVar(Trim(arrColVal(7)),"''","S")     & ","        
    ...
    lgStrSQL = lgStrSQL & " FND_BP_CD   =   " & FilterVar(arrColVal(8), "''", "S")   
    lgStrSQL = lgStrSQL & " AND FND_USER_SEQ   =   " & UNIConvNum(arrColVal(9),0) 
...
Sub SubBizSaveMultiDelete(arrColVal)
    ...
    Err.Clear                                                                        '☜: Clear Error status
		...
		lgStrSQL = ""
		lgStrSQL = " Delete  XXSB_DTI_BP_USER"
		lgStrSQL = lgStrSQL & " WHERE   "
        lgStrSQL = lgStrSQL & " FND_BP_CD   =   " & FilterVar(arrColVal(2), "''", "S")   
        lgStrSQL = lgStrSQL & " AND FND_USER_SEQ   =   " & UNIConvNum(arrColVal(3),0) 
       'lgStrSQL = lgStrSQL & " AND PU_START_DT   =   '" & UNIConvDate(Trim(arrColVal(4)))  & "'"   
    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
				...
				Case "R"
					   lgStrSQL = ""						   
					   lgStrSQL = lgStrSQL & vbCrLf & " Select TOP " & iSelCount & " a.FND_USER_NAME as user_name, a.FND_USER_DEPT_NAME as dept_name, "						   
					   lgStrSQL = lgStrSQL & vbCrLf & " a.FND_USER_TEL_NUM as tel_num, a.FND_USER_EMAIL as emai_id, a.SMART_ID, a.REMARK,  "
					   lgStrSQL = lgStrSQL & vbCrLf & " a.FND_BP_CD as bp_cd, a.FND_USER_SEQ as user_seq, a.FND_REGNO as reg_no "
					   lgStrSQL = lgStrSQL & vbCrLf & " from  XXSB_DTI_BP_USER A (NOLOCK)   "
       ...
       Case "<%=UID_M0001%>"                                                         '☜ : Query
          If Trim("<%=lgErrorStatus%>") = "NO" Then
              With Parent
                ...
                .frm1.txtHBpCd.Value		= "<%=ConvSPChars(Request("BizPartnerID"))%>"
