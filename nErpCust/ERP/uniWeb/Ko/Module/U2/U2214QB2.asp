    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
                    lgStrSQL =            " select	TOP " & iSelCount  & " c.po_dt, d.item_cd, e.item_nm, e.spec, d.po_unit, a.po_no, a.po_seq_no, "
					lgStrSQL = lgStrSQL & "			a.plan_dvry_qty, a.plan_dvry_dt, d.rcpt_qty, (a.plan_dvry_qty - d.rcpt_qty) unrcpt_qty, "
					lgStrSQL = lgStrSQL & "			isNULL(b.firm_dvry_qty,0) firm_dvry_qty, "
					lgStrSQL = lgStrSQL & "			(a.plan_dvry_qty - d.rcpt_qty - isNULL(b.firm_dvry_qty,0)) remain_qty, a.ret_flg, c.remark, "
					lgStrSQL = lgStrSQL & "			d.po_prc, d.po_prc_flg, d.po_doc_amt , D.SL_CD , G.SL_NM , D.CLS_FLG"  
					lgStrSQL = lgStrSQL & "   from	M_SCM_PLAN_PUR_RCPT a, "
					lgStrSQL = lgStrSQL & "			(select	po_no, po_seq_no, sum(case when rcpt_qty = 0 then confirm_qty else 0 end) firm_dvry_qty "
					lgStrSQL = lgStrSQL & "			 from M_SCM_FIRM_PUR_RCPT where	(plan_dvry_qty - rcpt_qty) > 0 group by po_no, po_seq_no) b, "
					lgStrSQL = lgStrSQL & "			m_pur_ord_hdr c, m_pur_ord_dtl d, b_item e, b_biz_partner f , B_STORAGE_LOCATION G  "
					lgStrSQL = lgStrSQL & "  where	a.split_seq_no = 0 and a.po_no = b.po_no "
					lgStrSQL = lgStrSQL & "  and	a.po_seq_no = b.po_seq_no and a.po_no = c.po_no and a.po_no = d.po_no "
					lgStrSQL = lgStrSQL & "  and	a.po_seq_no = d.po_seq_no and c.bp_cd = f.bp_cd and d.item_cd = e.item_cd and c.release_flg = 'Y' "
					lgStrSQL = lgStrSQL & "	 and	(d.cls_flg = 'N' or (d.cls_flg = 'Y' and d.rcpt_qty > 0)) "
					lgStrSQL = lgStrSQL & "	 and    D.SL_CD = G.SL_CD "
					...
					If lgkeystream(2) <> "" Then
						lgStrSQL = lgStrSQL & "  and G.bp_cd = " & FilterVar(lgKeyStream(2) & "" ,"''", "S")
					End If
