			...
			Select Case Mid(pDataType,2,1)
				Case "R"
					...
					lgStrSQL	= " SELECT	TOP " & iSelCount & " A.GRP_FLG, B.PO_NO, B.PO_SEQ_NO, B.PLAN_DVRY_DT, A.PLAN_DVRY_QTY, " _
								& "			B.RCPT_DT, A.RCPT_QTY, A.UNRCPT_QTY, ISNULL(B.LOT_NO, '') LOT_NO " _
								& "   FROM	( " _
								& "			 SELECT	GROUPING(PO_NO) + GROUPING(PO_SEQ_NO) + GROUPING(SPLIT_SEQ_NO) GRP_FLG, " _
								& "					PO_NO, PO_SEQ_NO, SPLIT_SEQ_NO, " _
								& "					SUM(ISNULL(PLAN_DVRY_QTY,0)) PLAN_DVRY_QTY, SUM(ISNULL(RCPT_QTY,0))	RCPT_QTY, " _
								& "					SUM(ISNULL((CASE WHEN RCPT_QTY = 0 AND RCPT_DT IS NULL THEN CONFIRM_QTY ELSE 0 END),0)) UNRCPT_QTY " _
								...
								& "			 HAVING SUM(ISNULL(PLAN_DVRY_QTY,0)) <> 0 OR SUM(ISNULL(RCPT_QTY,0)) <> 0 " _
								& "					OR SUM(ISNULL((CASE WHEN RCPT_QTY = 0 AND RCPT_DT IS NULL THEN CONFIRM_QTY ELSE 0 END),0)) <> 0 " _
								& "			) A " _
