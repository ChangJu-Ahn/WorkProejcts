...
'Call ServerMesgBox("(20)SubMakeSQLStatements", vbInformation, I_MKSCRIPT)   
	strBpCd		= FilterVar(Request("txtBpCd")&"%", "''", "S")
	strDlvyNo	= FilterVar(Request("txtDlvyNo")&"%", "''", "S")
	strItemCd	= FilterVar(Request("txtItemCd")&"%", "''", "S")	
...
'Call ServerMesgBox(arrCnt , vbInformation, I_MKSCRIPT)	
...
'Call ServerMesgBox(lgStrSQL , vbInformation, I_MKSCRIPT)	
		...
		adoRec.Close 
'Call ServerMesgBox("(10)SubBizSaveMulti" , vbInformation, I_MKSCRIPT)		
...
'Call ServerMesgBox("(20)SubBizSaveMulti" , vbInformation, I_MKSCRIPT)				
		strSql="DELETE FROM M_SCM_DOCUMENT_DTL_KO441 WHERE BP_CD='" & strBpCd & "' and DLVY_NO = '" & strDlvyNo & "' and DOCUMENT_NO = '" & strDocumentNo & "' "
		lgObjConn.execute strSql
		Response.Write strSQL		
'Call ServerMesgBox("(30)SubBizSaveMulti" , vbInformation, I_MKSCRIPT)		
	next
    ...
    End If
Call ServerMesgBox("(40)SubBizSaveMulti" , vbInformation, I_MKSCRIPT)		
%>
<Script Language=vbscript>
	With parent																	    '☜: 화면 처리 ASP 를 지칭함 
		CALL .DbSaveOk()
