...
Dim strBpCd, strDLVY_NO, strDocumentNo, blnStop
Dim strins_person, strTitle, strDOCUMENT_TEXT, strinsrt_user_id, strinsrt_dt
...
strDLVY_NO 		= FilterVar(Request("DLVY_NO"), "''", "S") 
strDocumentNo	= FilterVar(Request("document_no"), "''", "S") 

on error Resume Next
strins_person = "" : strTitle = "": strDOCUMENT_TEXT = "": strinsrt_user_id = "" : strinsrt_dt = ""
blnStop = False

	Call SubOpenDB(lgObjConn)
	Call SubCreateCommandObject(lgObjComm)	

If Not IsNull(Request("DLVY_NO")) And Request("DLVY_NO") <> "" Then
  
	filePath=server.MapPath (".") & "\files\"
    	
	Redim arrFileInfo(100)
	
	lgStrSQL = "SELECT ins_user, document_no, insrt_user_id, DOCUMENT_TEXT, insrt_dt FROM M_SCM_DOCUMENT_HDR_KO441 "
	lgStrSQL =lgStrSQL & " WHERE BP_CD = " & strBpCd
	lgStrSQL =lgStrSQL & "   AND DLVY_NO = " & strDLVY_NO
	...
	If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then                    'If data not exists	
		...
		strTitle = "" & lgObjRs("document_no")
		strinsrt_dt = "" & lgObjRs("insrt_dt")
		strDOCUMENT_TEXT = "" & lgObjRs("DOCUMENT_TEXT")
...
<TABLE  CLASS="BasicTB" CELLSPACING=0 CELLPADDING=0 border=1 >
	<TR HEIGHT=100%>
		...
		<TABLE  CLASS="BasicTB" valign=top CELLSPACING=0>
				...
				lgStrSQL = " SELECT DOCUMENT_NM, DOCUMENT_ID,DOCUMENT_SEQ_NO,DOCUMENT_PATH "
				lgStrSQL = lgStrSQL & "  FROM M_SCM_DOCUMENT_DTL_KO441 "
				lgStrSQL = lgStrSQL & " WHERE BP_CD = " & strBpCd
				lgStrSQL = lgStrSQL & "   AND DLVY_NO = " & strDLVY_NO 
				lgStrSQL = lgStrSQL & "   AND document_no = " & strDocumentNo 
				lgStrSQL = lgStrSQL & " ORDER BY DLVY_NO "
				...
				<TR>
					<TD WIDTH=100% HEIGHT=* valign=top>
						<TABLE WIDTH="100%"  CLASS="BasicTB" CELLSPACING=0>
							<TR>
								<TD HEIGHT="100%">
									<textarea tabIndex="-1"  name="DOCUMENT_TEXT"  readonly><%= strDOCUMENT_TEXT %></textarea>
								</TD>
							</TR>
						</TABLE>
					</TD>
...
'===============================================================
	...
	lgStrSQL = "SELECT DOCUMENT_NM, DOCUMENT_ID, DOCUMENT_PATH "
	lgStrSQL = lgStrSQL & "  FROM M_SCM_DOCUMENT_DTL_KO441 "
	lgStrSQL = lgStrSQL & " WHERE BP_CD = " & strBpCd	
	lgStrSQL = lgStrSQL & "   AND DLVY_NO = " & strDLVY_NO 
	lgStrSQL = lgStrSQL & " ORDER BY BP_CD, DLVY_NO "
