...
Const BIZ_PGM_ID	= "U2122MB1_ko441.asp"			'☆: List & Manage SCM Orders
...
Sub InitSpreadSheet()
	...
	Call InitSpreadPosVariables()
		...
		With frm1
			...
			.vspdData1.MaxCols = C_OrderType + 1
			.vspdData1.MaxRows = 0
            ...
            ggoSpread.SSSetEdit		C_PlantCd,		"납품창고"  , 10
			ggoSpread.SSSetEdit		C_PlantNm,		"납품창고명",	12
			ggoSpread.SSSetEdit		C_SPLITSEQNO,	"분할번호",	12
			ggoSpread.SSSetEdit		C_OrderType,	"구분",	12
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
 	...
 	Select Case Ucase(pvSpdNo)
 		Case "A"
			...
			C_DvryPlanDt	= iCurColumnPos(9) 
			C_DvryQty		= iCurColumnPos(10)
			C_PlantCd		= iCurColumnPos(11)
			C_PlantNm		= iCurColumnPos(12)
			C_SPLITSEQNO	= iCurColumnPos(13)
			C_OrderType		= iCurColumnPos(14)
			...
			Select Case Col
				...
				'Case C_SLYN
				    ...
				    'if .value = "1" or .value = "Y" then
						'ggoSpread.SpreadUnLock		C_SLCD		, .activeRow,	C_SLCD,		.activeRow
						'ggoSpread.SpreadUnLock		C_SLPOP		, .activeRow,	C_SLPOP,		.activeRow
						'ggoSpread.sssetrequired     C_SLCD		, .activeRow,	.activeRow
					'else
					'	ggoSpread.SSSetProtected	C_SLCD		,.activeRow,	.activeRow
					'	ggoSpread.SSSetProtected	C_SLPOP		,.activeRow,	.activeRow
					'end if
...
Function BtnPreview()
	...
	var1 = Trim(frm1.txtdlvyno2.VALUE)
		
	strUrl = strUrl & "DLVY_NO|" & var1
...
Function FncDelete() 
	...
	Call CommonQueryRs( " COUNT(A.DLVY_NO) " , " M_SCM_DLVY_PUR_RCPT A, M_SCM_FIRM_PUR_RCPT B , M_PUR_ORD_HDR C " , _
		" A.DLVY_NO = B.DLVY_NO AND (B.MVMT_NO <> '' OR B.MVMT_NO IS NOT NULL) AND B.PO_NO = C.PO_NO AND A.DLVY_NO = " _
		& FilterVar(frm1.txtdlvyno.VALUE, "", "S") & " AND C.BP_CD = " & FilterVar(frm1.txtBpCd.value, "", "S")  , _
		lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6)
	
	If Cint(lgF0) <> 0 Then
	    Call DisplayMsgBox("174424","X","X","X")
		Exit Function
	End If
	
	Call CommonQueryRs( " ISNULL(SUM(RCPT_QTY),0) " , "  M_SCM_FIRM_PUR_RCPT A , M_PUR_ORD_HDR B " , _
		 " (A.MVMT_NO = '' OR A.MVMT_NO IS NULL) AND A.PO_NO = B.PO_NO AND A.DLVY_NO = " _
		& FilterVar(frm1.txtdlvyno.VALUE, "", "S") & " AND B.BP_CD =  " & FilterVar(frm1.txtBPCD.VALUE, "", "S") , _
			lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6)
           ...
           .Col = 0
			...
			Select Case .Text
			    Case  ggoSpread.DeleteFlag
					...
					Call CommonQueryRs( " ISNULL(MVMT_NO,'') " , "  M_SCM_FIRM_PUR_RCPT A " , " A.DLVY_NO = " & "'" & frm1.txtdlvyno.VALUE & "'" & " AND A.PO_NO = " & "'" & strPoNo & "'" & " AND A.PO_SEQ_NO = " & "'" & strPoSeqNo & "'" & " AND A.SPLIT_SEQ_NO = " & "'" & strSPLITSeqNo & "'"   ,  lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6)
					...
					Call CommonQueryRs( " SUM(RCPT_QTY) " , "  M_SCM_FIRM_PUR_RCPT A " , " A.DLVY_NO = " & "'" & frm1.txtdlvyno.VALUE & "'" & " AND A.PO_NO = " & "'" & strPoNo & "'" & " AND A.PO_SEQ_NO = " & "'" & strPoSeqNo & "'"   ,  lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6)
...
Function FncDeleteRow() 
    ...
    Dim lDelRows
    Dim strPono , strPoSeqNo , strSplitSeqNo , iDx
...
Function DbSave() 
    ...
    'strcurr_dt = UniConvDateToYYYYMMDD("<%=GetSvrDate%>", Parent.gServerDateFormat, "")
       ...
       For lRow = 1 To .vspdData1.MaxRows
           ...
           Select Case .vspdData1.Text
               ...
               Case  ggoSpread.InsertFlag                                      '☜: Update
						...
						.vspdData1.Col = C_SPLITSEQNO	: strVal = strVal & Trim(.vspdData1.value) & parent.gColSep	'12
						.vspdData1.Col = C_LotNo	    : strVal = strVal & Trim(.vspdData1.value) & parent.gRowSep	'13
...
Function OpenReqRef()
	...
	iCalledAspName = AskPRAspName("U2123RA1_ko441")
	
	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", parent.VB_INFORMATION, "U2123RA1", "X")
		lblnWinEvent = False
		Exit Function
	End If
...
Function SetFirmPoRef(strRet)
	...
	const		C_Spec_Ref			= 4
	const		C_LotNo_Ref			= 5   
	const		C_OrderUnit_Ref		= 6   
	const		C_DvryPlanDt_Ref	= 7   
	const		C_DvryQty_Ref		= 8   
	const		C_PlantCd_Ref		= 9   
	const		C_PlantNm_Ref		= 10  
	const		C_SPLITSEQNO_Ref	= 11
	...
	With frm1.vspdData1
		...
		For index1 = 0 to Count1
			...
			Call .SetText(C_LotNo,		Row1, strRet(index1,C_LotNo_Ref))
