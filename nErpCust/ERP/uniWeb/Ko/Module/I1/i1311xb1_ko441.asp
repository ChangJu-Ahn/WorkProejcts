...
Sub SubBizQueryMulti()
    ...
    lgStrSQL = lgStrSQL & " SELECT   X.ITEM_CD,                                                                                        "
    lgStrSQL = lgStrSQL & "          X.ITEM_NM,                                                                                        "
    lgStrSQL = lgStrSQL & "          X.SPEC,                                                                                           "
    lgStrSQL = lgStrSQL & "          X.PRODT_ORDER_NO,                                                                                 "
    lgStrSQL = lgStrSQL & "          X.BASIC_UNIT,                                                                                     "
    lgStrSQL = lgStrSQL & "          X.REQ_QTY,                                                                                        "
    lgStrSQL = lgStrSQL & "          X.ISSUE_QTY,                                                                                      "
    lgStrSQL = lgStrSQL & "          X.REMIND_QTY,                                                                                     "
    lgStrSQL = lgStrSQL & "          X.ITEM_SEQ,                                                                                       "
    lgStrSQL = lgStrSQL & "          X.ISSUE_REQ_NO,                                                                                   "
    lgStrSQL = lgStrSQL & "          X.REQ_DT,                                                                                         "
    lgStrSQL = lgStrSQL & "          X.DEPT_CD,                                                                                        "
    lgStrSQL = lgStrSQL & "          X.DEPT_NM,                                                                                        "
    lgStrSQL = lgStrSQL & "          X.EMP_NO,                                                                                         "
    lgStrSQL = lgStrSQL & "          X.MOV_TYPE,                                                                                       "
    lgStrSQL = lgStrSQL & "          X.EMP_NM                                                                                          "
    lgStrSQL = lgStrSQL & " FROM                                                                                                       "
    lgStrSQL = lgStrSQL & "         (                                                                                                  "
    lgStrSQL = lgStrSQL & "         SELECT   B.ITEM_CD,                                                                                "
    lgStrSQL = lgStrSQL & "                  C.ITEM_NM,                                                                                "
    lgStrSQL = lgStrSQL & "                  C.SPEC,                                                                                   "
    lgStrSQL = lgStrSQL & "                  B.PRODT_ORDER_NO,                                                                         "
    lgStrSQL = lgStrSQL & "                  C.BASIC_UNIT,                                                                             "
    lgStrSQL = lgStrSQL & "                  ISNULL(B.REQ_QTY,0) REQ_QTY,                                                              "
    lgStrSQL = lgStrSQL & "                  ISNULL(B.ISSUE_QTY,0) ISSUE_QTY,                                                          "
    lgStrSQL = lgStrSQL & "                  (ISNULL(B.REQ_QTY,0) - ISNULL(B.ISSUE_QTY,0)) REMIND_QTY,                                 "
    lgStrSQL = lgStrSQL & "                  B.ITEM_SEQ,                                                                               "
    lgStrSQL = lgStrSQL & "                  A.ISSUE_REQ_NO,                                                                           "
    ...
    lgStrSQL = lgStrSQL & "                                      GETDATE()) DEPT_NM,                                                   "
    lgStrSQL = lgStrSQL & "                  A.EMP_NO,                                                                                 "
    lgStrSQL = lgStrSQL & "                  A.MOV_TYPE,                                                                               "
    lgStrSQL = lgStrSQL & "                  D.NAME EMP_NM,                                                                            "
    lgStrSQL = lgStrSQL & " 				 A.TRNS_TYPE                                                                               "
    lgStrSQL = lgStrSQL & "         FROM     M_ISSUE_REQ_HDR_KO441 A,                                                                  "
    lgStrSQL = lgStrSQL & "                  M_ISSUE_REQ_DTL_KO441 B,                                                                  "
    lgStrSQL = lgStrSQL & "                  B_ITEM C,                                                                                 "
    lgStrSQL = lgStrSQL & "                  HAA010T D                                                                                 "
    lgStrSQL = lgStrSQL & "         WHERE    A.PLANT_CD = B.PLANT_CD                                                                   "
    lgStrSQL = lgStrSQL & "                  AND A.ISSUE_REQ_NO = B.ISSUE_REQ_NO                                                       "
    lgStrSQL = lgStrSQL & "                  AND B.ITEM_CD = C.ITEM_CD                                                                 "
    lgStrSQL = lgStrSQL & "                  AND A.EMP_NO = D.EMP_NO                                                                   "
    lgStrSQL = lgStrSQL & "                  AND ISNULL(A.CONFIRM_FLAG,'N') = 'Y'                                                      "
    lgStrSQL = lgStrSQL & "                  AND A.TRNS_TYPE = 'ST'                                                                    "
    lgStrSQL = lgStrSQL & "                  AND A.MOV_TYPE like " & FilterVar(Trim(Request("txtMovType"))&"%","''", "S") & " "
    ...
    lgStrSQL = lgStrSQL & "                  AND (ISNULL(B.REQ_QTY,0) - ISNULL(B.ISSUE_QTY,0)) > 0                                     "
    lgStrSQL = lgStrSQL & "                                                                                                            "
    ...
    lgStrSQL = lgStrSQL & "                  AND A.EMP_NO = D.EMP_NO                                                                   "
    lgStrSQL = lgStrSQL & "                  AND ISNULL(A.CONFIRM_FLAG,'N') = 'Y'                                                      "
    lgStrSQL = lgStrSQL & "                  AND A.TRNS_TYPE = 'OI'                                                                    "
    lgStrSQL = lgStrSQL & "                  AND A.ISSUE_TYPE like " & FilterVar(Trim(Request("txtMovType"))&"%","''", "S") & " "
    lgStrSQL = lgStrSQL & "                  AND A.ISSUE_REQ_NO like " & FilterVar(Trim(Request("txtISSUE_REQ_NO"))&"%","''", "S") & " "
    lgStrSQL = lgStrSQL & "                  AND A.PLANT_CD = " & FilterVar(Trim(Request("txtPlantCd")),"''", "S") & " "
    lgStrSQL = lgStrSQL & "                  AND (ISNULL(B.REQ_QTY,0) - ISNULL(B.ISSUE_QTY,0)) > 0                                     "
    lgStrSQL = lgStrSQL & "         ) X                                                                                                "
    lgStrSQL = lgStrSQL & " WHERE X.TRNS_TYPE = " & FilterVar(Trim(Request("txtDnType")),"''", "S") & " "
    lgStrSQL = lgStrSQL & " ORDER BY X.ISSUE_REQ_NO,                                                                                   "
    lgStrSQL = lgStrSQL & "          X.ITEM_SEQ                                                                                        "
...
'	lgStrSQL = lgStrSQL & " order by d.wc_cd,b.lot_sub_no "
    ...
    Else    
       ...
       Do While Not lgObjRs.EOF
          ...
          lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs("REMIND_QTY"), ggQty.DecPoint, 0)
...
'          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("emp_no1))    '20080312::hanc
