...
'*  2. Function Name        : 
'*  3. Program ID           : i1813qa1_KO441
'*  4. Program Name         : 재고이월비정상 데이타 조회 
'*  5. Program Desc         : 
'*  6. Component List       : 
'*  7. Modified date(First) : 2011/07/16
'*  8. Modified date(Last)  : 
'*  9. Modifier (First)     : song tae ho
'* 10. Modifier (Last)      : 
'* 11. Comment
...
Const BIZ_PGM_ID		= "i1813qb1_KO441.asp"                 '☆: 비지니스 로직 ASP명 
...
Sub SetDefaultVal()
	...
	Dim StartDate
	StartDate = "<%=GetSvrDate%>"     'UNIDateAdd("m", -1,"<%=EndDate%>", parent.gServerDateFormat)
...
'2008-05-22 1:06오후 :: hanc :: 12->15 &  	ggAmtOfMoneyNo->ggQtyNo 	
		...
		ggoSpread.SSSetFloat  C_TransStockQty,	"전월말재고",		15, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec		
		ggoSpread.SSSetFloat  C_InProdQty,		"당월기초재고",	15, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec
		ggoSpread.SSSetFloat  C_InPurQty,		"차이수량",	15, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec
'		
'		
'		' 그리드 헤더 합침 
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
 	...
 	Case "A"
        ...
        C_SlCd			= iCurColumnPos(4)			'창고
	    C_TransStockQty	= iCurColumnPos(5)			'이월재고
	    C_InProdQty		= iCurColumnPos(6)		'생산입고
	    C_InPurQty		= iCurColumnPos(7)		'구매입고
...
Function DbQuery() 
    ...
    Dim strplantCD,strslcd, stryymm, stritemacct, stritemcd
...
'  	Call ExtractDateFrom(frm1.txtMovToDt.Text,frm1.txtMovToDt.UserDefinedFormat,parent.gComDateType,strYear1,strMonth1,strDay1)
    ...
    With Frm1
        ...
        strslcd = Trim(.txtSLCd.value)
        stryymm =  Trim(strYear&strMonth)
        stritemacct = Trim(.txtItemAcct.value)
        ...
        lgKeyStream = stryymm  & gColSep
        lgKeyStream = lgKeyStream &  stritemcd   & gColSep
        lgKeyStream = lgKeyStream &  strplantcd  & gColSep
        lgKeyStream = lgKeyStream &  strslcd  & gColSep
        lgKeyStream = lgKeyStream &  stritemacct  & gColSep        
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>이월및기초재고차이분조회(S)</font></td>
