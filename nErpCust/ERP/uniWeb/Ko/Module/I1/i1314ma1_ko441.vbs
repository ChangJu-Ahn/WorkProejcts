Const BIZ_PGM_QRY_ID    = "i1314mb1_ko441.asp"					
Const BIZ_PGM_DQRY_ID   = "i1314mb1_ko441.asp"					
Const BIZ_PGM_SAVE_ID   = "i1314mb2_ko441.asp"					
Const BIZ_PGM_DEL_ID    = "i1314mb3_ko441.asp"					
Const BIZ_PGM_LOOKUP_ID	= "i1314mb4_ko441.asp"					
...
Dim C_SeqNo         
Dim C_EntryQtyAfter     '20080313::hanc
Dim C_EntryAmount	'20090817::KimbyungSang
...
Sub InitSpreadPosVariables()
	...
	C_EntryAmount	  = 4	    '20090817::KimbyungSang

	C_EntryQty        = 5       '20080313::hanc
	C_TrnsItemCd      = 6   
	C_TrnsItemPopup   = 7
	C_EntryQtyAfter   = 8       '20080313::hanc
	C_EntryUnit       = 9 
	C_EntryUnitPopup  = 10 
...
Sub SetDefaultVal()
	...
	frm1.txtMovType.Value      = "TX9"
	frm1.txtMovTypeNm.Value    = "재고이동(사업장간금액보정)네패스"
'	frm1.txtMovType.disabled    = True
'    frm1.btnMovType2.disabled    = True
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		.ReDraw = false
		.MaxCols = C_SeqNo + 1	'20080313::hanc::C_SeqNo
		.MaxRows = 0
		...
		ggoSpread.SSSetFloat      C_EntryQty,		"이동전수량",		15, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , "Z"      '20080313::hanc
		...
		ggoSpread.SSSetFloat      C_EntryQtyAfter,		"이동후수량",		15, Parent.ggQtyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , , "Z" '20080313::hanc
		...
		ggoSpread.SSSetEdit 	  C_ItemStatus,     "품목상태",		1, 0
		ggoSpread.SSSetEdit 	  C_TrnsPlantCd,    "TrnsPlantCd",		1, 0
		ggoSpread.SSSetEdit 	  C_TrnsSLCd,       "TrnsSLCd",			1, 0
		ggoSpread.SSSetEdit 	  C_TrnsTrackingNo, "TrnsTrackingNo",	1, 0
		ggoSpread.SSSetEdit 	  C_SeqNo,          "순번",			1, 0

		'20090817::KimbyungSang
		ggoSpread.SSSetFloat      C_EntryAmount,	"이동금액", 15, Parent.ggAmtOfMoneyNo, ggStrIntegeralPart, ggStrDeciPointPart, Parent.gComNum1000, Parent.gComNumDec, , ,"Z"

		
		Call ggoSpread.SSSetColHidden(C_ItemStatus, C_SeqNo, True)
		Call ggoSpread.SSSetColHidden(.MaxCols, .MaxCols, True)
		ggoSpread.SpreadUnLock -1, -1
...
Dim iCurColumnPos
	...
	Select Case UCase(pvSpdNo)
		Case "A"
			...
			C_EntryAmount	  =iCurColumnPos(4)     '20090817::KimbyungSang
			C_EntryQty        =iCurColumnPos(5)     '20080313::hanc
			C_TrnsItemCd      =iCurColumnPos(6)
			C_TrnsItemPopup   =iCurColumnPos(7)
			C_EntryQtyAfter   =iCurColumnPos(8)  '20080313::hanc
			C_EntryUnit       =iCurColumnPos(9) 
			C_EntryUnitPopup  =iCurColumnPos(10) 
			C_ItemSpec        =iCurColumnPos(11)
			C_InvUnit         =iCurColumnPos(12)
			C_TrackingNo      =iCurColumnPos(13)
			C_TrackingNoPopup =iCurColumnPos(14)
			C_LotNo           =iCurColumnPos(15)
			C_LotSubNo        =iCurColumnPos(16)
			C_LotNoPopup      =iCurColumnPos(17)
			C_TrnsLotNo       =iCurColumnPos(18)
			C_TrnsLotSubNo    =iCurColumnPos(19)
			C_ItemStatus      =iCurColumnPos(20)
			C_TrnsPlantCd     =iCurColumnPos(21)
			C_TrnsSLCd        =iCurColumnPos(22)
			C_TrnsTrackingNo  =iCurColumnPos(23)
			C_SeqNo           =iCurColumnPos(24)
...
Sub SetSpreadColor(ByVal pvStartRow, ByVal pvEndRow)    	
   ...
   '20090817::KimbyungSang::Start
	Dim StrCompany
	Dim lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6

	If 	CommonQueryRs("REFERENCE","B_CONFIGURATION", " MAJOR_CD = " & FilterVar("I0001", "''", "S") & "" _
						       & " AND SEQ_NO = " & 97 _
						       & " AND MINOR_CD = " & FilterVar(UCase(frm1.txtMovType.value), "''", "S"), _
	...
	If UCase(frm1.txtMovType.value) = "TX9" Then 
		ggoSpread.SSSetProtected C_EntryQty,       pvStartRow, pvEndRow
		ggoSpread.SSSetProtected C_EntryQtyAfter,  pvStartRow, pvEndRow				'20080313::hanc
		ggoSpread.SSSetProtected C_EntryUnit,      pvStartRow, pvEndRow
	ElseIf UCase(.txtMovType.value) <> "TX9"  Then
		ggoSpread.SSSetRequired	 C_EntryQty,  	   pvStartRow, pvEndRow
		ggoSpread.SSSetRequired	 C_EntryQtyAfter,  pvStartRow, pvEndRow     			'20080313::hanc
...
Function OpenItem2(Byval strCode)
	...
	arrParam(0) = Trim(frm1.txtPlantCd2.value)	'2008-03-17 3:24오후 :: hanc
...
Function SetMoveInvRef1(arrRet)
	...
	With frm1.vspdData
		...
		iLngLoopCnt = Ubound(arrRet, 1)           '☜: Reference Popup에서 선택되어진 Row만큼 추가 
			...
			ggoSpread.SpreadUnLock	 C_EntryQtyAfter,	TempRow + 1,	C_EntryQtyAfter,			.MaxRows        '20080313::hanc
			...
			ggoSpread.SSSetRequired	 C_EntryQtyAfter,	TempRow + 1,	.MaxRows        '20080313::hanc
			...
			ggoSpread.SpreadUnLock	 C_EntryAmount,	TempRow + 1,	C_EntryAmount,		.MaxRows        '20090817::KimbyungSang
			ggoSpread.SSSetProtected C_EntryAmount, TempRow + 1,	.MaxRows				'20090817::KimbyungSang
		...
		For iLngCnt = 0 to iLngLoopCnt - 1
            ...
            '2008-03-26 5:35오후 :: hanc
            '뿌려질때 변경품목, 변경후수량에 값이 뿌려지게 수정

			.Col = 0		:		.Text = ggoSpread.InsertFlag
			.Col = C_TrnsItemCd     :		.text = arrRet(iLngCnt, 0)	
			.Col = C_TrackingNo     :		.text = arrRet(iLngCnt, 2)	
			.Col = C_EntryQtyAfter  :		.text = arrRet(iLngCnt, 3)	
		...
		For IntRows = 1 To .MaxRows
			...
			Select Case Trim(.Text)
				Case ggoSpread.InsertFlag, ggoSpread.UpdateFlag
				   ...
				   '20090817::KimbyungSang::이동금액::Start
					'.Col = C_EntryQty
					...
					.Col = C_EntryQty	
					strVal = strVal & Trim(.Text) & iColSep					
				   '20090817::KimbyungSang::이동금액::End
					...
					strVal = strVal & Trim(frm1.txtTrackingNo.value) & iColSep  '20080313::hanc::iRowSep->iColSep
				   ...
				   '20090817::KimbyungSang::이동금액::Start
					...
					.Col = C_EntryQtyAfter	
					strVal = strVal & Trim(.Text) & iColSep					'20080313::hanc::이동후수량
				   '20090817::KimbyungSang::이동금액::End
