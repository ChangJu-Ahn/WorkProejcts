...
strItem			= FilterVar("%" & Trim(Request("txtItemCd")) & "%", "''", "S")
...
Sub SubMakeSQLStatements()
    ...
    lgStrSQL	=	" SELECT	A.item_cd,C.item_nm,C.spec,A.tracking_no,A.lot_no,A.lot_sub_no,A.base_unit,D.minor_nm,B.document_dt," & _
					"			A.qty,A.price,A.amount,A.sl_cd,A.trns_item_cd,f.item_nm,A.trns_sl_cd,A.item_document_no,A.seq_no " & _
					...
					"			left outer join (select MOV_TYPE, GUI_CONTROL_FLAG, GUI_CONTROL_FLAG3 from I_MOVETYPE_CONFIGURATION where TRNS_TYPE = " & FilterVar("ST", "''", "S") & ") E" & _
					"			on e.MOV_TYPE = a.MOV_TYPE " & _
					"			left outer join B_ITEM F on a.trns_item_cd = f.item_cd " & _
					" WHERE		A.AUTO_CRTD_FLAG = " & FilterVar("N", "''", "S") & " "		& _
					" AND		A.TRNS_TYPE = " & FilterVar("ST", "''", "S") & ""			& _	
					" AND		E.GUI_CONTROL_FLAG3 = " & FilterVar("Y", "''", "S") & " "	& _
					" AND		A.PLANT_CD = "				& strPlantCd & _
					" AND		(B.DOCUMENT_DT BETWEEN " & FromDate & " AND " & ToDate & " ) " & _
					" AND		A.MOV_TYPE >= "				& strMovType & _
					" AND		A.ITEM_CD LIKE "			& strItem & _
