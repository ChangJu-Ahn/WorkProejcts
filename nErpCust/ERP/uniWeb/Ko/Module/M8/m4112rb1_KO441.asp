...
'		strVal = strVal & " AND F.BP_CD = " & FilterVar(Request("txtSppl"), "''", "S") & " "		
		strVal = strVal & " AND A.BP_CD = " & FilterVar(Request("txtSppl"), "''", "S") & " "		
	End If
	
    '추가 2006-07-19
    strVal = strVal & " AND A.MVMT_QTY - A.IV_QTY-(A.AFTER_LC_QTY-A.AFTER_LC_IV_QTY) > 0 "
...
'    UNIValue(0,UBound(UNIValue,2)) = " ORDER BY A.MVMT_RCPT_NO DESC,A.PO_NO DESC,A.PO_SEQ_NO ASC "			  '	UNISqlId(0)의 마지막 ?에 입력됨	
    UNIValue(0,UBound(UNIValue,2)) = " ORDER BY A.MVMT_RCPT_NO DESC "			  '	UNISqlId(0)의 마지막 ?에 입력됨	
