...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "M4111PA401"											' main query(spread sheet에 뿌려지는 query statement)
	...
	If Len(Trim(Request("txtFrRcptDt"))) Then
		strVal = strVal & " AND Z.MVMT_RCPT_DT >= " & FilterVar(UNIConvDate(Request("txtFrRcptDt")), "''", "S") & ""		
	End If		
	
	If Len(Trim(Request("txtToRcptDt"))) Then
		strVal = strVal & " AND Z.MVMT_RCPT_DT <= " & FilterVar(UNIConvDate(Request("txtToRcptDt")), "''", "S") & ""		
	End If	
	...
	strVal = strVal & "AND B.SUBCONTRA_FLG = " & FilterVar("N", "''", "S") & "  "  
	strVal = strVal & "order by Z.MVMT_RCPT_DT desc " 	  
