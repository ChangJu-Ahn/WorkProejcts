...
Const BIZ_PGM_ID			= "B1Z01MB1_site.asp"											 '☆: 비지니스 로직 ASP명
Const BIZ_PGM_ID_RUN 		= "B1Z010MB1_site.asp" 
...
Sub OpenQueryID()
	...
	iCalledAspName = AskPRAspName("B1Z01OA1_site")

	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", Parent.VB_INFORMATION, "B1Z01OA1_site", "X")
		IsOpenPop = False
		Exit Sub
	End If
...
On Error Resume Next
    ...
    StrSelectLt = Chr(-24100) & Trim(frm1.txtSelect.value)    
    If Trim(frm1.txtFrom.value) <> "" Then
		StrSelect = StrSelect & " FROM " & Trim(frm1.txtFrom.value)
		StrSelectLt = StrSelectLt & Chr(-24099) & Trim(frm1.txtFrom.value)
    End If
    If Trim(frm1.txtWhere.value) <> "" Then
		StrSelect = StrSelect & " Where " & Trim(frm1.txtWhere.value)
		StrSelectLt = StrSelectLt & Chr(-24098) & Trim(frm1.txtWhere.value)
    End If
    If Trim(frm1.txtEtc.value) <> "" Then
		StrSelect = StrSelect & " " & Trim(frm1.txtEtc.value)
		StrSelectLt = StrSelectLt & " " & Trim(frm1.txtEtc.value)
    End If
        
    frm1.StrSelect_RUN.value = StrSelect
    
    FPos = Instr(UCASE(StrSelectLt), Chr(-24100))
	LPos = Instr(UCASE(StrSelectLt), Chr(-24099))
	XPos = Instr(UCASE(StrSelectLt), Chr(-24098))
	...
	StrColList = Trim(MID(Trim(StrSelectLt), FPos + 1, LPos - (FPos + 1)))
	...
	if Instr(arrSelCol(0),"*") > 0 Then
		' where조건이 없는 경우
		if Trim(XPos) = "0" Then
			StrTableNm = Trim(UCase(MID(StrSelectLt, LPos + 4, Len(StrSelectLt))))
		Else
			StrTableNm = Trim(UCase(MID(StrSelectLt, LPos + 4, XPos - (LPos + 4))))
		End If
...
<TABLE <%=LR_SPACE_TYPE_00%>>
					...
					<TD WIDTH=100% HEIGHT=100% valign=top>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							...
							<TR>
								...
								<TD CLASS=TD6 NOWRAP COLSPAN=3><TEXTAREA NAME="txtFrom" rows=2 cols=100 TAG="21X" 
								ALT="FROM절"></TEXTAREA></TD>
							</TR>
							...
							<TD HEIGHT="60%" colspan=4>
								<script language =javascript src='./js/b1z01ma1_site_A_vspdData.js'></script>
							</TD>
							</TR>
							
							<TR>
							<TD HEIGHT=0 colspan=4>
								<script language =javascript src='./js/b1z01ma1_site_A1_vspdData1.js'></script>
							</TD>
						</TR>
	...
	<TR>
		<TD WIDTH=100% HEIGHT=100 <%=BizSize%>>
			<IFRAME NAME="MyBizASP" SRC="../../blank.htm" WIDTH=100% HEIGHT=100 FRAMEBORDER=0 SCROLLING=Yes noresize framespacing=0 tabindex="-1"></IFRAME>
		</TD>
	</TR>
</TABLE>
