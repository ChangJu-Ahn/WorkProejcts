...
Sub SubBizQueryMulti()
    ...
    Else
        ...
        Do While Not lgObjRs.EOF
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("to_sales_grp")) 'cost center
            lgstrData = lgstrData & Chr(11) & ""		 'button
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("sales_grp_nm")) 'cost center
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("to_alloc")) 'cost center
            lgstrData = lgstrData & Chr(11) & ""		 'button
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("item_group_nm")) 'cost center 명 
...
Sub SubBizSaveMultiCreate(arrColVal)
    ...
    lgStrSQL = lgStrSQL & FilterVar(UCase(arrColVal(3)), "''", "S")     & "," & FilterVar("*", "''", "S") & " ,"
...
Sub SubBizSaveMultiUpdate(arrColVal)
    ...
    lgStrSQL = lgStrSQL & " AND TO_SALES_GRP    = " & FilterVar(UCase(arrColVal(3)), "''", "S")
    ...
    Select Case Mid(pDataType,1,1)
        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "R"
                       lgStrSQL = "Select TOP " & iSelCount  & " g.to_sales_grp,c.sales_grp_nm,g.to_alloc, case when b.item_group_nm is null then g.to_alloc else b.item_group_nm end item_group_nm, IsNull(SUM(g.to_data),0) as A "
                       lgStrSQL = lgStrSQL & " From  g_alloc_data g, b_item_group b,b_sales_grp c "
                       lgStrSQL = lgStrSQL & " Where  g.to_alloc *= b.item_group_cd and g.to_sales_grp = c.sales_grp "
                       lgStrSQL = lgStrSQL & " and  g.alloc_kinds = " & FilterVar(4, "''", "S")
                       lgStrSQL = lgStrSQL & " and  g.yyyymm = " & pCode
                       lgStrSQL = lgStrSQL & " group by  g.to_sales_grp,c.sales_grp_nm,g.to_alloc, b.item_group_nm "
                       lgStrSQL = lgStrSQL & " order by g.to_sales_grp,g.to_alloc "
						
               Case "S"
                       lgStrSQL = "Select IsNull(SUM(g.to_data),0) as A "
                       lgStrSQL = lgStrSQL & " From  g_alloc_data g"
                       lgStrSQL = lgStrSQL & " Where  g.alloc_kinds = " & FilterVar(4, "''", "S")
                       lgStrSQL = lgStrSQL & " and  g.yyyymm = " & pCode
