...
Sub FixUNISQLData()
   ...
   IF Trim(lgKeyStream(2)) <> "" Then                 '사업부 
	...
	IF Trim(lgKeyStream(6)) <> "" Then                 '거래처 
  		Call CommonQueryRs("BP_NM","B_BIZ_PARTNER","BP_CD = " & FilterVar(lgKeyStream(6), "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
		If Trim(Replace(lgF0,Chr(11),"")) = "X" then
	...
	UNISqlId(0) = "Gh010MA01"      '품목별 손익비교 
	UNISqlId(1) = "Gh010MA02"      '총계합 
	...
	IF Trim(lgKeyStream(6)) = "" Then					 ' 거래처 
		lgCodeCond =  lgCodeCond & "" 
	ELSE
		lgCodeCond =  lgCodeCond & " and a.bp_cd = " & FilterVar(lgKeyStream(6), "''", "S")
	END IF
...
<Script Language="VBScript">
    ...
    With Parent.Frm1
             ...
             .txtCostNm.Value					= "<%=ConvSPChars(txtCostNm)%>"            
             .txtSalesOrgnm.Value				= "<%=ConvSPChars(txtSalesOrgnm)%>"
             .txtSalesGrpnm.Value				= "<%=ConvSPChars(txtSalesGrpnm)%>"
