...
'*  3. Program ID           : i1531ma1
'*  4. Program Name         : Floor Goods Receipt
'*  5. Program Desc         : VMI Goods Receipt for shop floor directly 
...
Const BIZ_PGM_QRY1_ID	= "i1531mb1.asp"								
Const BIZ_PGM_QRY2_ID	= "i1531mb2.asp"								
Const BIZ_PGM_SAVE_ID	= "i1531mb3.asp"
...
StartDate = UNIDateAdd("D",-7,LocSvrDate, parent.gDateFormat)	
EndDate = UNIDateAdd("D", 7,LocSvrDate, parent.gDateFormat)	
...
Sub InitSpreadSheet(ByVal pvSpdNo)
	...
	If pvSpdNo = "A" Or pvSpdNo = "*" Then
		With frm1.vspdData1 
			...
			ggoSpread.SSSetEdit		C_SLCd	,			"현장창고", 10,,,,2
			ggoSpread.SSSetEdit		C_SLNm,				"현장창고명", 12
...
Function OpenProdOrderNoFrom()
	...
	If IsOpenPop = True Or UCase(frm1.txtProdOrderNoFrom.className) = UCase(parent.UCN_PROTECTED) Then Exit Function
	...
	arrParam(3) = "RL"
	arrParam(4) = "ST"
	arrParam(5) = Trim(frm1.txtProdOrderNoFrom.value)
	arrParam(6) = ""
	arrParam(7) = ""
	arrParam(8) = ""	
...
Function OpenProdOrderNoTo()
	...
	If IsOpenPop = True Or UCase(frm1.txtProdOrderNoTo.className) = UCase(parent.UCN_PROTECTED) Then Exit Function
	...
	arrParam(3) = "RL"
	arrParam(4) = "ST"
	arrParam(5) = Trim(frm1.txtProdOrderNoTo.value)
	arrParam(6) = ""
	arrParam(7) = ""
	arrParam(8) = ""	
...
Function OpenMakerLotNo()
	...
	If arrRet(0) = "" Then
		Call SetActiveCell(frm1.vspdData2,C_MakerLotNo,frm1.vspdData2.ActiveRow,"M","X","X")
		Exit Function
	Else
...
'------------------------------------------  SetProdOrderNoFrom()  -------------------------------------------
Function SetProdOrderNoFrom(byRef arrRet)
	frm1.txtProdOrderNoFrom.Value    = arrRet(0)		
	frm1.txtProdOrderNoFrom.focus
End Function

'------------------------------------------  SetProdOrderNoTo()  -------------------------------------------
Function SetProdOrderNoTo(byRef arrRet)
	frm1.txtProdOrderNoTo.Value    = arrRet(0)		
	frm1.txtProdOrderNoTo.focus
End Function
...
'=======================================================================================================
'   Event Name : txtReqStartDt_DblClick(Button)
'=======================================================================================================
...
'=======================================================================================================
'   Event Name : txtReqEndDt_DblClick(Button)
'=======================================================================================================
...
Sub vspdData1_ScriptLeaveCell(ByVal Col, ByVal Row, Byval NewCol, Byval NewRow, Byval Cancel)
	...
	frm1.txtTOTBP.value				= ""
	frm1.txtTOTPORemainQty.value	= 0
	frm1.txtTOTTempGRQty.value		= 0
	frm1.txtTOTTempPORemainQty.value = 0
	frm1.txtTOTSL.value		= ""
	frm1.txtTOTGRQty.value	= 0
	frm1.txtTOTGOHQty.value = 0
	frm1.txtTOTTempGOHQty.value = 0
...
Sub vspdData2_TopLeftChange(ByVal OldLeft , ByVal OldTop , ByVal NewLeft , ByVal NewTop )
    ...
    if frm1.vspdData2.MaxRows < NewTop + VisibleRowCnt(frm1.vspdData2,NewTop) and lgStrPrevKey21 <> "" Then						
...
Function FncQuery() 
    ...
    If ValidDateCheck(frm1.txtReqStartDt, frm1.txtReqEndDt) = False Then 
   		frm1.txtReqEndDt.focus 
		Exit Function
	End If
    '-----------------------
    ...
    With frm1
		...
		If lgIntFlgMode1 = parent.OPMD_UMODE Then
			strVal =  BIZ_PGM_QRY1_ID &	"?txtMode="				& parent.UID_M0001					& _
										...
										"&txtProdOrderNoFrom="	& Trim(.txthProdOrderNoFrom.Value)	& _
										"&txtProdOrderNoTo="	& Trim(.txthProdOrderNoTo.Value)	& _
										"&txtReqStartDt="		& Trim(.txthReqStartDt.Value)		& _
										"&txtReqEndDt="			& Trim(.txthReqEndDt.Value)			& _
		...
		Else
			strVal =  BIZ_PGM_QRY1_ID & "?txtMode="				& parent.UID_M0001					& _
										...
										"&txtProdOrderNoFrom="	& Trim(.txtProdOrderNoFrom.Value)	& _
										"&txtProdOrderNoTo="	& Trim(.txtProdOrderNoTo.Value)		& _
										"&txtItemCd="			& Trim(.txtItemCd.Value)			& _
										"&txtReqStartDt="		& Trim(.txtReqStartDt.Text)			& _
										"&txtReqEndDt="			& Trim(.txtReqEndDt.Text)			& _
										"&txtMaxRows="			& .vspdData1.MaxRows
		End IF	
...
Function DbDtlQuery1Ok(ByVal FromNo, ByVal ToNo)
    ...
    Call SetActiveCell(frm1.vspdData2,C_GRQty,frm1.vspdData2.ActiveRow,"M","X","X")
	...
	With frm1.vspdData1
		...
		.Col = 0
		If 	.Text <> "" Then
			If CheckItemMaxRow(CurrentRow1, DtlFromNo, DtlToNo) = True Then 
				Call CopyHiddenToGrid2(CurrentRow1, DtlFromNo)
				Exit Function
			End if
		End If
	End With
...
Function TOTVMISLInfo(ByVal VMISLCd)
	...
	With frm1.vspdData2
		...
		frm1.txtTOTGRQty.value	= UNIFormatNumber(LngTOTSLGRQty,ggQty.DecPoint,-2,0,ggQty.RndPolicy,ggQty.RndUnit)
		frm1.txtTOTGOHQty.value = UNIFormatNumber(LngTOTGOHQty,ggQty.DecPoint,-2,0,ggQty.RndPolicy,ggQty.RndUnit)
		frm1.txtTOTTempGOHQty.value = UNIFormatNumber((LngTOTGOHQty - LngTOTSLGRQty),ggQty.DecPoint,-2,0,ggQty.RndPolicy,ggQty.RndUnit)
	End With
	TOTVMISLInfo = True	
End Function
...
Function TOTBPInfo(ByVal Row2, ByVal StrBPCd)
	...
	With frm1.vspdData2
		...
		frm1.txtTOTPORemainQty.value	= UNIFormatNumber(LngTOTPORemainQty2,ggQty.DecPoint,-2,0,ggQty.RndPolicy,ggQty.RndUnit)
		frm1.txtTOTTempGRQty.value		= UNIFormatNumber(LngTOTTempGRQty2,ggQty.DecPoint,-2,0,ggQty.RndPolicy,ggQty.RndUnit)
		frm1.txtTOTTempPORemainQty.value = UNIFormatNumber(LngTOTTempPORemainQty2,ggQty.DecPoint,-2,0,ggQty.RndPolicy,ggQty.RndUnit)
	End With
	TOTBPInfo = True	
End Function
...
Function CheckPOItem(ByVal INSRow)
	...
	With frm1.vspdData2
		...
		'For i = 1 To .MaxRows
		'	If i = .ActiveRow Then i = i + 1
		'	.Row = i
		'	.Col = C_INSNo
		'	If UNICDbl(.Text) = INSRow Then
		'		.Col = C_VMISLCd
		'		If ChkVMISLCd = Trim(.Text) Then
		'			.Col = C_MakerLotNo
		'			If ChkMakerLotNo = .Text Then
		'				.Col = C_MakerLotSubNo
		'				If ChkMakerLotSubNo = UNICDbl(.Text) Then
		'					.Col = C_GRQty 
							'If UNICDbl(.Text) <> 0 Then
							'	Call Displaymsgbox("162058", "x", "x", "x")'해당 발주내역에 대한 입고예정이 이미 존재합니다..
							'	Call SetFocusToDocument("M") 
							'	.focus
							'	Exit Function
							'End If
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>현장입고등록</font></td>
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								...
								<TR>
									...
									<TD CLASS="TD6">
										<script language =javascript src='./js/i1531ma1_OBJECT4_txtReqStartDt.js'></script>
										&nbsp;~&nbsp;
										<script language =javascript src='./js/i1531ma1_OBJECT5_txtReqEndDt.js'></script>
									</TD>																						
									<TD CLASS=TD5 NOWRAP>제조오더번호</TD>
									<TD CLASS=TD6 NOWRAP><INPUT TYPE=TEXT NAME="txtProdOrderNoFrom" SIZE=20 MAXLENGTH=18 tag="11xxxU" ALT="시작제조오더번호"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnProdOrderFrom" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenProdOrderNoFrom">	
									&nbsp;~&nbsp;</TD>
								</TR>
								<TR>
									...
									<INPUT TYPE=TEXT NAME="txtProdOrderNoTo" SIZE=20 MAXLENGTH=18 tag="11xxxU" ALT="종료제조오더번호"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnProdOrderTo" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenProdOrderNoTo"></TD>	
								</TR>
							</TABLE>
						</FIELDSET>
					</TD>
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE WIDTH=100% CELLSPACING=0 CELLPADDING=0>
								<TR>
									...
									<TD CLASS="TD6">
										<script language =javascript src='./js/i1531ma1_OBJECT4_txtGRDt.js'></script>
									</TD>
						...
						<TABLE <%=LR_SPACE_TYPE_20%>>
							<TR HEIGHT="50%">
								<TD WIDTH="100%" colspan=4>
									<script language =javascript src='./js/i1531ma1_A_vspdData1.js'></script>
								</TD>
							</TR>
							<TR HEIGHT="50%">
								<TD WIDTH="100%" colspan=4>
									<script language =javascript src='./js/i1531ma1_B_vspdData2.js'></script>
								</TD>
							</TR>
							<TR>
									...
									<LEGEND>공급처 합계</LEGEND>
										<TABLE CLASS="TB2" CELLSPACING=0 CELLPADDING=0>
											<TR>
												...
												<TD CLASS=TD6 NOWRAP>
													<script language =javascript src='./js/i1531ma1_OBJECT1_txtTOTPORemainQty.js'></script>
												</TD>																
											</TR>
											<TR>
												<TD CLASS=TD5 NOWRAP>예상잔량</TD>
												<TD CLASS=TD6 NOWRAP>
													<script language =javascript src='./js/i1531ma1_OBJECT1_txtTOTTempPORemainQty.js'></script>
												</TD>																
												<TD CLASS=TD5 NOWRAP>입고예정량</TD>
												<TD CLASS=TD6 NOWRAP>
													<script language =javascript src='./js/i1531ma1_OBJECT1_txtTOTTempGRQty.js'></script>
												</TD>																
											</TR>
										</TABLE>
									</FIELDSET>
									...
									<LEGEND>VMI창고 합계</LEGEND>
										<TABLE CLASS="TB2" CELLSPACING=0 CELLPADDING=0>
											<TR>
												...
												<TD CLASS=TD6 NOWRAP>
													<script language =javascript src='./js/i1531ma1_OBJECT1_txtTOTGOHQty.js'></script>
												</TD>																
											</TR>
											<TR>
												<TD CLASS=TD5 NOWRAP>예상재고량</TD>
												<TD CLASS=TD6 NOWRAP>
													<script language =javascript src='./js/i1531ma1_OBJECT1_txtTOTTempGOHQty.js'></script>
												</TD>																
												<TD CLASS=TD5 NOWRAP>입고수량</TD>
												<TD CLASS=TD6 NOWRAP>
													<script language =javascript src='./js/i1531ma1_OBJECT1_txtTOTGRQty.js'></script>
												</TD>																
											</TR>
										</TABLE>
									</FIELDSET>
...
<script language =javascript src='./js/i1531ma1_C_vspdData3.js'></script>
...
<INPUT TYPE=HIDDEN NAME="cbohMvmtType" tag="24" TABINDEX="-1">
<INPUT TYPE=HIDDEN NAME="txthProdOrderNoFrom" tag="24" TABINDEX="-1">
<INPUT TYPE=HIDDEN NAME="txthProdOrderNoTo" tag="24" TABINDEX="-1">
<INPUT TYPE=HIDDEN NAME="txthItemCd" tag="24" TABINDEX="-1">
<INPUT TYPE=HIDDEN NAME="txthReqStartDt" tag="24" TABINDEX="-1">
<INPUT TYPE=HIDDEN NAME="txthReqEndDt" tag="24" TABINDEX="-1">
</FORM>
