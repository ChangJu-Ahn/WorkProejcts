...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "MM112MA102" 											' header
  	      
    iStrBlNo = Trim(Request("txtIvNo"))    
...
Sub MakeSpreadSheetData()
	...
	Const	M_IV_DTL_IV_NO            =		0
	Const	M_IV_DTL_IV_SEQ_NO        =		1
	Const	M_IV_DTL_PLANT_CD         =		2
	Const	M_IV_DTL_ITEM_CD          =		3
	Const	M_IV_DTL_ITEM_NM          =		4
	Const	M_IV_DTL_SPEC             =		5
	Const	M_IV_DTL_IV_QTY           =		6
	Const	M_IV_DTL_IV_UNIT          =		7
	Const	M_IV_DTL_IV_PRC           =		8
	Const	M_IV_DTL_IV_DOC_AMT       =		9
	Const	M_IV_DTL_VAT_DOC_AMT      =		10
	Const	M_IV_DTL_VAT_TYPE         =		11
	Const	M_IV_DTL_VAT_TYPE_NM      =		12
	Const	M_IV_DTL_VAT_RT           =		13
	Const	M_IV_DTL_VAT_IO_FLG       =		14
	Const	M_IV_DTL_PO_NO		      =		15
	Const	M_IV_DTL_PO_SEQ_NO        =		16
	Const	M_IV_DTL_MVMT_NO	      =		17
	...
	'----- 레코드셋 칼럼 순서 ----------
	'A.IV_NO, A.IV_SEQ_NO, A.PLANT_CD, A.ITEM_CD, D.ITEM_NM, D.SPEC, 
	'A.IV_QTY, A.IV_UNIT, A.IV_PRC, A.IV_DOC_AMT,
	'A.VAT_DOC_AMT, A.VAT_TYPE, E.MINOR_NM, A.VAT_RT, A.VAT_IO_FLG
	'-----------------------------------    
   ...
   Do while Not (rs0.EOF Or rs0.BOF)
        ...
        iRowStr = ""
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_IV_NO))	 									'
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_IV_SEQ_NO))                                    '
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_PLANT_CD))                                   '품목          
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_ITEM_CD))                                                      '품목명        
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_ITEM_NM))                                                      '품목명        
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_SPEC))                                                         '품목규격      
		iRowStr = iRowStr & Chr(11) & UNINumClientFormat(rs0(M_IV_DTL_IV_QTY),ggQty.DecPoint,0)               '수량          
		iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_IV_UNIT))                                      '단위          
		iRowStr = iRowStr & Chr(11) & UNINumClientFormat(rs0(M_IV_DTL_IV_PRC), ggAmtOfMoney.DecPoint,0)       '단가           
		iRowStr = iRowStr & Chr(11) & UNINumClientFormat(rs0(M_IV_DTL_IV_DOC_AMT), ggAmtOfMoney.DecPoint,0)   '금액                   
		...
		iRowStr = iRowStr & Chr(11) & UNINumClientFormat(rs0(M_IV_DTL_VAT_RT), ggExchRate.DecPoint,6)         '부가세율      
		IF rs0(M_IV_DTL_VAT_IO_FLG) = "1" Then
			iStrVatIncFlg = "별도"
		Else
			iStrVatIncFlg = "포함"			
		End if	
		iRowStr = iRowStr & Chr(11) & ConvSPChars(iStrVatIncFlg)				                                        '부가세포함구분        
        iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_PO_NO))
        iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_PO_SEQ_NO))
        iRowStr = iRowStr & Chr(11) & ConvSPChars(rs0(M_IV_DTL_MVMT_NO))
