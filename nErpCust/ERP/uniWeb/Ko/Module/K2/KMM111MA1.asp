...
Dim	C_Check								'선택 
Dim	C_CfmFlg								'확정여부 
Dim	C_BlNo									'공급처세금계산서번호 
Dim	C_BlCur									'화폐 
Dim	C_BlDocAmt								'공급가액 
Dim	C_BlVatDocAmt							'부가세금액 
Dim	C_BlTotDocAmt							'합계금액 
Dim	C_BlVatType								'부가세유형 
Dim	C_BlVatTypeNm							'부가세유형명 
Dim	C_BlVatRt								'부가세율 
...
Sub InitSpreadSheet()
    ...
    Call GetSpreadColumnPos("A")
		...
		ggoSpread.SSSetEdit		C_BlNo						, "공급처세금계산서번호"	, 20
		ggoSpread.SSSetEdit		C_BlCur						, "화폐"					, 10
		ggoSpread.SSSetFloat	C_BlDocAmt					, "공급가액"				, 15,		parent.ggAmtOfMoneyNo,	ggStrIntegeralPart,	ggStrDeciPointPart,	parent.gComNum1000,	parent.gComNumDec,	,	,	"Z"
		ggoSpread.SSSetFloat	C_BlVatDocAmt				, "부가세금액"				, 15,		parent.ggAmtOfMoneyNo,	ggStrIntegeralPart,	ggStrDeciPointPart,	parent.gComNum1000,	parent.gComNumDec,	,	,	"Z"
		ggoSpread.SSSetFloat	C_BlTotDocAmt				, "합계금액"				, 15,		parent.ggAmtOfMoneyNo,	ggStrIntegeralPart,	ggStrDeciPointPart,	parent.gComNum1000,	parent.gComNumDec,	,	,	"Z"
		ggoSpread.SSSetEdit		C_BlVatType					, "부가세유형"				, 18
		ggoSpread.SSSetEdit		C_BlVatTypeNm				, "부가세유형명"			, 18
		ggoSpread.SSSetFloat	C_BlVatRt					, "부가세율"				, 12,		parent.ggExchRateNo,	ggStrIntegeralPart,	ggStrDeciPointPart,	parent.gComNum1000,	parent.gComNumDec,	,	,	"Z"
		ggoSpread.SSSetEdit		C_BlPayMeth					, "결제방법"				, 18
		ggoSpread.SSSetEdit		C_BlPayMethNm				, "결재방법명"				, 18
		ggoSpread.SSSetEdit		C_SoNo						, "수주번호"				, 18
		ggoSpread.SSSetEdit		C_CustPoNo					, "발주번호"				, 18
		ggoSpread.SSSetEdit		C_PoCompanyCd				, "발주법인"				, 18
		ggoSpread.SSSetEdit		C_SoCompanyCd				, "수주법인"				, 18
		ggoSpread.SSSetFloat	C_TaxQty					, "수량"				, 12,		parent.ggQtyNo,		  ggStrIntegeralPart,	ggStrDeciPointPart,	parent.gComNum1000,	 parent.gComNumDec,	,	,	"Z"
...
Sub InitSpreadSheet2()
    ...
    With frm1
	   ...
	   .vspdData2.MaxRows = 0
		...
		ggoSpread.SSSetEdit 	C_BillNo					, "BL번호"				, 15
		ggoSpread.SSSetEdit 	C_BillSeqNo					, "순번"				, 8
...
Sub InitSpreadPosVariables()
	...
	C_BlCur			    = 4			 '화폐 
	C_BlDocAmt		    = 5			 '공급가액 
	C_BlVatDocAmt	    = 6			 '부가세금액 
	C_BlTotDocAmt	    = 7			 '합계금액 
	C_BlVatType		    = 8			 '부가세유형 
	C_BlVatTypeNm	    = 9			 '부가세유형명 
	C_BlVatRt		    = 10		 '부가세율 
	C_BlPayMeth		    = 11		 '결제방법 
	C_BlPayMethNm	    = 12		 '결재방법명 
	C_SoNo			    = 13		 '수주번호 
	C_CustPoNo		    = 14		 '발주번호 
	C_PoCompanyCd	    = 15		 '발주법인 
	C_SoCompanyCd		= 16		 '수주법인 
	C_TaxQty			= 17
	...
	Select Case UCase(pvSpdNo)
		Case "A"
			...
			Call ggoSpread.GetSpreadColumnPos(iCurColumnPos)
				C_Check				=	iCurColumnPos(1)      '화폐 
				C_CfmFlg		    =	iCurColumnPos(2)      '공급가액 
				C_BlNo			    =	iCurColumnPos(3)      '부가세금액 
				C_BlCur			    =	iCurColumnPos(4)      '합계금액 
				C_BlDocAmt		    =	iCurColumnPos(5)      '부가세유형 
				C_BlVatDocAmt	    =	iCurColumnPos(6)      '부가세유형명 
				C_BlTotDocAmt	    =	iCurColumnPos(7)      '부가세율 
				C_BlVatType		    =	iCurColumnPos(8)      '결제방법 
				C_BlVatTypeNm	    =	iCurColumnPos(9)      '결재방법명 
				C_BlVatRt		    =	iCurColumnPos(10)     '수주번호 
				C_BlPayMeth		    =	iCurColumnPos(11)     '발주번호 
				C_BlPayMethNm	    =	iCurColumnPos(12)     '구매요청상태명 
				C_SoNo			    =	iCurColumnPos(13)     '구매요청구분 
				C_CustPoNo		    =	iCurColumnPos(14)     '구매요청구분명 
				C_PoCompanyCd	    =	iCurColumnPos(15)     '발주법인 
				C_SoCompanyCd		=	iCurColumnPos(16)     '수주법인 
		    ...
		    Case ggoSpread.UpdateFlag											'☜: 신규 
				...
				If Trim(.vspdData.Text) = "1" then
					...
					Else
							...
							.vspdData.Col = C_BlVatDocAmt		 								'부가세금액 
							strVal = strVal & UNIConvNum(Trim(.vspdData.Text),0) & parent.gColSep
							.vspdData.Col = C_BlTotDocAmt		 								'합계금액 
							strVal = strVal & Trim(.vspdData.Text) & parent.gColSep
							.vspdData.Col = C_BlVatType		 								'부가세유형 
							strVal = strVal & Trim(.vspdData.Text) & parent.gColSep
							.vspdData.Col = C_BlVatRt		 								'부가세율 
							...
							strVal = strVal & Trim(.vspdData.Text) & parent.gColSep
							.vspdData.Col = C_PoCompanyCd		 								'발주번호 
							strVal = strVal & Trim(.vspdData.Text) & parent.gColSep
							.vspdData.Col = C_SoCompanyCd		 								'발주번호 
							strVal = strVal & Trim(.vspdData.Text) & parent.gColSep
							strVal = strVal & UNIConvDate(Trim(.txtPayDt.text)) & parent.gColSep		'지급일 
							strVal = strVal & parent.gRowSep
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<TD background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>멀티컴퍼니 매입</font></TD>
						...
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><INPUT TYPE=TEXT ALT="매입형태"   NAME="txtIvTypeCd" SIZE=10 MAXLENGTH=5 tag="22NXXU" ><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnIvTypeCd" align=top TYPE="BUTTON"ONCLICK="vbscript:OpenIvTeypCd()" OnMouseOver="vbscript:PopUpMouseOver()" OnMouseOut="vbscript:PopUpMouseOut()">
								     <INPUT TYPE=TEXT ALT="매입형태" NAME="txtIvTypeNm" SIZE=20 MAXLENGTH=50 tag="24X">
...
<!--										<script language =javascript src='./js/kmm111ma1_fpDateTime2_txtBillDt.js'></script>		-->
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP>
									...
									<INPUT TYPE=TEXT ALT="구매그룹" NAME="txtPurGrpNm" SIZE=20 MAXLENGTH=50 tag="24X">
		...
		<TD WIDTH="100%">
			<TABLE <%=LR_SPACE_TYPE_30%>>
				<TR>
					...
					<td WIDTH="*" align="right"><a href="VBSCRIPT:CookiePage(1)">멀티컴퍼니매입조회</a></td>
					<TD WIDTH=10>&nbsp;</TD>
			 </TR>
			</TABLE>
		</TD>
