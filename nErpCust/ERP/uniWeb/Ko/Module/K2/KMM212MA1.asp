...
Sub InitSpreadSheet()
    ...
    Call GetSpreadColumnPos("A")
		...
		ggoSpread.SSSetEdit		C_BlIssueDt                           	,	"B/L접수일"     ,18
		ggoSpread.SSSetEdit		C_LoadingDt                           	,	"선적일"        ,18
		ggoSpread.SSSetEdit		C_Currency                            	,	"화폐"          ,10
		SetSpreadFloatLocal		C_DocAmt                              	,	"B/L금액"       ,15,1,5
		SetSpreadFloatLocal		C_LocAmt                              	,	"B/L자국금액"   ,15,1,5
		SetSpreadFloatLocal		C_XchRate	                          	,	"환율"          ,12,1,5
		...
		ggoSpread.SSSetEdit		C_Beneficiary							,	"수출자"		,18
		ggoSpread.SSSetEdit		C_Applicant								,	"수입자"		,18
		ggoSpread.SSSetEdit		C_PostedYN								,	"확정여부"		,18
		ggoSpread.SSSetEdit		C_RefIvNo								,	"매입번호"		,18
...
Sub InitSpreadSheet2()
    ...
    With frm1
	   ...
	   .vspdData2.MaxRows = 0
		...
		SetSpreadFloatLocal		C_Qty						, "수량"				, 12,1,5
		ggoSpread.SSSetEdit		C_Unit						, "단위"				, 10
		SetSpreadFloatLocal		C_Price						, "단가"				, 15,1,5
		SetSpreadFloatLocal		C_DDocAmt					, "금액"				, 15,1,5
		SetSpreadFloatLocal		C_DLocAmt					, "자국금액"			, 15,1,5
...
Sub InitSpreadPosVariables()
	...
	C_BlNo		            =	4         'B/L번호 
	C_BlDocNo               =	5         'B/L관리번호 
	C_PostedFlg             =	6         'B/L확정여부 
	C_BlIssueDt             =	7         'B/L접수일 
	C_LoadingDt             =	8         '선적일 
	C_Currency              =	9         '화폐 
	C_DocAmt                =	10        'B/L금액 
	C_LocAmt                =	11        'B/L자국금액 
	...
	Select Case UCase(pvSpdNo)
		Case "A"
			...
			Call ggoSpread.GetSpreadColumnPos(iCurColumnPos)
				...
				C_BpCd					=	iCurColumnPos(2)        '수주법인 
				C_BpNm                  =	iCurColumnPos(3)        '수주법인명 
				C_BlNo		            =	iCurColumnPos(4)        'B/L번호 
				C_BlDocNo               =	iCurColumnPos(5)        'B/L관리번호 
				C_PostedFlg             =	iCurColumnPos(6)        'B/L확정여부 
				C_BlIssueDt             =	iCurColumnPos(7)        'B/L접수일 
				C_LoadingDt             =	iCurColumnPos(8)        '선적일 
				C_Currency              =	iCurColumnPos(9)        '화폐 
				C_DocAmt                =	iCurColumnPos(10)       'B/L금액 
				C_LocAmt                =	iCurColumnPos(11)       'B/L자국금액 
				C_XchRate	            =	iCurColumnPos(12)       '환율 
				C_IvType	            =	iCurColumnPos(13)       '매입형태 
				C_IvTypeNm              =	iCurColumnPos(14)       '매입형태명 
				C_PayMethod             =	iCurColumnPos(15)       '결제방법 
				C_PayMethodNm           =	iCurColumnPos(16)       '결제방법명 
				C_PurGrp                =	iCurColumnPos(17)       '구매그룹 
				C_PurGrpNm		        =	iCurColumnPos(18)		'구매그룹명 
...
'				If Trim(UNICDbl(GetSpreadText(.vspdData,C_PostedFlg,lRow,"X","X"))) = "확정" then
'					Call DisplayMsgBox("970021", "X","B/L확정을 취소하세요", "X")
'					.vspdData.Row = lRow
'					.vspdData.Action = 0
'					Call LayerShowHide(0)
'					Exit Function
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<TD background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>멀티컴퍼니BL 확정/삭제</font></TD>
						...
						<TABLE <%=LR_SPACE_TYPE_40%>>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><!--<INPUT TYPE=radio Class="Radio" ALT="B/L확정처리여부" NAME="rdoCfmflg" id = "rdoCfmflg1" Value="A" checked tag="11"><label for="rdoCfmflg1">&nbsp;전체&nbsp;</label> -->
													   ...
													   <INPUT TYPE=radio Class="Radio" ALT="B/L확정처리여부" NAME="rdoCfmflg" id = "rdoCfmflg3" Value="N" tag="11" checked><label for="rdoCfmflg3">&nbsp;미확정&nbsp;</label></TD>
								<TD CLASS="TD5" NOWRAP>발주번호</TD>
								<TD CLASS="TD6" NOWRAP><INPUT TYPE=TEXT ALT="발주번호"   NAME="txtPoNo" SIZE=35 MAXLENGTH=18 tag="11NXXU" ></TD>
				...
				<TR>
					<TD WIDTH=100% HEIGHT=* VALIGN=TOP>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								<TD HEIGHT=10 WIDTH=100% CLASS="TD5" NOWRAP>B/L접수일</TD>
								<TD CLASS="TD6" NOWRAP>
