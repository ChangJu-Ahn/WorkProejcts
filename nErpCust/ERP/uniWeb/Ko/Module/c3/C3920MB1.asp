    ...
    Else
        ...
        Do While Not lgObjRs.EOF
			IF iDx < lgMaxCount + 1 Then
                ...
                lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs(2))		'Basic Unit
                lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs(3))		'Spec
                
                IF txtFlag = "RCPT" Then
					lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs(4),ggUnitCost.DecPoint,0)		'입고단가 
					lgstrData = lgstrData & Chr(11) & UNINumClientFormat(0,ggUnitCost.DecPoint,0)
				ELSE
					lgstrData = lgstrData & Chr(11) & UNINumClientFormat(0,ggUnitCost.DecPoint,0)							
					lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs(4),ggUnitCost.DecPoint,0)		'총평균단가		
				END IF                
                
                lgstrData = lgstrData & Chr(11) & UNINumClientFormat(lgObjRs(5),ggUnitCost.DecPoint,0)		'재고표준단가 
				...
				iDx = iDx + 1
			ELSE	
				lgStrPrevKey = ConvSPChars(lgObjRs(0))
				Exit Do
            End If
	    Loop
    End If
...
Sub SubBizSaveMulti()
	...
	IF iWhere = "S" Then		'☜: 선택반영 
		...
		For iDx = 0 To UBOUND(arrRowVal,1) - 1
		    ...
		    pCode = pCode & FilterVar(arrColVal(0), "''", "S") & ","
...
Sub SubMakeSQLStatements(iWhere,pCode)
    ...
    Select Case iWhere
        Case "QR"
            ...
            lgStrSQL = lgStrSQL & "			a.item_cd,c.item_nm,c.basic_unit,c.spec,a.temp_price_1,d.std_prc"
            lgStrSQL = lgStrSQL & " from	C_RCPT_DIFF a,B_ITEM_BY_PLANT b,B_ITEM c,I_MATERIAL_VALUATION d"
            ...
            lgStrSQL = lgStrSQL & " and		a.item_cd >= " & FilterVar(lgStrPrevKey, "''", "S") 
            ...
            lgStrSQL = lgStrSQL & " and		a.plant_cd = d.plant_cd and a.item_cd = d.item_cd "
            lgStrSQL = lgStrSQL & " and		a.temp_price_1 > 0 "
            lgStrSQL = lgStrSQL & " order by a.item_cd"
        
        Case "QT"
            lgStrSQL =			  " select  top  " & CStr(lgMaxCount + 1) 
            lgStrSQL = lgStrSQL & "			a.item_cd,c.item_nm,c.basic_unit,c.spec,a.temp_price_1,d.std_prc"
            lgStrSQL = lgStrSQL & " from	C_ITEM_BY_PLANT a,B_ITEM_BY_PLANT b,B_ITEM c,I_MATERIAL_VALUATION d"
            lgStrSQL = lgStrSQL & " where	a.yyyymm = " & FilterVar(txtYYYYMM, "''", "S") 
            lgStrSQL = lgStrSQL & " and		a.plant_cd = " & FilterVar(txtPlantCd, "''", "S") 
            lgStrSQL = lgStrSQL & " and		b.item_acct = " & FilterVar(txtItemAccntCd, "''", "S") 
            lgStrSQL = lgStrSQL & " and		a.item_cd >= " & FilterVar(lgStrPrevKey, "''", "S") 
        ...
        Case "UR"
            ...
            lgStrSQL = lgStrSQL & "	set		c.std_prc = a.temp_price_1 ,updt_user_id =  " & FilterVar(gUsrID , "''", "S") & ",updt_dt = getdate() "
			lgStrSQL = lgStrSQL & " from	I_MATERIAL_VALUATION c"
            lgStrSQL = lgStrSQL & " inner join	C_RCPT_DIFF a on a.plant_cd = c.plant_cd and a.item_cd = c.item_cd"
            ...
            IF pCode <> "" Then
				lgStrSQL = lgStrSQL & " and		a.item_cd in " & pCode
			END IF
        
        Case "UT"
            lgStrSQL = lgStrSQL & "	update	c"
            lgStrSQL = lgStrSQL & "	set		c.std_prc = a.temp_price_1,"
			lgStrSQL = lgStrSQL & "			updt_user_id =  " & FilterVar(gUsrID , "''", "S") & ",updt_dt = getdate() "
            lgStrSQL = lgStrSQL & " from	I_MATERIAL_VALUATION c"
            lgStrSQL = lgStrSQL & " inner join	C_ITEM_BY_PLANT a on a.plant_cd = c.plant_cd and a.item_cd = c.item_cd"
            lgStrSQL = lgStrSQL & " inner join  B_ITEM_BY_PLANT b on a.plant_cd = b.plant_cd and a.item_cd = b.item_cd"
            lgStrSQL = lgStrSQL & " where	a.yyyymm = " & FilterVar(txtYYYYMM, "''", "S") 
            lgStrSQL = lgStrSQL & " and		a.plant_cd = " & FilterVar(txtPlantCd, "''", "S") 
            lgStrSQL = lgStrSQL & " and		b.item_acct = " & FilterVar(txtItemAccntCd, "''", "S") 
