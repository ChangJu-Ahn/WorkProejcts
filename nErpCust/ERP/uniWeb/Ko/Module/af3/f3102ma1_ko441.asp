...
Const BIZ_PGM_ID        = "f3102mb1_ko441.asp"						'☆: 비지니스 로직 ASP명 
...
'==========================================================================================================
Sub InitSpreadSheet()
        Call SetZAdoSpreadSheet("F3102MA1_KO441","S","A","V20030410",Parent.C_SORT_DBAGENT, frm1.vspdData, C_MaxKey,"X","X")
...
Function OpenPopUp(Byval strCode, Byval iWhere)
	...
	Select Case iWhere
		Case 0
			...
			arrParam(1) = "B_BANK A(NOLOCK),B_BANK_ACCT	B(NOLOCK),F_DPST C(NOLOCK) " 			' TABLE 명칭 
			arrParam(2) = Trim(strCode)					' Code Condition
			arrParam(3) = ""							' Name Cindition
			arrParam(4) = " A.BANK_CD = B.BANK_CD "
		 	arrParam(4) = arrParam(4) & " AND A.BANK_CD = C.BANK_CD "
		 	arrParam(4) = arrParam(4) & " AND C.PAR_BANK_ACCT_NO = B.BANK_ACCT_NO "
		 	'arrParam(4) = arrParam(4) & " AND B.BANK_ACCT_PRNT = 'Y' "							' Where Condition
...
'			End If
			...
			arrHeader(2) = "모계좌번호"				' Header명(1)
			
		Case 1
			arrParam(0) = "모계좌번호 팝업"			' 팝업 명칭 
			arrParam(1) = "B_BANK A(NOLOCK),B_BANK_ACCT	B(NOLOCK),F_DPST C(NOLOCK)" 			' TABLE 명칭 
			arrParam(2) = Trim(strCode)					' Code Condition
			arrParam(3) = ""							' Name Cindition
			arrParam(4) = " A.BANK_CD = B.BANK_CD "
...
Sub CurFormatNumericOCX()
	...
	With frm1
		...
		If Trim(.txtDocCur.value) = "" Then
		  ...
		  'intRetCD = CommonQueryRs("top 1 B.doc_cur"," F_DPST A, f_dpst_item b, B_BANK_ACCT C "," A.BANK_CD = B.BANK_CD and A.BANK_ACCT_NO = B.BANK_ACCT_NO and A.BANK_CD = C.BANK_CD and A.PAR_BANK_ACCT_NO = C.BANK_ACCT_NO and C.BANK_ACCT_PRNT = 'Y' AND A.bank_cd =  " & FilterVar(strBankCd , "''", "S") & " and A.PAR_bank_acct_no =  " & FilterVar(strBankAcctNo , "''", "S") & " " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
		  intRetCD = CommonQueryRs("doc_cur"," F_DPST "," BANK_ACCT_NO = " & FilterVar(strBankAcctNo , "''", "S") & " " ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
								...
								<TD HEIGHT=20 WIDTH=100%>
									<TABLE <%=LR_SPACE_TYPE_60%>>
										<TR>
											...
											<TD class=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> CLASS=FPDS140 name="txtPreAmt" title=FPDOUBLESINGLE ALT="이월금액" tag="24X7X" id=OBJECT22></OBJECT>');</SCRIPT></TD>
										...
										<TR>
											...
											<TD class=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> CLASS=FPDS140 name="txtRcptAmt" title=FPDOUBLESINGLE ALT="입금합계" tag="24X7X" id=OBJECT22></OBJECT>');</SCRIPT></TD>											
										...
										<TR>
											...
											<TD class=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> CLASS=FPDS140 name="txtPaymAmt" title=FPDOUBLESINGLE ALT="출금합계" tag="24X7X" id=OBJECT22></OBJECT>');</SCRIPT></TD>											
										...
										<TR>
											...
											<TD class=TD6 NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDS%> CLASS=FPDS140 name="txtBalAmt" title=FPDOUBLESINGLE ALT="잔액" tag="24X7X" id=OBJECT22></OBJECT>');</SCRIPT></TD>											
