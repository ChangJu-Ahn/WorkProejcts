...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "F3102MA101"	'예적금조회 
    UNISqlId(1) = "F3102MA102"	'은행명 
    UNISqlId(2) = "F3102MA103"	'이월금액 
    UNISqlId(3) = "F3102MA104"	'입출합계 
    UNISqlId(4) = "F3102MA105"	'계좌번호 
	UNISqlId(5) = "A_GETBIZ"
    UNISqlId(6) = "A_GETBIZ"
...
Sub TrimData()
    ...
    If Trim(Request("txtDocCur")) = "" Then
		strDocCur = ""
	Else
		strDocCur = " AND A.DOC_CUR = " & Filtervar(UCase(Trim(Request("txtDocCur"))), "''", "S")
	End If
...
'	else
'		strWhere = strWhere & " AND A.BIZ_AREA_CD >= " & FilterVar(" ", "''", "S") & " "
	end if
