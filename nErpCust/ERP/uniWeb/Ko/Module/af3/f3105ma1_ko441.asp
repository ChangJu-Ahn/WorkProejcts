...
Const BIZ_PGM_ID = "f3105mb1_ko441.asp"  
...
Function fnButtonExec()
    ...
    if frm1.Rb_WK1.checked = true then
		strVal = strVal & "&txtRadio=" & "C"								'☆: 조회 조건 데이타 
		strVal  = strVal & "&txtOrgChangeId=" & Trim(frm1.hOrgChangeId.value) 
    else
		strVal = strVal & "&txtRadio=" & "D"								'☆: 조회 조건 데이타 
		strVal  = strVal & "&txtOrgChangeId=" & parent.gChangeOrgId
	end if

    if frm1.Rb_WK3.checked = true then
		strVal = strVal & "&txtRadio2=" & "U"								'☆: 조회 조건 데이타 
    else
		strVal = strVal & "&txtRadio2=" & "C"								'☆: 조회 조건 데이타 
	end if      
...
'msgbox strVal
	...
	If frm1.Rb_WK3.checked = True Then
		If frm1.Rb_WK1.checked = True Then
			...
			Call CommonQueryRs("COUNT(*)", "F_DPST_SHEET_KO441(nolock)", "JOB_DATE = " & Filtervar(strFrDt, "''", "S"), lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6) 
'MsgBox "S1 - " & Replace(lgF0, chr(11), "")
			If CInt(Replace(lgF0, chr(11), "")) > 0 Then
				Call LayerShowHide(0) 
				
				RetFlag = DisplayMsgBox("800064", "X","X","X")   '☜ 바뀐부분 
				Exit Function
			End If
		Else
			Call CommonQueryRs("COUNT(*)", "F_DPST_SHEET_KO441(nolock)", "CONF_FG = 'C' AND JOB_DATE = " & Filtervar(strFrDt, "''", "S"), lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6) 
'MsgBox "S2 - " & Replace(lgF0, chr(11), "")
			If CInt(Replace(lgF0, chr(11), "")) > 0 Then
				Call LayerShowHide(0) 
				
				RetFlag = DisplayMsgBox("117531", "X","X","X")   '☜ 바뀐부분 
				Exit Function
			End If			
	...
	Else
		If frm1.Rb_WK1.checked = True Then
			...
			Call CommonQueryRs("COUNT(*)", "F_DPST_SHEET_KO441(nolock)", "JOB_DATE = " & Filtervar(strFrDt, "''", "S") & " AND BIZ_AREA_CD = " & Filtervar(strBizAreaCd, "''", "S") & " AND DEPT_CD = " & Filtervar(strDeptCd, "''", "S"), lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6) 
															   
			If CInt(Replace(lgF0, chr(11), "")) <= 0 Then
				Call LayerShowHide(0) 
				
				RetFlag = DisplayMsgBox("970000", "X","전표처리할 대상","X")   '☜ 바뀐부분 
				Exit Function
			End If
	
				
			Call CommonQueryRs("COUNT(*)", "F_DPST_SHEET_KO441(nolock)", "JOB_DATE = " & Filtervar(strFrDt, "''", "S") & " AND BIZ_AREA_CD = " & Filtervar(strBizAreaCd, "''", "S") & " AND GL_DATE = " & Filtervar(strWkDt, "''", "S") & " AND DEPT_CD = " & Filtervar(strDeptCd, "''", "S") & " AND CONF_FG = 'C' ", lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6)
'MsgBox "S4 - " & Replace(lgF0, chr(11), "")																    
			If CInt(Replace(lgF0, chr(11), "")) > 0 Then
				Call LayerShowHide(0) 
		...
		Else	
			...
			Call CommonQueryRs("COUNT(*)", "F_DPST_SHEET_KO441(nolock)", "JOB_DATE = " & Filtervar(strFrDt, "''", "S") & " AND BIZ_AREA_CD = " & Filtervar(strBizAreaCd, "''", "S") & " AND DEPT_CD = " & Filtervar(strDeptCd, "''", "S") & " AND CONF_FG = 'C' ", lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6) 
'MsgBox "S5 - " & Replace(lgF0, chr(11), "")																   
			If CInt(Replace(lgF0, chr(11), "")) <= 0 Then
				Call LayerShowHide(0) 
				
				RetFlag = DisplayMsgBox("970000", "X","전표처리 취소할 대상","X")   '☜ 바뀐부분 
				Exit Function
			End if				
					
			Call CommonQueryRs("COUNT(*)", "F_DPST_SHEET_KO441(nolock)", "JOB_DATE = " & Filtervar(strFrDt, "''", "S") & " AND BIZ_AREA_CD = " & Filtervar(strBizAreaCd, "''", "S") & " AND DEPT_CD = " & Filtervar(strDeptCd, "''", "S") & " AND CONF_FG = 'C' AND (ISNULL('',GL_NO) <> '' OR ISNULL('',REV_GL_NO) <> '') ", lgF0, lgF1, lgF2, lgF3, lgF4, lgF5, lgF6) 
'MsgBox "S5 - " & Replace(lgF0, chr(11), "")																   
			If CInt(Replace(lgF0, chr(11), "")) > 0 Then
				Call LayerShowHide(0) 
				
				RetFlag = DisplayMsgBox("am0007", "X","X","X")   '☜ 바뀐부분 
				Exit Function
			End if			
		End If				
	End If
					...
					<TD WIDTH=100% HEIGHT=100% valign=top>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="RADIO" CLASS="Radio" NAME=Radio2 ID=Rb_WK3 Checked onclick=radio3_onchange()><LABEL FOR=Rb_WK3>미수수익계산</LABEL>&nbsp;<INPUT TYPE="RADIO" CLASS="Radio" NAME=Radio2 ID=Rb_WK4 onclick=radio4_onchange()><LABEL FOR=Rb_WK4>전표처리</LABEL></TD>
							</TR>
							<TR>
