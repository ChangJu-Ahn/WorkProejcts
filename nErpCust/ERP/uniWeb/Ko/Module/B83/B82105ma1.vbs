...
Const BIZ_PGM_QRY_ID  = "B82105mb1.asp"
Const BIZ_PGM_SAVE_ID = "B82105mb2.asp"

Const BIZ_PGM_JUMP_ID = "B82104ma1"
'========================================================================================================
...
Function FncNew()
    ...
    Call SetToolbar("111000000000111")
    Call SetDefaultVal
    Call InitVariables                                                          '⊙: Initializes local global variables
...
Function DbQueryOk()
    ...
    If frm1.txtFileNm.Value = "" Then
       Call ggoOper.SetReqAttr(frm1.button1, "Q")
       frm1.button1.disabled = true
    Else	
       frm1.button1.disabled = Flase	
    End If
   ...
   // '사용자 ID별 로 권한을 체크하여 버튼을 활성화 한다.   
    ...
    IntRetCD = CommonQueryRs("Isnull(R_GRADE,' ') +Isnull(T_GRADE,' ')+Isnull(P_GRADE,' ')+Isnull(q_GRADE,' ')","B_CIS_CHANGE_ITEM_REQ","REQ_NO = '" & TRIM(frm1.txtarReqNo.value) & "' " ,GradeVal,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)    
 
  IntRetCD = CommonQueryRs("ITEM_R + ITEM_T + ITEM_P + ITEM_Q","B_CIS_ROUTING_USER","ITEM_ACCT = '" & TRIM(frm1.htxtItemAcct.value) & "' AND ITEM_KIND = '" & TRIM(frm1.htxtItemKind.value) & "' AND USER_ID = '" & parent.gUsrId & "'" ,IsRouting,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)    
   
   
  configSet = CommonQueryRs("case ITEM_R when 'Y' then '1' else '' end+case ITEM_T when 'Y' then '2' else '' end+case ITEM_P when 'Y'  then '3' else '' end+case ITEM_Q when 'Y'  then '4' else '' end","B_CIS_CONFIG","ITEM_ACCT = '" & TRIM(frm1.htxtItemAcct.value) & "' AND ITEM_KIND = '" & TRIM(frm1.htxtItemKind.value) & "'" ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)    
...
Function BtnR()
    ...
    arrParam(5) = Trim(frm1.htxtItemAcct.value)
    arrParam(6) = Trim(frm1.htxtItemKind.value)
