...
Const BIZ_PGM_QRY_ID  = "B82102mb1.asp"
Const BIZ_PGM_SAVE_ID = "B82102mb2.asp"

Const BIZ_PGM_JUMP_ID = "B82101ma1"
...
Function DbQueryOk()
    ...
    IntRetCD = CommonQueryRs("Isnull(R_GRADE,' ') +Isnull(T_GRADE,' ')+Isnull(P_GRADE,' ')+Isnull(q_GRADE,' ')","B_CIS_NEW_ITEM_REQ","REQ_NO = '" & TRIM(frm1.txtarReqNo.value) & "' " ,GradeVal,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)    
    IntRetCD = CommonQueryRs("ITEM_R + ITEM_T + ITEM_P + ITEM_Q","B_CIS_ROUTING_USER","ITEM_ACCT = '" & TRIM(frm1.cboItemAcct.value) & "' AND ITEM_KIND = '" & TRIM(frm1.txtItemKind.value) & "' AND upper(USER_ID) = upper('" & parent.gUsrId & "')"  ,IsRouting,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)    
    configSet = CommonQueryRs("case ITEM_R when 'Y' then '1' else '' end+case ITEM_T when 'Y' then '2' else '' end+case ITEM_P when 'Y'  then '3' else '' end+case ITEM_Q when 'Y'  then '4' else '' end","B_CIS_CONFIG","ITEM_ACCT = '" & TRIM(frm1.cboItemAcct.value) & "' AND ITEM_KIND = '" & TRIM(frm1.txtItemKind.value) & "'" ,lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)    
    
    IsRouting = Trim(Replace(IsRouting,Chr(11),""))
...
Function BtnR()
    ...
    arrParam(5) = Trim(frm1.cboItemAcct.value)
