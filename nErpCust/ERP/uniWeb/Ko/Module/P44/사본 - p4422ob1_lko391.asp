        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
                    ...
                    lgStrSQL = lgStrSQL & " c.wc_cd    , d.wc_nm                 , a.prodt_order_no             , b.item_cd                   , e.item_nm               , "
                    lgStrSQL = lgStrSQL & " a.report_dt, A.REPORT_TYPE, "
                    lgStrSQL = lgStrSQL & " CASE	A.REPORT_TYPE "
                    lgStrSQL = lgStrSQL & "			WHEN	'G'	THEN	'양품' "
                    lgStrSQL = lgStrSQL & "			WHEN	'B'	THEN	'불량' "
                    lgStrSQL = lgStrSQL & " END	REPORT_TYPE_NM, "
                    lgStrSQL = lgStrSQL & " a.prod_qty_in_order_unit, a.insp_good_qty_in_order_unit, a.insp_bad_qty_in_order_unit, a.rcpt_qty_in_order_unit, "
                    lgStrSQL = lgStrSQL & " a.remark   , a.cur_cd                , a.seq                        , a.opr_no                   , convert(varchar(20),a.insrt_dt,120)  AS   insrt_dt                        "
                    lgStrSQL = lgStrSQL & " FROM p_production_results a                                                                           "
                    lgStrSQL = lgStrSQL & " INNER JOIN p_production_order_header b ON a.prodt_order_no = b.prodt_order_no                         "
                    lgStrSQL = lgStrSQL & " INNER JOIN p_production_order_detail c ON a.prodt_order_no = c.prodt_order_no AND a.opr_no = c.opr_no "
                    lgStrSQL = lgStrSQL & " LEFT OUTER JOIN p_work_center d ON a.plant_cd = d.plant_cd AND c.wc_cd = d.wc_cd                      "
                    lgStrSQL = lgStrSQL & " LEFT OUTER JOIN b_item e ON b.item_cd = e.item_cd                                                     "
                    lgStrSQL = lgStrSQL & " WHERE a.del_flg ='N' "
                    lgStrSQL = lgStrSQL & whereQuery                    
                                        
                    lgStrSQL = lgStrSQL & " union all  SELECT  "  
                    lgStrSQL = lgStrSQL & " '' wc_cd,'총계' wc_nm, '' prodt_order_no, '' item_cd, '' item_nm, "
                    lgStrSQL = lgStrSQL & " null report_dt, '' REPORT_TYPE, "
                    lgStrSQL = lgStrSQL & " '' REPORT_TYPE_NM, "
                    lgStrSQL = lgStrSQL & " sum(a.prod_qty_in_order_unit), sum(a.insp_good_qty_in_order_unit), sum(a.insp_bad_qty_in_order_unit), sum(a.rcpt_qty_in_order_unit), "
                    lgStrSQL = lgStrSQL & " '' remark, '' cur_cd,  null seq, null opr_no, null insrt_dt                        "
                    lgStrSQL = lgStrSQL & " FROM p_production_results a                                                                           "
                    lgStrSQL = lgStrSQL & " INNER JOIN p_production_order_header b ON a.prodt_order_no = b.prodt_order_no                         "
                    lgStrSQL = lgStrSQL & " INNER JOIN p_production_order_detail c ON a.prodt_order_no = c.prodt_order_no AND a.opr_no = c.opr_no "
                    lgStrSQL = lgStrSQL & " LEFT OUTER JOIN p_work_center d ON a.plant_cd = d.plant_cd AND c.wc_cd = d.wc_cd                      "
