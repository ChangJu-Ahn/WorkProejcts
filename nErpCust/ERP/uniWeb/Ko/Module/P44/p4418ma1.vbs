...
'**********************************************************************************************************
Const BIZ_PGM_QRY_ID	= "p4418mb1.asp"						'☆: 비지니스 로직 ASP명 
Const BIZ_PGM_SAVE_ID	= "p4417mb2.asp"						'☆: 비지니스 로직 ASP명 
...
Sub InitSpreadSheet()
    ...
    With frm1.vspdData
		...
		.MaxCols = C_LOT_NO+1							'☜: 최대 Columns의 항상 1개 증가시킴    
		.MaxRows = 0
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
		Case "A"
			...
			C_BadReason = iCurColumnPos(5)
			C_Remark	= iCurColumnPos(6)
			C_Seq		= iCurColumnPos(7)
			C_Milestone = iCurColumnPos(8)
			C_LOT_NO    = iCurColumnPos(9)
	End Select
End Sub    
...
Function OpenProdRef()
	...
	arrParam(0) = Trim(frm1.txtPlantCd.value)		'☆: 조회 조건 데이타 
	arrParam(1) = Trim(frm1.txtProdOrderNo.value)	'☜: 조회 조건 데이타 
	arrParam(2) = Trim(frm1.txtOprCd.value)			'☜: 조회 조건 데이타 
...
Sub vspdData_Click(ByVal Col , ByVal Row )
	...
	gMouseClickStatus = "SPC"
 	...
 	Else
		...
		frm1.vspdData.Col = C_Milestone
		strMilestone = frm1.vspdData.Text
		
		'Add 2005-10-05
 		If frm1.hDamperFlag.value = "Y" Then
 			If Row = frm1.vspdData.Maxrows and strMilestone = "Y" and frm1.txtStatus.value <> "CL" Then
				Call SetToolbar("11001011000111")										'⊙: 버튼 툴바 제어 
				Call SetPopupMenuItemInf("0101110111")
			Else
				frm1.vspdData.Row = Row + 1
				frm1.vspdData.Col = 0
				If frm1.vspdData.Text = ggoSpread.DeleteFlag and strMilestone = "Y" and frm1.txtStatus.value <> "CL" Then
					Call SetToolbar("11001011000111")										'⊙: 버튼 툴바 제어 
					Call SetPopupMenuItemInf("0101110111")
				Else
			...
			End If
		Else
			If frm1.txtStatus.value <> "CL" and strMilestone = "Y" Then
				Call SetToolbar("11001011000111")										'⊙: 버튼 툴바 제어 
				Call SetPopupMenuItemInf("0101110111")
			Else	
				Call SetToolbar("11001001000111")										'⊙: 버튼 툴바 제어 
...
Sub vspdData_LeaveCell(ByVal Col, ByVal Row, ByVal NewCol, ByVal NewRow, Cancel)
	...
	If NewRow = frm1.vspdData.Maxrows  and strMilestone = "Y" and frm1.txtStatus.value <> "CL" Then
		Call SetToolbar("11001011000111")										'⊙: 버튼 툴바 제어 
	Else
...
Function DBQueryOk()									'☆: 조회 성공후 실행로직 
	...
	strMilestone = frm1.vspdData.Text
	If frm1.hDamperFlag.value = "Y" Then
		If frm1.vspdData.MaxRows = 1  and strMilestone = "Y" and frm1.txtStatus.value <> "CL" Then
			Call SetToolbar("11001011000111")					'⊙: 버튼 툴바 제어 
		Else
			Call SetToolbar("11001001000111")					'⊙: 버튼 툴바 제어 
		End IF
	Else
		If strMilestone = "Y" and frm1.txtStatus.value <> "CL" Then
			Call SetToolbar("11001011000111")					'⊙: 버튼 툴바 제어 
		Else
			Call SetToolbar("11001001000111")					'⊙: 버튼 툴바 제어 
		End IF
...
Function DbSave() 
	...
	Select Case .Text
	    ...
	    Case ggoSpread.DeleteFlag
			...
			strDel = ""
			'P448_IG1_I1_prodt_order_no = 0    '[CONVERSION INFORMATION]  View Name : import p_production_order_header
			strDel = strDel & Trim(frm1.txtProdOrderNo.Value) & iColSep
			'P448_IG1_I2_opr_no = 1    '[CONVERSION INFORMATION]  View Name : import p_production_order_detail
			strDel = strDel & Trim(frm1.txtOprCd.Value) & iColSep
			'P448_IG1_I3_seq = 2    '[CONVERSION INFORMATION]  View Name : import p_production_results
			.Col = C_Seq
			strDel = strDel & Trim(.Text) & iColSep
			'P448_IG1_I3_report_dt = 3
			.Col = C_ReportDt
			strDel = strDel & UNIConvDate(.Text) & iColSep
			'P448_IG1_I3_report_type = 4
			strDel = strDel  & iColSep
			'P448_IG1_I3_prod_qty_in_order_unit = 5
			strDel = strDel  & iColSep
			'P448_IG1_I3_prod_qty_in_base_unit = 6
			strDel = strDel  & iColSep
			'P448_IG1_I3_rcpt_item_document_no = 7
			strDel = strDel  & iColSep
			'P448_IG1_I3_reason_cd = 8
			strDel = strDel  & iColSep
			'P448_IG1_I4_flag = 9    '[CONVERSION INFORMATION]  View Name : import_auto_rcpt ief_supplied
			strDel = strDel  & iRowSep

            '2008-05-26 11:26오후 :: hanc
			.Col = C_LOT_NO

'20080612::HANC::임시			If Trim(.Text) <> "" Then
'20080612::HANC::임시			    MsgBox "인터페이스 DATA는 삭제 할 수 없습니다"
'20080612::HANC::임시			    Call LayerShowHide(0)
'20080612::HANC::임시			    Exit Function
'20080612::HANC::임시			End If
