...
'*  7. Modified date(Last)  : 2003-09-14
...
%>
				...
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("OPR_NO"))%>"													'품목 
				...
				strData = strData & Chr(11) & "0"		'입력부 - 실적수량 
				strData = strData & Chr(11) & "G"		'입력부 - 양/불 
				strData = strData & Chr(11) & ""		'입력부 - 불량코드 
				strData = strData & Chr(11) & ""		'입력부 - 불량이유 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("CUR_CD"))%>"													'통화 
				...
				strData = strData & Chr(11) & ""		'입력부 - Remark
				strData = strData & Chr(11) & ""		'입력부 - Lot No.
				strData = strData & Chr(11) & ""		'입력부 - 입고전표번호 
				strData = strData & Chr(11) & ""		'입력부 - 출고전표번호 
				strData = strData & Chr(11) & ""		'입력부 - 검사의뢰번호 
				...
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("BP_CD"))%>"													'거래처 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("BP_NM"))%>"													'거래처명 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("JOB_CD"))%>"													'작업 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("JOB_NM"))%>"													'작업명 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("ROUT_ORDER"))%>"												'작업순서 
				strData = strData & Chr(11) & "<%=UniConvNumDBToCompanyWithOutChange(rs0("SUBCONTRACT_PRC"), 0)%>"				'외주금액 
				strData = strData & Chr(11) & "<%=UniConvNumDBToCompanyWithOutChange(rs0("SUBCONTRACT_AMT"), 0)%>"				'외주단가 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("ORDER_STATUS"))%>"											'지시상태 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("STATUS_NM"))%>"												'지시상태 
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("TAX_TYPE"))%>"												'세금 
				strData = strData & Chr(11) & "<%=rs0("INSIDE_FLG")%>"															'사내/외 
				...
				strData = strData & Chr(11) & "<%=rs0("INSP_FLG")%>"															'공정검사여부 
				strData = strData & Chr(11) & "<%=rs0("FINAL_INSPEC_FLG")%>"
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("item_group_cd"))%>"
				strData = strData & Chr(11) & "<%=ConvSPChars(rs0("item_group_nm"))%>"													'최종검사여부 
				strData = strData & Chr(11) & "<%=UniConvNumberDBToCompany(rs0("ORDER_QTY_IN_BASE_UNIT"), ggQty.DecPoint, ggQty.RndPolicy, ggQty.RndUnit, 0)%>"			'입고수량 
...
%>
		...
		Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData1, 1, .Frm1.vspdData1.MaxRows, .C_CurrencyCode,.C_SubcontractPriceIn, "C", "I", "X", "X")
		Call .ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData1, 1, .Frm1.vspdData1.MaxRows, .C_CurrencyCode,.C_SubcontractAmtIn, "A", "I", "X", "X")		
