...
%>
			If "<%=i%>" = 0 Then
				.lgShift = Trim("<%=ConvSPChars(rs0("Shift_Cd"))%>")
			End If
			strData = strData & Trim("<%=ConvSPChars(rs0("Shift_Cd"))%>")  & vbTab		'☆: Shift Code
<%		
			rs0.MoveNext
		Next
	End If
...
%>	
	.ggoSpread.SetCombo strData, .C_ShiftId
