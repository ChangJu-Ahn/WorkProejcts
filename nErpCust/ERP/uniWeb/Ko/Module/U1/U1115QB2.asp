        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
					...
					lgStrSQL = lgStrSQL & "                 CASE WHEN I.RET_FLG = 'Y' AND I.RCPT_FLG = 'N' THEN - MVMT_QTY ELSE MVMT_QTY END MVMT_QTY,  (CASE WHEN A.PO_NO IS NULL AND A.PO_SEQ_NO IS NULL THEN A.MVMT_PRC ELSE G.PO_PRC END) PO_PRC,  "
					lgStrSQL = lgStrSQL & "                 CASE WHEN I.RET_FLG = 'Y' AND I.RCPT_FLG = 'N' THEN - MVMT_QTY ELSE MVMT_QTY END * (CASE WHEN A.PO_NO IS NULL AND A.PO_SEQ_NO IS NULL THEN A.MVMT_PRC ELSE G.PO_PRC END) RCPT_AMT ,  "
					lgStrSQL = lgStrSQL & "                 A.MVMT_RCPT_SL_CD SL_CD , H.SL_NM  "
					lgStrSQL = lgStrSQL & "   FROM	M_PUR_GOODS_MVMT A, B_BIZ_PARTNER B,  B_ITEM D, B_PLANT F,   "
					lgStrSQL = lgStrSQL & " 	M_PUR_ORD_DTL G  , B_STORAGE_LOCATION H , M_MVMT_TYPE I  "
					lgStrSQL = lgStrSQL & "  WHERE 	A.BP_CD = B.BP_CD AND	A.ITEM_CD = D.ITEM_CD   "
					...
					lgStrSQL = lgStrSQL & "    AND  A.IO_TYPE_CD = I.IO_TYPE_CD "
					...
					If lgkeystream(5) = "B" Then
						lgStrSQL = lgStrSQL & "     and 	i.RET_FLG = 'N' "
					ElseIf	lgkeystream(5) = "C" Then
						lgStrSQL = lgStrSQL & "     and 	i.RET_FLG = 'Y' "
					End If
