           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "Y"
                    lgStrSQL	= " SELECT TOP " & iSelCount & " A.PO_NO, A.PO_SEQ_NO, A.PLANT_CD, X.D_BP_CD, A.ITEM_CD, " _
								...
								& " A.PO_PRC, A.PO_DOC_AMT, F.PO_CUR, A.DLVY_DT, ISNULL(X.RCPT_QTY,0) RCPT_QTY, " _
								...
								& " ISNULL(A.LC_RCPT_QTY,0) LC_RCPT_QTY, L.LOT_FLG, N.LOT_GEN_MTHD, " _
								& " ((CASE WHEN X.RCPT_QTY = 0 AND X.RCPT_DT IS NULL THEN X.CONFIRM_QTY ELSE 0 END) - X.RCPT_QTY) REMAIN_RCPT_QTY, " _
								& " X.LOT_NO SERIAL_NO, X.LOT_SUB_NO SERIAL_SUB_NO, X.PLAN_DVRY_DT, X.SPLIT_SEQ_NO " _
								...
								& " INNER JOIN M_SCM_FIRM_PUR_RCPT X ON A.PO_NO = X.PO_NO AND X.PO_SEQ_NO = A.PO_SEQ_NO AND X.CONFIRM_QTY > X.RCPT_QTY " _
					...
					lgStrSQL = lgStrSQL	& " WHERE A.CLS_FLG = " & FilterVar("N","''","S") _
										...
										& " AND (CASE WHEN A.OVER_TOL <> 0 THEN ((100 + A.OVER_TOL) * A.PO_QTY / 100) ELSE A.PO_QTY END) > A.RCPT_QTY " _
										& " AND (L.MATERIAL_TYPE <> " & FilterVar("20","''","S") & " OR M.DELIVERY_ORDER_FLG <> " & FilterVar("Y","''","S") & ") " _
										& " AND (CASE WHEN A.OVER_TOL <> 0 THEN ((100 + A.OVER_TOL) * A.PO_QTY / 100) ELSE A.PO_QTY END) > A.IV_QTY " _
										& " AND (CASE WHEN A.OVER_TOL <> 0 THEN ((100 + A.OVER_TOL) * A.PO_QTY / 100) ELSE A.PO_QTY END) > (A.RCPT_QTY + A.PRE_IV_QTY + A.INSPECT_QTY + A.LC_QTY - A.LC_RCPT_QTY) " _
										& " AND F.STO_FLG = " & FilterVar("N","''","S") _
										& " AND (X.MVMT_NO = '' OR X.MVMT_NO IS NULL)  "
					
					If lgkeystream(1) <> "" Then
						lgStrSQL = lgStrSQL & "  AND X.PLAN_DVRY_DT >= " & FilterVar(UniConvDate(lgKeyStream(1)),"''", "S")
					End If
					
					If lgkeystream(2) <> "" Then
						lgStrSQL = lgStrSQL & "  AND X.PLAN_DVRY_DT <= " & FilterVar(UniConvDate(lgKeyStream(2)),"''", "S")
					End If
               ...
               Case "N"
					...
					lgStrSQL = lgStrSQL	& " WHERE A.CLS_FLG = " & FilterVar("N","''","S") _
										& " AND F.RELEASE_FLG = " & FilterVar("Y","''","S") _
										& " AND (CASE WHEN A.OVER_TOL <> 0 THEN ((100 + A.OVER_TOL) * A.PO_QTY / 100) ELSE A.PO_QTY END) - A.RCPT_QTY) > 0 " _
										& " AND (CASE WHEN A.OVER_TOL <> 0 THEN ((100 + A.OVER_TOL) * A.PO_QTY / 100) ELSE A.PO_QTY END) > A.IV_QTY " _
										& " AND (CASE WHEN A.OVER_TOL <> 0 THEN ((100 + A.OVER_TOL) * A.PO_QTY / 100) ELSE A.PO_QTY END) > (A.RCPT_QTY + A.PRE_IV_QTY + A.INSPECT_QTY) " _
										& " AND F.STO_FLG = " & FilterVar("N","''","S") _
										& " AND (X.MVMT_NO = '' OR X.MVMT_NO IS NULL)  "
					
					If lgkeystream(1) <> "" Then
						lgStrSQL = lgStrSQL & "  AND F.PO_DT >= " & FilterVar(UniConvDate(lgKeyStream(1)),"''", "S")
					End If
					
					If lgkeystream(2) <> "" Then
						lgStrSQL = lgStrSQL & "  AND F.PO_DT <= " & FilterVar(UniConvDate(lgKeyStream(2)),"''", "S")
					End If
					...
					If lgkeystream(12) <> "" Then
						lgStrSQL = lgStrSQL & "  AND F.RCPT_TYPE = " & FilterVar(lgKeyStream(15),"''", "S")
					End If	
					...
					lgStrSQL = lgStrSQL & "  ORDER BY A.PO_NO, A.O_SEQ_NO "
