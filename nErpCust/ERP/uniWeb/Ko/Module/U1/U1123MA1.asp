...
'*  1. Module Name          : 
'*  2. Function Name        : 
'*  3. Program ID           : U1123MA1
'*  4. Program Name         : 구매반품등록(SCM) 
'*  5. Program Desc         :
'*  6. Comproxy List        : 
'*  7. Modified date(First) : 
'*  8. Modified date(Last)  : 2005/02/24
'*  9. Modifier (First)     : 
'* 10. Modifier (Last)      : 
'* 11. Comment              :
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.Spreadinit "V20030515",,Parent.gAllowDragDropSpread  
		...
		ggoSpread.SSSetEdit 		C_SplitSeqNo,		"", 10
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
	...
	Select Case UCase(pvSpdNo)
		Case "A"
			...
			C_LotNoPop		= iCurColumnPos(13)
			C_LotNoSeq		= iCurColumnPos(14)
			C_RetType	    = iCurColumnPos(15)
			C_RetTypeNm		= iCurColumnPos(16)
			C_PoNo			= iCurColumnPos(17)
			C_PoSeq			= iCurColumnPos(18)
			C_GmNo			= iCurColumnPos(19)
			C_GmSeq			= iCurColumnPos(20)
			C_MvmtNo		= iCurColumnPos(21)
			C_RetOrdQty		= iCurColumnPos(22)
			C_SplitSeqNo	= iCurColumnPos(23)
	End Select
...
Function OpenGrType()
	...
	arrParam(4) = "Ret_Flg='Y' AND USAGE_FLG='Y' "
	arrParam(5) = "입출고유형"			
...
Function OpenSL()
	...
	arrParam(4) = "PLANT_CD='" & FilterVar(UCase(Trim(GetSpreadText(frm1.vspdData,C_PlantCd,iCurRow,"X","X"))), " " , "SNM") & "'"
	arrParam(5) = "창고"
...
Function OpenMvmtNo()
		...
		iCalledAspName = AskPRAspName("U1123PA1")
	
		If Trim(iCalledAspName) = "" Then
			IntRetCD = DisplayMsgBox("900040", parent.VB_INFORMATION, "U1123PA1", "X")
			lblnWinEvent = False
			Exit Function
		End If
...
Function OpenSppl()
	...
	arrParam(0) = "공급처"			
	arrParam(1) = " (SELECT distinct B.BP_CD,C.BP_NM " _
				& " FROM M_SCM_FIRM_PUR_RCPT A(NOLOCK) INNER JOIN M_PUR_ORD_HDR B(NOLOCK) " _
				& " ON A.PO_NO = B.PO_NO " _
				& " INNER JOIN B_BIZ_PARTNER C(NOLOCK) ON B.BP_CD = C.BP_CD " _
				& " WHERE A.PLAN_DVRY_QTY > 0 " _
				& " AND A.RCPT_QTY = 0 AND B.RET_FLG = 'Y' AND A.DLVY_NO <> '') A "
...
Function OpenFirmPORcpt()
	...
	iCalledAspName = AskPRAspName("U1113RA1")
	
	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", parent.VB_INFORMATION, "U1113RA1", "X")
		lblnWinEvent = False
		Exit Function
	End If
...
Function SetFirmPoRef(strRet)
	...
	With frm1.vspdData
		...
		For index1 = 0 to Count1
			...
			Call .SetText(C_Unit,		Row1, strRet(index1,C_Unit_Ref))
			...
			Call .SetText(C_PoSeq,		Row1, strRet(index1,C_PoSeq_Ref))
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>구매반품(Procurement)</font></td>
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="right"><img src="../../../CShared/image/table/seltab_up_right.gif" width="10" height="23"></td>
								
						    </TR>
						</TABLE>
					</TD>
					<TD WIDTH=* align=right><A href="vbscript:OpenFirmPORcpt()">반품예정참조</A></TD>
					<TD WIDTH=10>&nbsp;</TD>
				</TR>
			</TABLE>
		</TD>
