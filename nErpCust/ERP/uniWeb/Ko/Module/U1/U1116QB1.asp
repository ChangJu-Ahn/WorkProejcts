...
Sub SubBizQueryMulti()
    ...
    Else
        ...
        Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & UniConvNumDBToCompanyWithOutChange(lgObjRs("PO_AMT"), 0)
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("LOT_NO"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("MAKER_LOT_NO"))
    ...
    Select Case Mid(pDataType,1,1)
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
					...
					lgStrSQL = lgStrSQL & "                 CASE WHEN C.RET_FLG = 'Y' AND C.RCPT_FLG = 'N' THEN - A.MVMT_QTY ELSE A.MVMT_QTY END MVMT_QTY ,  "
					lgStrSQL = lgStrSQL & " 	    	A.MVMT_BASE_QTY, A.IV_QTY, A.MVMT_PRC, A.MVMT_LOC_AMT, A.BP_CD, B.BP_NM,  "
					lgStrSQL = lgStrSQL & " 	    	A.LOT_NO, A.LOT_SUB_NO, A.MAKER_LOT_NO, A.MAKER_LOT_SUB_NO , (CASE WHEN A.PO_NO IS NULL AND A.PO_SEQ_NO IS NULL THEN A.MVMT_PRC ELSE G.PO_PRC END) PO_PRC ,  "
					lgStrSQL = lgStrSQL & "                 (CASE WHEN A.PO_NO IS NULL AND A.PO_SEQ_NO IS NULL THEN A.MVMT_PRC ELSE G.PO_PRC END) * CASE WHEN C.RET_FLG = 'Y' AND C.RCPT_FLG = 'N' THEN - A.MVMT_QTY ELSE A.MVMT_QTY END PO_AMT , H.SL_CD , H.SL_NM "
					lgStrSQL = lgStrSQL & "   FROM	M_PUR_GOODS_MVMT A, B_BIZ_PARTNER B, M_MVMT_TYPE C, B_ITEM D,  B_PLANT F , M_PUR_ORD_DTL G , B_STORAGE_LOCATION H "
					...
					If lgkeystream(5) <> "" and lgkeystream(5) <> "A" Then
						lgStrSQL = lgStrSQL & "				and		c.ret_flg = " & FilterVar(lgKeyStream(5),"''", "S")
					End If
					If lgkeystream(6) <> "" Then
						lgStrSQL = lgStrSQL & "            	and 	a.po_no like " & FilterVar("%" & lgKeyStream(6) & "%","''", "S")
					End If
					If lgkeystream(7) <> "" Then
						lgStrSQL = lgStrSQL & "            	and 	a.PUR_GRP = " & FilterVar(lgKeyStream(7),"''", "S")
					End If
        ...
        Case "C"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "G"
                    lgStrSQL =            " select PUR_GRP_NM from B_PUR_GRP where PUR_GRP = " & FilterVar(lgKeyStream(7) & "" ,"''", "S")          
           End Select  
