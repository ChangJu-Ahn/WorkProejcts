...
Sub SubBizQuery()
    ...
    If lgStrPrevKey = 0 Then
            ...
            lgStrSQL = "  				SELECT	DISTINCT CONVERT(CHAR(7), A.REPORT_DT, 120) REPORT_DT,                                                                            "
            lgStrSQL = lgStrSQL & vbCrLf & " 		A.PRODT_ORDER_NO,                                                                                                             "
            lgStrSQL = lgStrSQL & vbCrLf & " 		A.PLANT_CD,                                                                                                                   "      
            lgStrSQL = lgStrSQL & vbCrLf & " 			C.PLANT_NM,                                                                                                               "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			A1.ITEM_CD,                                                                                                               "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			B.ITEM_NM,                                                                                                                "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			B.SPEC,                                                                                                                   "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			COUNT(*)	CNT1,                                                                                                         "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			(SELECT COUNT(*) FROM P_COMPNT_REQ_BY_ROUTING WHERE PLANT_CD = A.PLANT_CD AND ITEM_CD = A1.ITEM_CD) CNT2,                 "                                                                                                  
            lgStrSQL = lgStrSQL & vbCrLf & " 			(SELECT COUNT(*) FROM  P_BOM_DETAIL WHERE  PRNT_PLANT_CD = A.PLANT_CD AND    PRNT_ITEM_CD  = A1.ITEM_CD)	CNT3,         "                                                                                                          
            lgStrSQL = lgStrSQL & vbCrLf & " 			(SELECT COUNT(*) FROM  P_ROUTING_DETAIL WHERE  PLANT_CD = A.PLANT_CD AND    ITEM_CD  = A1.ITEM_CD)			CNT4          "                                                                                                          
            lgStrSQL = lgStrSQL & vbCrLf & " 	FROM	P_PRODUCTION_RESULTS A                                                                                                    "          
            lgStrSQL = lgStrSQL & vbCrLf & " 		JOIN    B_ITEM_BY_PLANT		A1                                                                                                "      
            lgStrSQL = lgStrSQL & vbCrLf & " 		ON		A.PLANT_CD	=	A1.PLANT_CD                                                                                           "      
            lgStrSQL = lgStrSQL & vbCrLf & " 		AND		(SELECT ITEM_CD FROM P_PRODUCTION_ORDER_HEADER WHERE PRODT_ORDER_NO = A.PRODT_ORDER_NO)	=	A1.ITEM_CD                "      
            lgStrSQL = lgStrSQL & vbCrLf & " 			JOIN	B_ITEM		B                                                                                                     "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			ON		B.ITEM_cD	=	A1.ITEM_CD                                                                                        "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			JOIN	B_PLANT		C                                                                                                     "          
            lgStrSQL = lgStrSQL & vbCrLf & " 			ON		C.PLANT_CD	=	A1.PLANT_CD                                                                                       "          
            lgStrSQL = lgStrSQL & vbCrLf & " 				WHERE	A.PLANT_CD	=	 " & FilterVar(lgKeyStream(0),"''", "S")                                                                                                              
            lgStrSQL = lgStrSQL & vbCrLf & " 				AND		A.REPORT_DT >=   " & FilterVar(lgKeyStream(2),"''", "S") 
            lgStrSQL = lgStrSQL & vbCrLf & " 				AND		A.REPORT_DT <=   " & FilterVar(lgKeyStream(3),"''", "S") 
            lgStrSQL = lgStrSQL & vbCrLf & " 	AND		A1.ITEM_ACCT	IN	('10','20')                                                                                           "          
            lgStrSQL = lgStrSQL & vbCrLf & " 	GROUP	BY	A.REPORT_DT, A.PRODT_ORDER_NO,A.PLANT_CD,	C.PLANT_NM, A1.ITEM_CD, B.ITEM_NM,B.SPEC                                  "
...
'            lgStrSQL = lgStrSQL & vbCrLf & " 				AND		A.REPORT_DT <=   " & FilterVar(lgKeyStream(3),"''", "S") 
'            lgStrSQL = lgStrSQL & vbCrLf & " 				AND		A1.ITEM_ACCT	IN	('10','20')                                                                                                     "
'            lgStrSQL = lgStrSQL & vbCrLf & " 				GROUP	BY	A.PLANT_CD,	C.PLANT_NM, A1.ITEM_CD, B.ITEM_NM,B.SPEC                                                                            "
...
'            lgStrSQL = lgStrSQL & " select TOP " & iSelCount & " 	DISTINCT CONVERT(CHAR(7), A.REPORT_DT, 120) REPORT_DT,"
            lgStrSQL = "  				SELECT	DISTINCT CONVERT(CHAR(7), A.REPORT_DT, 120) REPORT_DT,                                                                            "
            lgStrSQL = lgStrSQL & vbCrLf & " 		A.PRODT_ORDER_NO,                                                                                                             "
            lgStrSQL = lgStrSQL & vbCrLf & " 		A.PLANT_CD,                                                                                                                   "      
            lgStrSQL = lgStrSQL & vbCrLf & " 			C.PLANT_NM,                                                                                                               "          
...
''          lgStrSQL = lgStrSQL & vbCrLf & " ORDER	BY	Z.PLANT_CD,Z.PLANT_NM,Z.ITEM_CD,Z.ITEM_NM,Z.SPEC "
    ...
    Else    
       ...
       Do While Not lgObjRs.EOF
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("REPORT_DT"))
