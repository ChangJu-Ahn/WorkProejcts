...
<%
'======================================================================================================
'*  1. Function Name        : 제31호(1)중소기업투자준비금조정 
'*  3. Program ID           : W4101MA1
'*  4. Program Name         : W4101MA1_HTF.asp
	...
	Private Sub SubMakeSQLStatements(pMode, pCode1, pCode2, pCode3)
	    Select Case pMode 
	      Case "H"
				...
				lgStrSQL = lgStrSQL & " FROM TB_31_1H	A  WITH (NOLOCK) " & vbCrLf	' 서식3호 
	      ...
	      Case "D"
				...
				lgStrSQL = lgStrSQL & " SELECT  "
	            lgStrSQL = lgStrSQL & " A.*  " & vbCrLf
				lgStrSQL = lgStrSQL & " FROM TB_31_1D1	A  WITH (NOLOCK) " & vbCrLf	' 서식3호 
				lgStrSQL = lgStrSQL & " WHERE A.CO_CD = " & pCode1 	 & vbCrLf
				lgStrSQL = lgStrSQL & "		AND A.FISC_YEAR = " & pCode2 	 & vbCrLf
				lgStrSQL = lgStrSQL & "		AND A.REP_TYPE = " & pCode3 	 & vbCrLf
...
' -- 데이타 존재 체크 
Class TYPE_DATA_EXIST_W4101MA1
	Dim A103

End Class

' ------------------ 메인 함수 --------------------------------
Function MakeHTF_W4101MA1()
    
	' -- 원래 전자신고가 아닌데, 참조하기 위해 만든 인크루드파일 
End Function

' ------------------ 조회 함수 --------------------------------
Sub SubMakeSQLStatements_W4101MA1(pMode, pCode1, pCode2, pCode3)
