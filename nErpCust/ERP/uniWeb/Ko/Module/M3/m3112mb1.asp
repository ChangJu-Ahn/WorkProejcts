...
'*  2. Function Name        : 
'*  3. Program ID           : M3112MB1
'*  4. Program Name         : 발주내역등록 
'*  5. Program Desc         : 발주내역등록 
'*  6. Component List       : PM3G119.cMLookupPurOrdHdrS/PM3G121.cMMaintPurOrdDtlS/PM3G128.cMListPurOrdDtlS/PM3G1R1.cMReleasePurOrdS/PM3G1P9.cMLookupPriceS/PB3S106.cBLkUpItemByPlt
...
'-------------------------------------------------------------------    
	...
	If CheckSYSTEMError2(Err,True,"","","","","") = true then 		
		...
		Response.Write "parent.frm1.btnCfm.disabled = True " & vbCr
		Response.Write "call parent.ggoOper.ClearField(Parent.Document, ""2"") " & vbCr	'Clearing field only when error occurred
	...
	If UCase(Trim(E22_m_pur_ord_hdr(M239_E22_ret_flg))) <> "N" then
	   Call DisplayMsgBox("17a014", vbInformation, "반품발주등록건", "조회", I_MKSCRIPT)
       Set M31119 = Nothing																	'☜: ComProxy UnLoad
		Response.Write "<Script Language=vbscript>"  	& vbCr
		Response.Write "parent.frm1.txtPoNo.focus" & vbCr
		Response.Write "parent.frm1.btnCfm.disabled = True " & vbCr
	...
	Response.Write "	.frm1.txtCurr.value 	  = """ & Trim(UCase(ConvSPChars(E22_m_pur_ord_hdr(M239_E22_po_cur))))	 			& """" & vbCr
	Response.Write " 	parent.CurFormatNumericOCX " &vbCr
	Response.Write "	.frm1.txtCurrNm.value     = """ & Trim(UCase(ConvSPChars(E18_b_currency)))									& """" & vbCr
	Response.Write "	.frm1.txtGrossAmt.value   = """ & UNIConvNumDBToCompanyByCurrency(cdbl(E22_m_pur_ord_hdr(M239_E22_tot_po_doc_amt)),lgCurrency,ggAmtOfMoneyNo, "X" , "X") & """" & vbCr
	Response.Write "	.frm1.txtSupplierCd.value = """ & Trim(UCase(ConvSPChars(E9_b_biz_partner(M239_E9_bp_cd))))              	& """" & vbCr
	Response.Write "	.frm1.txtSupplierNm.value = """ & Trim(UCase(ConvSPChars(E9_b_biz_partner(M239_E9_bp_nm))))              	& """" & vbCr
	Response.Write "	.frm1.txtGroupCd.value    = """ & Trim(UCase(ConvSPChars(E20_b_pur_grp(M239_E20_pur_grp))))                	& """" & vbCr
	Response.Write "	.frm1.txtGroupNm.value    = """ & Trim(UCase(ConvSPChars(E20_b_pur_grp(M239_E20_pur_grp_nm))))              & """" & vbCr
	Response.Write "	.frm1.txtPoTypeCd.value   = """ & Trim(UCase(ConvSPChars(E19_m_config_process(M239_E19_po_type_cd))))       & """" & vbCr
	Response.Write "	.frm1.txtPoTypeNm.value   = """ & Trim(UCase(ConvSPChars(E19_m_config_process(M239_E19_po_type_nm))))       & """" & vbCr
	Response.Write "	.frm1.txtRelease.value    = """ & Trim(UCase(ConvSPChars(E22_m_pur_ord_hdr(M239_E22_release_flg)))) 		& """" & vbCr
	Response.Write "	.frm1.txthdnPoNo.value    = """ & Trim(UCase(ConvSPChars(E22_m_pur_ord_hdr(M239_E22_po_no))))               & """" & vbCr
	Response.Write "	.frm1.txtPoNo.value       = """ & Trim(UCase(ConvSPChars(E22_m_pur_ord_hdr(M239_E22_po_no))))               & """" & vbCr
	Response.Write "	.frm1.txtPoDt.text       = """ & UNIDateClientFormat(E22_m_pur_ord_hdr(M239_E22_po_dt))       	& """" & vbCr
	if cDate(Trim(E22_m_pur_ord_hdr(M239_E22_fore_dvry_dt))) <= cDate("1900-01-01") then
		Response.Write "	.frm1.hdnDlvydt.value     = """"" & vbCr
	else
...
'        istrData = istrData & Chr(11) & ConvSPChars(EG1_exp_group(iLngRow, M192_EG1_E5_m_pur_ord_dtl_po_prc))  '14
		...
		'Response.Write  "msgbox """ & EG1_exp_group(iLngRow, M192_EG1_E5_m_pur_ord_dtl_po_doc_amt)  & """" & vbcr
...
'-------------------------------------------------
	...
	iStrPoNo   = Trim(Request("txthdnPoNo"))
...
Sub SubLookUpItemPlant()
    ...
    For LngRow = 1 To LngMaxRow  
		...
		Response.Write "		If  """ & ConvSPChars(E1_m_supplier_item_price_pur_prc(1)) & """ <> """" Then " 				& vbCr								& vbCr	
...
Sub lookupPriceForSelection()
    ...
    Const M106_E1_pur_prc_flg = 1   'add KSJ 
    ...
    If Len(Trim(Request("txtPoDt"))) Then
		If UNIConvDate(Request("txtPoDt")) = "" Then
		    ...
		    Call LoadTab("parent.frm1.txtPoDt", 0, I_MKSCRIPT)
		    Exit Sub	
		End If
	End If
   ...
   If CheckSYSTEMError(Err,True) = true Then 		
	...
	ReDim returnValue(iLngMaxRow,1)
    ...
    For iLngRow = 1 To iLngMaxRow
		...
		I1_m_supplier_item_price(M106_I1_valid_fr_dt) = UNIConvDate(Trim(Request("txtPoDt")))
		...
		'Modified by KSJ 2008-03-21
		'returnValue(iLngRow,M106_E1_pur_prc) = UNINumClientFormat(E1_m_supplier_item_price(M106_E1_pur_prc) ,ggUnitCost.DecPoint,0)
		returnValue(iLngRow,M106_E1_pur_prc) =UNIConvNumDBToCompanyByCurrency(E1_m_supplier_item_price(M106_E1_pur_prc), ConvSPChars(I1_m_supplier_item_price(M106_I1_pur_cur)), ggUnitCostNo,"X","X")
		
		returnValue(iLngRow,M106_E1_pur_prc_flg) = E1_m_supplier_item_price(M106_E1_pur_prc_flg)
	...
	For rowindex = 1 To rowCount
		...
		If iLngRow <= iLngMaxRow Then
			...
			Response.wirte  "=====<br>"
			'Response.wirte ConvSPChars(returnValue(resultindex,M106_E1_pur_prc)) & "<br>"
			'Response.wirte returnValue(resultindex,M106_E1_pur_prc_flg) & "<br>"
			Response.wirte  "=====<br>"
	Response.Write "<script language=vbscript>" & vbCr
	...
	'Modified by KSJ 2008-03-21 
	'Response.Write " Parent.frm1.vspdData.Text = """ & ConvSPChars(returnValue(resultindex,M106_E1_pur_prc)) & """" & vbCr
	'Response.Write " Parent.frm1.vspdData.Text = """ & UNIConvNumDBToCompanyByCurrency(E1_m_supplier_item_price(M106_E1_pur_prc), ConvSPChars(I1_m_supplier_item_price(M106_I1_pur_cur)), ggUnitCostNo,"X","X") & """" & vbCr
	Response.Write " Parent.frm1.vspdData.Text = """ & UNIConvNumDBToCompanyByCurrency(returnValue(resultindex,M106_E1_pur_prc), ConvSPChars(I1_m_supplier_item_price(M106_I1_pur_cur)), ggUnitCostNo,"X","X") & """" & vbCr
	
	
	Response.Write " Parent.frm1.vspdData.Col  = Parent.C_CostConCd "   & vbCr                              'Add by KSJ 2007.6.20
	Response.Write " Parent.frm1.vspdData.Text = """ & ConvSPChars(returnValue(resultindex,M106_E1_pur_prc_flg)) & """" & vbCr 'Add by KSJ 2007.6.20
	Response.Write " Parent.vspdData_Change Parent.C_Cost , """ & Cint(arrVal(5)) & """" & vbCr  
	'Response.Write " msgbox """ & returnValue(resultindex,M106_E1_pur_prc_flg) & """" & vbCr
	''Response.Write " Parent.frm1.vspdData.Col  = Parent.C_Cost "   & vbCr
	''Response.Write " Parent.frm1.vspdData.Text = """ & ConvSPChars(returnValue(resultindex)) & """" & vbCr
	''Response.Write " Parent.frm1.vspdData.Col  = Parent.C_Cost "   & vbCr
	''Response.Write " Parent.frm1.vspdData.Text = """ & ConvSPChars(returnValue(resultindex)) & """" & vbCr
...
Function CheckPriceType(Byref lgPriceType) 
    ...
    If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL,"X","X") = False Then
		...
		Exit Function
	Else
'		Call ServerMesgBox(lgObjRs(0) , vbInformation, I_MKSCRIPT)		
