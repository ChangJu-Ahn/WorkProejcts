...
Const ivType = "ST"
Const BIZ_PGM_ID        = "M3113RB1.asp"
'Const C_MaxKey          = 26                                    '☆☆☆☆: Max key value
Const C_MaxKey          = 27                                    '☆☆☆☆: Max key value 2007-12-27 Modified by KSJ
...
Sub InitVariables()
	...
	frm1.hdnPoNo.value			= arrParam(12)
	frm1.hdnCurrency.value		= arrParam(13)
	'반품내역등록 - 외주가공여부 조건 추가 
	If UBound(arrparam) > 13 Then
		frm1.hdnSubcontraflg.value	= arrParam(14)
	End if
...
'==========================================================================================================
Sub InitSpreadSheet()
	Call SetZAdoSpreadSheet("M3113RA1","S","A","V20051201",PopupParent.C_SORT_DBAGENT,frm1.vspdData, _
									C_MaxKey, "X","X")
	Call SetSpreadLock 
End Sub
...
Function OpenMvmtNo()
	...
	iCalledAspName = AskPRAspName("M4141PA1")
	
	If Trim(iCalledAspName) = "" Then
		IntRetCD = DisplayMsgBox("900040", PopupParent.VB_INFORMATION, "M4141PA1", "X")
		lblnWinEvent = False
		Exit Function
	End If
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------
		    ...
		    strVal = strVal & "&txtMvmtNo=" & Trim(frm1.hdnMvmtNo.value)				'☆: 조회 조건 데이타 
			strVal = strVal & "&txtFrIvDt=" & Trim(frm1.txtFrIvDt.text)
		    strVal = strVal & "&txtToIvDt=" & Trim(frm1.txtToIvDt.text)
		    strVal = strVal & "&hdnPoNo= " & Trim(frm1.hdnPoNo.value)
		    strVal = strVal & "&hdnSupplierCd= " & Trim(frm1.hdnSupplierCd.value)
		    strVal = strVal & "&hdnCurrency= " & Trim(frm1.hdnCurrency.value)
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------
		    strVal = BIZ_PGM_ID & "?txtMode=" & PopupParent.UID_M0001							'☜: 비지니스 처리 ASP의 상태 
		    strVal = strVal & "&txtMvmtNo=" & Trim(frm1.txtMvmtNo.value)				'☆: 조회 조건 데이타 
		    strVal = strVal & "&txtFrIvDt=" & Trim(frm1.txtFrIvDt.text)
		    strVal = strVal & "&txtToIvDt=" & Trim(frm1.txtToIvDt.text)
		    strVal = strVal & "&hdnPoNo= " & Trim(frm1.hdnPoNo.value)
		    strVal = strVal & "&hdnSupplierCd= " & Trim(frm1.hdnSupplierCd.value)
