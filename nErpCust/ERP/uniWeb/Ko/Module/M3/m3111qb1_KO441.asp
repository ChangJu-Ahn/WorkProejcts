...
'--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
    ...
    If Len(Trim(Request("txtPlantCd"))) Then
		iStrSQL = iStrSQL & " AND B.PLANT_CD =  " & FilterVar(Trim(UCase(Request("txtPlantCd"))), " " , "S") & " "
    End If
     '---구매그룹 
    If Len(Trim(Request("txtPurGrpCd"))) Then
		iStrSQL = iStrSQL & " AND A.PUR_GRP =  " & FilterVar(Trim(UCase(Request("txtPurGrpCd"))), " " , "S") & " "
    End If
     '---공급처 
    If Len(Trim(Request("txtBpCd"))) Then
		iStrSQL = iStrSQL & " AND A.BP_CD  =  " & FilterVar(Trim(UCase(Request("txtBpCd"))), " " , "S") & " "
    End If
     '---발주일 
    If Len(Trim(Request("txtPoFrDt"))) Then
		iStrSQL = iStrSQL & " AND A.PO_DT  >=  " & FilterVar(uniConvDate(Request("txtPoFrDt")), "''", "S") & ""
    Else
		iStrSQL = iStrSQL & " AND A.PO_DT  >= " & FilterVar("1900/01/01", "''", "S") & ""
    End If

    If Len(Trim(Request("txtPoToDt"))) Then
		iStrSQL = iStrSQL & " AND A.PO_DT  <=  " & FilterVar(uniConvDate(Request("txtPoToDt")), "''", "S") & ""
    Else
		iStrSQL = iStrSQL & " AND A.PO_DT  <= " & FilterVar("2999/12/30", "''", "S") & ""
    End If    
    '---품목 
    If Len(Trim(Request("txtItemCd"))) Then
		iStrSQL = iStrSQL & " AND B.ITEM_CD  =  " & FilterVar(Trim(UCase(Request("txtItemCd"))), " " , "S") & " "
    End If

    '---Tracking No
    If Len(Trim(Request("txtTrackNo"))) Then
		iStrSQL = iStrSQL & " AND B.TRACKING_NO  =  " & FilterVar(Trim(UCase(Request("txtTrackNo"))), " " , "S") & " "
    End If
     '---발주형태 
    If Len(Trim(Request("txtPoType"))) Then
		iStrSQL = iStrSQL & " AND A.PO_TYPE_CD  =  " & FilterVar(Trim(UCase(Request("txtPoType"))), " " , "S") & " "
    End If
     '---단가구분 
    If Len(Trim(Request("txtPrcFlg"))) Then
		iStrSQL = iStrSQL & " AND B.PO_PRC_FLG  =  " & FilterVar(Trim(UCase(Request("txtPrcFlg"))), " " , "S") & " "
    End If
     '---마감여부 
    If Len(Trim(Request("rdoClsFlg"))) Then
		iStrSQL = iStrSQL & " AND A.CLS_FLG		=  " & FilterVar(Trim(UCase(Request("rdoClsFlg"))), " " , "S") & " "
    End If

    If Len(Trim(Request("gPlant"))) Then
	    iStrSQL = iStrSQL & " AND B.PLANT_CD		=  " & FilterVar(Trim(UCase(Request("gPlant"))), " " , "S") & " "
    End If
...
'--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------
	 ...
	 UNIValue(0,1)  = UCase(Trim(iStrSQL))
...
<Script Language=vbscript>
    ...
    With Parent
         ...
         Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.Frm1.vspdData,"<%=iFrPoint+1%>",parent.frm1.vspddata.maxrows,Parent.GetKeyPos("A",23), Parent.GetKeyPos("A",24),"A", "Q" ,"X","X")	'발주금액 
         Call Parent.ReFormatSpreadCellByCellByCurrency2(Parent.Frm1.vspdData,"<%=iFrPoint+1%>",parent.frm1.vspddata.maxrows, Parent.Parent.gCurrency , Parent.GetKeyPos("A",25),"A", "Q" ,"X","X")	'발주자국금액				'매입자국금액 
