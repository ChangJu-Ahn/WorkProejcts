...
Sub SubBizSave()																	'☜: 저장 요청을 받음 
	...
	'NOTE ***	발주확정취소후, GroupWare 재상신 *** >>air
	'			함수호출 -> GroupWareBlank(인자1)
	Call GroupWareBlank(Trim(Request("txtPoNo2")))
...
Sub SubReleaseCheck()
	...
	Dim M31211
	Dim strMode, strMode1, lgIntFlgMode
	Dim txtSpread
	...
	lgIntFlgMode = CInt(Request("txtFlgMode"))										'☜: 저장시 Create/Update 판별 
'Call ServerMesgBox("SubReleaseCheck", vbInformation, I_MKSCRIPT)
	...
	'⊙: Lookup Pad 동작후 정상적인 데이타 이면, 저장 로직 시작 
	'NOTE *** >>air / 2008.03.10 ***
	Call GroupWareCheck(Trim(Request("txtPoNo")), strMode1)
'Call ServerMesgBox("GroupWareCheck-out1" , vbInformation, I_MKSCRIPT)	
...
'Call ServerMesgBox("g1" , vbInformation, I_MKSCRIPT)	
    '-----------------------
    'Com action result check area(OS,internal)
    '-----------------------
    If CheckSYSTEMError(Err,True) = true then
			...
			Exit Sub
	End If
'Call ServerMesgBox("g2" , vbInformation, I_MKSCRIPT)
	'-----------------------
	'Com Action Area
	'-----------------------
    ...
    If Len(ls_msg) >= 6 Then
		...
		Else
			If CheckSYSTEMError2(Err,True,"","","","","") = true then 
				Call ServerMesgBox(ls_msg , vbInformation, I_MKSCRIPT)
				Response.Write "<Script Language=VBScript>" & vbCr
			 ...
			 End If
		End If
		'Call ServerMesgBox("g3-1" , vbInformation, I_MKSCRIPT) 
	Else		
		If CheckSYSTEMError2(Err,True,"","","","","") = true then 		
		    Response.Write "<Script Language=VBScript>" & vbCr
		    Response.Write "parent.frm1.btnCfm.disabled = False" & vbCr
			...
			Response.End
		End If
		'Call ServerMesgBox("g3-2" , vbInformation, I_MKSCRIPT) 
	End If	 
'Call ServerMesgBox("g4" , vbInformation, I_MKSCRIPT) 
...
'============================================================================================================
' Name : GroupWareCheck()
' Desc : the case of GroupWareCheck
' >>air
'============================================================================================================
Sub GroupWareCheck(LG1_m_pur_ord_hdr, strMode1)

    
    Dim iVar_EXT1_CD1
    Dim iVar_RELEASE_FLG1
    ...
    iStrSQL = ""
    iStrSQL = iStrSQL & " SELECT  IsNull(EXT1_CD,''), IsNull(RELEASE_FLG,'')    "
    iStrSQL = iStrSQL & " FROM    M_PUR_ORD_HDR(UPDLock)   "
    iStrSQL = iStrSQL & " WHERE   PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
    
    If FncOpenRs("R",lgObjConn,lgObjRs,iStrSQL,"X","X") = True Then
        iVar_EXT1_CD1 = lgObjRs(0)		'결재상태(E:결재완료, Y:결재진행중, '':결재대기)
        iVar_RELEASE_FLG1 = lgObjRs(1)	'확정여부
        
        Call SubCloseRs(lgObjRs) 'Close RocordSet >>air 
        'debugging>>air
        'Call ServerMesgBox(iVar_RELEASE_FLG1, vbInformation, I_MKSCRIPT)
        
        Select Case iVar_EXT1_CD1 
        	Case "E"
					'Call ServerMesgBox("GroupWareE-1" , vbInformation, I_MKSCRIPT)	
        		Call GroupWareE(LG1_m_pur_ord_hdr, iVar_RELEASE_FLG1, strMode1)
					'Call ServerMesgBox("GroupWareE-2" , vbInformation, I_MKSCRIPT)	        		
        	Case "Y"
					'Call ServerMesgBox("GroupWareY-1" , vbInformation, I_MKSCRIPT)	        	
        		Call GroupWareY(LG1_m_pur_ord_hdr, iVar_RELEASE_FLG1)
					'Call ServerMesgBox("GroupWareY-2" , vbInformation, I_MKSCRIPT)	        	        		
        	'Case ""	
        	'	Call GroupWareBlank()
    	End Select
...
'결재완료상태
'>>air
Sub GroupWareE(LG1_m_pur_ord_hdr, iVar_RELEASE_FLG1, strMode1)
...
'Call ServerMesgBox("GroupWareE-IN" , vbInformation, I_MKSCRIPT)	
'	If strMode1 = "Y" Then	'발주확정
'		iStrSQL = ""
'		iStrSQL = iStrSQL & " UPDATE M_PUR_ORD_HDR SET RELEASE_FLG = 'Y' "
'		iStrSQL = iStrSQL & "                  WHERE PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
'		iStrSQL = iStrSQL & "                    AND EXT1_CD = 'E' "
'		iStrSQL = iStrSQL & "                    AND RELEASE_FLG = 'N' "
'		
'	    lgObjConn.Execute iStrSQL,,adCmdText + adExecuteNoRecords
'	    
'	    If CheckSYSTEMError(Err,True) = True Then
'	       lgErrorStatus    = "YES"
...
'	Else	'발주확정취소
'		iStrSQL = ""
'		iStrSQL = iStrSQL & " UPDATE M_PUR_ORD_HDR SET RELEASE_FLG = 'N' "
'		iStrSQL = iStrSQL & "                  WHERE PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
'		iStrSQL = iStrSQL & "                    AND EXT1_CD = 'E' "
'		iStrSQL = iStrSQL & "                    AND RELEASE_FLG = 'Y' "
'		
'	    lgObjConn.Execute iStrSQL,,adCmdText + adExecuteNoRecords
'	    
'	    If CheckSYSTEMError(Err,True) = True Then
...
'결재진행상태
Sub GroupWareY(LG1_m_pur_ord_hdr, iVar_RELEASE_FLG1)
	'Call ServerMesgBox("GroupWareY" , vbInformation, I_MKSCRIPT)	
	'If iVar_RELEASE_FLG1 = "N" Then
		Call DisplayMsgBox("899999", vbInformation, "", "", I_MKSCRIPT)
		'결재진행중입니다.
		Call ExitDbSave()	'결재진행중이므로, 발주확정취소
	'End If	
...
'결재대기상태 >>air
'no problem
Sub GroupWareBlank(LG1_m_pur_ord_hdr)
    ...
    On Error Resume Next                                                             '☜: Protect system from crashing
    Err.Clear                                                                        '☜: Clear Error status
	'Call ServerMesgBox("GroupWareBlank" , vbInformation, I_MKSCRIPT)	
    '---------- Developer Coding part (Start) ---------------------------------------------------------------
    ...
    Call SubOpenDB(lgObjConn)
		...
		iStrSQL = ""
		iStrSQL = iStrSQL & " UPDATE M_PUR_ORD_HDR SET EXT1_CD = ''  " 
		iStrSQL = iStrSQL & "                  WHERE PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
		iStrSQL = iStrSQL & "                    AND EXT1_CD <> '' "
		iStrSQL = iStrSQL & "                    AND RELEASE_FLG = 'N' "
		'
	'Call ServerMesgBox(iStrSQL , vbInformation, I_MKSCRIPT)
    '---------- Developer Coding part (End  ) ---------------------------------------------------------------
    lgObjConn.Execute iStrSQL,,adCmdText + adExecuteNoRecords
...
Sub ExitDbSave()
	...
	Response.Write "End With"   & vbCr
	Response.Write "</Script>" & vbCr
'Call ServerMesgBox("ExitDbSave" , vbInformation, I_MKSCRIPT)	
	Response.End	
End Sub
