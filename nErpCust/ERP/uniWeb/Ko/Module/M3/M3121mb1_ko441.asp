...
Sub GroupWareCheck(LG1_m_pur_ord_hdr, strMode1)
    ...
    lgStrSQL = ""
    lgStrSQL = lgStrSQL & " SELECT  IsNull(EXT1_CD,''), IsNull(RELEASE_FLG,'')    "
    lgStrSQL = lgStrSQL & " FROM    M_PUR_ORD_HDR(UPDLock)   "
    lgStrSQL = lgStrSQL & " WHERE   PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
...
'		lgStrSQL = ""
'		lgStrSQL = lgStrSQL & " UPDATE M_PUR_ORD_HDR SET RELEASE_FLG = 'Y' "
'		lgStrSQL = lgStrSQL & "                  WHERE PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
'		lgStrSQL = lgStrSQL & "                    AND EXT1_CD = 'E' "
'		lgStrSQL = lgStrSQL & "                    AND RELEASE_FLG = 'N' "
'		
...
'	Else	'발주확정취소
'		lgStrSQL = ""
'		lgStrSQL = lgStrSQL & " UPDATE M_PUR_ORD_HDR SET RELEASE_FLG = 'N' "
'		lgStrSQL = lgStrSQL & "                  WHERE PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
'		lgStrSQL = lgStrSQL & "                    AND EXT1_CD = 'E' "
'		lgStrSQL = lgStrSQL & "                    AND RELEASE_FLG = 'Y' "
'		
'	    lgObjConn.Execute lgStrSQL,,adCmdText + adExecuteNoRecords
'	    
'	    If CheckSYSTEMError(Err,True) = True Then
...
Sub GroupWareBlank(LG1_m_pur_ord_hdr)
    ...
    Call SubOpenDB(lgObjConn)
		...
		lgStrSQL = ""
		lgStrSQL = lgStrSQL & " UPDATE M_PUR_ORD_HDR SET EXT1_CD = ''  " 
		lgStrSQL = lgStrSQL & "                  WHERE PO_NO = " & FilterVar(LG1_m_pur_ord_hdr, "''", "S")
		lgStrSQL = lgStrSQL & "                    AND EXT1_CD <> '' "
		lgStrSQL = lgStrSQL & "                    AND RELEASE_FLG = 'N' "
		'
