...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "M3113RA1"									'* : 데이터 조회를 위한 SQL문 만듬 
 
    '--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
    UNIValue(0,0) = lgSelectList                                          '☜: Select list
    '--------------- 개발자 coding part(실행로직,Start)----------------------------------------------------

	strVal = " AND F.IV_FLG <> " & FilterVar("Y", "''", "S") & " "		' 재반입시 대금정산으로 출고된 건 조회 않되게 함 

	If Trim(Request("txtMvmtNo")) <> "" Then
		strVal = strVal & " AND A.MVMT_RCPT_NO >= " & FilterVar(Trim(UCase(Request("txtMvmtNo"))), " " , "S") & "  AND A.MVMT_RCPT_NO <=  " & FilterVar(Trim(UCase(Request("txtMvmtNo"))), " " , "S") & " "
	End If
    ...
    If Trim(Request("hdnPoNo")) <> "" Then
		strVal = strVal & " AND F.PO_NO >= " & FilterVar(Trim(UCase(Request("hdnPoNo"))), " " , "S") & "  AND F.PO_NO <=  " & FilterVar(Trim(UCase(Request("hdnPoNo"))), " " , "S") & " "
    End If
    
    If Trim(Request("hdnCurrency")) <> "" Then
        strVal = strVal & " AND A.MVMT_CUR = " & FilterVar(Trim(UCase(Request("hdnCurrency"))), " " , "S") & " "		
    ELSE
        strVal = strVal & " AND A.MVMT_CUR = " & FilterVar("zzzzzzzzz", "''", "S") & ""		
    End If
    
    If Trim(Request("hdnSupplierCd")) <> "" Then
        strVal = strVal & " AND A.BP_CD = " & FilterVar(Trim(UCase(Request("hdnSupplierCd"))), " " , "S") & " "		
    ELSE
	...
	If Len(Request("txtSubcontraflg")) Then
		strVal = strVal & " AND F.SUBCONTRA_FLG = " & FilterVar(Trim(UCase(Request("txtSubcontraflg"))), " " , "S") & ""		
	End If
