...
'========================================================================================================
Const BIZ_PGM_ID		= "C4110Mb1.asp"                                      'Biz Logic ASP 
'========================================================================================================
...
Sub InitSpreadSheet()
	...
	Call SetZAdoSpreadSheet("C4110MA1","S","A","V20070301", Parent.C_SORT_DBAGENT, frm1.vspdData, 3, "X", "X" )
...
Function OpenPopup(ByVal iWhere)
	...
	select case iWhere
		...
		case 3
			...
			arrParam(3) = ""							' Name Cindition
			arrParam(4) = "a.major_cd = 'I0002' "		' Where Condition
			arrParam(5) = "수불구분"			
		...
		case 4
			...
			arrParam(4) = "a.major_cd = 'I0001' and a.minor_cd = b.mov_type "		' Where Condition

			IF Trim(frm1.txtTrnsTypeCd.value) <> "" Then
				arrParam(4) = arrParam(4) & " and b.trns_type = " & FilterVar(frm1.txtTrnsTypeCd.value, "''", "S")
			END IF
		...
		case 5
			...
			arrParam(4) = "a.major_cd = 'P1001' and a.minor_cd = b.item_acct and b.item_acct_group in ('1FINAL','2SEMI','3RAW','4SUB','5GOODS') "
		...
		case 6
			...
			IF Trim(frm1.txtItemAcctCd.value) <> "" Then
				arrParam(4) = arrParam(4) & " and b.item_acct = " & FilterVar(frm1.txtItemAcctCd.value, "''", "S")
			End If
		...
		case 7
			arrParam(0) = "Tracking 팝업"				' 팝업 명칭 
			arrParam(1) = "pms_tracking"					' TABLE 명칭 
...
'				arrParam(4) = arrParam(4) & " a.item_cd = " & FilterVar(frm1.txtItemCd.value, "''", "S")
'			End If

			arrParam(5) = "Tracking No"			
	
			arrField(0) = "Tracking_No"					' Field명(0)
			arrField(1) = "Project_code"					' Field명(1)
    
			arrHeader(0) = "Tracking No"				' Header명(0)
			arrHeader(1) = "프로젝트코드"				' Header명(1)
	end select
...
Function DbQuery()
	...
	IF lgIntFlgMode <> Parent.OPMD_UMODE Then
		...
		IF frm1.txtTrnsTypeCd.value <> "" Then
			intRetCd = CommonQueryRs("minor_nm","b_minor","major_cd = 'I0002' and minor_cd = " & FilterVar(Trim(frm1.txtTrnsTypeCd.value),"''","S" ),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
			IF intRetcd = True Then
				arrTemp = Split(lgF0,Chr(11))
		...
		IF frm1.txtMovTypeCd.value <> ""  Then
			intRetCd = CommonQueryRs("minor_nm","b_minor","major_cd = 'I0001' and minor_cd = " & FilterVar(Trim(frm1.txtMovTypeCd.value),"''","S" ),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
			IF intRetcd = True Then
				arrTemp = Split(lgF0,Chr(11))
		...
		IF frm1.txtItemAcctCd.value <> "" Then
			intRetCd = CommonQueryRs("minor_nm","b_minor","major_cd = 'P1001' and minor_cd = " & FilterVar(Trim(frm1.txtItemAcctCd.value),"''","S" ),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
			IF intRetcd = True Then
				arrTemp = Split(lgF0,Chr(11))
...
'=======================================================================================================
Function ExeBatchJob(ByVal strFlag)
	Dim IntRetCD 
	...
	ExeBatchJob = False															 '☜: Processing is NG
	Err.Clear                                                                    '☜: Clear err status
	...
	strVal = BIZ_PGM_ID & "?txtMode=" & strFlag
	strVal = strVal & "&txtYyyymm=" & strYear&strMonth

	Call RunMyBizASP(MyBizASP, strVal)                                           '☜:  Run biz logic
	    
	ExeBatchJob = True                                                              '☜: Processing is OK
...
Function OpenPopupGL()
	...
	' ADO 필드에서 Key 값을 가져온다.
	Call SetSpreadColumnValue("A", frm1.vspdData, frm1.vspdData.Col, frm1.vspdData.Row)

	strItemDocumentNo = GetKeyPosVal("A",1)		' 수불번호 
	
	if Trim(strItemDocumentNo) = "" then
	...
	arrParam(0) = ""			'회계전표번호 
	arrParam(1) = Trim(strItemDocumentNo) & "-" & Trim(frm1.txtYyyymm.Year)   '수불번호'
...
Function OpenMoveDtlRef()
	...
	With frm1.vspdData	
		...
		Param3 = GetKeyPosVal("A",1)	'수불번호 
		Param6 = GetKeyPosVal("A",3)	'수불구분명. 재고 팝업에서 수불구분명을 꼭 필요로 하므로...이해안됨???
	    
		' 수불번호와 전표번호를 보내줘야 팝업이 사용됨 
		...
		' 레퍼런스팝업에 사업장을 보여주기 위해...이해안됨???
		
		IntRetCD = CommonQueryRs("a.biz_area_cd,b.biz_area_nm","i_goods_movement_header a(nolock), b_biz_area b(nolock)","a.biz_area_cd = b.biz_area_cd and a.item_document_no = " & FilterVar(Param3, "''", "S"),lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
					...
					<TD HEIGHT=20 WIDTH=100%>
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								...
								<TR>
									<TD CLASS="TD5">코스트센터</TD>
									<TD CLASS="TD6">
										<INPUT TYPE=TEXT NAME="txtCostCd" SIZE=10 MAXLENGTH=10 tag="11XXXU" ALT="코스트센터"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCostCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenPopup(2)">
								...
								<TR>
									<TD CLASS="TD5">품목</TD>
									<TD CLASS="TD6">
										<INPUT TYPE=TEXT NAME="txtItemCd" SIZE=15 MAXLENGTH=18 tag="11XXXU" ALT="품목"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnItemCd" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenPopup(6)">
										<INPUT TYPE=TEXT NAME="txtItemNm" SIZE=30 tag="14X">
									</TD>
									<TD CLASS="TD5">Tracking No</TD>
									<TD CLASS="TD6">
										<INPUT TYPE=TEXT NAME="txtTrackingNo" SIZE=15 MAXLENGTH=25 tag="11XXXU" ALT="Tracking No"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnTrackingNo" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:OpenPopup(7)">
									</TD>
								</TR>
		...
		<TD WIDTH=100%>
			<TABLE <%=LR_SPACE_TYPE_30%>>
					<TR>
					    <TD WIDTH=10>&nbsp;</TD>
						<TD>
						    <BUTTON NAME="btnExe" CLASS="CLSSBTN" ONCLICK="vbscript:ExeBatchJob('ExeReflect')" >반영</BUTTON>&nbsp;<BUTTON NAME="btnCnc" CLASS="CLSSBTN" ONCLICK="vbscript:ExeBatchJob('ExeCancel')" >취소</BUTTON>
						</TD>
						<TD WIDTH=*>&nbsp;</TD>
