...
'*  4. Program Name         : 제3호의2(1)(3)표준대차대조표 
'*  5. Program Desc         : 제3호의2(1)(3)표준대차대조표 등록/조회 
'*  6. Component List       : 
...
Const BIZ_PGM_ID 		= "W1101MB1.asp"											 '☆: 비지니스 로직 ASP명 
Const BIZ_PGM_ID2 		= "W1101MB2.asp"											 '☆: 비지니스 로직 ASP명 
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData
		...
		ggoSpread.SSSetFloat    C_DR_INV_AMT,			"차변잔액",				15,		Parent.ggAmtOfMoneyNo,		ggStrIntegeralPart,		ggStrDeciPointPart,		Parent.gComNum1000,		Parent.gComNumDec,  "1", True,   "Z"
		ggoSpread.SSSetFloat    C_DR_SUM_AMT,			"차변합계",				15,		Parent.ggAmtOfMoneyNo,		ggStrIntegeralPart,		ggStrDeciPointPart,		Parent.gComNum1000,		Parent.gComNumDec,  "1", True,   "Z"
		ggoSpread.SSSetEdit     C_GP_NM,				"계정과목",				25
		ggoSpread.SSSetEdit     C_FISC_CD,				"코드",					5, 2
		ggoSpread.SSSetFloat    C_CR_SUM_AMT,			"대변합계",				15,		Parent.ggAmtOfMoneyNo,		ggStrIntegeralPart,		ggStrDeciPointPart,		Parent.gComNum1000,		Parent.gComNumDec,  "1", True,   "Z"
		ggoSpread.SSSetFloat    C_CR_INV_AMT,			"대변잔액",				15,		Parent.ggAmtOfMoneyNo,		ggStrIntegeralPart,		ggStrDeciPointPart,		Parent.gComNum1000,		Parent.gComNumDec,  "1", True,   "Z"
...
'	Next
	...
	'-----------------------
	' 필수입력 금액 확인 
	' 	자산총계 (차변합계 - 대변합계) 와 부채와 자본총계 (대변합계 - 차변합계 ) 가 일치하여야 함.		
	'	자산총계의 대변합계가 0보다 작거나 같으면 오류		
	'	자산계정		
	'		각 계정의 (차변합계 - 대변합계)는 잔액기준 계정별 잔액과 일치하여야 함.	
	'	부채 및 자본계정		
	'		각 계정의 (대변합계 - 차변합계)는 잔액기준 계정별 잔액과 일치하여야 함.	
	'-----------------------
    ...
    If UNICDbl(Frm1.vspdData.text) <= 0 Then
        Call DisplayMsgBox("WC0006", "X", "자산총계의 대변합계", "X")                          
        Exit Function
    End If
    ...
    If iDRInvAmt <> iCRInvAmt Then
        Call DisplayMsgBox("WC0004", "X", "자산총계", "부채와 자본총계")                          
        Exit Function
    End If
