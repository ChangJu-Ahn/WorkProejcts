...
'*  4. Program Name         : 제3호의3(3) 부속명세서-분양원가명세서 
'*  5. Program Desc         : 제3호의3(3) 부속명세서-분양원가명세서 등록/조회 
'*  6. Component List       : 
...
'
'서식간 검증 
'      24.접대비 >0 인경우 47호(갑) (117)의 ③ < 0 이면 오류	오류	WC0024	법인세 제47호(갑) 주요계정명세서의 접대비중 ③회사계상금액이 존재하지 않습니다.	%1의 %2중 %3이 존재하지 않습니다.
'***********************************************************************k*********************** -->
...
Const BIZ_PGM_ID 		= "W1113MB1.asp"											 '☆: 비지니스 로직 ASP명 
Const BIZ_PGM_ID2 		= "W1113MB2.asp"											 '☆: 비지니스 로직 ASP명 
...
'      ggoSpread.SpreadLockWithOddEvenRowColor()
   ...
   With frm1
	...
	ggoSpread.SSSetProtected C_AMT, 3, 3
	ggoSpread.SSSetProtected C_AMT, 7, 7
	ggoSpread.SSSetProtected C_AMT, 12, 12
	ggoSpread.SSSetProtected C_AMT, 29, 29
	ggoSpread.SSSetProtected C_AMT, 32, 32
	ggoSpread.SSSetProtected C_AMT, 35, 35
...
'	FORM_REP_NO	:	01		02 + 03 를 계산하여 입력함 
'					02		사용자가 숫자로 직접입력함.
'					03		( 04 + 05 - 06 ) 를 계산하여 입력함.
'					04~06		사용자가 숫자로 직접입력함.
'					07		( 08 + 09 + 10 ) 를 계산하여 입력함.
'					08~11		사용자가 숫자로 직접입력함.
'					12		13 ~ 27 의 합계를 입력함.
'					13~27		사용자가 숫자로 직접입력함.
'					28		( 1 + 7 + 11 + 12 )를 계산하여 입력함.
'					29~30		사용자가 숫자로 직접입력함.
'					31		( 28 + 29 + 30 ) 를 계산하여 입력함.
'					32~33		사용자가 숫자로 직접입력함.
'					34		( 31 - 32 - 33 ) 를 계산하여 입력함.
'========================================================================================================
Function SubMakeSum()
	Dim iSchRow
	Dim iSumAmt
	...
	' -- 2006.03 개정 
	iSchRow = Frm1.vspdData.SearchCol(C_FORM_REP_NO, 0, Frm1.vspdData.MaxRows, "35", 0)
   	Frm1.vspdData.Row = iSchRow
	Frm1.vspdData.Col = C_AMT
	iSumAmt = iSumAmt + UNICDbl(Frm1.vspdData.text)


	iSchRow = Frm1.vspdData.SearchCol(C_FORM_REP_NO, 0, Frm1.vspdData.MaxRows, "12", 0)
   	Frm1.vspdData.Row = iSchRow
	Frm1.vspdData.Col = C_AMT
	Frm1.vspdData.text  = iSumAmt
	...
	iSchRow = Frm1.vspdData.SearchCol(C_FORM_REP_NO, 0, Frm1.vspdData.MaxRows, "11", 0)
   	Frm1.vspdData.Row = iSchRow
	Frm1.vspdData.Col = C_AMT
	iSumAmt = iSumAmt + UNICDbl(Frm1.vspdData.text)
					...
					<TD CLASS="CLSMTABP_BAK">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTABP_BAK"><font color=white>제3호의3(3) 부속명세 분양원가</font></td>
