...
Sub SubBizQueryMulti()
    ...
    if  Trim(lgKeyStream(3)) <> "" then
         iKey1 = iKey1 & " AND a.wc_cd LIKE " &	FilterVar(Trim(lgKeyStream(3)) & "%", "''", "S")
    end if
...
'            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("std_time"))
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("INSIDE_FLG"))
        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               Case "R"
                       ...
                       lgStrSQL = lgStrSQL & "   		A.WC_CD WC_CD,                                               "
                       lgStrSQL = lgStrSQL & "   		A.WC_NM WC_NM,                                               "
                       lgStrSQL = lgStrSQL & "   		CASE WHEN A.INSIDE_FLG  = 'N' THEN '사내'                    "
                       lgStrSQL = lgStrSQL & "                WHEN A.INSIDE_FLG = 'Y' THEN '외주'                    "
                       lgStrSQL = lgStrSQL & "           END INSIDE_FLG,                                             "
                       ...
                       lgStrSQL = lgStrSQL & "             WHEN A.MES_RECEIVE_FLAG = 'N' THEN NULL                   "
                       lgStrSQL = lgStrSQL & "             ELSE A.MES_RECEIVE_DT                                     "
                       lgStrSQL = lgStrSQL & "           END MES_RECEIVE_DT                                          "
                       lgStrSQL = lgStrSQL & "   FROM	T_IF_SND_WORK_CENTER_KO441 A                                "
                       lgStrSQL = lgStrSQL & "   WHERE	a.plant_cd like '%'                                            " & pCode
...
'                       lgStrSQL = lgStrSQL & "FROM    T_IF_SND_ITEM_KO441 a, B_ITEM b, B_MINOR c, B_ITEM_GROUP d "
