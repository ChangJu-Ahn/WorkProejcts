...
'--------------- 개발자 coding part(변수선언,Start)--------------------------------------------------------
    ...
    lgChargeCd		= Trim(Request("txtHConSalesGrp"))
...
Sub FixUNISQLData()
	...
	If Len(lgFromDt) Then
		iStrVal = iStrVal & " SC.CHARGE_DT >=  " & FilterVar(UNIConvDate(lgFromDt), "''", "S") & ""		
	End If		
	
	'조회기간끝===========================================================================================
	If Len(lgToDt) Then
		iStrVal = iStrVal & " AND SC.CHARGE_DT <=  " & FilterVar(UNIConvDate(lgToDt), "''", "S") & ""		
	End If
	...
	If Len(lgBizAreaCd) Then
		...
		iStrVal = iStrVal & " AND SC.BIZ_AREA =  " & FilterVar(lgBizAreaCd , "''", "S") & ""				
	End If	
	
	'경비항목명===========================================================================================	
    If Len(lgChargeCd) Then
		UNISqlId(2)		= "M6111QA102"	
		UNIValue(2,0)	= " " & FilterVar(lgChargeCd, "''", "S") & ""
		iStrVal = iStrVal & " AND SC.CHARGE_CD =  " & FilterVar(lgChargeCd , "''", "S") & ""				
	End If									

	'확정여부=====================================================================================	
	If lgConfFlag = "Y" Then
		iStrVal = iStrVal & " AND SC.POSTING_FLAG =" & FilterVar("Y", "''", "S") & " "
	ElseIf lgConfFlag = "N" Then
		iStrVal = iStrVal & " AND SC.POSTING_FLAG =" & FilterVar("N", "''", "S") & " "
	End If	
	
	UNISqlId(0) = "S6113QA101"					
    UNIValue(0,0) = lgSelectList                                      
	UNIValue(0,1) = iStrVal	         
    
    UNISqlId(3) = "S6113QA102"
    UNIValue(3,0) = iStrVal                                      
...
Sub QueryData()
	...
	If lgChargeCd <> "" Then
		If rs2.EOF And rs2.BOF Then
			rs2.Close
			Set rs2 = Nothing			
			Call ConNotFound("txtConCharge")			
			Exit Sub
		Else	
			Call WriteConDesc("txtConChargeNm", rs2(1))		
		End If
	Else
		Call WriteConDesc("txtConChargeNm", "")
	End If
...
Sub WriteResult()
	...
	Response.Write " Parent.frm1.txtChargeTotNetAmtLoc.text = """ & UNINumClientFormatByCurrency(rs3(0),lgCurrency,ggAmtOfMoneyNo) & """" & vbCr
	Response.Write " Parent.frm1.txtChargeNetAmtLoc.text = """ & UNINumClientFormatByCurrency(rs3(1),lgCurrency,ggAmtOfMoneyNo) & """" & vbCr
	Response.Write " Parent.frm1.txtChargeVATAmtLoc.text = """ & UNINumClientFormatByCurrency(rs3(2),lgCurrency,ggAmtOfMoneyNo) & """" & vbCr		
