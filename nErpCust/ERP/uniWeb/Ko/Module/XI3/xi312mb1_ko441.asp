...
Err.Clear
	...
	Else
		UNIValue(0, 7) = FilterVar(UCase(Trim(Request("rdoFlagF"))), "''", "S")
	End If
	
'Call ServerMesgBox("HANC : " &  , vbInformation, I_MKSCRIPT)
'	If Trim(Request("rdoFlagF")) = "A" Then
...
'	End If
	...
	If Trim(Request("txtProdtOrderNo")) <>"" Then 
		UNIValue(0, 8) = FilterVar(UCase(Trim(Request("txtProdtOrderNo")))&"%","''","S")
	Else
		UNIValue(0, 8) = FilterVar(""&"%", "''", "S") 
	End If
	If Trim(Request("txtToOper")) <>"" Then 
		UNIValue(0, 9) = FilterVar(UCase(Trim(Request("txtToOper")))&"%","''","S")
	Else
		UNIValue(0, 9) = FilterVar(""&"%", "''", "S") 
	End If
	If Trim(Request("txtWcCd")) <>"" Then 
		UNIValue(0, 10) = FilterVar(UCase(Trim(Request("txtWcCd")))&"%","''","S")
	Else
...
%>
			...
			strData = strData & Chr(11) & "<%=UniNumClientFormat(rs0("loss_qty"),ggQty.DecPoint,0)%>"
			strData = strData & Chr(11) & "<%=UniNumClientFormat(rs0("bonus_qty"),ggQty.DecPoint,0)%>"
			...
			strData = strData & Chr(11) & "<%=Trim(ConvSPChars(rs0("to_oper")))%>"					
