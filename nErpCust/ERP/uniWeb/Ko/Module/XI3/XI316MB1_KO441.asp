...
<%
    ...
    strSQL =          " SELECT  A.MACHINE_CD,                     "
    strSQL = strSQL & " 		A.MACHINE_NM,                     "
    strSQL = strSQL & " 		A.CREATE_TYPE,                    "
    strSQL = strSQL & " 		A.ERP_RECEIVE_DT,                 "
    strSQL = strSQL & " 		A.ERP_APPLY_FLAG,                 "
    strSQL = strSQL & " 		A.ERR_DESC                        "
    strSQL = strSQL & " FROM	T_IF_RCV_MACHINE_CD_KO441	A     "
	strSQL = strSQL & " WHERE"
	strSQL = strSQL & "      MACHINE_CD   BETWEEN (CASE '" & Request("txtBp_cdFrom") & "' WHEN '' THEN ''           ELSE '" & Request("txtBp_cdFrom") & "'     END)"
	...
	strSQL = strSQL & "	 AND ERP_APPLY_flag LIKE (CASE '" & Request("txtRadioFlag") & "' WHEN '' THEN '%' ELSE '" & Request("txtRadioFlag") & "' END)"
    strSQL = strSQL & " ORDER	BY SEND_DT                        "
...
'	strSQL = strSQL & "	FROM"
'	strSQL = strSQL & "  t_if_snd_biz_part_ko441 (nolock) "
'	strSQL = strSQL & " WHERE"
    ...
    Else
		...
		Do While Not lgObjRs.EOF
			If iLngRow < C1_SHEETMAXROWS_D Then
				Response.Write Chr(11) & lgObjRs("MACHINE_CD") 
				Response.Write Chr(11) & lgObjRs("MACHINE_NM")
