...
'		strVal = strval & " AND ISNULL(A.SEC_INVOICE_NO,'') = '' AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) = 'PS' AND A.DELIVERY_HOLD_FG <> 'Y' and a.pallet_no not in (select pallet_no from S_Prev_Integrate_Lbl_Ko119)"
   ...
   if Trim(Request("txtRadio")) = "MINV" or Trim(Request("txtRadio")) = "SINV" or Trim(Request("txtRadio")) = "MOUT" or Trim(Request("txtRadio")) = "SOUT" then
	...
	'양산입고
	UNIValue(1,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd where " & strVal3 & "AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) <> 'PS'" & strVal5 & ""
	'Sample입고
	UNIValue(2,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd where " & strVal3 & "AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) = 'PS'"	& strVal5 & ""
	'양산출고
	UNIValue(3,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd where " & strVal3 & "AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) <> 'PS' AND ISNULL(A.SEC_INVOICE_NO,'') <> '' " & strVal5 & ""
	'Sample출고
	UNIValue(4,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join  B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd where " & strVal3 & "AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) = 'PS' AND ISNULL(A.SEC_INVOICE_NO,'') <> '' " & strVal5 & ""
    '가상출고
    UNIValue(5,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd inner join  S_Prev_Integrate_Lbl_Ko119  C on  A.PALLET_NO  =  C.PALLET_NO Where " & strVal3 & strVal5 & ""	
    '양산Hold재고
    UNIValue(6,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join  B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd where " & strVal3 & "AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) <> 'PS' AND A.DELIVERY_HOLD_FG = 'Y' " & strVal5 & ""
	'Sample Hold 재고
	UNIValue(7,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join  B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd where " & strVal3 & "AND UPPER(LEFT(A.PRODT_ORDER_NO,2)) = 'PS' AND A.DELIVERY_HOLD_FG = 'Y' " & strVal5 & ""
    '양산가용재고
    UNIValue(8,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join  B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd "
	...
	'샘플가용재고
	UNIValue(9,0)  = "SELECT sum(isnull(A.tray_item_qty,0)) from T_IF_RCV_TRAY_INFO_KO119 A (nolock) inner join  B_ITEM_MAPPING_KO441 b (nolock) on a.item_cd = b.item_cd "
