...
'*  2. Function Name		: 
'*  3. Program ID			: xi313ma1_KO441
'*  4. Program Name			: 생산입고실적수신 
'*  5. Program Desc			:
...
'**********************************************************************************************************
Const BIZ_PGM_QRY_ID = "xi313mb1_KO441.asp"											'☆: 비지니스 로직(Qeury) ASP명 
'Post Production Results
Const BIZ_PGM_SAVE_ID	= "xi313mb2_KO441.asp"		
Const BIZ_PGM_SAVE_ID2	= "xi313mb3_KO441.asp"	
'==========================================  1.2.1 Global 상수 선언  ======================================
'==========================================================================================================
...
Sub Form_Load()
    ...
    Call ggoOper.FormatNumber(frm1.txtSendStartHH,    23, 0, False)
    Call ggoOper.FormatNumber(frm1.txtSendStartMM,    59, 0, False)
    Call ggoOper.FormatNumber(frm1.txtSendEndHH, 23, 0, False)
    Call ggoOper.FormatNumber(frm1.txtSendEndMM, 59, 0, False) 
    Call ggoOper.FormatNumber(frm1.txtPlanStartHH, 23, 0, False)
    Call ggoOper.FormatNumber(frm1.txtPlanStartMM, 59, 0, False) 
    Call ggoOper.FormatNumber(frm1.txtPlanEndHH, 23, 0, False)
    Call ggoOper.FormatNumber(frm1.txtPlanEndMM, 59, 0, False) 
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData 
		...
		.MaxCols = C_PlantCd + 1												'☜: 최대 Columns의 항상 1개 증가시킴    
		.MaxRows = 0
		
		Call GetSpreadColumnPos("A")
		
		ggoSpread.SSSetEdit		C_ProdtOrderNo,			"제조오더번호", 15
		ggoSpread.SSSetEdit		C_TagNo,				"TAG No",		15
		ggoSpread.SSSetEdit		C_ResultIfSeq,			"실적전송순번",	10
		...
		ggoSpread.SSSetEdit		C_LotNo,				"LOT번호",		15
		ggoSpread.SSSetFloat	C_ProdQtyInOrderUnit, 	"실적수량",		12, parent.ggQtyNo,ggStrIntegeralPart, ggStrDeciPointPart,parent.gComNum1000,parent.gComNumDec,,,"Z"
		ggoSpread.SSSetEdit 	C_JobStatus, 			"작업상태",		 8		
		ggoSpread.SSSetEdit 	C_JobTime, 				"실적시간",		 8,2
		ggoSpread.SSSetEdit		C_JobLine,				"라인",			 6,2
		ggoSpread.SSSetEdit		C_CreateType,			"생성구분",		 8,2			
		ggoSpread.SSSetEdit		C_ResultSeq,			"ERP실적순번",	10
		ggoSpread.SSSetEdit		C_SendDt ,				"MES송신일시" , 15,2
		ggoSpread.SSSetEdit		C_ReceiveDt ,			"ERP수신일시" , 25,2
		ggoSpread.SSSetEdit		C_ApplyFlag,			"ERP반영여부",	10,2		
		ggoSpread.SSSetEdit 	C_ErrDesc, 				"에러내역",		30
		
		ggoSpread.SSSetEdit		C_RcptDocumentNo,		"생산입고번호", 10,2		
		ggoSpread.SSSetEdit 	C_RcptDocumentYear, 	"생산입고년도", 12
		ggoSpread.SSSetEdit		C_PopInfKey,			"PopKey", 6
		ggoSpread.SSSetEdit		C_PlantCd,				"공장", 6

		Call ggoSpread.SSSetColHidden(C_PopInfKey, .MaxCols, True)
		Call ggoSpread.SSSetColHidden(C_TagNo, .MaxCols, True)          '20080225::hanc
 	...
 	Select Case UCase(pvSpdNo)
 		Case "A"
 			...
 			Call ggoSpread.GetSpreadColumnPos(iCurColumnPos)
				...
				C_JobStatus			= iCurColumnPos(9)
				C_JobTime			= iCurColumnPos(10)
				C_JobLine			= iCurColumnPos(11)
				C_CreateType		= iCurColumnPos(12)
				C_ResultSeq			= iCurColumnPos(13)
				C_SendDt			= iCurColumnPos(14)
				C_ReceiveDt			= iCurColumnPos(15)
				C_ApplyFlag			= iCurColumnPos(16)
				C_ErrDesc			= iCurColumnPos(17)

				C_RcptDocumentNo	= iCurColumnPos(18)
				C_RcptDocumentYear	= iCurColumnPos(19)
				C_PopInfKey			= iCurColumnPos(20)
				C_PlantCd			= iCurColumnPos(21)
...
Sub vspdData_TopLeftChange(ByVal OldLeft , ByVal OldTop , ByVal NewLeft , ByVal NewTop )
    ...
    if frm1.vspdData.MaxRows < NewTop + VisibleRowCnt(frm1.vspdData,NewTop) Then
		If lgStrPrevKey1 <> "" And lgStrPrevKey2 <> "" And lgStrPrevKey3 <> "" And _
		   lgStrPrevKey4 <> "" And lgStrPrevKey5 <> "" And lgStrPrevKey6 <> "" Then				'⊙: 다음 키 값이 없으면 더 이상 업무로직ASP를 호출하지 않음 
...
Function DbQuery() 
	...
	If lgIntFlgMode = parent.OPMD_UMODE Then
		...
		strVal = strVal & "&txtSendStartHh="   & Trim(.hSendStartHh.Value)
		strVal = strVal & "&txtSendStartMm="   & Trim(.hSendStartMm.Value)
		strVal = strVal & "&txtSendEndDt="     & Trim(.hSendEndDt.Value)
		strVal = strVal & "&txtSendEndHh="     & Trim(.hSendEndHh.Value)
		strVal = strVal & "&txtSendEndMm="     & Trim(.hSendEndMm.Value)
		strVal = strVal & "&txtProdtOrderNo="  & Trim(.hProdOrderNo.Value)		'☆: 조회 조건 데이타 
		strVal = strVal & "&txtPlanStartDt="   & Trim(.hPlanStartDt.value)
		strVal = strVal & "&txtPlanStartHh="   & Trim(.hPlanStartHh.value)
		strVal = strVal & "&txtPlanStartMm="   & Trim(.hPlanStartMm.value)
		strVal = strVal & "&txtPlanEndDt="     & Trim(.hPlanEndDt.value)
		strVal = strVal & "&txtPlanEndHh="     & Trim(.hPlanEndHh.value)
		strVal = strVal & "&txtPlanEndMm="     & Trim(.hPlanEndMm.value)
	...
	Else
		...
		strVal = strVal & "&txtSendStartHh="   & UNIConvNum(Trim(frm1.txtSendStartHh.Text), 0)
		strVal = strVal & "&txtSendStartMm="   & UNIConvNum(Trim(frm1.txtSendStartMm.Text), 0)		
		strVal = strVal & "&txtSendEndDt="     & Trim(.txtSendEndDt.Text)
		strVal = strVal & "&txtSendEndHh="     & UNIConvNum(Trim(frm1.txtSendEndHh.Text), 0)		
		strVal = strVal & "&txtSendEndMm="     & UNIConvNum(Trim(frm1.txtSendEndMm.Text), 0)		
		strVal = strVal & "&txtPlanStartDt="   & Trim(.txtPlanStartDt.Text)
		strVal = strVal & "&txtPlanStartHh="     & UNIConvNum(Trim(frm1.txtPlanStartHh.Text), 0)		
		strVal = strVal & "&txtPlanStartMm="     & UNIConvNum(Trim(frm1.txtPlanStartMm.Text), 0)				
		strVal = strVal & "&txtPlanEndDt="     & Trim(.txtPlanEndDt.Text)
		strVal = strVal & "&txtPlanEndHh="     & UNIConvNum(Trim(frm1.txtPlanEndHh.Text), 0)				
		strVal = strVal & "&txtPlanEndMm="     & UNIConvNum(Trim(frm1.txtPlanEndMm.Text), 0)						
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									&nbsp;									
									<SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtSendStartHH.js"></SCRIPT>&nbsp;:&nbsp;
 						            <SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtSendStartMM.js"></SCRIPT>
   									&nbsp;~&nbsp;
									<SCRIPT LANGUAGE=JAVASCRIPT>
									ExternalWrite('<OBJECT classid=<%=gCLSIDFPDT%> name=txtSendEndDt CLASS=FPDTYYYYMMDD title=FPDATETIME ALT="종료일" tag="12X1" id=fpDateTime1></OBJECT>');</SCRIPT>
									&nbsp;
									<SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtSendEndHH.js"></SCRIPT>&nbsp;:&nbsp;
 						            <SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtSendEndMM.js"></SCRIPT>									
								</TD>	
								</TR>
								<TR>
									...
									&nbsp;									
									<SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtPlanStartHH.js"></SCRIPT>&nbsp;:&nbsp;
 						            <SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtPlanStartMM.js"></SCRIPT>
 						            &nbsp;~&nbsp;
									<SCRIPT LANGUAGE=JAVASCRIPT>
									...
									&nbsp;
									<SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtPlanEndHH.js"></SCRIPT>&nbsp;:&nbsp;
 						            <SCRIPT LANGUAGE=JavaScript SRC="./js/xi313ma1_KO441_txtPlanEndMM.js"></SCRIPT>
								</TD>	
								</TR>
								<TR>	
									...
									<TD CLASS=TD5 NOWRAP>ERP수신여부</TD>
									<TD CLASS=TD6 NOWRAP>
									<INPUT TYPE="RADIO" NAME="rdoFlg" ID="rdoFlg1" CLASS="RADIO" tag="11" Value="A" CHECKED><LABEL FOR="rdoFlg1">전체</LABEL>
									     				 ...
									     				 <INPUT TYPE="RADIO" NAME="rdoFlg" ID="rdoFlg3" CLASS="RADIO" tag="11" Value="N" ><LABEL FOR="rdoFlg3">미반영</LABEL></TD>
									</TD>								
								</TR>								
							</TABLE>
						</FIELDSET>
		...
		<TD WIDTH=100%>
			<TABLE  CLASS="BasicTB" CELLSPACING=0>
				<TR>
					...
					&nbsp;<BUTTON NAME="btnReg" CLASS="CLSMBTN" ONCLICK="vbscript:RegProd()" >생산최종실적등록</BUTTON></TD>				
					<TD>&nbsp;</TD>
				</TR>
			</TABLE>
		</TD>
...
<INPUT TYPE=HIDDEN NAME="hSendStartHh" tag="24">
<INPUT TYPE=HIDDEN NAME="hSendStartMm" tag="24">
<INPUT TYPE=HIDDEN NAME="hSendEndDt" tag="24">
<INPUT TYPE=HIDDEN NAME="hSendEndHh" tag="24">
<INPUT TYPE=HIDDEN NAME="hSendEndMm" tag="24">
<INPUT TYPE=HIDDEN NAME="hPlanStartDt" tag="24">
<INPUT TYPE=HIDDEN NAME="hPlanStartHh" tag="24">
<INPUT TYPE=HIDDEN NAME="hPlanStartMm" tag="24">
<INPUT TYPE=HIDDEN NAME="hPlanEndDt" tag="24">
<INPUT TYPE=HIDDEN NAME="hPlanEndHh" tag="24">
<INPUT TYPE=HIDDEN NAME="hPlanEndMm" tag="24">
