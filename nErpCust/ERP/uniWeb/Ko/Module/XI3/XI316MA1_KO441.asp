...
'********************************************************************************************************
'*  1. Module Name          : 인터페이스
'*  2. Function Name        : 장비코드 수신현황
'*  3. Program ID           : XI316MA1_KO441
'*  4. Program Name         : 장비코드 수신현황
'*  5. Program Desc         : 장비코드 수신현황
'*  6. Comproxy List        :
'*  7. Modified date(First) : 2008/04/08
'*  8. Modified date(Last)  : 2008/04/08
...
Const BIZ_PGM_ID = "XI316MB1_KO441.asp"	
...
Sub InitSpreadSheet()
	...
	With frm1.vspdData  
		...
		.MaxCols = C_ERR_DESC + 1												'☜: 최대 Columns의 항상 1개 증가시킴 
		...
		ggoSpread.SSSetEdit  C_MACHINE_CD        , "장비코드",		10, , ,10           
		ggoSpread.SSSetEdit  C_MACHINE_NM        , "장비명",		18, , ,18           
		ggoSpread.SSSetEdit  C_CREATE_TYPE       , "생성구분",		9, 2   
		ggoSpread.SSSetEdit  C_ERP_RECEIVE_DT    , "ERP수신일시",	19, 2      
		ggoSpread.SSSetEdit  C_ERP_APPLY_FLAG    , "ERP반영여부",	9, 2         
		ggoSpread.SSSetEdit  C_ERR_DESC          , "에러내역",		50, , ,200           
...
Sub GetSpreadColumnPos(ByVal pvSpdNo)
    ...
    Select Case UCase(pvSpdNo)
       Case "A"
            ...
            C_MACHINE_CD        = iCurColumnPos(1)      
            C_MACHINE_NM        = iCurColumnPos(2)  
            C_CREATE_TYPE       = iCurColumnPos(3)      
            C_ERP_RECEIVE_DT    = iCurColumnPos(4)      
            C_ERP_APPLY_FLAG    = iCurColumnPos(5)      
            C_ERR_DESC          = iCurColumnPos(6) 
...
Sub SetSpreadColor(ByVal lRow)
	With frm1
		...
		ggoSpread.SSSetProtected  C_MACHINE_CD     , lRow, lRow        
		ggoSpread.SSSetProtected  C_MACHINE_NM     , lRow, lRow    
		ggoSpread.SSSetProtected  C_CREATE_TYPE    , lRow, lRow        
		ggoSpread.SSSetProtected  C_ERP_RECEIVE_DT , lRow, lRow        
		ggoSpread.SSSetProtected  C_ERP_APPLY_FLAG , lRow, lRow        
		ggoSpread.SSSetProtected  C_ERR_DESC       , lRow, lRow  
...
Function OpenBp_cd(Byval strCode, Byval iWhere)
	...
	arrParam(5) = "장비"								<%' TextBox 명칭 %>
	
    arrField(0) = "UD_MINOR_CD"								<%' Field명(0)%>
    arrField(1) = "UD_MINOR_NM"								<%' Field명(1)%>
    
    arrHeader(0) = "장비코드"								<%' Header명(0)%>
    arrHeader(1) = "장비TYPE"							<%' Header명(1)%>
							...
							<TABLE <%=LR_SPACE_TYPE_40%>>
								...
								<TR >
									...
									<TD CLASS="TD6" NOWRAP>
										...
										<input type=radio CLASS="RADIO" name="rdoUsage_flag" id="rdoUsage_flag2" value="Y" tag = "11"><label for="rdoUsage_flag2">반영</label>
										<input type=radio CLASS="RADIO" name="rdoUsage_flag" id="rdoUsage_flag3" value="N" tag = "11"><label for="rdoUsage_flag3">미반영</label>
									</TD>
								</TR>
		...
		<TD WIDTH=100%>
			<TABLE <%=LR_SPACE_TYPE_30%>>
			    <TR>
					...
					<TD WIDTH=500>&nbsp;[ERP반영여부] 반영중 오류발생시에도 'Y'</TD>
					<TD WIDTH=10>&nbsp;</TD>
				</TR>
			</TABLE>
		</TD>
