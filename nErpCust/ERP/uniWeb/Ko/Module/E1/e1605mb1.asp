...
<%   		
    ...
    if  lgPrevNext = "N" or lgPrevNext = "P" then
	    ...
	    iKey1 = iKey1 & " AND dilig_strt_dt =  " & FilterVar( lgKeyStream(4), "''", "S") & ""
...
'			Else
				...
				lgStrSQL = lgStrSQL & " WHERE emp_no = "        & FilterVar(Request("txtEmp_no"), "''", "S")
				lgStrSQL = lgStrSQL & "   AND dilig_strt_dt = " & FilterVar(UNIConvDateCompanyToDB(Request("txtDilig_dt"),NULL),"NULL","S")
				lgStrSQL = lgStrSQL & "   AND dilig_cd = "      & FilterVar(Request("txtdilig_cd"), "''", "S")
...
Sub SubBizSaveSingleCreate()
    ...
    lgStrSQL = lgStrSQL & FilterVar(UNIConvDateCompanyToDB(lgKeyStream(4),NULL), "''", "S")  & ","
...
Sub SubBizSaveSingleUpdate()
    ...
    'A developer must define field to update record
    '--------------------------------------------------------------------------------------------------------
    lgStrSQL = "UPDATE  E11070T set"
    lgStrSQL = lgStrSQL & "       dilig_hh = "        & FilterVar(lgKeyStream(5), "''", "S") & ","
    lgStrSQL = lgStrSQL & "       dilig_mm = "        & FilterVar(lgKeyStream(6), "''", "S") & ","    
...
'Response.Write	"***strType:"   & strType   	
    ...
    strDilig_dt = UNIConvDate(Request("txtDilig_dt"))
    iCnt = 0
    iHoliday_cnt = 0    
    ...
    Select Case pMode 
      Case "R"
             Select Case  lgPrevNext 
                Case ""
                      lgStrSQL = "Select top 1 a.emp_no,dilig_strt_dt,dilig_hh,dilig_mm,dilig_cd,app_emp_no,remark," 
