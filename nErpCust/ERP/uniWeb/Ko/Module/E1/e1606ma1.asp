...
Const BIZ_PGM_ID      = "e1606mb1.asp"						           '☆: Biz Logic ASP Name
Const BIZ_PGM_ID1     = "e1605ma1.asp"						           '☆: Biz Logic ASP Name
'========================================================================================================
...
'        lgKeyStream = lgKeyStream & Trim(DiligAuth) & gColSep
...
Function DbQuery(ppage)
	...
	With Frm1
		...
		If CompareDateByFormat(.txtfrom.value,.txtto.value,"시간외근무시작일","시간외근무종료일","970025", gDateFormat, gComDateType,TRUE) = False Then
		    .txtfrom.focus()
		    exit function
		end if
    End With
...
Function DoubleGetRow(pRow)
    ...
    If txtDilig_dt <> "" and txtDilig_cd <> "" then
        ...
        strVal = strVal & "&emp_no=" & Trim(frm1.txtEmp_no.Value)
                        ...
                        <TD colspan=4>
                            <TABLE cellSpacing=1 cellPadding=0 width="100%" border=0 bgcolor=#ffffff>
                                <TR bgcolor=#d0d6e4 height=20>
		                        	<TD></TD>
		                        	<TD class=TDFAMILY_TITLE1 colspan=1>근무날짜</TD>
		                        	<TD class=TDFAMILY_TITLE1 colspan=1>시간</TD>	
		                        	<TD class=TDFAMILY_TITLE1 colspan=1>분</TD>		                        		                        	
		                        	<TD class=TDFAMILY_TITLE1 colspan=1>근무구분</TD>
...
<%            
        For i=1 To 10
			...
			Response.Write "<TD><INPUT name='SPREADCELL_DILIG_STRT_DT" & i & "' tag='25x' flag='SPREADCELL' style='WIDTH: 70px; TEXT-ALIGN: left' onMouseOver='vbscript: Call MouseRow(" & i & ")' readonly></TD>"			
            Response.Write "<TD><INPUT name='SPREADCELL_DILIG_HOUR" & i & "'  tag='25X' flag='SPREADCELL' style='WIDTH:  30px; TEXT-ALIGN: center'' onMouseOver='vbscript: Call MouseRow(" & i & ")' readonly></TD>"
            Response.Write "<TD><INPUT name='SPREADCELL_DILIG_MIN" & i & "'  tag='25X' flag='SPREADCELL' style='WIDTH:  30px; TEXT-ALIGN: center'' onMouseOver='vbscript: Call MouseRow(" & i & ")' readonly></TD>"                        
        	...
        	Response.Write "<TD><INPUT name='SPREADCELL_app_emp_no' tag='25X' flag='SPREADCELL' style='WIDTH: 120px;' onMouseOver='vbscript: Call MouseRow(" & i & ")' readonly></TD>"
...
%>
    ...
    <INPUT TYPE=hidden NAME=GRID_TOTPAGES STYLE="WIDTH: 200px; HEIGHT: 20px">
