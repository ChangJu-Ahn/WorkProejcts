...
'*  2. Function Name        : Master Data(Monthly Exchange Rate)(고정환율 등록)
'*  3. Program ID           : B1703ma1.asp
'*  4. Program Name         : B1703ma1.asp
'*  5. Program Desc         :
'*  6. Modified date(First) : 2000/09/06
...
Const BIZ_PGM_ID = "B1703mb1.asp"												<%'비지니스 로직 ASP명 %>
...
Sub InitSpreadSheet()
    ...
    Temp = Replace(parent.gDateFormatYYYYMM,"Y","9")    
    Temp = Replace(Temp             ,"M","9")    
    Temp = Replace(Temp             ,parent.gComDateType,"X")
   ...
   'patch version
    ggoSpread.Spreadinit "V20021209",,parent.gAllowDragDropSpread    
    ...
    ggoSpread.SSSetMask C_ValidDt, "해당년월", 30,2, Temp
...
Function DbQuery() 
	...
	Dim strVal, strYYYYMM
	
	strYYYYMM = Trim(frm1.txtValidDt.text)
	    
	Call TransToYYYYMM(strYYYYMM)
	
    With frm1
    
    If lgIntFlgMode = parent.OPMD_UMODE Then		
		strVal = BIZ_PGM_ID & "?txtMode=" & parent.UID_M0001						'☜: 
		strVal = strVal & "&txtValidDt=" & .hValidDt.value 			 		'☆: 조회 조건 데이타 
		strVal = strVal & "&txtCurrency=" & .hCurrency.value 				'☆: 조회 조건 데이타		
		strVal = strVal & "&lgStrPrevKey=" & lgStrPrevKey
		strVal = strVal & "&lgStrPrevToKey=" & lgStrPrevToKey
    Else		
		strVal = BIZ_PGM_ID & "?txtMode=" & parent.UID_M0001						'☜: 
		strVal = strVal & "&txtValidDt=" & strYYYYMM						'☆: 조회 조건 데이타 
    ...
    For lRow = 1 To .vspdData.MaxRows
		    ...
		    Select Case .vspdData.Text
		        Case ggoSpread.InsertFlag, ggoSpread.UpdateFlag		'☜: 신규, 수정 
		            ...
		            '----------------------------------
					'저장하기 위해 YYYYMM 포멧으로 바꿈 
					'----------------------------------
					strTxt = Trim(.vspdData.Text)
				       
					Call TransToYYYYMM(strTxt)
						
					strApplyDt =  UNIGetLastDay(.vspdData.Text,  parent.gDateFormatYYYYMM)
                     
                    if (Trim(StrApplyDt) = "" AND .vspdData.Text <> "")  then
						Call LayerShowHide(0)
						Call DisplayMsgBox("970029", "X", "해당년월", "X")
						.vspdData.Row = lRow
						.vspdData.Action = 0						
						Exit Function
					end if
					...
					strVal = strVal & strTxt & parent.gColSep		            
		        ...
		        Case ggoSpread.DeleteFlag							'☜: 삭제 
		            ...
		            '----------------------------------
					'삭제하기 위해 YYYYMM 포멧으로 바꿈 
					'----------------------------------
					strTxt = Trim(.vspdData.Text)
			
					Call TransToYYYYMM(strtxt)
					
		            strDel = strDel & strTxt & parent.gColSep
...
<TABLE <%=LR_SPACE_TYPE_00%>>
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>고정환율</font></td>
						...
						<FIELDSET CLASS="CLSFLD">
							<TABLE <%=LR_SPACE_TYPE_40%>>
								<TR>
									...
									<TD CLASS="TD6">
									<script language =javascript src='./js/b1703ma1_fpDateTime1_txtValidDt.js'></script></TD>
				...
				<TR>
					<TD WIDTH=100% HEIGHT=* valign=top>
						<TABLE <%=LR_SPACE_TYPE_20%>>
							<TR>
								<TD HEIGHT="100%">
									<script language =javascript src='./js/b1703ma1_vaSpread1_vspdData.js'></script>
								</TD>
							</TR>
						</TABLE>
					</TD>
	...
	<TR>
		<TD WIDTH=100% HEIGHT=<%=BizSize%>><IFRAME NAME="MyBizASP" SRC="B1703mb1.asp" WIDTH=100% HEIGHT=<%=BizSize%> FRAMEBORDER=0 SCROLLING=NO noresize framespacing=0></IFRAME>
		</TD>
	</TR>
</TABLE>
