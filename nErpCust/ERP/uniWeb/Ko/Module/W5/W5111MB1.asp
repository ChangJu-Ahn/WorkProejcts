...
<%
	...
	Dim sFISC_YEAR, sREP_TYPE, sW6_TYPE
	Dim lgStrPrevKey
...
Sub SubBizQuery()
    ...
    iKey4 = FilterVar(sW6_TYPE,"''", "S")		' 구분 

    Call SubMakeSQLStatements("R",iKey1, iKey2, iKey3, iKey4)                                       '☜ : Make sql statements
    ...
    Else
        ...
        Do While Not lgObjRs.EOF
			...
			lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("W6_NM"))	
...
Sub SubMakeSQLStatements(pMode,pCode1, pCode2, pCode3, pCode4)
    Select Case pMode 
      Case "R"
			lgStrSQL =			  " SELECT  "
            lgStrSQL = lgStrSQL & " A.SEQ_NO, A.W1, B.ACCT_NM AS W1_NM, A.W2, A.W3, A.W4, A.W5, A.W6, dbo.ufn_GetCodeName('W1008', A.W6) AS W6_NM, A.W7 "
            lgStrSQL = lgStrSQL & " FROM TB_DONA_ADJUST A WITH (NOLOCK), " & vbCrLf
        	lgStrSQL = lgStrSQL & " 	(SELECT ACCT_CD, ACCT_NM FROM TB_WORK_6 C WITH (NOLOCK) " & vbCrLf
        	lgStrSQL = lgStrSQL & " 		WHERE C.CO_CD = " & pCode1 	 & vbCrLf
        	lgStrSQL = lgStrSQL & " 		AND C.FISC_YEAR = " & pCode2 	 & vbCrLf
        	lgStrSQL = lgStrSQL & " 		AND C.REP_TYPE = " & pCode3  & ") B "	 & vbCrLf

        	lgStrSQL = lgStrSQL & " WHERE A.W1 *= B.ACCT_CD " & vbCrLf
            ...
            If pCode4 <> "''" Then
            	lgStrSQL = lgStrSQL & " AND A.W6 = " & pCode4 	 & vbCrLf
            End If
...
Sub SubBizSaveMultiCreate(arrColVal)
	...
	lgStrSQL = lgStrSQL & FilterVar(Trim((arrColVal(C_W4))),"''","S")	& ","
...
Sub SubBizSaveMultiUpdate(arrColVal)
	...
	lgStrSQL = lgStrSQL & " W4		= " &  FilterVar(Trim((arrColVal(C_W4))),"''","S")	& ","
