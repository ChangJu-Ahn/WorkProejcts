...
Sub  TrimData()
	...
	If strRdo = "1" Then							'미결 
		...
		End If
	 	...
	 	'strCond = strCond & " AND A.GL_INPUT_TYPE = 'TG' AND isnull(A.HQ_BRCH_NO,'')  = '' "
	 	strCond = strCond & " AND A.GL_INPUT_TYPE = " & FilterVar("TG", "''", "S") & "  AND (A.HQ_BRCH_NO IS NULL OR RTrim(A.HQ_BRCH_NO) = '') "
	 	
	ElseIf strRdo = "2" Then						'완결 
		If striOpt = "A" Then						'차변 
			'strCond = strCond & " AND B.DR_CR_FG = 'DR' AND isnull(A.HQ_BRCH_NO,'')  <> '' "
			strCond = strCond & " AND B.DR_CR_FG = " & FilterVar("DR", "''", "S") & "  AND (A.HQ_BRCH_NO IS NOT NULL OR RTrim(A.HQ_BRCH_NO) <> '') "
		ElseIf striOpt = "B" Then					'대변 
