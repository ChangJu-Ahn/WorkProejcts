...
Dim lgBlnOpenedFlag
Dim lgBlnCfmChecked
Dim lgStrFromDt		' 확정구기간의 시작일 
Dim lgStrToDt		' 예시구간의 종료일 
	...
	With frm1
		If pvKubun = 1 Then
			...
			WriteCookie CookieSplit , "" & Parent.gColSep & "" & Parent.gColSep & _
									  ...
									  lgStrFromDt & Parent.gColSep & lgStrToDt
			<% Else %>
...
' 미확정현황 Popup
Function OpenNonDistrListPopup()
	Dim iArrParam(3)
	Dim iCalledAspName

	OpenNonDistrListPopup = False
	...
	With frm1
		iCalledAspName = AskPRAspName("s2214pa2")
		...
		iArrParam(2) = .txtPlantCd.value
		iArrParam(3) = "S2214PA202"
	End With
...
Function GetCfmPeriod(ByVal pvIntSpPeriodSeq)
	...
	iStrSelectList = " * "
	iStrFromList = "  dbo.ufn_s_GetCfmPeriod(" & FilterVar("S2216BA1", "''", "S") & ",  " & FilterVar(iStrCode, "''", "S") & ", " & FilterVar("1", "''", "S") & " , " & FilterVar("E", "''", "S") & " ,  " & FilterVar(C_StrGrpFlag, "''", "S") & ", " & pvIntSpPeriodSeq & ") "
	iStrWhereList = ""
	...
	If CommonQueryRs2by2(iStrSelectList,iStrFromList,iStrWhereList,iStrRs) Then
		...
		With frm1
			...
			lgStrFromDt = UNIDateClientFormat(iArrRs(7))
			lgStrToDt = UNIDateClientFormat(iArrRs(8))
		End With
...
Function GetCancelPeriod()
	...
	iStrSelectList = " * "
	iStrFromList = "  dbo.ufn_s_GetCancelPeriod(" & FilterVar("S2216BA1", "''", "S") & ",  " & FilterVar(iStrCode, "''", "S") & ", " & FilterVar("1", "''", "S") & " , " & FilterVar("E", "''", "S") & " ,  " & FilterVar(C_StrGrpFlag, "''", "S") & ") "
	iStrWhereList = ""
	...
	If CommonQueryRs2by2(iStrSelectList,iStrFromList,iStrWhereList,iStrRs) Then
		...
		With frm1
			...
			.txtFcToSPPeriod.value = ""
			.txtFcToSPPeriodDesc.value = ""
			lgStrFromDt = UNIDateClientFormat(iArrRs(3))
			lgStrToDt = ""
		End With
					...
					<TD CLASS="CLSMTABP">
						<TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0>
							<TR>
								...
								<td background="../../../CShared/image/table/seltab_up_bg.gif" align="center" CLASS="CLSMTAB"><font color=white>공장별일별품목판매계획확정</font></td>
	...
	<TR HEIGHT=20>
		<TD>
		    <TABLE <%=LR_SPACE_TYPE_30%>>
		        <TR>
					<TD WIDTH=10>&nbsp;</TD>
					<TD><BUTTON NAME="btnExe" CLASS="CLSMBTN" onclick="ExeReflect()" Flag=1>확정</BUTTON>
						<% If ((lgLngProcessByPlant And 1024) = 0) And ((lgLngProcessByPlant And 8192) > 0) Then %>
						&nbsp;
						<BUTTON NAME="btnNonDistrList" CLASS="CLSMBTN" onclick="OpenNonDistrListPopup()" Flag=1>미배분현황</BUTTON>
						<% End If %></TD>
					<TD WIDTH=* ALIGN="right"><%If (lgLngUseStep And 4096) > 0 Then %><a href = "vbscript:JumpChgCheck(BIZ_JUMP_ID_S2216MA1)">공장별일별품목판매계획조정</a><%End If%></TD>
					<TD WIDTH=10>&nbsp;</TD>
		        </TR>
		    </TABLE>
		</TD>
