...
Sub FixUNISQLData()
	...
	If Len(Request("txtConSalesGrp")) Then
		strVal = "WHERE SP.SALES_GRP = " & FilterVar(UCase(Request("txtConSalesGrp")), "''", "S") & " "
		arrVal(0) = Trim(Request("txtConSalesGrp"))
	End If
    ...
    '--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
    UNIValue(0,2) = "GROUP BY SP.SALES_GRP , SP.SP_YEAR, PLANFLAG.MINOR_NM, EXPORTFLAG.MINOR_NM," _
...
Sub QueryData()
    ...
    If  rs1.EOF And rs1.BOF Then
		...
		If Len(Request("txtConSalesGrp")) And FalsechkFlg = False Then
...
<Script Language=vbscript>
    With parent
         ...
         .lgStrPrevKey        =  "<%=lgStrPrevKey%>"                       '☜: set next data tag
  		...
  		.frm1.txtConSalesGrpNm.value		=  "<%=ConvSPChars(arrRsVal(1))%>" 
        ...
        .frm1.txtHConSalesGrp.value	        = "<%=ConvSPChars(Request("txtConSalesGrp"))%>"
