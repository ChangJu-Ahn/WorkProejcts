...
Const BIZ_PGM_ID 		= "S2214QB3.asp"                              '☆: Biz Logic ASP Name

Const C_MaxKey          = 89					                          '☆: SpreadSheet의 키의 갯수 
...
Sub InitSpreadSheet(ByVal pvPsdNo)
	If pvPsdNo = "A" then
	    Call SetZAdoSpreadSheet("S2214QA301","S","A", "V20030326", parent.C_SORT_DBAGENT,frm1.vspdData,C_MaxKey, "X", "X")
	    Call SetSpreadLock("A")
	Else
		Call SetZAdoSpreadSheet("S2214QA302","S","B", "V20030326", parent.C_SORT_DBAGENT,frm1.vspdData2,C_MaxKey, "X", "X")
...
Sub InitComboBox()
	...
	'조회기준 
	lgF0 = "G" & Chr(11) & "O" & Chr(11)
	lgF1 = "영업그룹" & Chr(11) & "영업조직" & Chr(11)
	Call SetCombo2(frm1.cboConGrpFlag,lgF0,lgF1,Chr(11))
	
	'영업조직레벨 
	Call CommonQueryRs(" MINOR_CD, MINOR_NM ", " B_MINOR ", " MAJOR_CD = " & FilterVar("S0016", "''", "S") & " ORDER BY MINOR_NM ",lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)
	Call SetCombo2(frm1.cboConOrgLvl,lgF0,lgF1,Chr(11))
...
'========================================================================================================
Sub cboConGrpFlag_onChange()
	lgBlnFlgChgValue = true	
	
	if frm1.cboConGrpFlag.value = "G" then

		Call ggoOper.SetReqAttr(frm1.cboConOrgLvl,"Q")
		Call ggoOper.SetReqAttr(frm1.txtConSalesOrg,"Q")
		frm1.btnConSalesOrg.Disabled = True
		Call ggoOper.SetReqAttr(frm1.txtConSalesGrp,"D")
		frm1.btnConSalesGrp.Disabled = False
		
		frm1.cboConOrgLvl.value = ""
		frm1.txtConSalesOrg.value = ""
		frm1.txtConSalesOrgNm.value = ""

	elseif frm1.cboConGrpFlag.value = "O" then

		Call ggoOper.SetReqAttr(frm1.txtConSalesGrp,"Q")
		frm1.btnConSalesGrp.Disabled = True
		Call ggoOper.SetReqAttr(frm1.cboConOrgLvl,"N")
		Call ggoOper.SetReqAttr(frm1.txtConSalesOrg,"D")

		frm1.btnConSalesOrg.Disabled = False
		frm1.txtConSalesGrp.value = ""
		frm1.txtConSalesGrpNm.value = ""
		frm1.cboConOrgLvl.value = "1"

	end if		
End Sub

'========================================================================================================
'	Description : 영업조직레벨 onChange 이벤트 처리 
'========================================================================================================
Sub cboConOrgLvl_onChange()
	lgBlnFlgChgValue = true	
	frm1.txtConSalesOrg.value = ""
	frm1.txtConSalesOrgNm.value = ""
End Sub
...
Function OpenConPopup(ByVal pvIntWhere)
	...
	Select Case pvIntWhere
		...
		Case 2 '영업조직 
			...
			iArrParam(2) = Trim(frm1.txtConSalesOrg.value)
			iArrParam(3) = ""
			iArrParam(4) = "USAGE_FLAG=" & FilterVar("Y", "''", "S") & "  AND LVL= " & FilterVar(frm1.cboConOrgLvl.value, "''", "S") & ""
			iArrParam(5) = "영업조직"
		...
		Case 3 '품목그룹 
			...
			iArrParam(2) = Trim(frm1.txtConItemGroupCd.value)
			iArrParam(3) = ""
			iArrParam(4) = "DEL_FLG=" & FilterVar("N", "''", "S") & "  AND ITEM_GROUP_LEVEL= " & FilterVar(frm1.cboConGrpLvl.value, "''", "S") & ""
			iArrParam(5) = "품목그룹"
...
Function SetConPopup(Byval pvArrRet,ByVal pvIntWhere)
	...
	With frm1
		...
		Case 2 '영업조직 
			.txtConSalesOrg.Value = pvArrRet(0)
...
'========================================================================================================
'	Description : 조회기준에 따라 Column Header 변경(영업조직<->영업그룹)
'========================================================================================================
Sub ChangeColHeader()
	If frm1.cboDisplayType.value = "H" then
		With frm1.vspdData
			.Row = 0
			If frm1.cboConGrpFlag.value = "G" Then
				.Col = GetKeyPos("A",3)	:	.Text = "영업그룹"
				.Col = GetKeyPos("A",4)	:	.Text = "영업그룹명"
			Else
				.Col = GetKeyPos("A",3)	:	.Text = "영업조직"
				.Col = GetKeyPos("A",4)	:	.Text = "영업조직명"
			End If
		End With
	Else
		With frm1.vspdData2
			.Row = 0
			If frm1.cboConGrpFlag.value = "G" Then
				.Col = GetKeyPos("A",4)	:	.Text = "영업그룹"
				.Col = GetKeyPos("A",5)	:	.Text = "영업그룹명"
			Else
				.Col = GetKeyPos("A",4)	:	.Text = "영업조직"
				.Col = GetKeyPos("A",5)	:	.Text = "영업조직명"
			End If
		End With
	End If
End Sub
...
Sub SetColHiddenByMonth()
	...
	iIntStartKey = 14 'Z_ADO_FIELD_INF 테이블에서 1월계획량에 해당하는 SEQ_NO
...
Sub FormatSpreadCellByCurrency()
	...
	With frm1
		If .vspdData.style.display = inline Then
			For	iLngRow = 0 To 12
				Call ReFormatSpreadCellByCellByCurrency(.vspdData,-1, -1,GetKeyPos("A",7),GetKeyPos("A",15 + iLngRow * 6),"A" ,"Q","X","X")
				Call ReFormatSpreadCellByCellByCurrency(.vspdData,-1, -1,GetKeyPos("A",7),GetKeyPos("A",16 + iLngRow * 6),"A" ,"Q","X","X")
			Next
		End If
		
		If .vspdData2.style.display = inline Then
			Call ReFormatSpreadCellByCellByCurrency(.vspdData2, -1, -1, GetKeyPos("B",8), GetKeyPos("B",12), "A" , "Q", "X", "X")
			Call ReFormatSpreadCellByCellByCurrency(.vspdData2, -1, -1, GetKeyPos("B",8), GetKeyPos("B",13), "A" , "Q", "X", "X")
		End If
	End With
  ...
  <TD WIDTH=100%>
   <TABLE <%=LR_SPACE_TYPE_10%>>
    <TR>
     ...
     <!--TD CLASS="CLSMTABP"-->
     <TD>
      <TABLE ID="MyTab" CELLSPACING=0 CELLPADDING=0 width=290>
       <TR>
        <td background="../../../CShared/image/table/seltab_up_bg.gif" width="9"><img src="../../../CShared/image/table/seltab_up_left.gif" width="9" height="23"></td>
        <td background="../../../CShared/image/table/seltab_up_bg.gif" align="center"><font color=white>판매계획대실적조회(영업조직/품목그룹)</font></td>
        <td background="../../../CShared/image/table/seltab_up_bg.gif" width="10" align="right"><img src="../../../CShared/image/table/seltab_up_right.gif" width="10" height="23"></td>
          </TR>
      </TABLE>
     </TD>
       ...
       <TR>       
         ...
         <TD CLASS="TD6" NOWRAP>
          <TABLE CELLSPACING=0 CELLPADDING=0>
           <TR>
            <TD>
             <script language =javascript src='./js/s2214qa3_fpDateTime1_txtConFromDt.js'></script>
            </TD>
            <TD>
             &nbsp;~&nbsp;
            </TD>
            <TD>
             <script language =javascript src='./js/s2214qa3_fpDateTime2_txtConToDt.js'></script>
            </TD>
           </TR>
          </TABLE>
         </TD>
        ...
        <TR>
		 ...
		 <TD CLASS="TD6"><SELECT Name="cboConGrpFlag" ALT="조회기준" STYLE="WIDTH: 150px" tag="12"></SELECT></TD> 
         <TD CLASS="TD5" NOWRAP>영업그룹</TD>
         <TD CLASS="TD6"><INPUT NAME="txtConSalesGrp" ALT="영업그룹" TYPE="Text" MAXLENGTH=4 SiZE=10 tag="1XXXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnConSalesGrp" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenConPopup 1">&nbsp;<INPUT NAME="txtConSalesGrpNm" TYPE="Text" MAXLENGTH="20" SIZE=25 tag="14"></TD>
        </TR>
        <TR>
         <TD CLASS="TD5" NOWRAP>영업조직레벨</TD>
		 <TD CLASS="TD6"><SELECT Name="cboConOrgLvl" ALT="영업조직레벨" STYLE="WIDTH: 150px" tag="12"><OPTION Value=""></OPTION></SELECT></TD> 
         <TD CLASS="TD5" NOWRAP>영업조직</TD>
         <TD CLASS="TD6"><INPUT NAME="txtConSalesOrg" ALT="영업조직" TYPE="Text" MAXLENGTH=4 SiZE=10 tag="1XXXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnConSalesOrg" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenConPopup 2">&nbsp;<INPUT NAME="txtConSalesOrgNm" TYPE="Text" MAXLENGTH="20" SIZE=25 tag="14"></TD>
        </TR>
        <TR>
         <TD CLASS="TD5" NOWRAP>품목그룹레벨</TD>
		 <TD CLASS="TD6"><SELECT Name="cboConGrpLvl" ALT="품목그룹레벨" STYLE="WIDTH: 150px" tag="12"></SELECT></TD> 
         <TD CLASS="TD5" NOWRAP>품목그룹</TD>
         <TD CLASS="TD6"><INPUT NAME="txtConItemGroupCd" ALT="품목그룹" TYPE="Text" MAXLENGTH=10 SiZE=10 tag="1XXXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnConItemGroupCd" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenConPopup 3">&nbsp;<INPUT NAME="txtConItemGroupNm" TYPE="Text" MAXLENGTH="50" SIZE=25 tag="14"></TD>
        </TR>
        <TR>
         ...
         <TD CLASS="TD6"><INPUT NAME="txtConCur" ALT="화폐" TYPE="Text" MAXLENGTH=3 SiZE=10 tag="1XXXXU"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnCur" align=top TYPE="BUTTON" ONCLICK="vbscript:OpenConPopup 4"></TD>
        </TR>
       </TABLE>
      </FIELDSET>
     </TD>
      ...
      <TABLE <%=LR_SPACE_TYPE_60%>>
       <TR>
        <TD HEIGHT="100%" WIDTH="100%" COLSPAN=4>
         <script language =javascript src='./js/s2214qa3_vspdData_vspdData.js'></script>
		 <script language =javascript src='./js/s2214qa3_vspdData2_vspdData2.js'></script>
        </TD>
       </TR>
      </TABLE>
     </TD>
