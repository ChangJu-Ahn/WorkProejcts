...
Sub SubBizBatch()
    ...
    With lgObjComm
		...
		If Request("txtWorkType") = "Y" Then
			.CommandText = "dbo.usp_s_CfmSSpItemByBp"
        Else
			.CommandText = "dbo.usp_s_CancelCfmSSpItemByBp"
        End If
		...
		End If
	    .Parameters.Append .CreateParameter("@sp_type", adXChar,adParamInput,1,Request("txtSpType"))		' 판매계획유형 
	    .Parameters.Append .CreateParameter("@loc_exp_flag", adXChar,adParamInput,1,"1")
	    .Parameters.Append .CreateParameter("@so_bill_flag", adXChar,adParamInput,1,"2")
    ...
    Elseif CDbl(intRetCD) = 800161 AND Request("txtWorkType") = "Y" Then '실행한 자료가 없을때 처리 (박정순 추가 (2006-04-06))
