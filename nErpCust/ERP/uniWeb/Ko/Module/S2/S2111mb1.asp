...
Sub SubBizQueryMulti()
	...
	If cStr(Err.Description) = "B_MESSAGE" & Chr(11) & "202100" then
		...
		If CheckSYSTEMError(Err,True) = True Then
			...
			Response.Write "   .frm1.txtConSalesOrgNm.value = """ & ConvSPChars(E5_b_sales_org(S213_E5_sales_org_nm))      & """" & vbCr    
			'계획구분 
    ...
    Response.Write "<Script language=vbs>  " & vbCr   
    Response.Write " With Parent        " & vbCr
    Response.Write "   .frm1.txtSalesOrg.value   = """ & ConvSPChars(E5_b_sales_org(S213_E5_sales_org))         & """" & vbCr 
    Response.Write "   .frm1.txtSalesOrgNm.value = """ & ConvSPChars(E5_b_sales_org(S213_E5_sales_org_nm))      & """" & vbCr    
    '계획년도 
    ...
    For iLngRow = 0 To UBound(exp_grp,1)
		...
		Response.Write " .ggoSpread.SSSetProtected .C_ItemSpec, LngMaxRow + " & iLngRow+1 & "  , LngMaxRow + " & iLngRow+1  & vbCr
		Response.Write " .ggoSpread.SSSetProtected .C_ItemName, LngMaxRow + " & iLngRow+1 & " , LngMaxRow + " & iLngRow+1  & vbCr
		Response.Write " .ggoSpread.SSSetRequired .C_PlanUnit, LngMaxRow + " & iLngRow+1 & " , LngMaxRow + " & iLngRow+1 & vbCr
