...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = Request("txtSqlId")							  ' main query(spread sheet에 뿌려지는 query statement)

	If UNISqlId(0) = "S2214PA201" Then
		Redim UNIValue(0,3)											  '☜: 각각의 SQL ID와 입력될 where 조건의 쌍으로 된 2차원 배열 
	Else
		Redim UNIValue(0,4)
	End If
	...
	UNIValue(0,1) = " " & FilterVar(Request("txtFrSpPeriod"), "''", "S") & ""
	UNIValue(0,2) = " " & FilterVar(Request("txtToSpPeriod"), "''", "S") & ""
	If UNISqlId(0) = "S2214PA202" Then
		UNIValue(0,3) = " " & FilterVar(Request("txtPlantCd"), "''", "S") & ""
	End If
