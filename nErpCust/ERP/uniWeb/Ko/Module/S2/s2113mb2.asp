...
'*  1. Module Name          : Sales
'*  2. Function Name        : 
'*  3. Program ID           : S2113MB2
'*  4. Program Name         : 그룹별 고객판매계획등록 
'*  5. Program Desc         :
'*  6. Comproxy List        : PS2G122.dll, PS2G123.dll, PS2G124.dll
'*  7. Modified date(First) : 2000/04/03
'*  8. Modified date(Last)  : 2002/06/16
...
'    I3_s_cust_sales_plan(S222_I3_qty_amt_flag) = TRIM(Request("txtConSpYear"))
'    I3_s_cust_sales_plan(S222_I3_sales_grp) = TRIM(Request("txtConSpYear"))
	...
	If cStr(Err.Description) = "B_MESSAGE" & Chr(11) & "202258" then    
		...
		If CheckSYSTEMError(Err,True) = True Then 		    		
			...
			Response.Write " Parent.frm1.txtConSalesOrgNm.value   = """ & ConvSPChars(E1_s_cust_sales_plan(S222_E1_sales_grp_nm))      & """" & vbCr    
    ...
    Response.Write " Parent.SetDefaultVal " & vbCr            
    Response.Write " Parent.frm1.txtSalesOrg.value   = """ & ConvSPChars(E1_s_cust_sales_plan (S222_E1_sales_grp ))     & """" & vbCr    
    Response.Write " Parent.frm1.txtSalesOrgNm.value   = """ & ConvSPChars(E1_s_cust_sales_plan(S222_E1_sales_grp_nm ))   & """" & vbCr        
...
Sub SubBizSaveMulti()                                                                          '☜: Clear Error status
    ...
    Const S224_I1_sp_year = 0
	Const S224_I1_sp_month = 1
	Const S224_I1_plan_flag = 2
	Const S224_I1_plan_seq = 3
	Const S224_I1_export_flag = 4
	Const S224_I1_cur = 5
	Const S224_I1_qty_amt_flag = 6
	Const S224_I1_sales_grp = 7
	Const S224_I1_sales_org = 8
    
 
    Dim I3_b_sales_org_Div 
    
	Dim E1_s_cust_sales_plan 
	Const S224_E1_plan_seq = 0
	
    Dim E3_b_sales_org
	Const S224_E3_sales_grp = 0
	Const S224_E3_sales_grp_nm = 1
	
	Dim E4_b_minor    
	Const S224_E4_minor_cd = 0
	Const S224_E4_minor_cd_nm = 1
	
	Dim E5_b_minor    
	Const S224_E5_minor_cd = 0
	Const S224_E5_minor_cd_nm = 1
	Dim strHang 
	...
	I3_b_sales_org_Div = "S"	
    
    iErrorPosition = ""
    
	I1_s_cust_sales_plan(S224_I1_sp_year) = Trim(Request("txtSpYear"))	
	I1_s_cust_sales_plan(S224_I1_plan_flag) = Trim(Request("txtPlanTypeCd"))
	I1_s_cust_sales_plan(S224_I1_export_flag) = Trim(Request("txtDealTypeCd"))
	I1_s_cust_sales_plan(S224_I1_plan_seq) = Trim(Request("txtPlanNum"))
	I1_s_cust_sales_plan(S224_I1_sales_grp) = Trim(Request("txtSalesOrg"))
	
	I1_s_cust_sales_plan(S224_I1_cur) = gCurrency
	
	I1_s_cust_sales_plan(S224_I1_sales_org) = ""
	I1_s_cust_sales_plan(S224_I1_qty_amt_flag) = ""
	
	
	Set PS2G123 = Server.CreateObject("PS2G123.CsCustSpByGp")	
	...
	CALL PS2G123.S_CUST_SP_BY_GROUP(gStrGlobalCollection ,cstr(itxtSpread), I1_s_cust_sales_plan , _
	...
	If CheckSYSTEMError2(Err, True, iErrorPosition & strHang ,"","","","") = True Then
		Response.Write "<Script language=vbs> " & vbCr      
		Response.Write " Parent.frm1.txtSalesOrgNm.value    = """ & ConvSPChars(E3_b_sales_org(S224_E3_sales_grp_nm)) & """" & vbCr
		Response.Write " Parent.frm1.txtPlanTypeNm.value    = """ & ConvSPChars(E4_b_minor(S224_E4_minor_cd_nm)) & """" & vbCr
		Response.Write " Parent.frm1.txtDealTypeNm.value    = """ & ConvSPChars(E5_b_minor(S224_E5_minor_cd_nm)) & """" & vbCr
	...
	Response.Write "<Script language=vbs> " & vbCr           
	If Trim(ConvSPChars(E1_s_cust_sales_plan(S224_E1_plan_seq))) <> "" then
		Response.Write " Parent.frm1.txtPlanNum.value    = """ & ConvSPChars(E1_s_cust_sales_plan(S224_E1_plan_seq)) & """" & vbCr        
		Response.Write " Parent.frm1.txtConPlanNum.value    = """ & ConvSPChars(E1_s_cust_sales_plan(S224_E1_plan_seq)) & """" & vbCr		
	End If
...
Sub SubConfirm()
    ...
    I2_s_cust_sales_plan(S223_I2_sales_grp)   = UCase(Trim(Request("txtSalesOrg")))
    I2_s_cust_sales_plan(S223_I2_sales_org)   = ""
