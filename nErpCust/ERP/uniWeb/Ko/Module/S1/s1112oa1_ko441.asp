...
Function BtnPrint() 
	...
	If lgPLCd <> "" Then
		vargBizArea = vargBizArea & " INNER JOIN (SELECT X.ITEM_CD,Y.BIZ_AREA_CD,Y.PLANT_CD"
		vargBizArea = vargBizArea & "       FROM B_ITEM_BY_PLANT X"
		vargBizArea = vargBizArea & " INNER JOIN B_PLANT Y ON X.PLANT_CD = Y.PLANT_CD AND Y.PLANT_CD = " & FilterVar(lgPLCd , "''", "S") & " ) PL ON E.ITEM_CD = PL.ITEM_CD  "
	Else
		vargBizArea = ""
	End If
	If lgBACd <> "" Then
		vargPlant = vargPlant & " INNER JOIN (SELECT X.ITEM_CD,Y.BIZ_AREA_CD,Y.PLANT_CD"
		vargPlant = vargPlant & "       FROM B_ITEM_BY_PLANT X"
		vargPlant = vargPlant & " INNER JOIN B_PLANT Y ON X.PLANT_CD = Y.PLANT_CD AND Y.BIZ_AREA_CD = " & FilterVar(lgBACd , "''", "S") & " ) BA ON E.ITEM_CD = BA.ITEM_CD  "
	Else
		vargPlant = ""
	End If
...
' Print 함수에서 호출 
'----------------------------------------------------------------
OBjName = AskEBDocumentName("s1112oa1_ko441","ebr")    
