...
Sub SubBizQueryMulti()
    ...
    Else
       ...
       Do While Not lgObjRs.EOF
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("TEST_APPLY_OPT"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("TERM_OPT"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("TERM_OPT"))
            ...
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("ROUND_RULE_CP"))
            lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("ROUND_RULE_FT"))
...
Sub SubBizSaveMultiCreate(arrColVal)
	...
	lgStrSQL = "INSERT INTO S_Pricing_Confuguration_Cust_KO441"
	lgStrSQL = lgStrSQL & "(APPLY_YYMM, BP_CD, PRICE_CUR, TEST_APPLY_OPT, TERM_OPT, RETEST_RATE, EFR_RATE, QEI_RATE, ADVANTAGE_SEC,   "
	lgStrSQL = lgStrSQL & " ROUND_RULE_CP, ROUND_RULE_FT, BASIC_PRICE_OPT, OPERATOR, OUTPUT_RATE, XCHG_RATE_OPT,  "
	lgStrSQL = lgStrSQL & " INSRT_USER_ID, INSRT_DT, UPDT_USER_ID, UPDT_DT ) "
	lgStrSQL = lgStrSQL & " VALUES(" 

	lgStrSQL = lgStrSQL & FilterVar(UCase(arrColVal(2)), "''", "S")     & ","	'C_Apply_yymm
	...
	lgStrSQL = lgStrSQL &  UNIConvNum(arrColVal(7),0)    & ","					'C_Retest_Rate
	...
	lgStrSQL = lgStrSQL &  UNIConvNum(arrColVal(15),0)    & ","					'C_Output_rate
...
Sub SubBizSaveMultiUpdate(arrColVal)
    ...
    lgStrSQL = "UPDATE  S_Pricing_Confuguration_Cust_KO441 "
    lgStrSQL = lgStrSQL & " SET " 
    lgStrSQL = lgStrSQL & " PRICE_CUR	    =       " & FilterVar(UCase(arrColVal(4)), "''", "S")		& ","
    lgStrSQL = lgStrSQL & " TEST_APPLY_OPT	=       " & FilterVar(UCase(arrColVal(5)), "''", "S")		& ","
    lgStrSQL = lgStrSQL & " TERM_OPT	    =       " & FilterVar(UCase(arrColVal(6)), "''", "S")		& ","
    ...
    lgStrSQL = lgStrSQL & " ROUND_RULE_CP	=       " & UNIConvNum(arrColVal(11),0)	& ","
    lgStrSQL = lgStrSQL & " ROUND_RULE_FT	=       " & UNIConvNum(arrColVal(12),0)	& ","
    lgStrSQL = lgStrSQL & " BASIC_PRICE_OPT	=       " & FilterVar(UCase(arrColVal(13)), "''", "S")		& ","
    lgStrSQL = lgStrSQL & " OPERATOR		=       " & FilterVar(UCase(arrColVal(14)), "''", "S")		& ","
    lgStrSQL = lgStrSQL & " OUTPUT_RATE		=       " & UNIConvNum(arrColVal(15),0)	& ","
    lgStrSQL = lgStrSQL & " XCHG_RATE_OPT	=       " & FilterVar(UCase(arrColVal(16)), "''", "S")		& ","
        
    lgStrSQL = lgStrSQL & " UPDT_DT			=       " & FilterVar(lgSvrDateTime, "''", "S") & ","
    lgStrSQL = lgStrSQL & " UPDT_USER_ID	=       " & FilterVar(gUsrId, "''", "S")  
    lgStrSQL = lgStrSQL & " WHERE					"
    lgStrSQL = lgStrSQL & " BP_CD			=       " & FilterVar(UCase(arrColVal(3)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " APPLY_YYMM		=       " & FilterVar(UCase(arrColVal(2)), "''", "S")
...
Sub SubBizSaveMultiDelete(arrColVal)
    ...
    lgStrSQL = "DELETE  S_Pricing_Confuguration_Cust_KO441 WHERE "
    lgStrSQL = lgStrSQL & " BP_CD		=       " & FilterVar(UCase(arrColVal(3)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " APPLY_YYMM	=       " & FilterVar(UCase(arrColVal(2)), "''", "S")
'Response.Write lgStrSQL
'Response.End 
        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "R"
                       ...
                       lgStrSQL = lgStrSQL & " BP_CD, APPLY_YYMM = LEFT(APPLY_YYMM,4)+'-'+RIGHT(APPLY_YYMM,2) , PRICE_CUR, TEST_APPLY_OPT, TERM_OPT, RETEST_RATE, EFR_RATE, QEI_RATE, "   
                       lgStrSQL = lgStrSQL & "    ADVANTAGE_SEC, ROUND_RULE_CP, ROUND_RULE_FT, BASIC_PRICE_OPT, OPERATOR, OUTPUT_RATE, XCHG_RATE_OPT, "  
                       lgStrSQL = lgStrSQL & "    BP_NM = dbo.ufn_x_getcodename('b_biz_partner', BP_CD,'') "  
                       lgStrSQL = lgStrSQL & " FROM S_Pricing_Confuguration_Cust_KO441 "
                       lgStrSQL = lgStrSQL & " WHERE 1=1 "  & pCode 
                       lgStrSQL = lgStrSQL & " ORDER BY BP_CD, APPLY_YYMM " 
