...
Sub SubBizSaveMultiCreate(arrColVal)
	...
	lgStrSQL = "INSERT INTO S_Test_Basic_Price_Item_KO441"
	lgStrSQL = lgStrSQL & "( ITEM_CD, BP_CD, TESTER, TEST_METHOD, TEST_TYPE, CURRENCY, VALID_FROM_DT, BASIC_PRICE,  "
...
Sub SubBizSaveMultiUpdate(arrColVal)
    ...
    lgStrSQL = "UPDATE  S_Test_Basic_Price_Item_KO441 "
    lgStrSQL = lgStrSQL & " SET " 
    lgStrSQL = lgStrSQL & " BASIC_PRICE		=       " & UNIConvNum(arrColVal(9),0)	& ","
        
    lgStrSQL = lgStrSQL & " UPDT_DT			=       " & FilterVar(lgSvrDateTime, "''", "S") & ","
    lgStrSQL = lgStrSQL & " UPDT_USER_ID	=       " & FilterVar(gUsrId, "''", "S")  
    lgStrSQL = lgStrSQL & " WHERE					"
    lgStrSQL = lgStrSQL & " ITEM_CD			=       " & FilterVar(UCase(arrColVal(2)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " BP_CD			=       " & FilterVar(UCase(arrColVal(3)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " TESTER			=       " & FilterVar(UCase(arrColVal(4)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " TEST_METHOD 	=       " & FilterVar(UCase(arrColVal(5)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " TEST_TYPE 		=       " & FilterVar(UCase(arrColVal(6)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " CURRENCY   		=       " & FilterVar(UCase(arrColVal(7)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " VALID_FROM_DT	=       " & FilterVar(UNIConvDate(arrColVal(8)),NULL,"S")	
...
Sub SubBizSaveMultiDelete(arrColVal)
    ...
    lgStrSQL = "DELETE  S_Test_Basic_Price_Item_KO441 WHERE "
    lgStrSQL = lgStrSQL & " BP_CD			=       " & FilterVar(UCase(arrColVal(3)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " ITEM_CD			=       " & FilterVar(UCase(arrColVal(2)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " TESTER			=       " & FilterVar(UCase(arrColVal(4)), "''", "S") & " AND "
    lgStrSQL = lgStrSQL & " TEST_METHOD 	=       " & FilterVar(UCase(arrColVal(5)), "''", "S") & " AND "
        ...
        Case "M"
           ...
           Select Case Mid(pDataType,2,1)
               ...
               Case "R"
                       ...
                       lgStrSQL = lgStrSQL & " FROM S_Test_Basic_Price_Item_KO441 "
