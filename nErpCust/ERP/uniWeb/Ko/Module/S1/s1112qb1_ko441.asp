...
'*  1. Module Name          : Sales
'*  2. Function Name        : 
'*  3. Program ID           : s1112qb1
'*  4. Program Name         : 고객별품목단가조회 
'*  5. Program Desc         : 고객별품목단가조회 
'*  6. Comproxy List        :
'*  7. Modified date(First) : 2002/07/08
...
Sub FixUNISQLData()
	...
	If Len(Request("txtconBiz_partner")) Then
		strVal = strVal & " AND A.BP_CD = " & FilterVar(UCase(Request("txtconBiz_partner")), "''", "S") & " "
		arrVal(0) = Trim(Request("txtconBiz_partner"))
	End If

	
	If Len(Request("txtconItem_cd")) Then
		strVal = strVal & " AND A.ITEM_CD = " & FilterVar(UCase(Request("txtconItem_cd")), "''", "S") & " "
		arrVal(1) = Trim(Request("txtconItem_cd"))
	End If

 	If Len(Request("txtconPay_terms")) Then
		strVal = strVal & " AND A.PAY_METH = " & FilterVar(UCase(Request("txtconPay_terms")), "''", "S") & " "		
		arrVal(2) = Trim(Request("txtconPay_terms")) 
	End If		
    
    If Len(Request("txtconDeal_type")) Then
		strVal = strVal & " AND A.DEAL_TYPE = " & FilterVar(UCase(Request("txtconDeal_type")), "''", "S") & " "		
		arrVal(3) = Trim(Request("txtconDeal_type")) 
	End If	

	If Len(Request("txtconSales_unit")) Then
		strVal = strVal & " AND A.SALES_UNIT = " & FilterVar(UCase(Request("txtconSales_unit")), "''", "S") & " "		
		arrVal(4) = Trim(UCase(Request("txtconSales_unit")))
	End If

	If Len(Request("txtconCurrency")) Then
		strVal = strVal & " AND A.CURRENCY = " & FilterVar(UCase(Request("txtconCurrency")), "''", "S") & " "		
		arrVal(5) = Trim(UCase(Request("txtconCurrency")))
	End If
 	
	If Len(Request("gPlant")) Then
		strVal2 = strVal2 & " INNER JOIN (SELECT X.ITEM_CD,Y.BIZ_AREA_CD,Y.PLANT_CD"
		strVal2 = strVal2 & "       FROM B_ITEM_BY_PLANT X"
		strVal2 = strVal2 & " INNER JOIN B_PLANT Y ON X.PLANT_CD = Y.PLANT_CD AND Y.PLANT_CD = " & FilterVar(Request("gPlant"), "''", "S")  & " ) PL ON E.ITEM_CD = PL.ITEM_CD  "
        End If
        
        If Len(Request("gBizArea")) Then
		strVal2 = strVal2 & " INNER JOIN (SELECT X.ITEM_CD,Y.BIZ_AREA_CD,Y.PLANT_CD"
		strVal2 = strVal2 & "       FROM B_ITEM_BY_PLANT X"
		strVal2 = strVal2 & " INNER JOIN B_PLANT Y ON X.PLANT_CD = Y.PLANT_CD AND Y.BIZ_AREA_CD = " & FilterVar(Request("gBizArea"), "''", "S")  & " ) BA ON E.ITEM_CD = BA.ITEM_CD  "
        End If
	
'Call svrmsgbox(strVal2,0,1)

    UNIValue(0,1) = strVal 
    UNIValue(0,2) = strVal2 
    ...
    UNIValue(4,0) = FilterVar("S0001", " " , "S")
    UNIValue(4,1) = FilterVar(arrVal(3), " " , "S")
    UNIValue(5,0) = FilterVar(arrVal(4), " " , "S")
    UNIValue(6,0) = FilterVar(arrVal(5), " " , "S")
    
    '--------------- 개발자 coding part(실행로직,End)------------------------------------------------------
   ' UNIValue(0,UBound(UNIValue,3)) = UCase(Trim(lgTailList))
...
<Script Language=vbscript>
    With parent
		...
		Call .ReFormatSpreadCellByCellByCurrency(.frm1.vspdData,"<%=iFrPoint+1%>",.frm1.vspdData.MaxRows,.GetKeyPos("A",12),.GetKeyPos("A",13),"C","Q","X","X")

        .lgStrPrevKey        =  "<%=lgStrPrevKey%>"                       '☜: set next data tag
        
        .frm1.txtHconBiz_partner.value		= "<%=ConvSPChars(Request("txtconBiz_partner"))%>"
        ...
        .frm1.txtconBiz_partner_nm.value = "<%=ConvSPChars(arrRsVal(1))%>"
        .frm1.txtconItem_nm.value = "<%=ConvSPChars(arrRsVal(3))%>" 
        .frm1.txtconPay_terms_nm.value = "<%=ConvSPChars(arrRsVal(5))%>" 
        .frm1.txtconDeal_type_nm.value = "<%=ConvSPChars(arrRsVal(7))%>" 
