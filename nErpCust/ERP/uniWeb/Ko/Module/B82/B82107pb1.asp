...
Sub SubBizQuery()
    ...
    lgStrSQL = lgStrSQL & "           B.ITEM_KIND, "
	lgStrSQL = lgStrSQL & "           ITEM_KIND_NM = dbo.ufn_GetCodeName('Y1001' , B.ITEM_KIND ), "
    lgStrSQL = lgStrSQL & "           A.ITEM_CD,"
    lgStrSQL = lgStrSQL & "           A.OLD_ITEM_NM,"
    lgStrSQL = lgStrSQL & "           A.OLD_ITEM_SPEC,"
    lgStrSQL = lgStrSQL & "           A.NEW_ITEM_NM,"
    lgStrSQL = lgStrSQL & "           A.NEW_ITEM_SPEC,"
    lgStrSQL = lgStrSQL & "           R_GRADE = dbo.ufn_GetCodeName('Y1007' , A.R_GRADE ),"
    lgStrSQL = lgStrSQL & "           T_GRADE = dbo.ufn_GetCodeName('Y1007' , A.T_GRADE ),"
    lgStrSQL = lgStrSQL & "           P_GRADE = dbo.ufn_GetCodeName('Y1007' , A.P_GRADE ),"
    lgStrSQL = lgStrSQL & "           Q_GRADE = dbo.ufn_GetCodeName('Y1007' , A.Q_GRADE ),"
    lgStrSQL = lgStrSQL & "           A.TRANS_DT,"
    lgStrSQL = lgStrSQL & "           A.REQ_REASON "
    lgStrSQL = lgStrSQL & "      FROM B_CIS_CHANGE_ITEM_NM_REQ A, "
    lgStrSQL = lgStrSQL & "           B_CIS_ITEM_MASTER        B "
    lgStrSQL = lgStrSQL & "     WHERE A.ITEM_CD = B.ITEM_CD "
    lgStrSQL = lgStrSQL & "       AND A.INTERNAL_CD = B.INTERNAL_CD "
    lgStrSQL = lgStrSQL & "       AND A.REQ_DT >= " & FilterVar(UniConvDate(lgKeyStream(0)),"","S")
    ...
    If Trim(lgKeyStream(3)) <> "" Then
		lgStrSQL = lgStrSQL & " AND B.ITEM_ACCT = " & FilterVar(lgKeyStream(3),"","S")
	End If
    If Trim(lgKeyStream(4)) <> "" Then
		lgStrSQL = lgStrSQL & " AND B.ITEM_KIND = " & FilterVar(lgKeyStream(4),"","S")
	End If
	If Trim(lgKeyStream(5)) <> "" Then
		lgStrSQL = lgStrSQL & " AND A.REQ_ID = " & FilterVar(lgKeyStream(5),"","S")
	End If	
	If Trim(lgKeyStream(6)) <> "" Then
		lgStrSQL = lgStrSQL & " AND A.New_ITEM_SPEC LIKE " & FilterVar(lgKeyStream(6) & "%","","S")
	End If
    ...
    Else
       ...
       Do While Not lgObjRs.EOF
          ...
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("NEW_ITEM_NM"))
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("NEW_ITEM_SPEC"))
          ...
          lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("REQ_REASON"))
