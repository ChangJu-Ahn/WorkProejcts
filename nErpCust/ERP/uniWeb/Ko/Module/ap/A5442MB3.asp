...
'----------------------------------------------------------------------------------------------------------
Sub  TrimData()
    strFrDt	= FilterVar(UNIConvDate(Request("txtApFrDt")), "''", "S")
    strToDt	= FilterVar(UNIConvDate(Request("txtApToDt")), "''", "S")
...
Sub SubBizQueryMulti()
	...
	lgStrSQL = lgStrSQL & " A.AP_NO, CONVERT(CHAR(10),A.AP_DT,20) AP_DT, CONVERT(CHAR(10),I.GL_DT,20) GL_DT,"
	lgStrSQL = lgStrSQL & " ISNULL(A.AP_LOC_AMT,0)-ISNULL(D.ITEM_LOC_AMT,0) DIFF_AMT, "
	lgStrSQL = lgStrSQL & " ISNULL(A.AP_LOC_AMT,0) AP_LOC_AMT,ISNULL(D.ITEM_LOC_AMT,0) GL_ITEM_LOC_AMT,"
	...
	lgStrSQL = lgStrSQL & " ,A.AP_TYPE,C.MINOR_NM "
	lgStrSQL = lgStrSQL & " ,A.BIZ_AREA_CD,G.BIZ_AREA_NM "
	lgStrSQL = lgStrSQL & " ,A.PAY_BP_CD,H.BP_NM "
	lgStrSQL = lgStrSQL & " FROM A_OPEN_AP A  "
	lgStrSQL = lgStrSQL & " LEFT JOIN A_ACCT B ON A.ACCT_CD=B.ACCT_CD "
	lgStrSQL = lgStrSQL & " LEFT JOIN B_MINOR C ON (A.AP_TYPE=C.MINOR_CD  AND C.MAJOR_CD=" & FilterVar("A1001", "''", "S") & "  ) "
	...
	lgStrSQL = lgStrSQL & "	                                         where acct_cd in (select acct_cd from a_acct where acct_type LIKE " & FilterVar("%J%", "''", "S") & " )) a "
	...
	lgStrSQL = lgStrSQL & " LEFT JOIN (SELECT AA.REF_NO,CASE WHEN BB.ITEM_LOC_AMT > 0 AND CC.AP_LOC_AMT < 0 THEN (-1)*BB.ITEM_LOC_AMT "
	...
	lgStrSQL = lgStrSQL & "             INNER JOIN A_OPEN_AP CC ON AA.REF_NO=CC.REF_NO "	
	...
	lgStrSQL = lgStrSQL & " LEFT JOIN B_BIZ_PARTNER H ON A.PAY_BP_CD=H.BP_CD "
	lgStrSQL = lgStrSQL & " LEFT JOIN A_GL I ON A.GL_NO=I.GL_NO "
	lgStrSQL = lgStrSQL & " WHERE (A.GL_NO <> '' OR A.TEMP_GL_NO <> '') "
    lgStrSQL = lgStrSQL & "  AND  A.AP_NO >= " & FilterVar(lgStrPrevKey, "''", "S")
	lgStrSQL = lgStrSQL & "  AND  A.AP_DT <= " & strToDt 
	lgStrSQL = lgStrSQL & "  AND  A.AP_DT >= " & strFrDt

	If Trim(Request("txtGlInputType")) <> "" Then lgStrSQL = lgStrSQL & " AND  A.AP_TYPE  = " & strInputType
	...
	If UCase(Trim(Request("DispMeth"))) Then lgStrSQL = lgStrSQL & " AND ISNULL(A.AP_LOC_AMT,0) <> ISNULL(D.ITEM_LOC_AMT,0) "
	lgStrSQL = lgStrSQL & " ORDER BY A.AP_NO "
	
	lgStrSQL2 = ""
	lgStrSQL2 = lgStrSQL2 & " SELECT "
	lgStrSQL2 = lgStrSQL2 & "  ISNULL(SUM(A.AP_LOC_AMT),0) AP_TOT_LOC_AMT, ISNULL(SUM(E.ITEM_LOC_AMT),0) BATCH_TOT_LOC_AMT ,"
	lgStrSQL2 = lgStrSQL2 & "  ISNULL(SUM(D.ITEM_LOC_AMT),0) GL_TOT_LOC_AMT, ISNULL(SUM(F.ITEM_LOC_AMT),0) TEMP_GL_TOT_LOC_AMT, "
	lgStrSQL2 = lgStrSQL2 & "  (ISNULL(SUM(A.AP_LOC_AMT),0) -  ISNULL(SUM(D.ITEM_LOC_AMT),0) ) Diff_TOT_LOC_AMT "
	lgStrSQL2 = lgStrSQL2 & " FROM A_OPEN_AP A  "
	...
	lgStrSQL2 = lgStrSQL2 & "	                                         where acct_cd in (select acct_cd from a_acct where acct_type LIKE " & FilterVar("%J%", "''", "S") & " )) a "
	...
	lgStrSQL2 = lgStrSQL2 & " LEFT JOIN (SELECT AA.REF_NO,CASE WHEN BB.ITEM_LOC_AMT > 0 AND CC.AP_LOC_AMT < 0 THEN (-1)*BB.ITEM_LOC_AMT "
	...
	lgStrSQL2 = lgStrSQL2 & "             INNER JOIN A_OPEN_AP CC ON AA.REF_NO=CC.REF_NO "	
	...
	lgStrSQL2 = lgStrSQL2 & "  AND  A.AP_DT <= " & strToDt 
	lgStrSQL2 = lgStrSQL2 & "  AND  A.AP_DT >= " & strFrDt

	If Trim(Request("txtGlInputType")) <> "" Then lgStrSQL2 = lgStrSQL2 & " AND  A.AP_TYPE  = " & strInputType
	...
	If UCase(Trim(Request("DispMeth"))) Then lgStrSQL2 = lgStrSQL2 & " AND ISNULL(A.AP_LOC_AMT,0) <> ISNULL(D.ITEM_LOC_AMT,0) "   
	...
	Else
		...
		Do While Not lgObjRs.EOF
			...
			lgstrData = lgstrData & Chr(11) & UNIConvNumDBToCompanyByCurrency(lgObjRs("AP_LOC_AMT"), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")
			...
			lgstrData = lgstrData & Chr(11) & ConvSPChars(lgObjRs("AP_TYPE"))
	...
	If 	FncOpenRs("R",lgObjConn,lgObjRs,lgStrSQL2,"X","X") = True Then
		lgArTotLocAmt = UNIConvNumDBToCompanyByCurrency(lgObjRs("AP_TOT_LOC_AMT"), gCurrency,  ggAmtOfMoneyNo, gLocRndPolicyNo, "X")
