...
Sub FixUNISQLData()
    ...
    UNIValue(2,0) = "SELECT BIZ_AREA_NM FROM B_BIZ_AREA WHERE BIZ_AREA_CD = " & FilterVar(strBizCd, "''", "S")              
...
Sub  TrimData()
	...
	If Trim(Request("txtAllcDt")) <> "" Then strCond = strCond & " AND A.PRPAYM_DT <= " & FilterVar(UNIConvDate(Trim(Request("txtAllcDt"))), "''", "S")
...
<Script Language=vbscript>
    If "<%=lgDataExist%>" = "Yes" Then
       ...
       Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.Frm1.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,Parent.GetKeyPos("A",9),Parent.GetKeyPos("A",10),"A", "I" ,"X","X")
       Call Parent.ReFormatSpreadCellByCellByCurrency(Parent.Frm1.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,Parent.GetKeyPos("A",9),Parent.GetKeyPos("A",12),"A", "I" ,"X","X")
