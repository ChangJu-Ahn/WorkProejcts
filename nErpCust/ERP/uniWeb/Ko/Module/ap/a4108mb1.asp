...
Sub FixUNISQLData()
    ...
    UNISqlId(0) = "A4108MA104"
    UNISqlId(1) = "A4108MA105"
    UNISqlId(2) = "ADEPTNM"
    UNISqlId(3) = "ABPNM"
...
Sub TrimData()
	...
	strcboApSts			= Trim(Request("cboApSts"))
	...
	strCond = "AND A.ap_dt >=  " & FilterVar(strFromDt , "''", "S") & " AND A.ap_dt <=  " & FilterVar(strToDt , "''", "S") & "" 
	...
	If strcboApSts <> ""	Then strCond = strCond & " AND A.ap_sts = "		& FilterVar(strcboApSts ,null,"S")
    ...
    If "<%=lgDataExist%>" = "Yes" Then
       ...
       If "<%=lgPageNo%>" = "1" Then   ' "1" means that this query is first and next data exists
			...
			With parent.frm1
				...
				.hConfFg.value = Trim(.cboApSts.value)
				.hApSts.value = Trim(.cboConfFg.value)
       ...
       With Parent
			...
			Call Parent.ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,.GetKeyPos("A",1),.GetKeyPos("A",2),"D", "Q" ,"X","X")			
			Call Parent.ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,.GetKeyPos("A",1),.GetKeyPos("A",3),"A", "Q" ,"X","X")
			Call Parent.ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,.GetKeyPos("A",1),.GetKeyPos("A",4),"A", "Q" ,"X","X")
			Call Parent.ReFormatSpreadCellByCellByCurrency(.Frm1.vspdData,<%=iPrevEndRow+1%>,<%=iEndRow%>,.GetKeyPos("A",1),.GetKeyPos("A",5),"A", "Q" ,"X","X")
			...
			.frm1.txtTotApLocAmt =  "<%=UNINumClientFormat(txtTotApLocAmt, ggAmtOfMoney.DecPoint, 0)%>"
