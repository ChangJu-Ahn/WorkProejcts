...
'*  2. Function Name        : Account Payable
'*  3. Program ID           : a4203oa1_len000
'*  4. Program Name         : (P) Aged Payable
'*  5. Program Desc         : Preview/Print
'*  6. Comproxy List        : 
'*  7. Modified date(First) : 2001.01.31
'*  8. Modified date(Last)  : 2005/05/17
'*  9. Modifier (First)     : Song, Mun Gil
'* 10. Modifier (Last)      : Joo,Sungho	
'* 11. Comment              : * 시점기준(만기일, 발생일) 조건 추가 
'======================================================================================================= -->
<HTML>
<HEAD>
...
Function OpenPopUp(Byval strCode, Byval iWhere)
	...
	Select Case iWhere
		...
		Case 1, 2
				...
				IF frm1.txtApDateFr.Text<>"" THEN		arrParam(1) = arrParam(1) & " AND AP_DT >= " & FilterVar(UNIConvDate(frm1.txtApDateFr.Text), "''", "S") & ""
				IF frm1.txtApDateTo.Text<>"" THEN		arrParam(1) = arrParam(1) & " AND AP_DT <= " & FilterVar(UNIConvDate(frm1.txtApDateTo.Text), "''", "S") & ""								
				arrParam(1) = arrParam(1) & " AND AP_DT <= " & FilterVar(UNIConvDate(frm1.txtAsofDate.Text), "''", "S") & ""
...
'=======================================================================================================
Sub txtApDateFr_DblClick(Button)
    If Button = 1 Then
        frm1.txtApDateFr.Action = 7
        Call SetFocusToDocument("M")
		Frm1.txtApDateFr.Focus         
    End If
End Sub

Sub txtApDateTo_DblClick(Button)
    If Button = 1 Then
        frm1.txtApDateTo.Action = 7
        Call SetFocusToDocument("M")
		Frm1.txtApDateTo.Focus         
    End If
End Sub
...
Sub rdoReport1_OnClick()
	Call SetOrderByOption
End Sub

Sub rdoReport2_OnClick()
	Call SetOrderByOption
End Sub
...
'======================================================================================================
' Function Name : SetOrderByOption
' Function Desc : If Summary is selected, Order_by option is disabled.
'                 If Detail is selected, Order_by option is enabled.
'=======================================================================================================
Function SetOrderByOption()
	With frm1
		If .rdoReport1.checked = True Then	'Summary
			Call ggoOper.SetReqAttr(.rdoOrder1,   "Q")
			Call ggoOper.SetReqAttr(.rdoOrder2,   "Q")
			
			.rdoOrder1.checked = True
		Else	'Detail
			Call ggoOper.SetReqAttr(.rdoOrder1,   "D")
			Call ggoOper.SetReqAttr(.rdoOrder2,   "D")
		End If
	End With
End Function
...
Function SetPrintCond(strEbrFile, strCond)
	...
	With frm1
		...
		If Trim(.txtBizAreaCd.value) <> "" and   Trim(.txtBizAreaCd1.value) <> "" Then				
			If Trim(.txtBizAreaCd.value) > Trim(.txtBizAreaCd1.value) Then
				IntRetCd = DisplayMsgBox("970025", "X", .txtBizAreaCd.Alt, .txtBizAreaCd1.Alt)
				.txtBizAreaCd.focus
				Exit Function
			End If
		End If
		...
		If .txtApDateFr.Text <> "" And .txtApDateTo.Text <> "" Then
			If ValidDateCheck(.txtApDateFr, .txtApDateTo) = False Then Exit Function
		End If
		
'--------------------------------------------------------------------------------
'	1) strEbrFile = "a3203oa1a"		-> 집계 + 만기일(기존)
'	2) strEbrFile = "a3203oa1b"		-> 상세 + 만기일 + 거래처별(기존)
'   3) strEbrFile = "a3203oa1c"		-> 상세 + 만기일 + 채무만기일자별(기존)
'	4) strEbrFile = "a3203oa1d"		-> 집계 + 발생일 
'	5) strEbrFile = "a3203oa1e"		-> 상세 + 발생일 + 거래처별 
'   6) strEbrFile = "a3203oa1f"		-> 상세 + 발생일 + 채무만기일자별 
'--------------------------------------------------------------------------------

		If .rdoReport1.checked = True and .rdoViewPoint1.checked = True Then
			strEbrFile = "a4203oa1a"																		'Summary + 만기일 
		ElseIf .rdoReport1.checked = True and .rdoViewPoint2.checked = True Then
			strEbrFile = "a4203oa1d"																		'집계 + 발생일 
		ElseIf .rdoReport2.checked = True and .rdoViewPoint1.checked = True and .rdoOrder1.checked = True Then
			strEbrFile = "a4203oa1b"																		'상세 + 만기일 + 거래처별(기존)
		ElseIf .rdoReport2.checked = True and .rdoViewPoint1.checked = True and .rdoOrder2.checked = True Then
			strEbrFile = "a4203oa1c"																		'상세 + 만기일 + 채무만기일자별(기존)
		ElseIf .rdoReport2.checked = True and .rdoViewPoint2.checked = True and .rdoOrder1.checked = True Then
			strEbrFile = "a4203oa1e"																		'상세 + 발생일 + 거래처별 
		ElseIf .rdoReport2.checked = True and .rdoViewPoint2.checked = True and .rdoOrder2.checked = True Then
			strEbrFile = "a4203oa1f"																		'상세 + 발생일 + 채무만기일자별 
		End If

		If .txtApDateFr.Text = "" Then
			strCond = strCond & "ApDateFrom|" & "1900-01-01"
		Else
			strCond = strCond & "ApDateFrom|" & UNIConvDate(.txtApDateFr.Text)
		End If

		If .txtApDateTo.Text = "" Then
			strCond = strCond & "|ApDateTo|" & "2999-12-31"
		Else
			strCond = strCond & "|ApDateTo|" & UNIConvDate(.txtApDateTo.Text)
		End If
		...
		If .txtBizAreaCd.value = "" then 
			...
			strCond = strCond & "|BizAreaCd|" & FilterVar(.txtBizAreaCD.value,"","SNM")
		end if
	
		If .txtBizAreaCd1.value = "" then
			.txtBizAreaNm.value = ""
			strCond = strCond & "|BizAreaCd1|" & "ZZZZZZZZZZ"
		else 
			strCond = strCond & "|BizAreaCd1|" & FilterVar(.txtBizAreaCD1.value,"","SNM")
		end if
	...
	If lgAuthBizAreaCd <> "" Then			
		strAuthCond		= strAuthCond	& " AND AP.BIZ_AREA_CD = " & FilterVar(lgAuthBizAreaCd, "''", "S")  		
	End If			

	If lgInternalCd <> "" Then			
		strAuthCond		= strAuthCond	& " AND AP.INTERNAL_CD = " & FilterVar(lgInternalCd, "''", "S")  		
	End If			

	If lgSubInternalCd <> "" Then	
		strAuthCond		= strAuthCond	& " AND AP.INTERNAL_CD LIKE " & FilterVar(lgSubInternalCd & "%", "''", "S")  
	End If	

	If lgAuthUsrID <> "" Then	
		strAuthCond		= strAuthCond	& " AND AP.INSRT_USER_ID = " & FilterVar(lgAuthUsrID, "''", "S")  
	End If	
				...
				<TR>
					<TD WIDTH=100% HEIGHT=100% valign=top>
						<TABLE <%=LR_SPACE_TYPE_60%>>
							...
							<TR>
								<TD CLASS=TD5 NOWRAP>정  렬</TD>
								<TD CLASS=TD6 NOWRAP><INPUT TYPE=RADIO CLASS="RADIO" NAME="rdoOrder" ID="rdoOrder1" VALUE="1" TAG="11" Checked><LABEL FOR="rdoOrder1">거래처</LABEL>&nbsp;&nbsp
													 <INPUT TYPE=RADIO CLASS="RADIO" NAME="rdoOrder" ID="rdoOrder2" VALUE="2" TAG="11"><LABEL FOR="rdoOrder2">채무만기일자</LABEL>
								</TD>
							</TR>
							<TR>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtPayBpCdFr" SIZE=10 MAXLENGTH=10 tag="11XXXU" ALT="Vendor Code"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnPopup" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopUp(frm1.txtPayBpCdFr.Value, 1)">
													   <INPUT TYPE="Text" NAME="txtPayBpNmFr" SIZE=25 tag="14X" ALT="Vendor Name">
								</TD>
							</TR>
							<TR>
								<TD CLASS="TD5" NOWRAP>~&nbsp;</TD>
								<TD CLASS="TD6" NOWRAP><INPUT TYPE="Text" NAME="txtPayBpCdTo" SIZE=10 MAXLENGTH=10 tag="11XXXU" ALT="Vendor Code"><IMG SRC="../../../CShared/image/btnPopup.gif" NAME="btnPopup" ALIGN=top TYPE="BUTTON" ONCLICK="vbscript:Call OpenPopUp(frm1.txtPayBpCdTo.Value, 2)">
													   <INPUT TYPE="Text" NAME="txtPayBpNmTo" SIZE=25 tag="14X" ALT="Vendor Name">
								</TD>
							</TR>
							<TR>
							...
							<TR>
								...
								<TD CLASS="TD6" NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtApDateFr" CLASS=FPDTYYYYMMDD tag="11X1" Title="FPDATETIME" ALT="Start A/P Date" id=txtApDateFr></OBJECT>');</SCRIPT>&nbsp;~&nbsp;
													   <SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtApDateTo" CLASS=FPDTYYYYMMDD tag="11X1" Title="FPDATETIME" ALT="End A/P Date" id=txtApDateTo></OBJECT>');</SCRIPT>
								</TD>
							</TR>
							<TR>
								<TD CLASS="TD5" NOWRAP>조회기준일</TD>
								<TD CLASS="TD6" NOWRAP><SCRIPT LANGUAGE=JavaScript> ExternalWrite('<OBJECT CLASSID=<%=gCLSIDFPDT%> NAME="txtAsofDate" CLASS=FPDTYYYYMMDD tag="12X1" Title="FPDATETIME" ALT="As of Date" id=txtAsofDate></OBJECT>');</SCRIPT>
								</TD>
							</TR>
							<TR>
