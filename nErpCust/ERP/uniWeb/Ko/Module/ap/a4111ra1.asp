...
'*  2. Function Name        : 
'*  3. Program ID           :a4111ra1 
'*  4. Program Name         :채무/채권상계 
'*  5. Program Desc         :
'*  6. Comproxy List        : 
'*  7. Modified date(First) : 
'*  8. Modified date(Last)  : 
'*  9. Modifier (First)     : Hwang Eun Hee
'* 10. Modifier (Last)      : Hwang Eun Hee
'* 11. Comment              :
...
Const BIZ_PGM_ID 		= "a4111rb1.asp"                              '☆: Biz Logic ASP Name
...
'--------------- 개발자 coding part(실행로직,Start)--------------------------------------------------
	...
	frm1.txtFrClearDt.Text	= StartDate
	frm1.txtToClearDt.Text	= EndDate
...
Function OpenDeptOrgPopup()
	...
	arrParam(0) = frm1.txtFrClearDt.text								'  Code Condition
   	arrParam(1) = frm1.txtToClearDt.Text
...
Function SetDept(Byval arrRet)	
		...
		frm1.txtFrClearDt.text = arrRet(4)
		frm1.txtToClearDt.text = arrRet(5)
		frm1.txtDeptCd.focus		
End Function
...
Sub InitSpreadSheet()
    ...
    Call SetZAdoSpreadSheet("A4111RA1","S","A","V20021211",PopupParent.C_SORT_DBAGENT,frm1.vspdData, C_MaxKey, "X","X")
    Call SetSpreadLock() 
...
'==========================================================================================
'   Event Name : txtFrClearDt
'   Event Desc :
'==========================================================================================

Sub  txtFrClearDt_DblClick(Button)
	if Button = 1 then
		frm1.txtFrClearDt.Action = 7
		Call SetFocusToDocument("P")
		Frm1.txtFrClearDt.Focus
				
	End if
End Sub

Sub  txtFrClearDt_KeyPress(KeyAscii)
...
'==========================================================================================
'   Event Name : txtToClearDt
'   Event Desc :
'==========================================================================================

Sub  txtToClearDt_DblClick(Button)
	if Button = 1 then
		frm1.txtToClearDt.Action = 7
		Call SetFocusToDocument("P")
		Frm1.txtToClearDt.Focus
				
	End if
End Sub

Sub  txtToClearDt_KeyPress(KeyAscii)
	On Error Resume Next
	If KeyAscii = 27 Then 
		Call CancelClick()
	ElseIf KeyAscii = 13 Then 
...
Dim IntRetCD
	...
	If PopupParent.CompareDateByFormat(frm1.txtFrClearDt.text,frm1.txtToClearDt.text,frm1.txtFrClearDt.Alt,frm1.txtToClearDt.Alt, _
        	               "970025",frm1.txtFrClearDt.UserDefinedFormat,PopupParent.gComDateType, true) = False Then
	   frm1.txtFrClearDt.focus
	   Exit Function
	End If
			
	IF NOT CheckOrgChangeId Then
		  IntRetCD = DisplayMsgBox("800600","X",frm1.txtFrClearDt.alt,"X")            '⊙: Display Message(There is no changed data.)
		Exit Function
	End if
...
Function  DbQuery() 
    ...
    '--------- Developer Coding Part (Start) ----------------------------------------------------------
        If lgIntFlgMode  <> PopupParent.OPMD_UMODE Then   ' This means that it is first search
			strVal = strVal & "?txtFrClearDt="       & Trim(.txtFrClearDt.Text)
			strVal = strVal & "&txtToClearDt="		& Trim(.txtToClearDt.Text)
			strVal = strVal & "&txtFrClearNo="		& Trim(.txtFrClearNo.value)
			strVal = strVal & "&txtToClearNo="		& Trim(.txtToClearNo.value)
			strVal = strVal & "&txtdeptcd="			& Trim(.txtdeptcd.value) 
			strVal = strVal & "&txtDeptCd_Alt="		& Trim(.txtdeptcd.alt)	
        Else
			strVal = strVal & "?txtFrClearDt="       & Trim(.htxtFrClearDt.value)
			strVal = strVal & "&txtToClearDt="		& Trim(.htxtToClearDt.value)
			strVal = strVal & "&txtFrClearNo="		& Trim(.htxtFrClearNo.value)
			strVal = strVal & "&txtToClearNo="		& Trim(.htxtToClearNo.value)
...
Function CheckOrgChangeId()
	...
	With frm1
		...
		If LTrim(RTrim(.txtDeptCd.value)) <> "" Then
			...
			strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(.txtFrClearDt.Text, popupparent.gDateFormat,""), "''", "S") & ")"
			strWhere = strWhere & " AND ORG_CHANGE_DT <= " 
			strWhere = strWhere & " (select max(org_change_dt) from B_ACCT_DEPT where org_change_dt<= " & FilterVar(UNIConvDateToYYYYMMDD(.txtToClearDt.Text, popupparent.gDateFormat,""), "''", "S") & ") "
		...
		<TD HEIGHT=20>
			<FIELDSET CLASS="CLSFLD">
				<TABLE <%=LR_SPACE_TYPE_40%>>
					<TR>				
						<TD CLASS=TD5 NOWRAP>상계일자</TD>
						<TD CLASS=TD6 NOWRAP>
							<script language =javascript src='./js/a4111ra1_fpDateTime1_txtFrClearDt.js'></script>&nbsp;~&nbsp;
							<script language =javascript src='./js/a4111ra1_fpDateTime2_txtToClearDt.js'></script>
						</TD>												
						<TD CLASS=TD5 NOWRAP>상계번호</TD>				
						<TD CLASS=TD6 NOWRAP>
						<INPUT TYPE="Text" NAME="txtFrClearNo" SIZE=15 MAXLENGTH=18 STYLE="TEXT-ALIGN: left" tag="1XXXXU" ALT="출금번호">&nbsp;~&nbsp;
						<INPUT TYPE="Text" NAME="txtToClearNo" SIZE=15 MAXLENGTH=18 STYLE="TEXT-ALIGN: left" tag="1XXXXU" ALT="출금번호">
						</TD>
					</TR>			
					<TR>
	...
	<TR>
		<TD WIDTH=100% HEIGHT=* VALIGN=TOP>
			<TABLE <%=LR_SPACE_TYPE_20%>>
				<TR HEIGHT=100%>
					<TD WIDTH=100%>
						<script language =javascript src='./js/a4111ra1_vspdData_vspdData.js'></script>
					</TD>
				</TR>
			</TABLE>
		</TD>
...
<INPUT TYPE=HIDDEN NAME="htxtFrClearDt"     tag="24">
<INPUT TYPE=HIDDEN NAME="htxtToClearDt"		tag="24">
<INPUT TYPE=HIDDEN NAME="htxtFrClearNo"		tag="24">
<INPUT TYPE=HIDDEN NAME="htxtToClearNo"        tag="24">
