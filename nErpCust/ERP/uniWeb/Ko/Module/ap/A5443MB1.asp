...
Sub SubBizQueryMulti()
	...
	lgStrSQL = lgStrSQL & " FROM ( " 
	'*****************************************A_cls_AP에서 가져오기 시작******************************************			
	lgStrSQL = lgStrSQL & " SELECT A.ACCT_CD,B.ACCT_NM, SUM(A.CLS_LOC_AMT)  ITEM_LOC_AMT," & FilterVar("AP", "''", "S") & "  AP_FG" 
	...
	lgStrSQL = lgStrSQL & " FROM (" 
	lgStrSQL = lgStrSQL & "			SELECT A.AP_NO, A.CLS_AP_NO, A.CLS_DT, A.ACCT_CD, A.CLS_AMT, A.CLS_LOC_AMT, A.GL_NO, A.TEMP_GL_NO, B.BIZ_AREA_CD, B.PAY_BP_CD" 
	lgStrSQL = lgStrSQL & "			FROM (SELECT A.AP_NO,  CASE WHEN B.ALLC_TYPE = " & FilterVar("B", "''", "S") & "  THEN B.REF_NO ELSE A.CLS_AP_NO END CLS_AP_NO, A.CLS_DT, A.ACCT_CD ACCT_CD, " 
	lgStrSQL = lgStrSQL & "					A.CLS_AMT+A.DC_AMT CLS_AMT, A.CLS_LOC_AMT+A.DC_LOC_AMT CLS_LOC_AMT, B.GL_NO, B.TEMP_GL_NO" 
	lgStrSQL = lgStrSQL & "				   FROM A_CLS_AP A JOIN (SELECT PAYM_NO, GL_NO,	TEMP_GL_NO, REF_NO, ALLC_TYPE FROM A_ALLC_PAYM UNION" 
	lgStrSQL = lgStrSQL & "										 SELECT ALLC_NO, GL_NO, TEMP_GL_NO, '' , '' FROM A_ALLC_RCPT UNION" 
	lgStrSQL = lgStrSQL & "										 SELECT CLEAR_NO, GL_NO, '', '', '' FROM A_CLEAR_AP_AR) B ON B.PAYM_NO = A.CLS_AP_NO" 
	lgStrSQL = lgStrSQL & "				UNION ALL" 
	lgStrSQL = lgStrSQL & "				SELECT A.AP_NO, A.ADJUST_NO, A.ADJUST_DT, B.ACCT_CD,A.ADJUST_AMT, A.ADJUST_LOC_AMT,  A.GL_NO, A.TEMP_GL_NO" 
...
'************************A_BATCH_GL_ITEM중에서  가져오기 시작*******************		
	...
	lgStrSQL = lgStrSQL & " " & FilterVar("BATCH", "''", "S") & "  AP_FG " 
...
'********************************************A_GL_ITEM중에서 가져오기 시작***********************************		
	...
	lgStrSQL = lgStrSQL & " SELECT  A.ACCT_CD,F.ACCT_NM,ISNULL(SUM( A.ITEM_LOC_AMT),0) ITEM_LOC_AMT, " & FilterVar("GL", "''", "S") & "  AP_FG " 
...
'********************************************A_Temp_GL_ITEM중에서 가져오기 시작***********************************
	...
	lgStrSQL = lgStrSQL & " SELECT A.ACCT_CD,F.ACCT_NM,ISNULL(SUM( A.ITEM_LOC_AMT),0) ITEM_LOC_AMT, " & FilterVar("TEMP_GL", "''", "S") & "  AP_FG  " 
