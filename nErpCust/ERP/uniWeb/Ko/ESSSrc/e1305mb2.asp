...
'------------  
	 ...
	 iKey1 = iKey1 & " AND YY = "  &  FilterVar(lgKeyStream(1),"'%'", "S")	 
	 iKey1 = iKey1 & " AND FAMILY_NAME =" &  FilterVar(lgKeyStream(2),"''", "S")
	 iKey1 = iKey1 & " AND EMP_NO =" &  FilterVar(lgKeyStream(0),"''", "S")	    
	 iKey1 = iKey1 & " AND FAMILY_TYPE =" &  FilterVar(lgKeyStream(4),"''", "S")
	 iKey1 = iKey1 & " AND SUBMIT_FLAG =" &  FilterVar(lgKeyStream(8),"''", "S")	
	 	 
	' YY, EMP_NO, FAMILY_NAME, FAMILY_REL, FAMILY_TYPE, SUBMIT_FLAG
...
<Script Language=vbscript>
       With Parent	
            ...
            .frm1.txtAmt.value = "<%=UNINumClientFormat(lgObjRs("EDU_AMT"),ggAmtOfMoney.DecPoint, 0)%>"
...
Sub SubBizDelete()
    ...
    lgStrSQL = lgStrSQL & "   AND YY = "			& FilterVar(lgKeyStream(1),"''","S")
    lgStrSQL = lgStrSQL & "   AND FAMILY_NAME = "	& FilterVar(lgKeyStream(2),"''","S")
	lgStrSQL = lgStrSQL & "   AND FAMILY_TYPE ="	& FilterVar(lgKeyStream(4),"''", "S")
	lgStrSQL = lgStrSQL & "   AND SUBMIT_FLAG ="	& FilterVar(lgKeyStream(6),"''", "S")	
...
Sub SubBizSaveSingleCreate()
	...
	'중복 데이터를 check 한다 
	Call CommonQueryRs(" count(EMP_NO) "," HFA090T "," EMP_NO = " & FilterVar(lgKeyStream(0),"''","S")& " AND YY = " & FilterVar(lgKeyStream(1),"''","S") &_
	"   AND FAMILY_NAME = "	& FilterVar(lgKeyStream(2),"''","S") & " AND FAMILY_TYPE = " & FilterVar(lgKeyStream(4),"''","S")  & " AND SUBMIT_FLAG = " & FilterVar(lgKeyStream(6),"''","S")  _
...
Sub SubBizSaveSingleUpdate()
    ...
    lgStrSQL = lgStrSQL & " SET   EDU_AMT = "		& UNIConvNum(lgKeyStream(5),0) & ","
...
Sub SubMakeSQLStatements(pMode,pCode)
    ...
    Select Case pMode 
      Case "R"
            ...
            lgStrSQL = lgStrSQL & "  FAMILY_TYPE , dbo.ufn_GetCodeName('H0070',FAMILY_TYPE) as FAMILY_TYPE_NM, EDU_AMT,SUBMIT_FLAG "
