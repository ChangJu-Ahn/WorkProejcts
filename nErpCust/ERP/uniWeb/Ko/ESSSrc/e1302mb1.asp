...
'============================================================================================================
Sub SubBizQuery()
    Dim strYear, strEmpNo
    Dim strWhere
    Dim a_pay_tot_amt, a_bonus_tot_amt, a_after_bonus_amt, b_amt
    Dim supp_old_cnt, supp_young_cnt
'    On Error Resume Next                                                            '☜: Protect system from crashing
    Err.Clear                                                                        '☜: Clear Error status
...
<%
    ...
    Else
		...
		Call CommonQueryRs(" SUPP_OLD_CNT, SUPP_YOUNG_CNT "," HDF020T ", "emp_no = " & FilterVar(lgKeyStream(0),"''", "S"), lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)

		supp_old_cnt   = Replace(lgF0, Chr(11), "")
		supp_young_cnt = Replace(lgF1, Chr(11), "")

		strWhere = "year_yy = " & FilterVar(lgKeyStream(2), "''", "S")
		strWhere = strWhere & " AND emp_no = " & FilterVar(lgKeyStream(0),"''", "S")
		
		Call CommonQueryRs(" SUM(A_PAY_TOT_AMT), SUM(A_BONUS_TOT_AMT), SUM(A_AFTER_BONUS_AMT), SUM(A_PAY_TOT_AMT)+SUM(A_BONUS_TOT_AMT)+SUM(A_AFTER_BONUS_AMT) "," HFA040T ", strWhere, lgF0,lgF1,lgF2,lgF3,lgF4,lgF5,lgF6)

		a_pay_tot_amt   = Replace(lgF0, Chr(11), "")
		a_bonus_tot_amt = Replace(lgF1, Chr(11), "")
		a_after_bonus_amt = Replace(lgF2, Chr(11), "")
		b_amt = Replace(lgF3, Chr(11), "")
...
<Script Language=vbscript>
       With Parent	
		'소득사항 
            ...
            .frm1.txtbonus_tot_amt.value           = "<%=UNINumClientFormat(a_bonus_tot_amt, ggAmtOfMoney.DecPoint,0)%>"          
            .frm1.txtold_after_bonus_amt.value     = "<%=UNINumClientFormat(a_after_bonus_amt, ggAmtOfMoney.DecPoint,0)%>"          
            .frm1.txtpay_tot_amt.value             = "<%=UNINumClientFormat(a_pay_tot_amt, ggAmtOfMoney.DecPoint,0)%>"        
            .frm1.txtb_amt.value                   = "<%=UNINumClientFormat(b_amt, ggAmtOfMoney.DecPoint,0)%>"      
		...
		'인적공제 
            ...
            .frm1.txtsupp_old_cnt.value            = "<%=UNINumClientFormat(supp_old_cnt, ggAmtOfMoney.DecPoint,0)%>"     
            ...
            .frm1.txtsupp_young_cnt.value          = "<%=UNINumClientFormat(supp_young_cnt, ggAmtOfMoney.DecPoint,0)%>"     
		...
		'특별세액공제          
            ...
            .frm1.txtTaxLaw_contr_amt2.value         = "<%=UNINumClientFormat(lgObjRs("hfa030t_TaxLaw_contr_amt2"), ggAmtOfMoney.DecPoint,0)%>"    
            ...
            .frm1.txthfa030t_Retire_pension.value   = "<%=UNINumClientFormat(lgObjRs("hfa030t_Retire_pension"), ggAmtOfMoney.DecPoint,0)%>"
            .frm1.txthfa050t_Retire_pension.value   = "<%=UNINumClientFormat(lgObjRs("hfa050t_Retire_pension"), ggAmtOfMoney.DecPoint,0)%>"
            ...
            .frm1.txtTax_Union_Ded.value			= "<%=UNINumClientFormat(lgObjRs("hfa050t_TAX_UNION_DED"), ggAmtOfMoney.DecPoint,0)%>"
            .frm1.txtPolicontr_tax_sub_amt.value	= "<%=UNINumClientFormat(lgObjRs("hfa050t_poli_tax_sub"), ggAmtOfMoney.DecPoint,0)%>"                            
		'2004 
...
Sub SubMakeSQLStatements(pMode,pCode)
    ...
    Select Case pMode 
      Case "R"
            ...
            Case ""
                lgStrSQL = "SELECT  b.SPOUSE hfa050t_spouse, b.LADY hfa050t_lady, b.PARIA_CNT hfa050t_paria_cnt, "  
                ...
                lgStrSQL = lgStrSQL & " b.INCOME_TAX_SUB hfa050t_income_tax_sub_amt,b.fore_pay hfa050t_fore_pay, b.TAX_UNION_DED hfa050t_TAX_UNION_DED,b.HOUSE_REPAY hfa050t_house_repay_amt,"
                lgStrSQL = lgStrSQL & " b.STOCK_SAVE hfa050t_stock_save_amt, b.TAX_SUB_SUM hfa050t_tax_sub_sum_amt, b.poli_tax_sub hfa050t_poli_tax_sub,"   
                ...
                lgStrSQL = lgStrSQL & " c.retire_pension hfa030t_retire_pension, "  '2005           
                lgStrSQL = lgStrSQL & " b.retire_pension hfa050t_retire_pension, "  '2005                               
                ...
                lgStrSQL = lgStrSQL & " c.APP_CONTR hfa030t_app_contr_amt,c.TaxLaw_contr_amt hfa030t_TaxLaw_contr_amt,c.TaxLaw_contr_amt2 hfa030t_TaxLaw_contr_amt2, c.PRIV_CONTR hfa030t_priv_contr_amt, c.PRIV_CONTR hfa030t_priv_contr_amt,"  
                ...
                lgStrSQL = lgStrSQL & " c.AFTER_BONUS_AMT after_bonus_amt, "  
                ...
                lgStrSQL = lgStrSQL & " (b.PER_SUB + b.SPOUSE_SUB + b.SUPP_SUB + b.OLD_SUB + b.PARIA_SUB + b.LADY_SUB + "  
                lgStrSQL = lgStrSQL & " b.CHL_REAR_SUB + b.SMALL_SUB + b.STD_SUB + b.INDIV_ANU  + b.INDIV_ANU2  + b.National_pension_sub_amt + b.TECH_SUB + b.INVEST_SUB_SUM + "
                ...
                lgStrSQL = lgStrSQL & " FROM HFA050T b, "  
                lgStrSQL = lgStrSQL & "		 HFA030T c "  
                lgStrSQL = lgStrSQL & " WHERE b.YEAR_YY = " & FilterVar(lgKeyStream(2), "''", "S")
                lgStrSQL = lgStrSQL & " AND b.emp_no = " & FilterVar(lgKeyStream(0),"''", "S")                       
